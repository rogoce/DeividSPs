...
create procedure "informix".sp_pro493e(v_poliza char(10), v_endoso char(5), v_factor decimal(9,6))
...
--SET DEBUG FILE TO "\\NEMESIS\Ancon\Store Procedures\Debug\sp_victor.trc";  
--TRACE ON;                                                                 
...
--    Let _cant = 0;
--	select count(*) into _cant from endedde2
--	 where no_poliza   = v_poliza
--	   and no_endoso   = v_endoso
...
--	Let v_descto = 0.00;
--    select sum(porc_descuento) Into v_descto 
--      From endeddes 
--	 where no_poliza = v_poliza
--	   and no_endoso = v_endoso;
--	If v_descto is null Then
--	   let v_descto = 0.00;
--	End If
--	if v_descto = 0.00	Then
--	   update endedcob
--	      set endedcob.descuento   = 0.00
--	    where endedcob.no_poliza   = v_poliza
--	      and endedcob.no_endoso   = v_endoso;
--	end if

   foreach
      select x.cod_cobertura, x.prima_anual, x.prima into v_cobertura, r_prima_anual, v_prima
        from endedcob x
       where x.no_poliza = v_poliza
         and x.no_endoso = v_endoso
         and x.no_unidad = v_unidad

-- endedcob.deducible   = endedcob.deducible * v_factor

	  if v_tipocalc = "005" Or v_tipocalc = "006" Then
	     update endedcob
	        set endedcob.prima_anual = endedcob.prima_anual * v_factor,
   ...
   select sum(x.prima_anual), sum(x.prima), SUM(x.descuento), SUM(x.recargo), SUM(x.prima_neta)
