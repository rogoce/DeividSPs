...
-- Creado    : 13/05/2013 - Autor: Armando Moreno

drop procedure sp_rec160a;

create procedure sp_rec160a(a_periodo char(7)) 
returning integer,
          char(50);
...
monto			dec(16,2),
primary key (numrecla,cod_cobertura)
) with no log;
...
foreach
	...
	foreach
		...
		if _cantidad = 0 then
			...
			values (_numrecla, _cod_ramo,_cod_cobertura, _monto);
		else
			update tmp_promedio
