...
--Creado 30/01/2018	Armando Moreno M.

DROP PROCEDURE sp_che115_carga2018;
CREATE PROCEDURE sp_che115_carga2018(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
let _fec_aa_ini     = "01/01/2018";	--para sacar cancelada o anulada en el periodo
let _per_ini        = "2018-01";
let _per_ini_ap     = "2017-01";
let _ano            = a_periodo[1,4];  --2018
let _ano            = _ano - 1;		   --2017
let _per_fin_ap     = _ano || a_periodo[5,7]; --2017-01

let _per_fin_dic    = "2017-12";         
let _fecha_aa_ini   = sp_sis36(_per_ini);    --31/01/2018	 
let _fecha_aa       = sp_sis36(a_periodo);	 --31/01/2018 
let _fecha_ap_ini   = sp_sis36(_per_ini_ap); --31/01/2017 
let _fecha_ap       = sp_sis36(_per_fin_ap); --31/12/2017
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2018',_fecha_aa,my_sessionid);
	...
	foreach
		...
		let _monto3 = 0.00;
		let _monto3 = _pri_dev_aa * _porc_partic_agt /100;		
		begin work;
		insert into fis_che115a(no_documento,pri_sus_pag,tipo,cod_agente,pri_pag) --pri_pag = corresponde a prima sus. devengada
		values (_no_documento,_monto2,3,_cod_agente,_monto3);
		commit work;
	end foreach	
end foreach
--***********************
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2017',_fecha_ap,my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
foreach
	...
	--Quitar facultativo a la prima cobrada
	...
	--Para Coaseguro Mayoritario, tomar solo nuestra participacion
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/01/2018' and '31/12/2018';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/01/2018' and _fecha_anulado <= '31/12/2018' then
					let _monto_dev = 0;
				end if
			end if			
		else
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2018',_fecha_aa, my_sessionid);
	call sp_dev06f(_no_documento,_fecha_aa,'01/01/2018',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
	...
	foreach
		...
		let _monto3 = _pri_devengada * _porc_partic_agt /100;
		begin work;
		--insert into fis_che115a(no_documento, pri_can_ap, tipo,cod_agente)
		--values (_no_documento, _monto2, 8,_cod_agente);
		insert into fis_che115a(no_documento, pri_can_ap, tipo,cod_agente,pri_dev)
		values (_no_documento, _monto2, 8,_cod_agente,_monto3);
		commit work;
	end foreach
end foreach	 
--**********************************
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2017','31/12/2017',my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
foreach
	select no_documento,
		   ...
		   sum(pri_pag)			    --   prima suscrita devengada
	  into _no_documento,
		   _cod_agente,
		   _sin_pen_aa,
		   _sin_pen_dic,
	...
	if _cod_ramo = '023' then
		...
		let _monto_90_aa   = _monto_90_aa   * (70 / 100);	--Quitar los riesgos catastroficos a prima cobrada neta
		let _pri_devengada = _pri_devengada * (70 / 100);	--Quitar los riesgos catastroficos a prima cobrada neta devengada
		let _pri_dev_aa    = _pri_dev_aa    * (70 / 100);	--Quitar los riesgos catastroficos a prima suscrita devengada
	end if
	...
	--************************************************
	--   Calculos para incremeto de PSP 2018 vs 2017
	--************************************************
		let _pri_sus_aa_tmp = 0;
		let _pri_sus_ap_tmp = 0;		
		...
		let _prima_cobrada   = _monto_90_aa;	--prima cobrada neta
		let _pri_susc_dev_aa = _pri_devengada;	--prima cobrada neta devengada
		let _pri_susc_dev_ap = _pri_dev_aa;     --prima suscrita devengada

		{-- Prima Suscrita Devengada	Año Actual
		let _porc_res_mat    = 100 - _porc_res_mat;
		--let _pri_susc_dev_aa = _pri_sus_aa_tmp * _porc_res_mat / 100;
		let _pri_susc_dev_aa = _prima_cobrada * _porc_res_mat / 100;}

		if _pri_sus_aa_tmp is null then
			let _pri_sus_aa_tmp = 0;
		end if

		-- Prima Suscrita Devengada	Año Pasado
...
{		if _prima_cobrada <> 0 then
			let _siniestralidad = (_sini_incu / _prima_cobrada) * 100;
		else
			--continue foreach;
		end if}
				 ...
				 where periodo  = a_periodo
				   ...
				   and _siniestralidad between rango_inicial and rango_final;
				    ...
				    if _porcentaje <> 0 then
					    ...
					    --let _prima_beneficio = _pri_susc_aa * (_porcentaje/100); --ESTE BASADO EN PRIMA SUSCRITA
					    --let _prima_beneficio = _prima_cobrada * (_porcentaje/100);
						--let _prima_beneficio = _pri_devengada * (_porcentaje/100);
