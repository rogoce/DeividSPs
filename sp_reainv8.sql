-- Cambio en distribucion de reaseguro, restore serie 2020
-- 
-- Creado     : 17/11/2021 - Autor: Amado Perez M
--
-- SIS v.2.0 - DEIVID, S.A.

drop procedure sp_reainv8;

create procedure sp_reainv8()
 returning integer,
           char(200);
...
set debug file to "sp_reainv8.trc";
trace on;
...
foreach WITH HOLD
	select no_poliza,
	  ...
	  from camrea2
	 where actualizado = 0
	--   and no_documento = '0120-00488-01'
	   and tipo = 1
	 order by 1,3,2
	...
	foreach
		select a.no_unidad
		 ...
		 where a.no_poliza = b.no_poliza
		   ...
		   and (a.no_endoso = '00000' or b.cod_endomov = '004')
		...
		foreach 
		    select no_endoso 
             ...
             where no_poliza = _no_poliza
               and periodo >= '2020-06'
			   and cod_endomov not in ('015', '010', '012', '013')
    ...
    -- Corrigiendo distribución de reaseguro en reclamos
	foreach
		select no_reclamo
	      into _no_reclamo
          from recrcmae
	    ...
	    call sp_sis18cam(_no_reclamo) returning _error, _error_desc; --Procedure que crea el reaseguro recreaco, recreafa, rectrrea, rectrref
	    ...
	    if _error = 0 then
			 FOREACH
				select distinct sac_notrx 
				  into _sac_notrx
				from sac999:reacompasie where no_registro in (
				select no_registro from sac999:reacomp a, rectrmae b 
				 where a.no_tranrec  = b.no_tranrec
			       and b.no_reclamo  = _no_reclamo
				   and a.periodo >= '2020-06'
