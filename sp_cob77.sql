-- Procedimiento que Genera el Saldo para un Documento a una Fecha
-- 
...
DROP PROCEDURE sp_cob77;

CREATE PROCEDURE "informix".sp_cob77(
a_compania     CHAR(3), 
a_sucursal     CHAR(3), 
...
FOREACH
 SELECT prima_bruta
  ...
  WHERE no_documento   = a_no_documento	-- Facturas de la Poliza
	...
	AND activa         = 1
    AND fecha_emision  = a_fecha        -- Hechas durante y antes de la fecha seleccionada
		LET v_por_vencer = v_por_vencer + _monto;
END FOREACH
...
FOREACH
 SELECT monto
  ...
  WHERE doc_remesa   = a_no_documento	-- Recibos de la Poliza
    ...
    AND tipo_mov     IN ('P', 'N')		-- Pago de Prima(P) y Notas de Credito(N)
    AND fecha        = a_fecha          -- Hechas durante y antes de la fecha seleccionada
		LET v_exigible = v_exigible + _monto;
END FOREACH
...
LET v_corriente = _monto_devolucion;

-- Anulacion de Cheques de Devolucion de Primas
...
LET v_corriente = v_corriente - _monto_devolucion;
