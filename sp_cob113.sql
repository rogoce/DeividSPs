-- Procedimiento que Genera los Cobros por Cobrador	semanal, por cobra poliza detallado por poliza
-- 
-- Creado    : 25/06/2003 - Autor: Marquelda Valdelamar 
-- Modificado: 25/06/2003 - Autor: Marquelda Valdelamar

DROP PROCEDURE sp_cob113;

CREATE PROCEDURE "informix".sp_cob113(
a_compania CHAR(3),
a_agencia  CHAR(3),
a_periodo  CHAR(7)
...
FOREACH
 SELECT doc_remesa, 
  ...
  WHERE actualizado  = 1			              -- Recibo este actualizado
    ...
    AND periodo      = a_periodo                  -- Periodo de Seleccion
 ...
 SELECT	nombre_cobrador,
   ...
   INTO	_nombre_cobrador,
		...
		_doc_poliza,
	   	_nombre_cobra
   FROM	tmp_moros
 GROUP BY nombre_cobra , nombre_cobrador, no_documento
 ORDER BY nombre_cobra , nombre_cobrador, no_documento  
