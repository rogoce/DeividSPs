...
drop procedure sp_cob309a;

create procedure "informix".sp_cob309a(a_no_poliza char(10), a_cod_agente char(5))
...
if _tipo_agente <> 'A' or _status_lic <> 'A' then	--Clausula 6
	return 0,'El Corredor no tiene la licencia activa o no es tipo Corredor Normal';
end if
...
if _cnt_canc <> 0 then
	return 0,'La Póliza fue Cancelada';
end if

if _status_poliza <> 1 then
	return 0,'La Póliza no esta Vigente';
end if
...
if _pago_comis_ade = 0 then 
	return 0,'No tiene el flag de Adelanto de Comisión';
end if
...
if _cod_ramo not in ('018','019') then  
	...
	-- Numero de Pagos
	if _no_pagos > 8 and a_cod_agente <> '00270' then
		return 0,'La Cantidad de pagos es mayor a 8';
	elif a_cod_agente = '00270' and _no_pagos > 10 then
		return 0,'La Cantidad máxima de pagos para este corredor es 10';
	end if
...
if _cod_formapag not in ("003", "005", "008") then
	return 0,'No es forma de pago Electronico o Corredor';
end if

-- Frontings

if _fronting = 1 then
	return 0,'La Poliza es Fronting';
end if

-- Polizas del Estado

if _cod_grupo in ("00000", "1000") then
	return 0,'La Póliza es del Estado';
end if

-- Coaseguro Minoritario

if _cod_tipoprod = "002" then
	return 0,'La Póliza es Coaseguro Minoritario';
end if

-- Si Aplica para el Pago

return 1,'La Póliza Aplica';
