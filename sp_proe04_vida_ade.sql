...
drop procedure sp_proe04_vida;
create procedure sp_proe04_vida(a_poliza char(10),a_unidad char(5),a_ruta char(5) default "",a_suma dec(16,2),a_cia char(3))
returning	integer;
...
define _cant,_wf_aprob				smallint;
...
define ld_suma_plenos,_suma_adelanto		dec(16,2);
...
define ld_letra,ld_prima_acum,_suma_unidad				dec(16,2);
define ld_suma,_suma_queda	dec(16,2); 
define _prima_dif        	dec(16,2);
define _suma_dif,_porc_riesgo,_valor_recargo          dec(16,2);
...
on exception set _error,_error_isam,_error_desc
    if _error = -530 then
		let _error = 341;
		if _nueva_renov <> "N" then	--solo renovadas por prima insuficiente.
...
foreach
	...
	if ls_cober_reas in('020','051') then	--COBERTURAS DE REASEGURO VIDA IND. Y VIDA ADELANTO 35%
		foreach
