...
drop procedure sp_par296u;
create procedure "informix".sp_par296u(a_no_registro char(10))
returning integer,
		  char(100);
...
define _cod_subramo_o     	char(3);
define _cod_coasegur		char(3);
define _centro_costo		char(3);
define _cod_tipotran		char(3);
define _cod_subramo     	char(3);
define _cod_ramo_o			char(3);
...
if _tipo_registro = 1 then -- Produccion
	...
	foreach
		...
		select nvl(cod_subramo_e,_cod_subramo_o)
		  into _cod_subramo
		  from prdequiph
		 where cod_ramo_o = _cod_ramo_o
		   and cod_subramo_o = _cod_subramo_o
		   and cod_ramo_e = _cod_ramo;
		...
		if _cant_reaseguro = 0 and _prima_suscrita <> 0.00 Then
			drop table if exists tmp_reas;
	...
	foreach
		...
		if _bouquet = 1 then
			if _porc_proporcion = 0 then		
				return 1, "% Proporcion es cero para la Remesa: " || _no_remesa || " Renglon: " || _renglon;				 
			end if
	...
	if _cod_ramo_o in ("001","003") then

		let _bouquet = 1;
		
		if _cod_ramo_o = '001' then
			let _porc_inc = .70;
			let _porc_ter = .30;		
		else
			let _porc_inc = .90;
					  ...
					  where no_tranrec     = _no_tranrec
						...
						if _monto <> 0.00 Then
							...
							if _tipo_transaccion = 4 then	-- Pago
								...
								let _cuenta    = sp_sis15('RIPDRES', '06', _no_poliza,_no_unidad);
								let _tipo_comp = 2;

							elif _tipo_transaccion = 5 then	-- Salvamento

								let _cuenta    = sp_sis15('RIPDRESAL', '06',_no_poliza,_no_unidad);
								let _tipo_comp = 3;

							elif _tipo_transaccion = 6 then	-- Recupero

								let _cuenta    = sp_sis15('RIPDREREC', '06', _no_poliza,_no_unidad);
								let _tipo_comp = 4;

			   				elif _tipo_transaccion = 7 then	-- Deducible

								let _cuenta    = sp_sis15('RIPDREDED', '06', _no_poliza,_no_unidad);
								let _tipo_comp = 5;
