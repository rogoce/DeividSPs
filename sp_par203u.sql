...
--execute procedure sp_par203u('')

drop procedure sp_par203u;

create procedure "informix".sp_par203u(a_no_remesa char(10))
returning integer,
		  char(100);
...
define _prima_neta_c       	dec(16,2);
define _prima_neta       	dec(16,2);
define _cod_tipoprod     	char(3);  
define _tipo_produccion  	smallint; 
define _no_poliza        	char(10); 
define _monto_descontado_c	dec(16,2);
...
define _comis_manual_c     	dec(16,2);
...
foreach
	...
	select cod_tipoprod,
 	...
 	if _cod_ramo = '024' then
		...
		if _tipo_mov         = 'M' AND
		   _monto_descontado_c <> 0  THEN
			let _monto_banco = 0 - _monto_descontado_c;
		elif _tipo_mov       = 'X' then
			let _monto_banco = _prima_neta_c;
		else
			let _monto_banco = _monto_c - _monto_descontado_c;
		end if

		IF _tipo_mov in ('P', 'N','X') THEN -- Pago a Prima, Nota Credito
			foreach
				select r.cod_ramo,
					   sum(distinct porc_proporcion)
				  into _cod_ramo,
					   _porc_proporcion
				  from cobreaco c, reacobre r
				...
				let _monto = _monto_c * (_porc_proporcion / 100);
				let _impuesto = _impuesto_c * (_porc_proporcion / 100);
				let _prima_neta = _prima_neta_c * (_porc_proporcion / 100);
				let _monto_descontado = _monto_descontado_c * (_porc_proporcion / 100);
				...
				IF _tipo_mov in ('P', 'N') then
					...
					IF _tipo_produccion = 3 THEN 
						LET _cuenta = sp_sis15('PACXCC','06', _no_poliza,_no_unidad);  -- Coaseguro Minoritario
					ELIF _tipo_produccion = 4 THEN 
						LET _cuenta = sp_sis15('PAPXCRA','06', _no_poliza,_no_unidad);  -- Reaseguro Asumido
					ELSE						 
						LET _cuenta = sp_sis15('PAPXCSD','06', _no_poliza,_no_unidad);  -- Produccion Directa
					END IF
						...
						foreach
						 select	porc_comis_agt,
						  ...
						  where	no_remesa = a_no_remesa
							...
							let _comis_manual = _comis_manual_c * (_porc_proporcion/100);
							...
							if _porc_comis_agt <> 0 THEN
								...
								if _monto <> 0.00 Then
									...
									Let _cuenta    = sp_sis15('PPCOMXPCO','06', _no_poliza,_no_unidad);  
									CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
								...
								if _monto <> 0.00 Then
									...
									Let _cuenta    = sp_sis15('CPCXPAUX','06', _no_poliza,_no_unidad); 
									CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
									CALL sp_sis86(a_no_remesa, _renglon, _cuenta, _cod_auxiliar, _debito, _credito);
									...
									if _adelanto_comis = 1 then
										...
										let _cuenta    = sp_sis15('CPCADECOM','06', _no_poliza,_no_unidad); 
										call sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
										call sp_sis86(a_no_remesa, _renglon, _cuenta, _cod_auxiliar, _debito, _credito);
							...
							If _monto_descontado <> 0.00 Then
								...
								let _cuenta = sp_sis15('CPCXPAUX','06', _no_poliza,_no_unidad);  -- Comision por Pagar
							...
							select porc_partic_coas,
							 ...
							 where no_poliza    = _no_poliza
							   and cod_coasegur <> _cod_lider
								...
								Let _cuenta = sp_sis15("PPCOAMDIF",'06', _no_poliza,_no_unidad);    
								CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
								CALL sp_sis86(a_no_remesa, _renglon, _cuenta, _cod_auxiliar, _debito, _credito);
								...
								let _cuenta = sp_sis15("PPCOASXP",'06', _no_poliza,_no_unidad);    
								call sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
								call sp_sis86(a_no_remesa, _renglon, _cuenta, _cod_auxiliar, _debito, _credito);
...
--*************************************************************************************************************************************
		...
		ELIF _tipo_mov = 'D' THEN -- Pago de Deducible
			...
			IF   _tipo_produccion = 4 THEN -- Reaseguro Asumido
				...
				LET  _cuenta  = sp_sis15('SGPDDRA',  '06', _no_poliza,_no_unidad); 
				LET  _debito  = 0;
				LET  _credito = _monto;
				CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
			...
			ELIF _tipo_produccion = 2 THEN -- Coaseguro Mayoritario
				...
				LET  _cuenta  = sp_sis15('SGPDDSD',  '06', _no_poliza,_no_unidad); 
				LET  _debito  = 0;
				LET  _credito = _monto;
				CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
				...
				foreach
					...
					LET  _cuenta  = sp_sis15('SARXCC',  '06', _no_poliza,_no_unidad); 
					LET  _debito  = 0;
					LET  _credito = _monto;
					CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
					CALL sp_sis86(a_no_remesa, _renglon, _cuenta, _cod_auxiliar, _debito, _credito);
		...
		ELIF _tipo_mov = 'S' THEN -- Pago de Salvamento
			...
			IF   _tipo_produccion = 2 THEN -- Coaseguro Mayoritario
				...
				LET  _monto   = _valor_pago / 100 * _porc_partic; 
				LET  _cuenta  = sp_sis15('SISAL',  '06', _no_poliza,_no_unidad); 
				LET  _debito  = 0;
				LET  _credito = _monto;
				CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
		...
		ELIF _tipo_mov = 'R' THEN -- Pago de Recupero
			...
			IF   _tipo_produccion = 2 THEN -- Coaseguro Mayoritario
				...
				LET  _monto   = _valor_pago / 100 * _porc_partic; 
				LET  _cuenta  = sp_sis15('SIREC',  '06', _no_poliza,_no_unidad); 
				LET  _debito  = 0;
				LET  _credito = _monto;
				CALL sp_sis16(a_no_remesa, _renglon, _cuenta, _debito, _credito);
