-- Reporte Pool de Endoso - Logistica - CASA MATRIZ
-- Creado    : 12/04/2017 - Autor: Henry Giron
DROP PROCEDURE sp_log028;
CREATE PROCEDURE "informix".sp_log028(a_desde DATE,a_hasta DATE,a_sucursal CHAR(255))
...
drop table if exists tmp_log028;

Create Temp Table tmp_log028(
...
Foreach
select numero,fecha_adicion,usuario_captura 
  into _numero,_fecha_impresion,_user_imprimio
  from logcaja0 
 where (fecha_adicion >= a_desde ) AND
       (fecha_adicion <= a_hasta ) AND     
	   (activo <> 4)
