...
define _tipo_pago,_fronting       smallint;
...
define _monto_bono_det dec(16,2); 
define _porc_bono_det  dec(16,2);
...
foreach
	select e.no_documento,
	 ...
	 where cod_compania = '001'
	   ...
	   and e.cod_ramo = '019'
	   and e.cod_grupo not in ("00000", "1000")
	...
	if _cod_tipoprod in ("004") AND _motivo = '' then   -- Excluir Reaseguro Asumido   
	--    continue foreach;
		let _motivo  = 'Poliza con Reaseguro Asumido.';
	...
	{if _prima_cobrada < _prima_mensual then
		let _motivo  = 'Poliza no se pago su primera letra.';
	end if}

	if _cnt > 0  AND _motivo = '' then			                
		--continue foreach;
		let _motivo  = 'Poliza Rehabilitada.';
	end if 	 	
				
	if _cod_formapag = '006'  AND _motivo = '' then	--Pago Voluntario	
		...
		if _saldo = 0 or _cod_perpago = '007' then	                    --  Pto-5 Voluntario excepto sin saldo  , EMAIL:ASTANCIO 14/03/2018 tomar periocidad SEMESTRAL 
		   ...
		   --continue foreach;
		   let _motivo  = 'Poliza pago voluntario con saldo.';		   
	   end if
	end if		
	...
	foreach
		...
		IF _tipo_agente <> "A"  AND _motivo = '' then	       -- Solo Corredores
			let _motivo  = 'Poliza no aplica por tipo agente.';	
		END IF

		IF _estatus_licencia <> "A"  AND _motivo = '' then     -- El Corredor Debe Estar Activo
			let _motivo = ''; -- = 'Poliza de corredor con licencia inactiva.';	-- Al estar activo pagar BONO
		END IF
		...
		select nombre
		 ...
		 where cod_cliente = _cod_contratante;		 		 
			...
			if _prima_cobrada < _prima_mensual then
				if _prima_cobrada - _prima_mensual >= 0.05 then		-- KLITUMA 24/09/2018 Rango de Holgura US $0.05
					let _motivo  = 'Poliza no se pago su primera letra.';
				end if
			end if	 		 
			...
			update chqbono019 
			   set prima_sus_nva = prima_sus_nva + _prima_sus_agt
			 where cod_agente  = _cod_agente 
...
foreach
	...
	if _monto_bono <> 0 then  	
		...
		let _monto_bono_det = 0.00;
		let _porc_bono_det  = 0.00;
		let _prima_sus_agt  = 0.00;
		let _dif_redondeo   = _monto_bono;
		
		foreach
			select no_documento,prima_sus_nva
			  into _no_documento,_prima_sus_agt
			  from chqbono019 
			 where cod_agente = _cod_agente
			   and periodo    = a_periodo
			   and motivo     = ''
			 order by prima_sus_nva asc
			   
			   let _monto_bono_det = round((_monto_bono * round((_prima_sus_agt/_prima_neta),4)),2);
			   let _porc_bono_det  = round(round((_prima_sus_agt/_prima_neta),4)*100,2);
			   let _dif_redondeo   = _dif_redondeo - _monto_bono_det;
			   
			   if _dif_redondeo < 0.03 then  --- diferencia de redondeo hasta 3cts se aplica a la ultima RSERRACI 04/05/2021 HG
					let _monto_bono_det = _monto_bono_det + round(_dif_redondeo,2);
			   end if

			 update chqbono019  
				set monto_bono = _monto_bono_det,  
					porc_bono  = _porc_bono_det, 
					aplica = 1  
			  where cod_agente = _cod_agente
				and periodo    = a_periodo
				and motivo     = ''
