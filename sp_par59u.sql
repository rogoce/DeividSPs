...
drop procedure sp_par59u;
create procedure sp_par59u(a_no_poliza char(10), a_no_endoso char(5))
returning integer,
		  char(100);
...
if _cod_endomov = '018' then	--Si la factura es cambio de coaseguro may, entonces se debe sacar el tipo de produccion de emision.
	Let _cod_tipoprod = _cod_tipoprod2;
end if
...
foreach
	...
	elif _cod_ramo in ('024') then	        -- Ramo combinado
		call sp_sis3en101(a_no_poliza,a_no_endoso) returning _error_cod, _error_desc;
	...
	If _tipo_produccion = 1 Or 
	   ...
	   _tipo_produccion = 3 Then
		...
		If _tipo_produccion = 2 THEN -- Coaseguro Mayoritario
			...
			foreach
				...
				if _monto <> 0.00 then
					...
					let _cuenta    = sp_sis15('PPCOAMDIF', '06', a_no_poliza, _no_unidad);   
			...
			if _prima_bruta <> 0.00 then
				...
				let _cuenta    = sp_sis15('PAPXCSD', '06', a_no_poliza, _no_unidad);
				call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
			end if			
			...
			if _prima_bruta <> 0.00 then
				...
				let _cuenta    = sp_sis15('PACXCC', '06', a_no_poliza, _no_unidad);   
				call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
			end if
		...
		if _prima_suscrita <> 0.00 then
			...
			Let _cuenta    = sp_sis15('PIPSSD' , '06', a_no_poliza, _no_unidad);
			call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
...
--			let _cuenta   = sp_sis15('PXCGASMA', '06', a_no_poliza, _no_unidad); -- Esta es la cuenta que falta

			let  _cuenta   = sp_sis15('PGGASMA', '06', a_no_poliza, _no_unidad); -- Esta es la cuenta que falta
			let  _debito   = _gastos_manejo;
			let  _credito  = 0;
			call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
--			CALL sp_par339(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _cod_auxiliar, _tipo_comp);

			let  _cuenta   = sp_sis15('PGGASMA', '06', a_no_poliza, _no_unidad);
			let  _debito   = 0;
	    ...
	    Foreach 
			...
			if _monto <> 0.00 then
				...
				let _cuenta    = sp_sis15('PGCOMCO', '06', a_no_poliza, _no_unidad);   
				call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
				...
				let _cuenta    = sp_sis15('PPCOMXPCO', '06', a_no_poliza, _no_unidad); 
				call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
			end if
	    end foreach
			...
			If _imp_gob = 1 Then -- Verifica si al Ramo se le Aplican los Impuestos
				...
				foreach
					...
					if _monto <> 0.00 then
						...
						let _cuenta    = sp_sis15(_cuenta_inc, '06', a_no_poliza, _no_unidad);    
						call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
						...
						let _cuenta    = sp_sis15(_cuenta_dan, '06', a_no_poliza, _no_unidad);   
						call sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
					end if
				end foreach
			end if
...
--trace on;
	...
	--	 WHERE no_poliza = a_no_poliza;
		...
		If _cantleg > 0 Then 
			...
			If _prima_bruta <> 0.00 Then
				...
				Let _cuenta    = sp_sis15('PCANCOBL8', '06', a_no_poliza, _no_unidad);
				CALL sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
				...
				Let _cuenta    = sp_sis15('PCANCOBL9', '06', a_no_poliza, _no_unidad);
				CALL sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
	...
	Elif _tipo_produccion = 4 THEN 	 
		...
		If _prima_suscrita <> 0.00 Then
			...
			Let _cuenta    = sp_sis15('PIPSRA' , '06', a_no_poliza, _no_unidad);
			Let _debito    = 0.00;
			...
			If _monto <> 0.00 Then
				...
				Let _cuenta    = sp_sis15('PPCPRA' , '06', a_no_poliza, _no_unidad);
				Let _debito    = _monto;
				Let _credito   = 0.00;
				Let _tipo_comp = 4;
				CALL sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
			...
			If _monto <> 0.00 Then
				Let _monto2    = _monto2 - _monto;
				Let _cuenta    = sp_sis15('PPIPRA' , '06', a_no_poliza, _no_unidad);
				Let _debito    = _monto;
				Let _credito   = 0.00;
				Let _tipo_comp = 4;
				CALL sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
			...
			Let _cuenta    = sp_sis15('PAPXCRA' , '06', a_no_poliza, _no_unidad);
			Let _debito    = _monto2;
			Let _credito   = 0.00;
			Let _tipo_comp = 4;
			CALL sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
	    ...
	    -- Comprobante de Retrocesion
  		...
  		Foreach
			...
			If _monto <> 0.00 Then
				...
				Let _monto3    = _monto;
				Let _cuenta    = sp_sis15('PGREARET', '06', a_no_poliza, _no_unidad);   
				Let _debito    = _monto;
				Let _credito   = 0.00;
				Let _tipo_comp = 5;
				CALL sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
				...
				If _monto2 <> 0.00 Then
					...
					Let _cuenta    = sp_sis15('PICGRET', '06', a_no_poliza, _no_unidad);   
					Let _debito    = 0.00;
				...
				If _monto2 <> 0.00 Then
					Let _monto3    = _monto3 - _monto2;
					Let _cuenta    = sp_sis15('PIIRRET', '06', a_no_poliza, _no_unidad);   
					Let _debito    = 0.00;
					Let _credito   = _monto2;
					Let _tipo_comp = 5;
					CALL sp_par60(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
