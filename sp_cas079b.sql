--polizas con forma de pago ancon y existen en estructura de ach o visa.

drop procedure sp_cas079b;

create procedure "informix".sp_cas079b()
returning smallint,
          char(20),
          char(3),
          char(50),
 ...
 select no_documento 
  ...
  where cod_formapag = "006"   --forma de pago ANCON
   ...
   group by 1
	...
	if _estatus <> 3 then	   --solo vencidas
		continue foreach;
	end if
	...
	if _cod_formapag <> "006" then --solo formapag ancon
		continue foreach;
	end if
	...
	if _cantidad = 0 then  --no existe en callcenter
