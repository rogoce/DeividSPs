...
-- modificado: 04/09/2006 - Autor: Demetrio Hurtado Almanza

				-- Las primas a reflejar seran las primas netas correspondientes a las parte de Ancon.

-- SIS v.2.0 - DEIVID, S.A.

DROP PROCEDURE sp_cob06;

CREATE PROCEDURE "informix".sp_cob06(
  ...
  WHERE	no_documento   = a_no_documento -- Facturas de la Poliza
	...
	-- FIN Cambio de Distribución de Morosidad por Facturas en Positivo
		...
		FOR _ciclo = 1 TO _no_pagos
			...
			ELIF _dias >= 0 AND _dias <= 85 THEN
		        LET v_corriente = v_corriente   + _prima_bruta;
			ELIF _dias > 85 AND _dias <= 170 THEN
			    LET v_monto_30 = v_monto_30     + _prima_bruta;
			ELIF _dias > 170 AND _dias <= 255 THEN
...
-- Eliminar el Impuesto y dejar solo la participacion de Ancon
...
let _saldo_neto      = v_saldo      / (1 + (_porc_impuesto / 100));
let _por_vencer_neto = v_por_vencer / (1 + (_porc_impuesto / 100));
let _exigible_neto   = v_exigible   / (1 + (_porc_impuesto / 100));
let _corriente_neto  = v_corriente  / (1 + (_porc_impuesto / 100));
let _monto_30_neto   = v_monto_30   / (1 + (_porc_impuesto / 100));
let _monto_60_neto   = v_monto_60   / (1 + (_porc_impuesto / 100));
let _monto_90_neto   = v_monto_90   / (1 + (_porc_impuesto / 100));
