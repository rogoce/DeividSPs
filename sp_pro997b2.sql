...
DROP procedure sp_pro997b2;
CREATE PROCEDURE sp_pro997b2(a_cia CHAR(3),a_agencia CHAR(3),a_codsucursal CHAR(255) DEFAULT "*",a_corredor char(255) DEFAULT "*",a_codramo CHAR(255) DEFAULT "*",a_subramo CHAR(255) DEFAULT "*",a_periodo1 DATE, a_cod_cliente CHAR(255) DEFAULT "*", a_no_documento CHAR(255) DEFAULT "*", a_estado smallint DEFAULT 0) 
RETURNING CHAR(3),
 		  CHAR(50),
 		  CHAR(20),
          DATE,
...
FOREACH
 ...
 {   SELECT count(*)
    ...
    SELECT count(distinct cli.nombre)
	  INTO _cant_ase
      FROM endedmae mae
	 INNER JOIN endeduni u ON mae.no_poliza = u.no_poliza AND mae.no_endoso = u.no_endoso 
	 INNER JOIN emipomae emi ON emi.no_poliza = mae.no_poliza AND mae.actualizado = 1 AND emi.estatus_poliza in( 1,3) 
	 INNER JOIN emipouni uni ON uni.no_poliza = emi.no_poliza AND uni.fecha_emision <= a_periodo1 AND ((uni.activo = 1) or (uni.activo = 0 AND uni.no_activo_desde >= a_periodo1))
	 INNER JOIN cliclien cli ON cli.cod_cliente = uni.cod_asegurado 
	 WHERE mae.no_poliza = v_nopoliza  and mae.vigencia_inic <= a_periodo1 and mae.date_added <= a_periodo1 ;
 --and cli.date_added <= a_periodo1 	   	   
    ...
    end if
{
    ...
    SELECT COUNT( distinct cli.nombre ) 
	 ...
	 INNER JOIN endeduni u ON mae.no_poliza = u.no_poliza AND mae.no_endoso = u.no_endoso 
	 INNER JOIN emipomae emi ON emi.no_poliza = mae.no_poliza AND mae.actualizado = 1 AND emi.estatus_poliza in( 1,3) 
	 INNER JOIN emidepen uni ON uni.no_poliza = emi.no_poliza AND ((uni.activo = 1) or (uni.activo = 0 AND uni.no_activo_desde >= a_periodo1))
	 INNER JOIN cliclien cli ON cli.cod_cliente = uni.cod_cliente 
	 WHERE mae.no_poliza = v_nopoliza  and mae.vigencia_inic <= a_periodo1 and mae.date_added <= a_periodo1 ;
...
FOREACH
    SELECT distinct uni.cod_asegurado
	  INTO _cod_asegurado
      FROM endedmae mae
	 INNER JOIN endeduni u ON mae.no_poliza = u.no_poliza AND mae.no_endoso = u.no_endoso 
	 INNER JOIN emipomae emi ON emi.no_poliza = mae.no_poliza AND mae.actualizado = 1 AND emi.estatus_poliza in( 1,3) 
