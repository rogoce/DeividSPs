...
-- Modificado: 01/03/2002 - Autor: Marquelda Valdelamar
-- SIS v.2.0 - DEIVID, S.A.

--DROP PROCEDURE sp_pa51c;		

CREATE PROCEDURE "informix".sp_pa51c(
a_compania   CHAR(3),
a_agencia    CHAR(3),
a_fecha      DATE
...
FOREACH 
 SELECT no_documento
  ...
  WHERE cod_compania       = a_compania		   -- Seleccion por Compania
	...
	AND cod_tipoprod       <> _cod_tipoprod4
	...
	FOREACH
	 SELECT	no_poliza,
	  ...
	  WHERE no_documento       = _doc_poliza
		AND actualizado        = 1			   	   -- Poliza este actualizada
		AND cod_tipoprod       <> _cod_tipoprod4   -- Reaseguro Asumido
...
-- Selecciona el Primer Acreedor de la Poliza
   ...
   FOREACH
	SELECT a.fecha, 
	 ...
	 WHERE a.doc_remesa     = _doc_poliza	-- Recibos de la Poliza
	   AND a.actualizado    = 1			    -- Recibo este actualizado
	   AND a.tipo_mov        IN ('P', 'N')		   
	   ANd a.periodo      = _periodo
	   AND a.no_poliza    = b.no_poliza
	   AND b.cod_tipoprod <> _cod_tipoprod4
	 ORDER BY fecha DESC
		EXIT FOREACH;
	END FOREACH
