...
--execute procedure sp_rec271('2017-02')

drop procedure sp_rec271;
create procedure sp_rec271(a_periodo char(8))
RETURNING	integer,
			varchar(100);
...
foreach
	select t.no_reclamo,
	  ...
	  into _no_reclamo,
		   _no_tranrec
	  from rectrmae t, rectrrea r, reacomae c
	 where t.no_tranrec = r.no_tranrec
	   and r.cod_contrato = c.cod_contrato
	   and t.periodo = a_periodo--'2017-02'
	   and t.numrecla[1,2] in ('02','20','23')
	   and c.tipo_contrato = 1
	   and t.fecha <> '12/02/2017'
	   and r.porc_partic_prima <> 30
	   and t.actualizado = 1
	...
	foreach
		...
		call sp_sac77(_notrx) returning _error_cod,_error_desc;
	end foreach
