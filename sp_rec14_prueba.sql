...
create procedure "informix".sp_rec14_prueba2(
a_compania	char(3),
a_agencia	char(3),
a_periodo1	char(7),
a_periodo2	char(7),
...
--let _tipo = sp_sis04(a_poliza);  -- separa los valores del string en una tabla de codigos		

foreach 
	select periodo1,periodo2
	  into _periodo1,_periodo2
	  from tmp_vigencia2
	  where seleccionado = 1

	foreach 
		select periodo, prima_suscrita,		
			   no_poliza
		  into _periodot, _prima_suscrita,
			   _no_poliza
		  from endedmae
		 where cod_compania = a_compania
		   and actualizado  = 1
           --and trim(no_documento) in (select trim(codigo) from tmp_codigos)
		   and periodo     >= _periodo1 
		   and periodo     <= _periodo2		   
		   
		  --call sp_rec14_vig_real(_no_poliza,_periodot) returning _periodo1r,_periodo2r;	 		  

		insert into tmp_montos(
				no_poliza,
				prima_suscrita,periodo1,periodo2)
		values(	_no_poliza,
				_prima_suscrita,_periodo1,_periodo2);		
--				_prima_suscrita,_periodo1r,_periodo2r);
	end foreach
...
foreach
	select no_poliza,
	 ...
	 where cod_compania = a_compania
	   and actualizado  = 1
	   and tipo_mov in ('P', 'N')
	   --and trim(doc_remesa) in (select trim(codigo) from tmp_codigos)
	   and periodo     >= _periodo1
	   and periodo     <= _periodo2
	   and renglon     <> 0
	...
	INSERT INTO tmp_montos(
	no_poliza,           
	prima_pagada,periodo1,periodo2
	)
	VALUES(
	_no_poliza,
    _prima_pagada,_periodo1,_periodo2	
--	_prima_pagada,_periodo1r,_periodo2r
	);
...
--let v_filtros = " Sucursal: " ||  trim(a_poliza);
--let _tipo = sp_sis04(a_poliza);  -- separa los valores del string en una tabla de codigos		
...
FOREACH 
 SELECT	incurrido_bruto,
   ...
   INTO	_incurrido_bruto,
		...
		_periodot
   FROM	tmp_sinis
   --where doc_poliza in (select codigo from tmp_codigos)
	...
	_salv_y_recup,
	_pago_y_ded,
	_var_reserva, _periodo1, _periodo2	
--	_var_reserva,_periodo1r,_periodo2r
	);
...
FOREACH 
 SELECT SUM(prima_suscrita),
   ...
   INTO	_prima_suscrita,
		...
		_no_poliza
   FROM tmp_montos
  GROUP BY no_poliza,periodo1,periodo2
