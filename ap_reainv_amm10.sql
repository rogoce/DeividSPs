...
--
drop procedure ap_reainv_amm10;
create procedure ap_reainv_amm10()
...
foreach
	select distinct no_poliza, no_unidad, no_documento
      into _no_poliza, _no_unidad, _no_documento
      from camrea2
	...
	if _no_unidad is not null and trim(_no_unidad) <> "" then
		...
		foreach
			...
			where rea.porc_partic_prima <> 5
			  and emi.no_poliza = _no_poliza
			union all	 
			select distinct rea.no_unidad
			  from emipomae emi
			inner join endedmae fac on fac.no_poliza = emi.no_poliza
			...
			where rea.porc_partic_prima <> 95
			  and emi.no_poliza = _no_poliza
			order by 1
