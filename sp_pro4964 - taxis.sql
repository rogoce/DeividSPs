DROP procedure sp_pro4964;
   CREATE procedure "informix".sp_pro4964(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
       ...
       FOREACH 
		SELECT 	a.no_unidad,          		 
            ...
            INTO _no_unidad,
            	 _suma_asegurada,
                 _suscrita_unidad				 
		   FROM emipouni a, emiauto b, emivehic c, emimodel d 
	...
	WHILE _3ciclo != 4
	 ...
	 --return _no_documento,_cod_ramo, _total_pri_sus,_sa_poliza with resume; 
		FOREACH WITH HOLD
			...
			(case when cod_ramo = '020' then (case when uso_auto = 'C' then 1  else 0 end )  else 0 end ) C11,
			(case when cod_ramo = '020' then (case when uso_auto = 'C' then 1  else 0 end )  else 0 end ) C12
			into _suma_asegurada,_no_unidad,_cobertura,_uso_auto,_prioridad,_primera_u,_c1,_c2,_c3,_c4,_c5,_c6,_c7,_c8,_c9,_c10,_c11,_c12
			 from tmp_unidad_v 
			WHERE no_documento = _no_documento
			  AND cod_ramo = _cod_ramo			 
			 order by prioridad
