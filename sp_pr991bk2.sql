...
--DROP PROCEDURE sp_pr991bk2;

CREATE PROCEDURE "informix".sp_pr991bk2(a_compania CHAR(03),a_periodo1 CHAR(07),a_periodo2 CHAR(07),a_agente CHAR(255) DEFAULT "*",a_tipo CHAR(2) DEFAULT "01")
...
select cod_contrato,nombre,nombre
  into a_tipo,s_tipo,m_contrato
  from reacontr
 where activo = 1
...
if a_tipo = "08" or a_tipo = "03" or a_tipo = "02" then --Cuaota parte acc y vida-50%Mapfre-RunOff
	...
	FOREACH
	  ...
	  order by reacoest.cod_coasegur,
			...
			--LET s_des_cod_clase = trim(s_des_clase)||" - "||trim(s_desc_contrato)||" - "||trim(s_cod_contrato) ;	
			  LET s_des_cod_clase = 'Saldo Cuenta Tecnica Trimestre '|| _trim_reas || '   ' || _anio_reas;
...
elif a_tipo = "06" then --Facilidad CAR
	LET s_renglon =	0 ;
	LET s_debito  =	0 ;
	LET s_credito =	0 ;
   ...
   {	  SELECT distinct reasegurador,p_partic,tipo,cod_ramo,cod_clase,fecha_transf,sum(monto)
	...
	ORDER BY reasegurador,p_partic,tipo,cod_ramo,cod_clase,fecha_transf}
	   ...
	   SELECT distinct reasegurador,p_partic,tipo,cod_ramo,cod_clase,fecha_transf,monto,no_remesa
		...
		if t_tipo = "01" then
			LET s_des_cod_clase = "Remesa Enviadas Al Reasegurador";
		elif t_tipo = "02" then
			LET s_des_cod_clase = "Remesa Recibidas del Reasegurador";
		else
			select descrip
...
FOREACH
  ...
  group by periodo1,periodo2,reasegurador,concepto2
  order by reasegurador,periodo1,periodo2,concepto2
