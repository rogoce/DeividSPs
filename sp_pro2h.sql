--------------------------------------------
---  SUMA ASEGURADA 1ER. EXCEDENTE X UBICACION ---
---  INCENDIO Y MULTIRIESGO POLIZA VIGENTES    ---
---  Creado por Amado Perez Noviembre 2003 	   ---
---  Corregido el 11/11/2003                   ---
---  Ref. Power Builder - d_sp_pro02
--------------------------------------------

--DROP procedure sp_pro02h;
CREATE procedure "informix".sp_pro02h(a_compania CHAR(3),a_agencia CHAR(03) DEFAULT "*",a_periodo DATE,a_codsucursal  CHAR(255) DEFAULT "*",a_codramo CHAR(255) DEFAULT "*")
   RETURNING CHAR(50),SMALLINT,DEC(16,2),DEC(16,2),
             DEC(16,2),DEC(16,2),SMALLINT,SMALLINT,CHAR(03),CHAR(45),CHAR(03),CHAR(45),
             DATE,VARCHAR(45),CHAR(255);
 ...
 BEGIN
	...
	DEFINE v_compania_nombre, v_ubicacion   CHAR(50);

    DEFINE v_codramo,v_codsucursal,v_subramo, _cod_ubica  CHAR(3);
    DEFINE v_desc_ramo,v_desc_subramo           CHAR(45);
	DEFINE descr_cia		 VARCHAR(45);
    ...
    DEFINE v_fecha_cancel, _fecha_cancelacion, _fecha_emision   DATE;
   ...
   CREATE TEMP TABLE temp_ubica
	  ...
	  CREATE INDEX iend2_temp_civil ON temp_civil(cod_ubica);
...
--SET DEBUG FILE TO "sp_pro02h.trc";
--TRACE ON;

       FOREACH

       SELECT a.no_poliza,e.prima_suscrita,e.prima_retenida,
              a.fecha_cancelacion,a.cod_ramo,a.cod_subramo,a.cod_sucursal,
              e.suma_asegurada,a.cod_tipoprod,a.fecha_cancelacion,e.no_endoso
	     INTO _no_poliza,v_prima_excedente2,v_prima_retenida,
	          v_fecha_cancel,v_codramo,v_subramo,v_codsucursal,
	          v_suma_asegurada2,v_cod_tipoprod,_fecha_cancelacion,_no_endoso
      ...
      FOREACH
       ...
       ORDER BY x.cod_ramo, x.cod_ubica
         ...
         SELECT prdsubra.nombre
                INTO v_desc_subramo
                FROM prdsubra
               WHERE prdsubra.cod_ramo = v_codramo
			     AND prdsubra.cod_subramo = v_subramo;
