...
drop procedure sp_sis198e;

create procedure "informix".sp_sis198e(a_documento CHAR(20))
returning integer, 
          char(100),
          char(30);
...
--SET DEBUG FILE TO "sp_sis198e.trc"; 
--trace on;
...
foreach
 ...
 foreach with hold
	 select cod_producto,
	   ...
	   from emipouni
	  where no_poliza = _no_poliza
	    and cod_producto in (select valor_auto from parautflot where tipo_valor = "cod_producto")
	 ...
	 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
		...
		DROP TABLE tmp_cobredet;
		DROP TABLE tmp_emiglofa;
		return -1, "cod_producto",_cod_producto;
	 end if
	 ...
	 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
	    ...
	    let _cant = 0;
	    select count(*) into _cant from parautflot where tipo_valor = "cod_ruta" and valor_flota = _cod_ruta;
		if _cant = 0 then
		    rollback work;
			DROP TABLE tmp_emireaco;
			DROP TABLE tmp_emifafac;
     ...
     foreach with hold
		...
		select valor_flota
		 ...
		 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
		    ...
		    let _cant = 0;
 		    select count(*) into _cant from parautflot where tipo_valor = "cod_cobertura" and valor_flota = _cod_cobertura;
 			if _cant = 0 then
			    rollback work;
				DROP TABLE tmp_emireaco;
				DROP TABLE tmp_emifafac;
     ...
     foreach with hold
        ...
        if _valor_flota is not null and TRIM(_valor_flota) <> "" then
			insert into tmp_emireaco 
 		    ...
 		    let _cant = 0;
 		    select count(*) into _cant from parautflot where tipo_valor = "cod_cober_reas" and valor_flota = _cod_cober_reas;
 			if _cant = 0 then
			    rollback work;
				DROP TABLE tmp_emireaco;
				DROP TABLE tmp_emifafac;
		 ...
		 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
		    ...
		    select count(*) into _cant from parautflot where tipo_valor = "cod_cobertura" and valor_flota = _cod_cobertura;
 			if _cant = 0 then
				...
				DROP TABLE tmp_cobredet;
				DROP TABLE tmp_emiglofa;
				return -1, "recreafa cod_cobertura",_cod_cobertura;
 			end if
		 end if
		...
		select valor_flota
		 ...
		 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
 			...
 			if _cant = 0 then
				...
				DROP TABLE tmp_cobredet;
				DROP TABLE tmp_emiglofa;
				return -1, "recreaco cod_contrato", _cod_contrato;
			end if
		 end if
			...
			select valor_flota
			 ...
			 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
 				...
 				if _cant = 0 then
					...
					DROP TABLE tmp_cobredet;
					DROP TABLE tmp_emiglofa;
					return -1, "rectrcob cod_cobertura", _cod_contrato;
				end if
			 end if
			...
			select valor_flota
			 ...
			 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
 				...
 				if _cant = 0 then
					...
					DROP TABLE tmp_cobredet;
					DROP TABLE tmp_emiglofa;
 					return -1, "recreaco cod_cober_reas", _cod_contrato;
 				end if
			 end if
		 end foreach
	   end foreach
