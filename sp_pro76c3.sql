...
drop procedure sp_pro76c3;

CREATE PROCEDURE "informix".sp_pro76c3(a_no_documento  varchar(20))
...
--trace on;
 ...
 --Seleccion de los Asegurados
   ...
   --Datos del corredor
			...
			Select email
			 ...
			 where cod_agente = _cod_agente
			   and tipo_correo = 'PER'
				...
				if _email_agtmail not like '%@%' or _email_agtmail not like '%.%' or _email_agtmail like '%/%' or _email_agtmail like '@%' or _email_agtmail like '% %' then
					continue foreach;
				end if
	...
	if _email_para is null or trim(_email_para) = '' then
		let _email_para = _email_ejecutivo;
	end if
	
	
	--let _email_para = 'fcoronado@asegurancon.com';
	--let _email_cc  = 'fcoronado@asegurancon.com';
