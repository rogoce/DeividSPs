--*****************************************************************************************
-- Procedimiento que genera el Reporte para convencion a Madrid-Marruecos 2019 para los corredores
--*****************************************************************************************
--CONVENCION Madrid-Marruecos 2019	*******************************************************************************************************
...
DROP PROCEDURE sp_che86mama;
CREATE PROCEDURE sp_che86mama(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
--return 0; --se detuvo la corrida 01/10/2018 Armando
...
let _per_ini      = "2018-10";
let _fecha_fin_ap = sp_sis36(_per_ini);	--31/10/2018

-- Periodo Final del Concurso

if a_periodo > "2019-09" then
	let a_periodo = "2019-09";
end if

-- Periodo Pasado
let _ano            = _per_ini[1,4];		  --2018
let _ano            = _ano - 1;				  --2017
let _per_ini_ap     = _ano || _per_ini[5,7];  --2017-10
let _ano            = a_periodo[1,4];		  --2019
let _ano            = _ano - 1;				  --2018
let _per_fin_ap     = _ano || a_periodo[5,7]; --2018-09

-- Diciembre
let _per_fin_dic    = _per_ini[1,4] || "-12"; --2018-12
-- Fechas de los Periodos
let _fecha_aa_ini = mdy(_per_ini[6,7], 1, _per_ini[1,4]);         --es del 01/10/2018
let _fecha_ap_ini = mdy(_per_ini_ap[6,7], 1, _per_ini_ap[1,4]);	  --es del 01/10/2017
let _fecha_aa     = sp_sis36(a_periodo);   --31/01/2019
let _fecha_ap     = sp_sis36(_per_fin_ap); --31/01/2018

SET ISOLATION TO DIRTY READ;

delete from milan08;
delete from fis_concursobk2;
...
let _nombre_tipo = "";
--**********************************************************************************************
-- Prima neta devengada periodo convencion 2018-10 A 2019-09
--**********************************************************************************************
...
foreach
	select doc_remesa
	  into _no_documento
	  from cobredet
	 where periodo     >= _per_ini			--2018-01
	   and periodo     <= a_periodo			--2018-12
	   and actualizado = 1
	   and tipo_mov    in ("P", "N")
	 group by doc_remesa
	 
	let _valor = sp_sis101a(_no_documento,'01/10/2018','30/09/2019', my_sessionid);
	call sp_dev06f(_no_documento,_fecha_aa,'01/10/2018',_fecha_aa) returning _valor,_mensaje,_pri_dev_aa;
	...
	foreach
		...
		begin work;
			insert into fis_concursobk2(no_documento, pri_dev_ap, tipo,cod_agente) --pri_pag = corresponde a prima neta cobrada devengada
			values (_no_documento, _monto2, 8,_cod_agente);
		commit work;
	end foreach
end foreach
--**********************************************************************************************
-- Prima Pagada periodo convencion 2018-10 A 2019-09
--**********************************************************************************************
foreach
	select doc_remesa,
		   sum(prima_neta)
	  into _no_documento,
		   _monto
	  from cobredet
	 where periodo     >= _per_ini			--2018-10
	   and periodo     <= a_periodo			--2019-09
	   and actualizado = 1
	   and tipo_mov    in ("P", "N")
	 group by doc_remesa
	...
	select sum(d.prima_neta * (c.porc_partic_prima/100) * (c.porc_proporcion/100))
	 ...
	 where c.no_remesa = d.no_remesa
	   ...
	   and d.periodo >= _per_ini			--2018-10
	   and d.periodo <= a_periodo			--2019-09
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/10/2018' and '30/09/2019';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/10/2018' and _fecha_anulado <= '30/09/2019' then
					let _monto_dev = 0;
				end if
			end if			
		else
	...
	let _valor = sp_sis101a(_no_documento,'01/10/2018','30/09/2019',my_sessionid); --se cambia a tabla fisica 05/06/2018
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
		...
		insert into fis_concursobk2(no_documento, pri_pag,tipo,cod_agente)
...
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Saliendo del Ciclo Prima Pagada periodo convencion...2.', CURRENT,CURRENT);
INSERT INTO fis_concursobk2
SELECT * FROM fis_concurso1;
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Entrando al Ciclo Group by Fis_Concurso...6.', CURRENT,CURRENT);
commit work;
--**********************************************************************************************
...
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Entrando al Ciclo Siniestros pagados año actual...3.', CURRENT,CURRENT);
 ...
 select doc_poliza,
   ...
   from tmp_sinis
	...
	foreach
		...
		insert into fis_concursobk2(no_documento, sin_pag_aa,tipo, cod_agente)
		values (_no_documento, _monto2, 3,_cod_agente);
	end foreach
end foreach
drop table tmp_sinis;

INSERT INTO log_jb (mensaje, hora,hora_real) values ('Saliendo del Ciclo Siniestros pagados año actual...3.', CURRENT,CURRENT);
--**********************************************************************************************
-- Siniestros Pendientes AÑO ACTUAL
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Entrando al Ciclo Siniestros pendientes año actual...4.', CURRENT,CURRENT);
...
foreach 
 ...
 having sum(variacion) > 0 
	...
	foreach
		...
		insert into fis_concursobk2(no_documento, sin_pen_aa, tipo, cod_agente)
		values (_no_documento, _monto2, 4, _cod_agente);
	end foreach
end foreach

INSERT INTO log_jb (mensaje, hora,hora_real) values ('Saliendo del Ciclo Siniestros pendientes año actual...4.', CURRENT,CURRENT);
---**********************************************************************************************
-- Polizas Nuevas ACTUAL a la fecha
---**********************************************************************************************
call sp_bo077conv(_fecha_aa_ini, _fecha_aa) returning _error, _error_desc;	--01/10/2017    -    31/01/2018

INSERT INTO log_jb (mensaje, hora,hora_real) values ('Entrando al Ciclo Nuevas año actual...5.', CURRENT,CURRENT);
foreach
		select no_documento,
			   sum(no_pol_nueva)
		  into _no_documento,
...
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Saliendo del Ciclo Nuevas año actual...5.', CURRENT,CURRENT);
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Entrando al Ciclo Group by Fis_Concurso...6.', CURRENT,CURRENT);
foreach
	select no_documento,
		   cod_agente,
   ...
   --rehabilitada o cancelada en el periodo del concurso no va
    select count(*)
     ...
     where no_poliza     = _no_poliza
       ...
       and fecha_emision >= '01/10/2018'
       and fecha_emision <= '30/09/2019';
...
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Saliendo del Ciclo Group by Fis_Concurso...9.', CURRENT,CURRENT);
--Aplicar Rango a los corredores tomando como base la Primas Suscrita año pasado periodo del concurso. (2016-10 a 2017-09)

INSERT INTO log_jb (mensaje, hora,hora_real) values ('Entrando al Ciclo ubicacion de agentes...10.', CURRENT,CURRENT);
...
INSERT INTO log_jb (mensaje, hora,hora_real) values ('Saliendo del Ciclo ubicacion de agentes...10.', CURRENT,CURRENT);
--COMMIT work;
end
return 0;
END PROCEDURE; 