-- Procedimiento que genera las cancelaciones por lote segun archivo enviado por Daivis 12/02/2026
-- 
-- Creado     : 24/10/2002 - Autor: Demetrio Hurtado Almanza
-- Modificado : 20/09/2016 - Autor: Armando Moreno M.
-- SIS v.2.0 - DEIVID, S.A.

drop procedure sp_par184_esp;
create procedure sp_par184_esp()
returning integer,
          char(100);
...
foreach	with hold
	select poliza,prima
	  into _no_documento,_saldo
	  from deivid_tmp:deivis_corp
	 where procesado = 0
	 order by poliza
	...
	begin work;
				...
				call sp_par130_esp(_no_poliza, _user_added, _saldo) returning _error, _descripcion, _no_endoso;
				...
				_no_poliza,
				current,
				"CANCELACION DE LA POLIZA POR SOL. ACREEDOR HIPOTECARIO",
				_user_added,
				_no_documento,
				null,
				0,
