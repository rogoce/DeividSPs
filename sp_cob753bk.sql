...
drop procedure sp_cob753bk;
create procedure sp_cob753bk()
returning integer,
          char(100);
...
foreach
	select b.fecha,
	 ...
	 where a.no_remesa = b.no_remesa
	   and b.actualizado  = 1
	   and trim(b.doc_remesa) in (select distinct trim(no_documento) from avisocanc where estatus not in ('Y','Z'))
