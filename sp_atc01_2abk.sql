drop procedure sp_atc01_2abk;
create procedure "informix".sp_atc01_2abk(
a_compania		char(3),
a_sucursal		char(3),
...
define _ded,_copago,_coaseguro,_ahorro decimal(16,2);
DEFINE _fecha_actual	    date;
DEFINE _cadena_fecha        CHAR(50);
DEFINE _periodo_fijo        CHAR(100);
define _cod_agente  		char(5);
define _pagadon				dec(16,2);
define _deduciblen			dec(16,2);
define _copagon				dec(16,2);
define _coaseguron			dec(16,2);
define _monto_no_cubierton	dec(16,2);
define _ahorron				dec(16,2);
define _cod_tipotrann		char(3);
define _cod_tipopagon		char(3);
define _gasto_inc_prov      dec(16,2);
...
let _no_unidad 			= '';
let _gasto_inc_prov 	= 0;
let _pago_prov	 		= 0;
let _monto_no_cubierto 	= 0;
...
--//////////////////////////////////////////////////////////////////////////
 ...
 select nombre,cedula
   into _nombre_asegurado,_cedula_pagador
   from cliclien
...
/* correcion para las cartas de salud*/
foreach
	 select pagado,
			deducible,
	        copago,
	  ...
	  where year(fecha) = a_ano
{
		if _cod_tipotrann = '004' and _cod_tipopagon = '003' then
			let _pago_prov	 = _pago_prov + _pagadon;
			let _monto_no_cubierto = _monto_no_cubierton + _ahorron + _monto_no_cubierto;
			--_monto_no_cubierto,
		end if 
		
		if _cod_tipotrann = '004' or _cod_tipotrann = '013' then
			let _monto_no_cubierto = _deduciblen + _copagon + _coaseguron + _monto_no_cubierto;
				if _cod_tipotrann = '013' and _cod_cliente = _cod_asegurado then
					let _monto_no_cubierto = _monto_no_cubierton + _ahorron + _monto_no_cubierto;
				elif _cod_tipotrann = '013' and _cod_cliente = _cod_asegurado then
					let _gasto_inc_prov = _monto_no_cubierton + _gasto_inc_prov;
				end if
		end if
		
		if _cod_tipotrann = '004' and _cod_tipopagon = '001' then
			let _gasto_inc_prov = _pagadon + _ahorron + _monto_no_cubierton + _gasto_inc_prov;
			--_monto_no_cubierto,
		end if 
	}	
		
			if _cod_tipotrann = '013' and trim(_cod_cliente) <> trim(_cod_asegurado) then --Declinar Reclamo Otros
				let _gasto_inc_prov =  _pagadon + _ahorron + _monto_no_cubierton + _gasto_inc_prov;
			elif _cod_tipotrann = '013' and trim(_cod_cliente) = trim(_cod_asegurado) then --Declinar Reclamo Asegurado
				let _monto_no_cubierto = _monto_no_cubierton +  _deduciblen + _copagon + _coaseguron + _ahorron + _monto_no_cubierto ;					
			elif _cod_tipotrann = '004' and _cod_tipopagon = '001' then -- Transacciones de Pago a Proveedor
				let _gasto_inc_prov =  _pagadon + _ahorron + _monto_no_cubierton  + _gasto_inc_prov;
				let _monto_no_cubierto = _deduciblen + _copagon + _coaseguron + _monto_no_cubierto;
			elif _cod_tipotrann = '004' and _cod_tipopagon = '003' then -- Transacciones de Pago a Asegurado
				let _pago_prov	 = _pago_prov + _pagadon;
				let _monto_no_cubierto = _deduciblen + _copagon + _coaseguron + _ahorron + _monto_no_cubierto + _monto_no_cubierton;
			end if
...
if _gasto_inc_prov is null then
	let _gasto_inc_prov = 0.00;
end if
...
--PERSONAS KATHERINE CESAR
--PATRIMONIALES JUAN SILVA
--AUTOMÃ“VIL SABISH CASTILLO
--OTROS LMORENO
let v_cedula_cartas = '';
let v_firma_cartas = '';

if _ramo_sis in (5,6,7,9) then
	--let v_firma_cartas = 'KCESAR';
	select valor_parametro 
	  into v_firma_cartas
	  from inspaag
	 where codigo_parametro = "firma_cd_1"; 
	
	select valor_parametro 
	  into v_cargo
	  from inspaag
	 where codigo_parametro = "cargo_cd_1";	
else
	{
	if _ramo_sis in (1,4) then
		--let v_firma_cartas = 'SCASTILL';
			select valor_parametro 
			  into v_firma_cartas
			  from inspaag
			 where codigo_parametro = "firma_cd_2"; 
			
			select valor_parametro 
			  into v_cargo
			  from inspaag
			 where codigo_parametro = "cargo_cd_2";	
	else
		if _ramo_sis in (99) then
			--let v_firma_cartas = 'JSILVA';
			select valor_parametro 
			  into v_firma_cartas
			  from inspaag
			 where codigo_parametro = "firma_cd_3"; 
			
			select valor_parametro 
			  into v_cargo
			  from inspaag
			 where codigo_parametro = "cargo_cd_3";				
	 -- SegÃºn ultima modificaciÃ³n por team de JEPEREZ 02/10/2020
		else --2,8,3
...
let v_fecha_genera = current;
    ...
    select nombre
	 ...
	 if _monto_no_cubierto is null then
	   let _monto_no_cubierto = 0.00;
	 end if
  ...
  return a_no_documento,
		 ...
		 v_fecha_max,  
		 abs(v_monto),
		 trim(_cedula_pagador), --trim(_cedula),
		 ...
		 trim(_cedula), --trim(_cedula_pagador),
		 _periodo_fijo,
		 _no_unidad,
