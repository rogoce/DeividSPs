...
define _cob_cont_otros		dec(16,2);
...
define _cob_otros,_ret_var	dec(16,2);
...
foreach
	...
	foreach					 -- se desglosa por tipo, buscar primero si es bouquet
			...
			if _facilidad_car = 1 then
				...
				if _ramo_sis = 1 then
					if v_cobertura in ('002','033','025') then
						...
						let _cob_cont_otros = _cob_cont_otros + v_prima2;
		...
		let v_prima_tipo = 0;
		let v_prima2 = 0;
		let _prima_total = v_prima_1 + _ret_casco + _ret_var + v_prima_ot + _sum_fac_car + v_prima_bq + v_prima_3 + _cob_otros + _cob_cont_otros + _cont_casco;
		...
		foreach
			...
			begin
				on exception in(-239)
				   update tmp_tabla
					  set cant_polizas   = cant_polizas   + 1,
						  ...
						  p_cont_otros   = p_cont_otros   + _cob_cont_otros * v_porcentaje/100,
				...
				values(	v_cod_tipo, 
						...
						_cob_cont_otros	* v_porcentaje/100,			 
