...
drop procedure sp_emite09b;

create procedure "informix".sp_emite09b(a_no_documento char(20)) 
...
define _act_cod_formapag    char(3);
...
--begin work;
	...
	foreach
		...
		inner join emipomae ant on ant.no_documento = act.no_documento and ant.vigencia_final = act.vigencia_inic 
		 ...
		 where act.no_factura not like '%-%'
		   and act.no_documento = a_no_documento
		   and act.actualizado = 0
		 order by act.no_documento
		
	/*	if _cod_formapag not in('006','008') and _act_cod_formapag not in('006','008') then
			update emipomae
	...
	*/
	...
	return 0,"Actualización Exitosa: "|| _no_documento;
	end
end procedure;
