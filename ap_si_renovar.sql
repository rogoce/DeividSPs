...
DROP PROCEDURE ap_si_renovar;
CREATE PROCEDURE ap_si_renovar() 
...
FOREACH
  SELECT emipomae.no_documento
   ...
   WHERE ( emipouni.no_poliza = emipomae.no_poliza ) and  
         ...
         ( ( emipouni.cod_producto in ('01961')) and
...
order by no_documento  
    ...
    FOREACH 	
	  SELECT emipomae.no_poliza,   
	   ...
	   WHERE ( emipouni.no_poliza = emipomae.no_poliza ) and  
			 ( prdprod.cod_producto = emipouni.cod_producto ) and  
             ( emipomae.no_poliza = _no_poliza )
			 
		if _estatus_poliza in (1,3) and _no_renovar = 1 then			 
