...
DROP PROCEDURE sp_imp02_esp;

CREATE PROCEDURE "informix".sp_imp02_esp(a_poliza CHAR(10), a_unidad CHAR(5),a_general INT)
...
IF a_general = 1 THEN
    ...
    FOREACH
	  SELECT y.prima, y.no_poliza,y.no_unidad, y.cod_cobertura 
		...
		AND z.acepta_desc = 1
		and x.no_unidad in (select no_unidad from endeduni where no_poliza = a_poliza and no_endoso = '00000')
	    and x.no_unidad in (select no_unidad from emipouni where no_poliza = a_poliza)
