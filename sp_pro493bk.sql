...
drop procedure sp_pro493bk;

create procedure "informix".sp_pro493bk(
v_poliza char(10),
v_endoso char(5),
v_factor dec(9,6))
...
if _tipo_produccion = 2 then  --Coas Mayoritario
...
if v_tipocalc = "004" then -- POR SALDO
	...
	if v_impuesto > 0 then
		...
		if v_tipo_mov = "4" or v_tipo_mov = "6" or v_tipo_mov = "1" then --Inclusion de unidades ,modificacion de unidades ,aumento de vig.
			select cod_ramo,
				   cod_subramo,
				   cod_origen
			  into _cod_ramo_i,
	...
	if v_tipo_mov = "3" Then --rehabilitacion de poliza
		let r_signo = 1;
	end if
	...
	--Canc,desc. pronto pag,aumento,reversar descuento pp,rehabil,disminucion de vig.
...
foreach
   ...
   --  Cargar Reaseguros Facultativos
	...
	foreach
		...
		if v_tipocalc = "005" or v_tipocalc = "006" then	--Perdida Total, Manual
		...
		elif v_tipocalc = "001" Then  --Prorrata
			select acepta_desc
	...
	if  v_tipocalc = "006" and v_tipo_mov = "2"  then  --Por saldo y cancelacion
...
if (v_tipocalc = "006" or v_tipocalc = "004") and _ramo_sis <> 5 Then	--Manual y Por saldo
	select prima_neta
	  into r_prima_neta
	  from endedmae
	 where no_poliza = v_poliza
	...
	if r_prima_anual <> 0 then
		if v_poliza <> '705916' and v_endoso <> '00005' then
			if r_prima_anual > 0 then
				let v_descto = -0.01;
			else
				let v_descto = +0.01;
