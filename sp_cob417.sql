-- Procedimiento que carga los pagos diarios de un corredor Felix ABADIA
-- Creado    : 20/09/2018 - Autor: Henry Giron
-- SIS v.2.0 - DEIVID, S.A.
-- execute procedure sp_cob417_ult('001','001','2018-01','02532','125')

DROP PROCEDURE sp_cob417_ult;
CREATE PROCEDURE sp_cob417_ult(a_compania CHAR(3), a_sucursal CHAR(3), a_periodo char(7), a_cod_agente CHAR(5) DEFAULT "*", a_cod_grupo CHAR(5) DEFAULT "*")
...
define _tot_id,_tot_monto_banisi,_tot_monto_agente,_tot_comision,_tot_dif dec(16,2);


--set debug file to "sp_cob417.trc";
--trace on;
...
select e.no_poliza, count(*), max(c.fecha), sum(c.prima_neta)
 ...
 where c.no_poliza = e.no_poliza
   ...
   and e.cod_grupo matches a_cod_grupo  
   and a.cod_agente matches a_cod_agente
 group by 1
 order by 1
	 ...
	 where c.no_poliza = e.no_poliza   
	   ...
	   EXIT FOREACH;	   
		...
		if _fecha <= _fecha_comparar then	   
	       if _prima_neta_cobrada >= _prima_suscrita then	
				let _porc_agente = 1;												
...
END FOREACH
    ...
    into _tot_id,_tot_monto_banisi,_tot_monto_agente,_tot_comision
    from temp_banisi ;
...
foreach
 select cod_agente,
			    ...
			    porc_comis,
				porc_banisi,								
				round(monto_banisi,2),								
				porc_agente,
				round(monto_agente,2),									
				round(comision,2),
				fecha_ult_pp				
   into	_cod_agente,
				_nombre_agente,
