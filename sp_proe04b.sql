...
drop procedure sp_proe04b;
create procedure sp_proe04b(a_poliza char(10), a_unidad char(5), a_suma dec(16,2), a_no_endoso char(5))
returning	integer;
...
define _vigencia_inic,_vigencia_inic_e		date;
define _cod_producto        char(5);

if a_poliza = '1530997' and a_no_endoso = '00001' and a_unidad = '00001' then
...
if ll_rea_glo is null or ls_ruta is null then
   let ll_rea_glo = 0;
end if
...
select vigencia_inic
 ...
 if ls_ramo = '018' then --se usa la vigencia inic del endoso Armando 24/02/2026
	let _vigencia_inic = _vigencia_inic_e;
 end if
...
foreach
	...
	if _mult_plenos > 0 then
		...
		if _tipo_mov = 4 then
			if ls_ramo = '004' or ls_ramo = '016' then
				foreach
					select orden,
						   cod_contrato,
						   porc_partic_suma,
					 ...
					 where cod_ruta       = ls_ruta
					   and cod_cober_reas = ls_cober_reas
					   --and porc_partic_prima <> 0   -- caso 5669 -- Meivis se creÃ³ un producto 08742 poliza 0423-00001-01 con un sola cobertura y en rearucon estÃ¡ en 0 los porcentajes de participaciÃ³n y cae en un loop en deivid
					   --and porc_partic_suma <> 0
					 order by orden
					 
					if ld_porc_suma = 0.00 and ld_porc_prima = 0.00 and _cod_producto <> '08742' then 
						continue foreach;
					end if
