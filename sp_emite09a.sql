...
drop procedure sp_emite09a;

create procedure "informix".sp_emite09a() 
	...
	foreach
		select act.no_poliza,
		  ...
		  into _no_poliza,
			   ...
			   _fecha_exp
		  from emipomae act
		 inner join emipomae ant on ant.no_documento = act.no_documento
		    and ant.vigencia_final = act.vigencia_inic 
			and ant.no_pagos <> act.no_pagos--ant.cod_formapag <> act.cod_formapag
		 where act.actualizado = 0
		   and act.no_factura not like '%-%'
		 order by act.no_documento
		
		update emipomae
		   set cod_perpago = _cod_perpago,
			   no_pagos = _no_pagos
