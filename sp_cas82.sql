...
foreach
	select cod_pagador,
	 ...
	 where date(fecha_ini) between a_fecha_desde and a_fecha_hasta
	  and cod_cobrador  in (select cod_cobrador
	  from cobcobra where  tipo_cobrador = 7 and activo = 1 )
	...
	select trim(nombre)||" - "||trim(cod_cobrador)
	  into _nombre_cobrador
	  from cobcobra
	 where cod_cobrador = _cod_cobrador;  --a_cod_cobrador;
