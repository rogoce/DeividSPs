...
returning	char(20)		as poliza,
			...
			char(50)		as cliente,
			date			as fecha_suscripcion,    	--20
			varchar(30)		as cedula,  -- ruc
			varchar(50)		as subramo,  -- subramo
			dec(16,2)		as retencion_casco,
			char(15)		as manzana,
			char(50)        as name_manzana,
			dec(16,2)		as retencion_otros,
			dec(16,2)		as cont_rc,
			dec(16,2)		as contrato_casco,
			char(5)         as no_unidad,
			date            as fecha_emision,			--30
			dec(16,2)       as retencion_varios,
			dec(16,2)       as contrato_varios,
			varchar(50)     as tipo_prod,
			dec(7,4)        as porc_partic_coas,
			date			as fecha_nacimiento,
			char(10)        as sexo,
			char(2)    	    as fumador,
			dec(16,2)   	as prima_cont_cp,
			dec(16,2)		as prima_cont_exc,
			dec(16,2)		as suma_cont_cp,			--40
			dec(16,2)		as suma_cont_exc,
			dec(16,2)		as suma_ret,
			dec(16,2)		as suma_facult,
			dec(16,2)       as ret_rc,
			dec(16,2)       as contrato_otro,			--45
			char(50)        as contratante,
			varchar(50)    	as cobertura_varios;
begin
...
define _flag,_ano				smallint;
define _cnt,_ano_contable	smallint;
...
define _fecha_emision,_vigencia_ruta		date;
define _cont_varios			dec(16,2);
define _ret_varios			dec(16,2);
define _tipoprod            varchar(50);
define _fecha_nacimiento    date;
define _sexo                char(1);
define _fumador				smallint;
define _prima_cont_cp		dec(16,2);
define _prima_cont_exc		dec(16,2);
define _suma_ret			dec(16,2);
define _suma_cont_cp		dec(16,2);
define _suma_cont_exc		dec(16,2);
define _suma_facult			dec(16,2);
define _suma_1              dec(16,2);
...
no_endoso       char(5),
no_unidad       char(5),
primary key(cod_ramo, cod_subramo, cod_origen, cod_contrato, cod_cobertura, desc_cob, serie, no_poliza, no_endoso, no_unidad)) with no log;
...
cont_otros		dec(16,2),
cont_casco		dec(16,2),
cont_varios     dec(16,2), 
no_endoso       char(5),
no_unidad       char(5),
prima_cont_cp   dec(16,2),
prima_cont_exc	dec(16,2),
suma_ret		dec(16,2) default 0.00,
suma_cont_cp	dec(16,2) default 0.00,
suma_cont_exc	dec(16,2) default 0.00,
suma_facult		dec(16,2) default 0.00,
ret_rc          dec(16,2),
cont_rc          dec(16,2),
primary key (no_documento,no_recibo,no_unidad,vigencia_ini,vigencia_fin,cod_ramo)) with no log;
...
--set debug file to "sp_pr1008a.trc";
--trace on;

foreach with hold
	select no_poliza,		--first 100 															 
		   no_endoso																		 
	  into v_nopoliza,
		   v_noendoso
	  from temp_det
...
foreach with hold
	select cod_ramo,		  --se busca por polizas
	  ...
	  from temp_produccion
	 where seleccionado = 1
	 group by cod_ramo, no_poliza, no_endoso, no_unidad
	...
	--*************vigencias para salud
	--*************vigencia inicial para salud, Armando 03/04/2025
	if v_cod_ramo = '018' then
		let _vigencia_ini = sp_sis517(a_periodo2, _vigencia_ini);
	end if
	...
	foreach					 -- se desglosa por tipo, buscar primero si es bouquet
		...
		let _cont_varios  = 0;
		let _ret_varios	  = 0;
		let _prima_cont_cp = 0;
		let _prima_cont_exc = 0;
		let _suma_ret = 0;
		let _suma_cont_cp = 0;
		let _suma_cont_exc = 0;
		let _suma_facult = 0;
		let _ret_rc = 0;
		let _cont_rc = 0;
			...
			if _facilidad_car = 1 then
				...
				if v_cod_ramo in ('002','020','023') then
					if v_cobertura in ('025','002','033') then
						...
						let _cont_rc = _cont_rc + v_prima_tipo;
						if v_tipo_contrato = 5 then
							let _prima_cont_cp = _prima_cont_cp + v_prima_tipo;
						else
							let _prima_cont_exc = _prima_cont_exc + v_prima_tipo;
						end if
					elif v_cobertura in ('044','046') then
						let v_prima_bq = v_prima_bq + v_prima_tipo;
					...
					elif v_cobertura in ('045','047','048') then	
						let v_prima_bq = v_prima_bq + v_prima2; 
						let _cont_varios = _cont_varios + v_prima2;
						if v_tipo_contrato = 5 then
							let _prima_cont_cp = _prima_cont_cp + v_prima2;
						else
							let _prima_cont_exc = _prima_cont_exc + v_prima2;
						end if
					else
					    let v_prima_bq = v_prima_bq + v_prima_tipo;
				...
				if _tipo_cont = 1 then		--	retencion
					if v_cod_ramo in ('002','020','023') then
						if v_cobertura in ('025','002','033') then
							...
							let _ret_rc = _ret_rc + v_prima_tipo;
						elif v_cobertura in ('044','046') then	
							let v_prima_1 = v_prima_1 + v_prima2; 
							let _cob_otros = _cob_otros + v_prima2;						
						elif v_cobertura in ('045','047','048') then -- Se agrega la retencion de varios Amado 27-05-2025	
							let v_prima_1 = v_prima_1 + v_prima_tipo; 
							let _ret_varios = _ret_varios + v_prima_tipo;
						else
							let v_prima_1 = v_prima_1 + v_prima_tipo;
	...
	--end foreach
		...
		let _prima_total = v_prima_1 + v_prima_ot + _sum_fac_car + v_prima_bq + v_prima_3;
		select nombre
		  into v_desc_contrato
		  from reacomae
		 where cod_contrato = v_cod_contrato;
		...
		foreach
			...
			begin
				on exception in(-239)
					update tmp_tabla
					   set cant_polizas   = cant_polizas   + 1,
						   ...
						   ret_varios     = ret_varios	   + _ret_varios * v_porcentaje/100,
						   cont_otros     = cont_otros      + _cont_cob_otros * v_porcentaje/100,
						   cont_casco     = cont_casco      + _cont_casco * v_porcentaje/100,
						   cont_varios	  = cont_varios		+ _cont_varios * v_porcentaje/100,
						   prima_cont_cp  = prima_cont_cp	+ _prima_cont_cp * v_porcentaje/100,
						   prima_cont_exc = prima_cont_exc	+ _prima_cont_exc * v_porcentaje/100,
						   ret_rc		  = ret_rc			+ _ret_rc * v_porcentaje/100,
						   cont_rc        = cont_rc			+ _cont_rc * v_porcentaje/100
				...
				values(	_no_documento, 
						...
						_ret_varios * v_porcentaje/100,
						_cont_cob_otros * v_porcentaje/100,
						_cont_casco * v_porcentaje/100,
						_cont_varios * v_porcentaje/100,
						v_noendoso,
						_no_unidad,
						_prima_cont_cp * v_porcentaje/100,
						_prima_cont_exc * v_porcentaje/100,
						_ret_rc * v_porcentaje/100,
						_cont_rc * v_porcentaje/100);				 
			end
		end foreach
...
foreach 
	...
	select first 1 cod_manzana,cod_asegurado
	  into _cod_manzana,_cod_asegurado
	  from emipouni
	 where no_poliza = _no_poliza;
	...
	foreach
		select fecha_emision,
		  ...
		  from endedmae
		 where actualizado = 1
		   and no_factura = v_no_recibo
		exit foreach;
	end foreach
	
    let _porc_partic_coas = 0;
	
	if _cod_tipoprod = '001' then --Coseguro Mayoritario
	...
	if v_cod_ramo in ('002','020') then
		foreach
			select a.cod_contrato, sum(a.suma_asegurada)
			  into v_cod_contrato,  _suma_1
			...
			--   and a.cod_cober_reas in ('031','025','034')
			   and b.actualizado = 1
			...
			if v_tipo_contrato = 1 then
			   let _suma_ret = _suma_1;
			elif v_tipo_contrato = 3 then
 			   let _suma_facult = _suma_1;
			elif v_tipo_contrato = 5 then
 			   let _suma_cont_cp = _suma_1;
			else
 			   let _suma_cont_exc = _suma_1;
           end if						   			   
		end foreach
	elif v_cod_ramo = '023' then
		foreach
			select a.cod_contrato, sum(a.suma_asegurada)
			  into v_cod_contrato,  _suma_1
			  from emifacon a, endedmae b
		...
		if trim(v_desc_ramo) IN ('INCENDIO-INCENDIO','MULTI RIESGO-INCENDIO') THEN 
			foreach
				select a.cod_contrato, sum(a.suma_asegurada)
				  into v_cod_contrato,  _suma_1
				  from emifacon a, endedmae b
				 where a.no_poliza = b.no_poliza
				   ...
				   and a.no_endoso = v_noendoso
				   and a.no_unidad = _no_unidad
				   and a.cod_cober_reas in ('001','003')
				   and b.actualizado = 1
				group by a.cod_contrato
		...
		elif trim(v_desc_ramo) IN ('INCENDIO-TERREMOTO','MULTI RIESGO-TERREMOTO') THEN
			foreach
				select a.cod_contrato, sum(a.suma_asegurada)
				  into v_cod_contrato,  _suma_1
				  from emifacon a, endedmae b
				 where a.no_poliza = b.no_poliza
				   ...
				   and a.no_endoso = v_noendoso
				   and a.no_unidad = _no_unidad
				   and a.cod_cober_reas in ('021','022')
				   and b.actualizado = 1
				group by a.cod_contrato
	...
	let v_cobertura_nombre = null;
	
--	if _ret_varios <> 0 then
		foreach
			select b.nombre
			 ...
			 where a.cod_cobertura = b.cod_cobertura
			   ...
			   and a.no_endoso = v_noendoso
			   and a.no_unidad = _no_unidad
			   and b.cod_cober_reas in (select cod_cober_reas from reacobre where nombre like '%VARIOS%')		   
			exit foreach;
        end foreach		
--	end if
	...
	return	_no_documento,
			...
			_porc_partic_coas,
		    _fecha_nacimiento,
  		   (case when _sexo = 'M' then 'MASCULINO' else (case when _sexo = 'F' then 'FEMENINO' else null end) end),
  		   (case when _fumador = 1 then 'SI' else (case when _fumador = 0 then 'NO' else null end)end),
		    _prima_cont_cp,
		    _prima_cont_exc,
		    _suma_cont_cp,
		    _suma_cont_exc,
