drop procedure sp_actuario19d;
-- copia de sp_actuario
create procedure "informix".sp_actuario19d(a_periodo_desde char(7), a_periodo_hasta char(7))
...
foreach with hold
	select a.no_poliza,
	 ...
	 where a.no_poliza = b.no_poliza
	   ...
	   and a.prima_neta <> 0
	   and a.actualizado = 1
	   and a.no_factura in (select id_recibo from deivid_ttcorp:tmp_reaseguro_prob where cod_situacion = _cod_situacion)
	begin work;
	...
	if abs(_mto_prima_ac - _prima_neta_end) > 0.02  then
		update tmp_movim_tec_pri_ttco
		   set flag = 1
		 where id_recibo = _id_recibo;
		--rollback work;
		--let _error_desc = 'La Prima Suscrita por Calculo Diferente de Prima Suscrita de la Factura: ' || trim(_id_recibo) || '. no_poliza: ' || trim(_no_poliza) || 'no_endoso: ' ||  trim(_no_endoso);
		--return _error,_error_desc;
	end if
