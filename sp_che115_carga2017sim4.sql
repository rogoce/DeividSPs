--Procedimiento para realizar la carga en Rentabilidad1 para simular el bono de RENTABILIDAD agregando ramo de incendio pero 70%
--Creado 22/03/2017	Armando Moreno M.

DROP PROCEDURE sp_che115_carga2017sim4;
CREATE PROCEDURE sp_che115_carga2017sim4(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
foreach
    ...
    if _cod_ramo in("001","002","020","004","016","012","014","017","010","022","013","003","006","015","005","011","021","009","007",'023') then
		{if _cod_ramo in("001") then
			let _pri_susc_aa = _pri_susc_aa * 70/100;
			let _pri_susc_ap = _pri_susc_ap * 70/100;
		end if}
	else
		continue foreach;
    end if
	...
	{if _cod_ramo in("001") then
		let _sini_incu = _sini_incu * 70/100;
	end if}
	...
	foreach
	  SELECT cod_agente,
	   ...
	   if _cod_agente not in('00141') then
			continue foreach;
	   end if
	   --*********************UNIFICACIONES*******************
	   ...
	   --*****************************************************
		...
		--************************************************
		--   Calculos para incremeto de PSP 2017 vs 2016
		--************************************************
		let _pri_sus_aa_tmp = 0;
		let _pri_sus_ap_tmp = 0;		
		...
		-- Prima Suscrita Devengada	Año Actual 2017
		...
		-- Prima Suscrita Devengada	Año Pasado 2016
