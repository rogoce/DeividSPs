...
drop procedure sp_pro30c;
create procedure sp_pro30c(a_no_poliza char(10))
returning smallint,
          char(50);
...
define _cnt_recargo     smallint;
define _cnt_recargo_dep smallint;
...
if _ano_salno is not null then
	if _ano_salno = _anno then
		update prdsalno
		   set liberar = 1
		 where no_documento = _no_documento;
	end if
...
foreach
	...
	let _producto_orig = _cod_producto;
    ...
    If _cod_grupo in ('01007', '983') AND _periodo = '2011-04' Then  --> Apavimed y Conafar todas deben cambiar de producto
	Else
		...
		if month(_vigencia_inic) <> month(_vigencia_final) then	-- Para que? solo se trabajan las que cumplen aniversario
			continue foreach;
		end if
	End If
	...
	--trace off;
   	--let _cod_producto = sp_pro30k(a_no_poliza, _cod_producto, _periodo); --  Se agrega para el cambio de tarifa 2012 -- Henry Giron. 19/12/2011
	--trace on;
	...
	if _cnt_recargo > 0 and _cod_producto = _producto_orig then --Si tiene recargo del 31.5% y es el mismo producto
		continue foreach;
	end if
	...
	if _tar_salud in (1,4,5) then -- Por edad, Por Edad - Sexo - Fumador, Por Edad (Aseg + Dep)
		IF _cod_subramo = '012' and _cod_grupo = '77942' THEN  -- Se excluye grupo FAES Amado 02-03-2023
		ELSE
			IF _periodo in ('2020-05','2020-06','2020-07','2020-08','2020-09') OR _cod_subramo = '012' THEN 
				IF _cod_subramo = '012' THEN
					insert into prdsalex
					VALUES(_no_documento, _periodo, "SUBRAMO COLECTIVO NO APLICA TARIFA POR EDAD");
					continue foreach;
				ELSE
					insert into prdsalex
					VALUES(_no_documento, _periodo, "COVID19");
					continue foreach;
				END IF
			END IF
		END IF
...
--		let _recargo    	= (_prima - _descuento) / 100 * _porc_recargo;
	...
	-- if _cod_producto = _producto_nuevo then --No hubo cambio de producto  Lo puse en comentario porque despues no tomaria el cambio 
		-- else								   -- cuando el asegurado o dependiente cumple aniversario o cualquier cambio en las coberturas	 AMADO 21/11/2012
