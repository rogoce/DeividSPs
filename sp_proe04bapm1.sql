...
drop procedure sp_proe04bapm1;
create procedure sp_proe04bapm1(a_poliza char(10), a_unidad char(5), a_suma dec(16,2), a_no_endoso char(5))
...
--IF a_poliza = '1917194' and a_no_endoso = '00013' and a_unidad = '04330' then
...
--	   and ruta_web = 1
	...
	if _cnt_rearumae is null then
		let _cnt_rearumae = 0;
	end

	if _cnt_rearumae = 1 then}
		select cod_ruta
		  into ls_ruta
		  from rearumae
		 where cod_ramo = ls_ramo
...
if ls_ramo = '002' then
	let ls_ruta = '00843';
elif ls_ramo = '020' then
	let ls_ruta = '00842';
elif ls_ramo = '023' then
	let ls_ruta = '00841';
end if
			...
			if ls_ramo = '004' or ls_ramo = '016' then
				foreach
					select orden,
					 ...
					 where cod_ruta       = ls_ruta
					   and cod_cober_reas = ls_cober_reas
					   --and porc_partic_prima <> 0   -- caso 5669 -- Meivis se creó un producto 08742 poliza 0423-00001-01 con un sola cobertura y en rearucon está en 0 los porcentajes de participación y cae en un loop en deivid
					   --and porc_partic_suma <> 0
					 order by orden
