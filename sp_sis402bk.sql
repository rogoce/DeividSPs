...
create procedure sp_sis402(a_no_poliza char(10),a_fecha_hoy date)
returning	smallint,
			varchar(100),
			decimal(16,2);
...
if _cod_ramo in ('004','008','016','018','019','020') then
	let _error_desc = 'El Ramo de esta Póliza NO Aplica para este Descuento.';
...
if _dias > 30 then
	let _error_desc = "La Póliza no cumple con las condicones.Han pasado mas de 30 días desde la suscripcion de la Póliza.";
	return 1,_error_desc,0.00;
end if
...
if _cod_grupo <> "00967" then --Grupo Felix B Maduro
	let _descuento = (_prima_bruta_end * 0.05);
else
	let _descuento = (_prima_bruta_end * 0.07);
end if
...
if a_monto_pago = 0 then	--Vengo de Endoso
	let _valor = _prima_bruta_end - _descuento;
