...
-- execute procedure sp_cob419('001','001','2019-01','00035;','00115;')  definir cliente y grupo?? 02618 y 1122

DROP PROCEDURE sp_cob419;
CREATE PROCEDURE sp_cob419(a_compania CHAR(3), a_sucursal CHAR(3), a_periodo char(7), a_cod_agente CHAR(255) DEFAULT "*", a_cod_grupo CHAR(255) DEFAULT "*")
...
define _tot_id,_tot_monto_banisi,_tot_monto_agente,_tot_comision,_tot_dif,_tot_monto_prt_pago  dec(16,2);
...
let _porc_banisi = 5;      -- Porcentaje de Manejo - Banco BANISI
--let _porc_pronto_pago = 3; -- Porcentaje de Pronto Pago
let _porc_agente = 0;      -- Porcentaje de Manejo - Corredor
let v_filtros = '';
let _tipo = '';
let _dif_pago = 0;
...
if a_cod_agente <> "*" then
	...
	drop table if exists tmp18_agente;
	select * from tmp_codigos into temp tmp18_agente;	
	drop table tmp_codigos;
end if

--Filtro por Grupo
if a_cod_grupo <> "*" then
	let v_filtros = trim(v_filtros) ||"Grupo "||trim(a_cod_grupo);
	let _tipo = sp_sis04(a_cod_grupo); -- separa los valores del string
	drop table if exists tmp18_grupo;
	select * from tmp_codigos into temp tmp18_grupo;	
	drop table tmp_codigos;
end if
...
FOREACH
select e.no_poliza, count(*), max(c.fecha), sum(e.prima_bruta)
 ...
 where c.no_poliza = e.no_poliza
   ...
   and e.cod_grupo in (select codigo from tmp18_grupo)
   and a.cod_agente in (select codigo from tmp18_agente)
 group by 1
 order by 1
...
--		call sp_sis36(_periodo_real) returning _fecha_ult_dia;
		let _fecha_ult_dia = MDY(a_periodo[6,7], 1, a_periodo[1,4]);
...
into _tot_id, _tot_monto_banisi, _tot_monto_agente, _tot_monto_prt_pago
from temp_banisi ;
