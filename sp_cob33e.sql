...
DROP PROCEDURE sp_cob33e;

CREATE PROCEDURE "informix".sp_cob33e(
a_compania     CHAR(3), 
a_sucursal     CHAR(3), 
  ...
  WHERE	no_documento   = a_no_documento -- Facturas de la Poliza
	...
	AND activa         = 1
		...
		FOR _ciclo = 1 TO _no_pagos
			...
			ELIF _dias > 30 AND _dias <= 45 THEN
			    LET v_monto_30 = v_monto_30     + _prima_bruta;
			ELIF _dias > 45  AND _dias <= 60 THEN
			    LET v_monto_45 = v_monto_45     + _prima_bruta;
			ELIF _dias > 60 THEN
...
WHILE _monto_recibo != 0
    ...
    ELIF v_monto_45 > 0 THEN  --Mayor de 45

		IF v_monto_45 > _monto_recibo THEN
		    LET v_monto_45    = v_monto_45 - _monto_recibo;
		    LET _monto_recibo = 0;
		ELSE 
		    LET _monto_recibo = _monto_recibo - v_monto_45;
		    LET v_monto_45    = 0;
		END IF
    ELIF v_monto_30 > 0 THEN  --Mayor de 30
    ...
    ELIF v_corriente > 0 THEN	-- de 0 a 30
...
LET v_por_vencer = 0;
LET v_exigible   = v_corriente + v_monto_30 + v_monto_45 + v_monto_60;
