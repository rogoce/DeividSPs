...
DROP PROCEDURE sp_rec30bk;
--DROP TABLE tmp_arreglo;
CREATE PROCEDURE sp_rec30bk(a_compania CHAR(3), a_agencia CHAR(3), a_reclamo CHAR(10))
			RETURNING   CHAR(18),
						CHAR(20),
						DATE,
						DATE,
...
DEFINE _ejecutivo_temp   VARCHAR(50);
...
DEFINE v_ejecutivo       VARCHAR(255);
...
{if a_reclamo = '643871' then
  set debug file to "sp_rec30bk.trc";
  trace on;	
end if}
		FOREACH
  	SELECT numrecla,
 ...
 SELECT no_poliza,      
   ...
   FROM tmp_arreglo
    ...
    -- Lectura de Agente
		...
		SELECT nombre 
		  INTO _ejecutivo_temp
		  FROM agtvende
		 WHERE cod_vendedor = _cod_vendedor;		

		IF TRIM(v_ejecutivo) = "" THEN
		   LET _largo2 = LENGTH(_ejecutivo_temp);
		   IF _largo2 >= 255 THEN
		   	EXIT FOREACH;
		   END IF
    	   LET v_ejecutivo = TRIM(_ejecutivo_temp);
		ELSE
		   LET _largo2 = LENGTH(TRIM(v_ejecutivo) || ", " || TRIM(_ejecutivo_temp));
		   IF _largo2 >= 255 THEN
		   	EXIT FOREACH;
		   END IF
		   LET v_ejecutivo = TRIM(v_ejecutivo) || ", " || TRIM(_ejecutivo_temp);
		END IF
	END FOREACH
	...
	END FOREACH
    if _prima_suscri_p is null then
		let _prima_suscri_p = 0;
	end if	
	if _prima_suscri_u is null then
		let _prima_suscri_u = 0;
	end if
	if v_incurr_bruto_p is null then
		let v_incurr_bruto_p = 0;
	end if	
	if v_incurr_bruto_u is null then
		let v_incurr_bruto_u = 0;
	end if
	if _prima_suscri_p = 0 then
		LET v_sinies_p = 0;
	else
		LET v_sinies_p =  v_incurr_bruto_p / _prima_suscri_p;
	end if	
	
	if _prima_suscri_u = 0 then
		let v_sinies_u = 0;
	else
