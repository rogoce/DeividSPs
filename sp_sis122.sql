...
drop procedure sp_sis122;
create procedure sp_sis122(a_no_poliza char(10), a_no_endoso char(5))
returning	integer,
			char(100);
...
if a_no_poliza = "2837487" And a_no_endoso in("00001","00003") then	--Especial factura Ricamar
	let _cod_ramo = '002';
end if

if _cod_ramo in ('001','003') and a_no_poliza <> '1194091' and _cod_endomov <> '017' then
	foreach
		select cod_contrato,
			   cod_cober_reas,
			   sum(prima),
	...
	if a_no_poliza = '0001815499' then
		foreach
			select no_unidad,
				   cod_cober_reas,
				   cod_contrato
