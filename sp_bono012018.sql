...
DROP PROCEDURE sp_bono012018;
CREATE PROCEDURE sp_bono012018()
RETURNING INTEGER;
...
delete from bono_prod_d_copy2;
let _ano_actual = year(current);	--2018
let _periodo_actual = _ano_actual || '-12';	--2018-12

--SET DEBUG FILE TO "sp_bono012018.trc";
--TRACE ON;
	...
	foreach
			 select e.no_documento,e.no_poliza
			  ...
			  where e.no_poliza         = t.no_poliza
				...
				and e.actualizado       = 1
				and e.nueva_renov       = "N"
				and e.periodo >= "2018-01"
				and e.periodo <= "2018-12"	
			...
			if _cnt > 0 then					--Excluye Facultativos
				let _prima_suscrita = _prima_retenida;
			end if
			...
			elif _cod_ramo = '009' and _cod_subramo in('001','002','004','003') then	--Si es Transporte, perimite terrestre y maritimo y Aereo.
			else
				continue foreach;
			end if
			...
			select count(*)
			 ...
			 where no_poliza     = _no_poliza
			   ...
			   and fecha_emision >= '01/01/2018'
			   and fecha_emision <= '31/12/2018';
	...
	foreach
			select d.no_poliza,
			 ...
			 where	d.no_remesa    = m.no_remesa
			   ...
			   and d.fecha        >= '01/01/2018'
			   and d.fecha        <= '31/12/2018'
