...
drop procedure sp_bono12a;
create procedure sp_bono12a(a_compania char(3), a_sucursal char(3))
...
--let _per_fin_aa = '2017-03';

--*****************************
-- Periodo Inicial del Bono
--*****************************

let _per_ini      = '2017-04';
let _fecha_fin_ap = sp_sis36(_per_ini);

{-- Periodo Final del Concurso
if _per_fin_aa > '2017-09' then
	let _per_fin_aa = '2017-09';
end if}
...
foreach
	...
	insert into tmp_concurso(no_documento, pri_pag_aa,tipo,nueva_renov,periodo)
...
foreach
	...
	insert into tmp_concurso(no_documento, pri_pag_ap,tipo,nueva_renov,periodo)
...
foreach
	...
	insert into tmp_concurso(no_documento, pri_sus_aa,nueva_renov,periodo)
...
foreach
	...
	insert into tmp_concurso(no_documento, pri_sus_ap,nueva_renov,periodo)
	values (_no_documento, _prima_suscrita,_nueva_renov,_periodo);
end foreach
...
foreach
	...
	insert into tmp_concurso(no_documento, sin_pag_aa,nueva_renov)
	values (_no_documento, _sin_pag_aa,_nueva_renov);
...
foreach 
	...
	insert into tmp_concurso(no_documento, sin_pen_ap,nueva_renov)
...
foreach 
	...
	insert into tmp_concurso(no_documento, sin_pen_aa,nueva_renov)
...
foreach
	...
	if _cod_ramo = '023' then --Excluir Flotas de la Persistencia
		let _no_pol_ren_aa_per = 0;
		let _no_pol_ren_ap_per = 0;
		let _no_pol_nue_ap_per = 0;
	end if
	...
	foreach
		...
		if _cnt_somos is null then
			let _cnt_somos = 0;
		end if

		if _cnt_somos > 0 then
