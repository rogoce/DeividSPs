...
DROP PROCEDURE sp_che95bb;
CREATE PROCEDURE sp_che95bb(a_cia CHAR(3),a_cod_agente CHAR(5) default "*",a_periodo char(7))
...
FOREACH
	...
	order by 1,2,3,4,5,6,7,8 desc
			...
			if a_periodo >= "2011-12" then
		        let _reserva_ant = _prima_ant * _porc_res_mat / 100;
				if  _monto_90 > 0  then
					let _sini = 0;
					let _prima_aplica = 0;
					let _reserva = 0;
		 ...
		 insert into tmpche95b(cia,asegurado,poliza,prima_suscrita,monto_90,prima_exc_monto90,reserva,prima_exc_reserva,siniestro,categoria,ramo,agente,reserva_ant,prima_ant)
		 values (v_nombre_cia,_nombre_cliente,_no_documento,_prima_aa,_monto_90,_prima_exc_m90,_reserva,_prima_aplica,_sini,_nombre_tipo,_nombre_ramo,_nombre_agente,_reserva_ant,_prima_ant);

END FOREACH


FOREACH
	select cia,agente,categoria,ramo,asegurado,poliza,prima_suscrita,monto_90,prima_exc_monto90,reserva,prima_exc_reserva,siniestro,reserva_ant,prima_ant
