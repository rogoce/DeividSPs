...
-- SIS v.2.0 - DEIVID, S.A.	 execute procedure sp_cob753bk()
 ...
 drop procedure sp_cob753bk_7228;
create procedure sp_cob753bk_7228(a_fecha_pagar date)
returning integer,
          char(100);
...
--set debug file to "sp_cob753b.trc";
--trace on;
...
'1921-00027-11'
	   ...
	   and a.date_posteo	>= a_fecha_pagar --_fecha_actual --'25/02/2014'
	 group by 1,2
	 order by 1,2
	...
	foreach
		select distinct trim(no_poliza)
		  into _no_poliza
		  from avisocanc
		 where trim(no_documento) = _no_documento
		--   and estatus not in ('Y')
		 order by 1 desc
		exit foreach;
	end foreach
	...
	foreach
		select no_aviso,
		 ...
		 where no_documento = _no_documento
		   ...
		   and estatus      = 'Y' -- and no_aviso = '02439'
		...
		--if _saldo_con_mora <= 5 then
				...
				foreach
					Select no_recibo,sum(monto)
					 ...
					 where doc_remesa   = _no_documento	    -- Recibos de la Poliza
					   ...
					   and fecha        = _fecha_ult_pago	-- Fecha Ultimo Pagos
					 group by 1
					 order by 2 desc
					exit foreach;
				end foreach
			...
			update avisocanc
			   set estatus         = _estatus,
				   ...
				   saldo		   = _saldo_c,
				   fecha_ejecuto   = _fecha_actual
			 where no_poliza       in (_no_poliza,_no_poliza2) ;
			   and no_aviso        = _no_aviso
			   and renglon         = _renglon;

			return _renglon,'La Poliza '|| trim(_no_documento) ||'Desmarcadas hoy; '||trim(_no_aviso) with resume;
		--end if
	end foreach
end foreach
end
