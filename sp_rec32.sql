...
CREATE PROCEDURE sp_rec32(v_no_tranrec CHAR(10), v_usuario    CHAR(8))
 RETURNING char(50), 
 		   char(10), 
 		   char(50), 
 		   char(10), 
...
DEFINE v_tr_ant_no2    CHAR(10);

define _fecha_hora		datetime year to minute;
...
foreach
  ...
  { select MAX(no_tranrec_int)
   ...
   -- Se cambio la formar de buscar la transaccion anterior 22/05/2008 - Amado
   LET _contador = 1;

   FOREACH
      select fecha, transaccion
        into v_tr_ant_fecha, v_tr_ant_no2
        from rectrmae
       where no_reclamo = v_reclamo
	     and actualizado = 1
	  order by 2

      If v_tr_ant_no2 = v_transaccion Then
		If _contador = 1 Then
	      LET v_tr_ant_no    = NULL;
	      LET v_tr_ant_fecha = NULL;
		End If
