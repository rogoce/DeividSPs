...
drop procedure sp_proe04xx;
create procedure sp_proe04xx(
a_poliza	char(10),
a_unidad	char(5),
a_suma		dec(16,2),
a_cia		char(3))
...
foreach
	...
	if _mult_plenos > 0 then
		...
		foreach
			...
			if a_poliza = '3305842' then
			  if ls_contrato = '00794'then
					let ld_porc_suma  = 100;
					let ld_porc_prima = 100;
			  else
			  continue foreach;
			  end if
			end if
			...
			if li_return = 0 Or li_return is null then
				...
				if ls_contrato = '00749' and ls_cober_reas = '040' then
					...
					update prueba
					   set no_unidad = a_unidad;
						...
						select no_poliza,no_endoso,no_unidad,cod_cober_reas,orden,cod_contrato,cod_coasegur,porc_partic_reas,porc_comis_fac,porc_impuesto,suma_asegurada,prima,impreso,fecha_impresion,no_cesion,subir_bo,monto_comision,monto_impuesto,cant_garantia_pago,cod_perfac,fecha_primer_pago 
						  from prueba;
