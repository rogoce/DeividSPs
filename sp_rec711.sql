...
drop procedure sp_rec711;
create procedure sp_rec711(a_compania char(3), a_fecha1 date, a_fecha2 date, a_ramo char(255) )
...
--   and fecha        <= a_fecha2
 ...
 select nombre
  ...
  if trim(_cod_proveedor) = '' or _cod_proveedor is null then
	 select nombre
	   into _n_proveedor
	   from cliclien
 ...
 select nombre
  ...
  where cod_cliente = _cod_asegurado;
	...
	if _pagado = 0 and _generar_cheque = 1 then 
	   let _grupo = "1 - NO PAGADAS";
	elif (_pagado = 1 and _generar_cheque = 1) OR (_pagado = 1) then
	   let _grupo = "2 - PAGADAS";
	elif trim(_anular_nt) <> "" AND _anular_nt is not null then --	elif _pagado = 0 and _anular_nt <> "" then 
	   let _grupo = "3 - ANULADAS";
	end if

	let _no_orden_compra = null;

	foreach
		select no_orden
		  into _no_orden_compra
		  from recordma 
