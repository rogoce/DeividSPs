...
drop procedure sp_par296_dep;
create procedure "informix".sp_par296_dep(a_no_poliza char(10), a_no_endoso char(5))
returning integer,
		  char(100);
...
--if _tipo_registro = 1 then -- Produccion
	...
	-- Centro de Costo
	call sp_sac93(a_no_poliza, 1) returning _error, _error_desc, _centro_costo;

	if _error <> 0 then
		drop table tmp_reas;
		let _error_desc = "Error en sp_sac93" || " Poliza " || a_no_poliza || " Endoso " || a_no_endoso;
		return _error, _error_desc;
	end if
		...
		if _tipo_contrato = 1 Then -- Retencion
			...
			foreach
				...
				if _monto <> 0.00 then
					...
					call sp_par60a(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _tipo_comp);
				...
				if _monto3 <> 0.00 then
					...
					call sp_par144a(a_no_poliza, a_no_endoso, _cuenta, _debito, _credito, _cod_coasegur, _tipo_comp);
