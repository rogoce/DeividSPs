...
drop procedure sp_par319_jango;
create procedure sp_par319_jango() 
...
--		   and sender like '%tracking%'
--		   and secuencia in (5122411,5122412)
--           and today - date(date_added) <= 1
		 order by secuencia
		...
		if _cod_tipo in ('00030','00031') and _date_added < '02/12/2019' and _enviado = 0 then
			continue foreach;
		end if	
        ...
        --let _email = 'jeperez@asegurancon.com;rgordon@asegurancon.com;aperez@asegurancon.com;';
		...
		if _cod_tipo in ('00004','00015','00047') then
		    ...
		    let _no_requis = null;
			if _cod_tipo in ('00016') then   --SD#1932 IVASQUEZ
				foreach			
					select no_remesa
					  into _no_requis
					  from parmailcomp
		...
		--	let _asunto = 'CARTA SALUD POLIZA ' || trim(_no_documento);		
			let _asunto = trim(_asunto) ||' - ' || trim(_no_documento);		
		elif _cod_tipo = '00042' then
            let _email =  trim(_email) || ";fianzas@asegurancon.com;";
		elif _cod_tipo = '00056' then
			--continue foreach;
			select no_remesa
			  into _no_tranrec
			  from parmailcomp
			...
			if _cod_tipopago = '001' then
				...
				let _asunto = 'Asegurado: ' || trim(_nombre) || ', Póliza Nro: ' || trim(_no_documento) || ', Nro. de Reclamo: ' || trim(_numrecla);						
			else
				select nombre 
				  into _nombre
				  from cliclien
				...
				let _asunto = 'Reclamante: ' || trim(_nombre) || ', Póliza Nro: ' || trim(_no_documento) || ', Nro. de Reclamo: ' || trim(_numrecla);						
			end if
