...
-- SIS v.2.0 - DEIVID, S.A.
 ...
 DROP PROCEDURE sp_pro468;

create procedure sp_pro468(a_ano char(4))
...
let _fecha2 = _fecha - 1 units year;
	...
	select  p.cod_ramo,
	  ...
	  where p.actualizado  = 1
		and p.estatus_poliza = "3"
	 	and p.vigencia_final > _fecha2
	 	and p.vigencia_final <= _fecha
		and p.renovada = 0
	  group by p.cod_ramo, p.cod_subramo

	if _cod_ramo = '020' then
		let _suma_pol_ven = 5000 * _cant_pol_ven;
	end if
