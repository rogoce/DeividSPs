--************************************************************
-- Procedimiento que Carga las Bonificaciones de cobranza 2023
--************************************************************
...
--drop procedure sp_che81esp;
create procedure sp_che81esp(a_compania char(3), a_sucursal char(3), a_periodo char(7), a_usuario char(8))
returning smallint;
...
let _valor = sp_che108aesp(a_compania,a_sucursal,a_periodo); --06/04/2010,acumula prima cobrada hasta mes que se va a pagar, para luego evaluar si cumple para el bono.
if _valor <> 0 then
	return 2;
end if
 --11
...
foreach
	...
	if _cod_agente in('02354','02831') then
	else
		continue foreach;
	end if
	...
	foreach
		select d.no_poliza,
		 ...
		 where	d.no_remesa    = m.no_remesa
		   ...
		   and c.cod_agente   in ('02354','02831')
...
foreach
	...
	if _cod_agente in('02354','02831') then --Correo 27/10/2023 Zuleyka
		let _porc_comis2 = 3;
	end if
