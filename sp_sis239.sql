...
drop procedure sp_sis239;
create procedure "informix".sp_sis239() 
returning	smallint,
			char(100),
			char(10);
...
let _motivo_rechazo = "RECUPERO DE DEVOLUCION INCORRECTA";
...
foreach
	select no_lote,
	  ...
	  into _no_lote,
		   ...
		   _no_documento,
		   _monto_recibo
      from deivid_tmp:error_recupero
	 where recuperado = 0 --deivid_tmp:error_devolucion
	...
	update deivid_tmp:error_recupero
	   set recuperado = 1
	 where no_lote = _no_lote
	   and renglon = _det_renglon;
	
	update deivid_tmp:error_devolucion
	   set recuperado = 1
	 where no_lote = _no_lote
	   and renglon = _det_renglon;
end foreach
