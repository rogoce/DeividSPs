...
drop procedure sp_bo043_prueba1;
create procedure sp_bo043_prueba1()
returning date,date
--integer,		  char(100);
...
let a_periodo = current;
-- Pólizas vencidas anio anterior
	let _ano = year(a_periodo);
	let _ano = _ano - 1;
	let _fecha_menos_ini = mdy(1, 1, _ano);
	let _fecha_menos_fin = mdy(12, 31, _ano);
	
	return _fecha_menos_ini, _fecha_menos_fin;
...
let _per_fin_aa = '2019-04';	--************ELIMINAR ESTA LINEA
...
foreach								--Se agrego este ciclo, debido a que arrojaba error -229 debido a la cantidad de registros almacenado al momento de hacer el sum. 03/12/2015
	...
	foreach
		select no_documento,		--1
			   ...
			   sum(pri_sus_map),
			   sum(pri_sus_maa),
			   sum(no_pol_ren_ap_per),	--25
			   sum(no_pol_nue_ap_per),	--26
			   sum(no_pol_ren_aa_per),	--27
			   sum(pri_sus_ap_mes),
			   sum(pri_sus_map_mes),
			   sum(pri_cob_ap_mes),
			   sum(pri_cob_neto_aa),
		  ...
		  into _no_documento,		--1
			   ...
			   _pri_sus_map,
			   _pri_sus_maa,
			   _no_pol_ren_ap_per,		--25
			   _no_pol_nue_ap_per,		--26
			   _no_pol_ren_aa_per,		--27
			   _pri_sus_ap_mes,
			   _pri_sus_map_mes,
			   _pri_cob_ap_mes,
			   _pri_cob_neto_aa,
