...
drop procedure sp_sis198b;

create procedure "informix".sp_sis198b(a_documento CHAR(20))
returning integer, 
          varchar(100),
          varchar(30);
...
foreach
 select no_poliza, cod_subramo
   into _no_poliza, _cod_subramo
   from emipomae
 ...
 {	 update emipomae 
    ...
    update tmp_autoflota
	   set error = "cod_subramo " || _cod_subramo
	 where no_documento = a_documento;
	  return -1, a_documento  || "cod_subramo " , _cod_subramo with resume;
 end if

 foreach
 	select cod_ruta, no_endoso, orden
	  into _cod_ruta, _no_endoso, _orden
	  from emigloco
	 where no_poliza = _no_poliza
	...
	select valor_flota
	 ...
	 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
	  {   update emigloco
	    ...
	    update tmp_autoflota
		   set error = trim(error) || " emigloco cod_ruta " || _cod_ruta
		 where no_documento = a_documento;
	   	--return -1, a_documento  || "emigloco cod_ruta" , _cod_ruta with resume;
	 end if
 end foreach
 ...
 foreach
	 ...
	 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
	  {   update emipouni
	    ...
	    update tmp_autoflota
		   set error = trim(error) || " cod_producto " || _cod_producto
		 where no_documento = a_documento;
		return -1, a_documento  || "cod_producto " ,_cod_producto with resume;
	 end if
     ...
     foreach
		...
		select valor_flota
		 ...
		 {    update emipocob
		    ...
		    update tmp_autoflota
			   set error = trim(error) || " cod_cobertura " || _cod_cobertura
			 where no_documento = a_documento;
   		  	return -1, a_documento  || "cod_cobertura",_cod_cobertura with resume;
		 end if
	 end foreach
     ...
     foreach
        ...
        if _valor_flota is not null and TRIM(_valor_flota) <> "" then
		  {	insert into tmp_emireaco 
		    ...
		    update tmp_autoflota
			   set error = trim(error) || " emireaco cod_cober_reas " || _cod_cober_reas
			 where no_documento = a_documento;
		   	return -1, a_documento  || "emireaco cod_cober_reas" , _cod_cober_reas with resume;
		 end if
	 end foreach
		...
		select valor_flota
		 ...
		 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
		  {   update emireama
		    ...
		    update tmp_autoflota
			   set error = trim(error) || " emireama cod_cober_reas " || _cod_cober_reas
			 where no_documento = a_documento;		  
		   	return -1, a_documento  || "emireama cod_cober_reas ", _cod_cober_reas with resume; 
		 end if
	 end foreach
 ...
 {    insert into emireaco(
		...
		select valor_flota
		 ...
		 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
		  {	insert into tmp_emifafac 
		    ...
		    {update tmp_autoflota
			   set error = trim(error) || " emifafac cod_cober_reas " || _cod_cober_reas
			 where no_documento = a_documento;		   }
		 	return -1, a_documento  || "emifafac cod_cober_reas",_cod_cober_reas with resume;
		 end if
     ...
     foreach
     	select cod_ruta, cod_cober_reas, no_endoso, orden
		  into _cod_ruta, _cod_cober_reas, _no_endoso, _orden
		  from emifacon
		 where no_poliza = _no_poliza
		   and no_unidad = _no_unidad
		...
		select valor_flota
		 ...
		 {    update emifacon
		   ...
		   { update tmp_autoflota
			   set error = trim(error) || " emifacon cod_ruta " || _cod_ruta 
			 where no_documento = a_documento;}
		   --	return -1, a_documento  || "emifacon cod_ruta", _cod_ruta  with resume;
		 end if
	 end foreach
     ...
     foreach
		...
		select valor_flota
		 ...
		 {    update emifacon
		    ...
		    {update tmp_autoflota
			   set error = trim(error) || " emifacon cod_cober_reas " || _cod_cober_reas
			 where no_documento = a_documento;		   }
		  	return -1, a_documento  || "emifacon cod_cober_reas", _cod_cober_reas  with resume;
		 end if
	 end foreach
  ...
  {   insert into emifafac(
   ...
   foreach
     ...
     foreach
		...
		select valor_flota
		 ...
		 {	insert into tmp_recreafa 
		   ...
		   { update tmp_autoflota
			   set error = trim(error) || " recreafa cod_cober_rea " || _cod_cober_reas	 
			 where no_documento = a_documento;					}
		  	return -1, a_documento  || "recreafa cod_cober_reas",_cod_cober_reas;
		 end if
     ...
     foreach
     	select cod_contrato, cod_cober_reas, orden
		  into _cod_contrato, _cod_cober_reas, _orden
		  from recreaco
		 where no_reclamo = _no_reclamo
		...
		{     update recreaco
		  ...
		  {  update tmp_autoflota
			   set error = trim(error) || " recreaco cod_cober_rea " || _cod_cober_reas
			 where no_documento = a_documento;	 }
		   	return -1, a_documento  || "recreaco cod_cober_reas", _cod_cober_reas;
		 end if
	 end foreach
		...
		select valor_flota
		 ...
		 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
		  {   update recrccob
		    ...
		    {update tmp_autoflota
			   set error = trim(error) || " recreaco cod_contrato " || _cod_contrato
			 where no_documento = a_documento;	   }
		   	return -1, a_documento  || "recreaco cod_cobertura", _cod_cobertura with resume;
		 end if
	     ...
	     foreach
			...
			select valor_flota
			 ...
			 {    update rectrcob
			   ...
			   { update tmp_autoflota
				   set error = trim(error) || " rectrcob cod_cobertura " || _cod_contrato
				 where no_documento = a_documento;			  }
			  	return -1, a_documento  || "rectrcob cod_cobertura", _cod_cobertura with resume;
			 end if

		 end foreach

	     foreach
	     	select no_tranrec, orden, cod_contrato, cod_cober_reas
			  into _no_tranrec, _orden, _cod_contrato, _cod_cober_reas
			  from rectrrea
			 where no_tranrec = _no_tranrec
			...
			select valor_flota
			 ...
			 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
			   ...
			   { update tmp_autoflota
				   set error = trim(error) || " recreaco cod_cober_reas " || _cod_cober_reas
				 where no_documento = a_documento; }
			   	return -1, a_documento  || "recreaco cod_cober_reas", _cod_cober_reas with resume;
			 end if
		 end foreach

	     foreach
	     	select no_tranrec, orden, cod_coasegur, cod_contrato, cod_cober_reas
			  into _no_tranrec, _orden, _cod_coasegur, _cod_contrato, _cod_cober_reas
			  from rectrref
			 where no_tranrec = _no_tranrec
			...
			select valor_flota
			 ...
			 if _valor_flota is not null and TRIM(_valor_flota) <> "" then
			   ...
			   { update tmp_autoflota
				   set error = trim(error) || " rectrref cod_cober_reas " || _cod_cober_reas
				 where no_documento = a_documento; }
