...
drop procedure ap_rwf103;

create procedure "informix".ap_rwf103(a_sucursal CHAR(3), a_no_reclamo CHAR(10))
returning  char(3),
           varchar(8), 
           varchar(40),
           varchar(30),
...
if a_sucursal in ('003','005','011','002') then --ID de la solicitud	# 5007 -- Se agrega Colón Solicitud # 6097
	select ajust_interno
	  into _cod_ajustador
	  from recrcmae
	 where no_reclamo = a_no_reclamo;
	   ...
	   let _sucursal4 = "002";
		...
		if _cantidad = 0 then
			select count(*)
			 ...
			 where activo     = 1
			   and dist_equitativa = 1
			   and cod_sucursal in ('003','005','011','002');
		else
