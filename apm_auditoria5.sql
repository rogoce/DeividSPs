...
drop procedure apm_auditoria5;

create procedure apm_auditoria5()
...
FOREACH
  SELECT no_reclamo, cod_asegurado, no_documento, cod_evento, fecha_reclamo, fecha_siniestro, numrecla, no_poliza, cod_producto
   ...
   WHERE numrecla[1,2] in ('02','20')
	 AND estatus_audiencia = 1
	 AND actualizado = 1

--periodo >= '2013-01' and periodo <= '2013-07'
--     AND 
--  order by numrecla
  ...
  if _no_recobro is null or trim(_no_recobro) = "" then
     continue foreach;
  end if
