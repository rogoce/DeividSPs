...
------------------------------------------------
--DROP PROCEDURE sp_pr860bkkk;
CREATE PROCEDURE sp_pr860bkkk(a_compania CHAR(03),a_agencia CHAR(03),a_periodo1 CHAR(07),a_periodo2 CHAR(07),a_codsucursal CHAR(255) DEFAULT "*",a_codgrupo  CHAR(255) DEFAULT "*",a_codagente CHAR(255) DEFAULT "*",a_codusuario CHAR(255) DEFAULT "*",a_codramo  CHAR(255) DEFAULT "*",a_reaseguro   CHAR(255) DEFAULT "*",a_serie CHAR(255) DEFAULT "*")
...
RETURNING CHAR(3),CHAR(3),CHAR(5),CHAR(3),DECIMAL(16,2),DECIMAL(16,2),DECIMAL(16,2),DECIMAL(16,2),DECIMAL(16,2),DECIMAL(16,2),DECIMAL(16,2),DECIMAL(16,2),CHAR(50),CHAR(50), CHAR(50), char(255);
   BEGIN
	  ...
	  define _facilidad_car,_tipo            smallint;
				 ...
				 Where serie         = _serie
				   ...
				   and borderaux     = _borderaux 
					...
					if _porc_cont_partic < 100 then 
					  ...
					  --Se pone en comentario este parrafo, hoy 17/09/2012 Armando M.	 	  
					  {	if 	_cod_coasegur = '042' then	-- SWISS REINSURANCE

							LET _comision70 = v_prima70 * 0.48 ;
							if 	v_cod_contrato in ("00602","00613") then
								LET _comision70 = v_prima70 * 0.40 ;
							end if

						elif _cod_coasegur = '076' then	         -- REASEGURADORA INTERNACIONAL DE VENEZUELA
							LET _comision70 = v_prima70 * 0.48 ;
						elif _cod_coasegur = '063' then			 -- MAPFRE
						   	LET _comision70 = v_prima70 * 0.42 ;
						elif _cod_coasegur = '050' then			 -- MUENCHENER DE MEXICO, S.A.
...
FOREACH
       SELECT r.serie,t.cod_ramo,t.cod_contrato,sum(t.pagado_neto) 
        ...
        where r.cod_contrato = t.cod_contrato 
          ...
          and t.tipo_contrato not in ('3','1') 
          and r.serie >= 2008 
          and t.cod_ramo in ("001","003","006","010","011","012","013","014","008","080","004","019","016")
     group by r.serie,t.cod_ramo,t.cod_contrato
     order by r.serie,t.cod_ramo,t.cod_contrato

		if v_cod_ramo = "006" or v_cod_ramo = "010" or v_cod_ramo = "011" or v_cod_ramo = "012" or v_cod_ramo = "013" or v_cod_ramo = "014" or v_cod_ramo = "001"  or v_cod_ramo = "003"  or v_cod_ramo = "008"  or v_cod_ramo = "080" or v_cod_ramo = "004"  or v_cod_ramo = "019" or v_cod_ramo = "016" then 
		else
			continue foreach;
		end if	    
