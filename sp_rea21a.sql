	...
	FOREACH WITH HOLD
	  SELECT d.no_poliza,d.no_documento,d.no_factura,d.sucursal_origen,
	         ...
	         d.vigencia_inic,d.vigencia_final,d.fecha_suscripcion,
	      	 d.user_added,d.suma_asegurada,d.fecha_cancelacion
	    INTO _no_poliza,_no_documento,_no_factura,v_cod_sucursal,
	         ...
	         v_usuario,v_suma_asegurada,_fecha_cancelacion
	     FROM emipomae d
				...
				FOREACH
				 SELECT f.cod_contrato
				  ...
				  WHERE u.no_poliza = _no_poliza
					...
					AND f.cod_cober_reas = "008"
					AND f.prima     <> 0.00

						SELECT serie,fronting
						  INTO _serie,_front
					      FROM reacomae
...
-- solo los bouquet
	UPDATE temp_perfil
     ...
     WHERE seleccionado = 1
       AND no_poliza NOT IN(SELECT Distinct no_poliza FROM temp_bouquet where bouquet = 1);

-- solo <> fronting
	UPDATE temp_perfil
       SET seleccionado = 0
     WHERE seleccionado = 1
       AND no_poliza IN(SELECT Distinct no_poliza FROM temp_bouquet where front = 1 );
