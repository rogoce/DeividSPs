-- Procedimiento que Genera el Cheque de Bonificacion por rentabilidad opcion 2
-- Creado    : 19/04/2016 - Autor: Henry Girón
-- SIS v.2.0 - DEIVID, S.A.

DROP PROCEDURE sp_che223;
CREATE PROCEDURE sp_che223(
a_compania       CHAR(3),
a_sucursal       CHAR(3),
a_cod_agente     CHAR(5),
a_usuario        CHAR(8),
...
let _origen_cheque = 'G';  -- Rentabildiad 2
let _error         = 0;
...
if a_cod_agente = '01001' then	-- Unificar un solo cheque para: "	Felix Alberto Abadia Pretelt

	SELECT SUM(comision)
	  INTO _comision
	  FROM chqrentaii
	 WHERE cod_agente IN('01001','02129','02130')
	   AND periodo    = a_periodo;

end if

if a_cod_agente = '00623' then	-- Unificar un solo cheque para: CHUNG WAI CHUN 

	SELECT SUM(comision)
	  INTO _comision
	  FROM chqrentaii
	 WHERE cod_agente IN('00623','01836')
	   AND periodo    = a_periodo;
...
SELECT cod_origen,
 ...
 WHERE cod_banco = a_banco;
	...
	LET _descripcion = 'PAGO DE BONIFICACION POR RENTABILIDAD 2 DEL ' || a_periodo[1,4];
	LET _tipo_pago = 0;
	...
	if a_cod_agente = '01001' then

		UPDATE chqrentaii
		   SET no_requis   = _no_requis,
		       tipo_requis = _tipo_requis
		 WHERE cod_agente  in('02129','02130')
		   AND periodo     = a_periodo;

	end if

	if a_cod_agente = '00623' then

		UPDATE chqrentaii
		   SET no_requis   = _no_requis,
		       tipo_requis = _tipo_requis
		 WHERE cod_agente  in('01836')
		   AND periodo     = a_periodo;

	end if

	-- Registros Contables de Cheques de Comisiones para Rentabilidad 2

	call sp_par276(_no_requis, "G") returning _error, _error_desc;
