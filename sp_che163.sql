...
if a_periodo >= '2022-12' then
	CALL sp_che163c(_fecha_desde, _fecha_hasta);
end if

--CALL sp_che240(a_compania, a_sucursal, a_periodo);
...
FOREACH
	...
	INNER JOIN emitipro d
		 ...
		 WHERE	a.actualizado      = 1
		   ...
		   AND a.fecha >= _fecha_desde
		   AND a.fecha <= _fecha_hasta	   
		   AND a.monto_descontado <> 0
		 ORDER BY a.no_remesa, a.renglon  
...
FOREACH
		 SELECT	distinct y.no_cheque,
		        y.monto
		   INTO	_no_cheque,
		        v_comision
		   FROM	chqchcta x, chqchmae y
		  WHERE x.no_requis = y.no_requis
            AND y.fecha_impresion >= _fecha_desde
            AND y.fecha_impresion <= _fecha_hasta	   
		    AND y.pagado = 1
	     	AND y.anulado = 0  
...
END FOREACH
  ...
  VALUES (_agente_agrupado,
          ...
          _prima_acc, 			 
          _prima_salud, 		
          _prima_colec,	 --0  SD#7145ZULEYKA	17072023 HG 
          0,		 
          0,		 
          0,		 
          0,	
