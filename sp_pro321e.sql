...
drop procedure sp_pro321e;

create procedure sp_pro321e(a_no_poliza char(10), a_no_poliza_ant char(10))
returning  integer;
...
define _cnt_lote            integer;
define _incremento          decimal(16,2);
define _ld_prima_anual_anter dec(16,2);
define _ld_prima_neta_anter	 dec(16,2);
define _ld_deduc_nuevo_dec   dec(16,2);
...
foreach
   ...
   --*** porcentaje de depreciacion
	...
	if _retorno = 0 then
		...
		if _cod_prod in ('10461','10288','03138') then -- Se agregan 2 más 10461 y 10288 -- Amado 08-09-2025
			if _cnt_ren = 1  then
				let _resultado = 1;
			elif _cnt_ren >= 2 then
				let _resultado = 3;
			end if		
	...
	if _ld_sum_aseg_1 < 4000 and _ld_sum_aseg_2 < 4000 then
		let _ld_sum_aseg_1 = _ld_sum_aseg_2;
	end if
	...
	/**/
		...
		-- Verificar si la renovación automatica lleva incremento -- Amado 08-08-2024
{		let _cnt_lote = 0;
		let _incremento = 0;
		...
		foreach
            ...
            --if _cnt_lote > 0 and _incremento = 0 then			
			...
			{	if _cod_prod in ('02282','02283') then --DRN # 13869 -- Amado 31-05-2025 ya no va por que está en el producto
				   if _cod_cobertura in ("00119", "00121") then
					let _ld_deduc_nuevo = _ld_deduc_nuevo + (_ld_deduc_nuevo * 30 / 100);
				   end if
				elif _cod_prod in ('07754','08278') then
				   if _cod_cobertura in ("00119", "00121") then
					let _ld_deduc_nuevo = _ld_deduc_nuevo + (_ld_deduc_nuevo * 20 / 100);
				   end if
				end if}
