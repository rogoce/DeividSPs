...
DROP PROCEDURE sp_che86_ha;
CREATE PROCEDURE sp_che86_ha(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando a Ciclo Prima Pagada este año...1.', CURRENT);
foreach
	select doc_remesa,
		   sum(prima_neta)
	  into _no_documento,
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Prima Pagada este año...1.', CURRENT);
--**********************************************************************************************
-- Prima Pagada AÑO PASADO a la fecha
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando a Ciclo Prima Pagada año pasado...2.', CURRENT);
foreach
	 select no_poliza,
			sum(prima_neta)
	   into _no_poliza,
	...
	select sum(d.prima_neta * (c.porc_partic_prima/100) * (c.porc_proporcion/100))
	 ...
	 where c.no_remesa = d.no_remesa
	   ...
	   and d.actualizado = 1
	   and d.tipo_mov in ("P", "N")
	   and d.periodo >= _per_ini_ap			--2017-10
	   and d.periodo <= _per_fin_ap			--2018-09
	   and r.tipo_contrato = 3;
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Prima Pagada año pasado...2.', CURRENT);
--**********************************************************************************************
...
--*************************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Prima Suscrita año pasado...4.', CURRENT);
foreach
 select no_poliza,
		no_endoso,
		no_documento,
...
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Siniestros pendientes año actual...6.', CURRENT);
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Siniestros pendientes año actual...6.', CURRENT);
---**********************************************************************************************
...
--let _fecha_ap = '30/09/2017';
--call sp_bo077_cambio(_fecha_ap_ini, _fecha_ap) returning _error, _error_desc;	-- 01/10/2016   A 30/091/2017	--se coloca para cantidad año pasado hasta el final del concurso. se puso el 13/07/2018.
call sp_bo077_camsalud(_fecha_ap_ini, _fecha_ap) returning _error, _error_desc;

INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Nuevas y Renovadas año pasado...7.', CURRENT);
foreach
		select no_documento,
			   sum(no_pol_nueva),
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Nuevas y Renovadas año pasado...7.', CURRENT);
----**********************************************************************************************
-- Polizas Nuevas y Renovadas AÑO ACTUAL a la fecha
----**********************************************************************************************
call sp_bo077_camsalud(_fecha_aa_ini, _fecha_aa) returning _error, _error_desc;	--01/10/2017    -    31/01/2018

INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Nuevas y Renovadas año actual...8.', CURRENT);
foreach
		select no_documento,
			   sum(no_pol_nueva),
			   sum(no_pol_renov),
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Nuevas y Renovadas año actual...8.', CURRENT);
