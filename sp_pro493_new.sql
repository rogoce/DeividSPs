...
create procedure "informix".sp_pro493(v_poliza char(10), v_endoso char(5), v_factor dec(9,6))
...
--if v_poliza = "0001031731" and v_endoso = '00001' then
--	let v_factor = -.430;
--end if
...
if v_tipocalc = "004" then -- POR SALDO
	...
	if v_tipo_mov in ("2","24","1","25","3","19") then
		select sum(prima),
			   sum(prima_neta)
		  into v_prima_bruta,
		       _prima_neta
...
--	END IF
		...
		if v_tipocalc = "005" or v_tipocalc = "006" then
			...
			if _acepta_descuento = 1 then
			   ...
			   if _tipo_descuento = 1 and _cod_tipo_tar = '002' then	--> Descuento RC, solo polizas nuevas  
		   	   ...
		   	   if _tipo_descuento in (1,2) and _cod_tipo_tar = '001' and _fecha_suscripcion >= "28/07/2014" then--and _nueva_renov = "R" then
			   ...
			   if _cod_tipo_tar = '004' or _cod_tipo_tar = '005' or _cod_tipo_tar = '006' or _cod_tipo_tar = '007' then -- Descuentos por modelo y siniestralidad
  ...
  --		let v_tot_descto = 0.00;
	...
	if _tot_reaseguro <> _neta Then
		...
		if r_signo < 0.00 and v_prima_suscrita > 0.00 and v_tipo_mov <> '025' Then
...
--if v_poliza = '58337' then
-- call ap_rehab() returning r_error, r_descripcion;
--end if
