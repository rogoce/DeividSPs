...
--DROP procedure sp_super15;
CREATE procedure sp_super15(a_agente CHAR(5), a_desde DATE, a_hasta DATE )
RETURNING   CHAR(3),   -- _cod_compania
	 ...
	 FOREACH  
		...
		if _cod_formapag in('003','005') then	-- si es electronico no va
			continue foreach;
		end if
		...
		RETURN  _cod_compania, _descr_cia, _cod_ramo, _ramo, _no_documento, _vigencia_inic, _vigencia_final,
		        _estatus_poliza, _cod_formapag, _no_recibo, _nombre, _prima_neta, _desc_formapag,_no_remesa,_fecha_remesa  WITH RESUME;
