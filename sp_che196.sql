...
---  Amado - 11/03/2022 --
------------------------------------------------
drop procedure sp_che196;
create procedure sp_che196(a_opc smallint, a_cod_banco char(3))
...
if a_cod_banco = '001' then
	if a_opc = 2 then
		...
		if a_opc = 5 then -- Accidentes Personales
...
foreach
  SELECT no_requis,   
	...
	INTO _no_requis,   
         ...
         _finiquito_firmado		 
    FROM chqchmae  
   WHERE ( origen_cheque = _origen_cheque) AND  
...
ORDER BY a_nombre_de
	...
	if a_cod_banco = '001' then -- Global Bank
  		if a_opc = 1 then
            ...
            if _cod_cliente = '417605' then
         		if _numrecla[1,2] not in ('18','04','19','16') then
        			continue foreach;
        		end if	
            else
    			if _numrecla[1,2] not in ('18','04','19') then
    				continue foreach;
    			end if
            end if		
  		elif a_opc = 3 then
	...
	elif a_cod_banco = '295' then -- Banisi
		if a_opc = 1 then
			if _numrecla[1,2] not in ('02','23') then
				continue foreach;
			end if
			if _cant <> 0 then
				continue foreach;
			...
			if _numrecla[1,2] not in ('16','19') then
				continue foreach;
			end if		
		elif a_opc = 3 then
			if _numrecla[1,2] not in ('02','23') then
