...
-- execute procedure sp_cob410('001','001','2018-01','02531','124')

DROP PROCEDURE sp_cob410;
CREATE PROCEDURE sp_cob410(a_compania CHAR(3), a_sucursal CHAR(3), a_periodo char(7), a_cod_agente CHAR(5) default "*", a_cod_grupo CHAR(5) default "*")
...
define _tot_id,_tot_monto_banisi,_tot_monto_web,_tot_monto_agente,_tot_comision,_tot_dif dec(16,2);
...
let _porc_banisi = 4;      -- Porcentaje de Manejo - Banco BANISI 4%
let _porc_web = 1;         -- Porcentaje Web 1%
let _porc_agente = 1;      -- Porcentaje de Manejo - Corredor 1%
...
--		LET _monto_banisi = round((_prima_neta_cobrada * round((_porc_banisi / 100),4)),4);								
LET _monto_banisi = _prima_neta_cobrada * (_porc_banisi / 100);		
		...
		if _fecha <= _fecha_comparar then	   
	       if _prima_neta_cobrada >= _prima_suscrita then		  -- Siempre y cuando haiga cubierto prima suscrita 
				let _porc_agente = 1;												
--		        LET _monto_agente = round((_prima_neta_cobrada * round((_porc_agente / 100),4)),4);	
LET _monto_agente = _prima_neta_cobrada * (_porc_agente / 100);	
		   end if
	   end if		 		 		 	 				
...
--		LET _monto_web          = round(((_prima_neta_cobrada - _monto_banisi - _monto_agente ) * round((_porc_web / 100), 4)  ), 4);							 	
--		LET _comision           = round(((_prima_neta_cobrada - _monto_banisi - _monto_agente ) * round((_porc_comis / 100), 4)   ), 4);	 			 
...
trace on;
    ...
    into _tot_id,_tot_monto_banisi,_tot_monto_web,_tot_monto_agente,_tot_comision
    from temp_banisi ;
