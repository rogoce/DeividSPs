...
DROP PROCEDURE sp_pro68;

CREATE PROCEDURE "informix".sp_pro68(a_compania CHAR(3),a_agencia CHAR(3),a_periodo1 CHAR(7),a_periodo2 CHAR(7),a_sucursal CHAR(255) DEFAULT "*",a_ramo CHAR(255) DEFAULT "*",a_grupo CHAR(255) DEFAULT "*",a_usuario  CHAR(255) DEFAULT "*",a_reaseguro CHAR(255) DEFAULT "*",a_agente CHAR(255) DEFAULT "*") 
RETURNING CHAR(255);
...
FOREACH WITH HOLD
 SELECT  e.no_poliza, e.no_endoso, e.prima_suscrita, e.prima_retenida, e.no_factura
   INTO  _no_poliza, _no_endoso, _total_prima_sus, _total_prima_ret, _no_factura
   FROM endedmae e
  WHERE e.periodo BETWEEN a_periodo1 AND a_periodo2
	...
	SELECT cod_sucursal, cod_tipoprod, cod_ramo,	cod_grupo,    cod_subramo,user_added
	  INTO _cod_sucursal, _cod_tipoprod, _cod_ramo,	_cod_grupo,   _cod_subramo,_user_added
	  FROM emipomae
	 	...
	 	SELECT 	porc_partic_agt, cod_agente
		  ...
		  WHERE	no_poliza = _no_poliza
			...
			-- Insercion / Actualizacion a la tabla temporal tmp_prod
				...
				no_poliza,			no_endoso,
				no_factura,         cod_subramo,
				cod_ramo,       	cod_grupo,
				cod_agente,			user_added,
				total_pri_sus,		total_pri_ret,
				total_pri_ced,	  	tipo_produccion,
				cod_sucursal
				)
				VALUES(
				_no_poliza,			_no_endoso,
				_no_factura,        _cod_subramo,
				_cod_ramo,   		_cod_grupo,
				_cod_agente,        _user_added,	
				t_total_prima_sus,	t_total_prima_ret,
				t_total_prima_ced, 	_tipo_produccion,
				_cod_sucursal
				);
