...
drop procedure sp_sis264;
create procedure sp_sis264(a_no_poliza char(10))
returning integer,varchar(150),dec(16,2),dec(5,4);


define _cod_asegurado char(10);
define _sexo		  char(1);
define _fecha_aniv,_vigencia_inic    date;
	...
	if _edad >= 0 and _edad <= 120 then
		...
		if _sexo = 'M' then			--es hombre
			if _fumador = 1 then	--es fumador
				...
				return _error,_desc_error,_h_fumador,_tasa_adelanto;
			else
			    let _h_no_fumador = _h_no_fumador * _porcentaje / 100;
				return _error,_desc_error,_h_no_fumador,_tasa_adelanto;
			end if
		elif _sexo = 'F' then			--es Mujer
			if _fumador = 1 then	--es fumador
			    let _m_fumador = _m_fumador * _porcentaje / 100;
				return _error,_desc_error,_m_fumador,_tasa_adelanto;
			else
			    let _m_no_fumador = _m_no_fumador * _porcentaje / 100;			
				return _error,_desc_error,_m_no_fumador,_tasa_adelanto;
			end if
		end if
	else
		let _error = 337;
...
return _error,_desc_error,0,_tasa_adelanto;
end procedure