...
--		end if
		...
		if _notrx in (877946,877940,877942,877946,877948,957938,957939,957940,957941,957942,957943,957944,957945) then
			continue foreach;
		end if
		--{
...
foreach
 select m.no_remesa, 
  ...
  group by 1, 2
	...
	if _no_remesa in ('872951','1572610') then
		continue foreach;
	end if
...
foreach
 select no_remesa,
  ...
  where periodo     >= _periodo
    ...
    if _no_remesa = '1822813' and _renglon = 90 then
        continue foreach;
    end if
