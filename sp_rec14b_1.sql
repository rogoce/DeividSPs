drop procedure sp_rec14b_1;
CREATE PROCEDURE sp_rec14b_1(
a_compania CHAR(3),
a_agencia CHAR(3),
a_periodo1 CHAR(7),
a_periodo2 CHAR(7),
...
IF a_codvend <> "*" THEN   -- F9:32147:DALBA 
...
IF a_producto <> "*" THEN
	...
	IF _tipo <> "E" THEN -- (I) Incluir los Registros
		...
		drop table if exists tmp_productohg; --26/05/2022.SD#3499
	  SELECT distinct no_poliza
		FROM emipouni
	   WHERE no_poliza in (
			 select no_poliza from tmp_siniest WHERE seleccionado = 1				
			 ) and cod_producto IN (SELECT codigo FROM tmp_codigos)
			 into temp tmp_productohg;

		UPDATE tmp_siniest
		   SET seleccionado = 0
		 WHERE seleccionado = 1
		   AND no_poliza NOT IN (SELECT no_poliza FROM tmp_productohg);
		   
	       LET v_saber = "";
	ELSE		        -- (E) Excluir estos Registros
			drop table if exists tmp_productohg;
	  SELECT distinct no_poliza
		FROM emipouni
	   WHERE no_poliza in (
			 select no_poliza from tmp_siniest WHERE seleccionado = 1				
		...
		UPDATE tmp_siniest
		   SET seleccionado = 0
		 WHERE seleccionado = 1
		   AND no_poliza IN (SELECT no_poliza FROM tmp_productohg);
	       LET v_saber = " Ex";
	END IF
