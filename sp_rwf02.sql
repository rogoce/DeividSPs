...
CREATE PROCEDURE sp_rwf02(a_cod_cliente CHAR(10), a_motor VARCHAR(30) DEFAULT "%")
...
FOREACH
 SELECT	a.no_poliza,
  ...
  WHERE a.cod_cliente = a_cod_cliente
	...
	AND b.cod_ramo in ('002','020','023')	--> Se agrego para que traiga solo automovil
...
FOREACH
 SELECT	no_poliza,
   ...
   FROM	emipomae 
  WHERE actualizado      = 1
    AND cod_ramo in ('002','020','023')  --> Se agrego para que traiga solo automovil
