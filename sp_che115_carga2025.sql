--Procedimiento para realizar la carga en Rentabilidad1 para bono de RENTABILIDAD 2025
--Usando prima neta cobrada devengada
--Creado 02/04/2025	Armando Moreno M.

DROP PROCEDURE sp_che115_carga2025;
CREATE PROCEDURE sp_che115_carga2025(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
let _fec_aa_ini     = "01/01/2025";	--para sacar cancelada o anulada en el periodo
let _per_ini        = "2025-01";
let _per_ini_ap     = "2024-01";
let _ano            = a_periodo[1,4];
let _ano            = _ano - 1;
let _per_fin_ap     = _ano || a_periodo[5,7];

let _per_fin_dic    = "2024-12";         
...
foreach
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2025',_fecha_aa, my_sessionid);
	if _tiene_fac = 1 then	--Tiene Facultativo
		call sp_dev06f_renta(_no_documento,_fecha_aa,'01/01/2025',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
	else	
		call sp_dev06f(_no_documento,_fecha_aa,'01/01/2025',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
	end if
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2024',_fecha_ap,my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
