...
--
      ...
      FOREACH WITH HOLD
         SELECT x.cod_no_renov,x.cod_ramo,x.no_factura,x.no_documento,x.cod_contratante,
                x.estatus,x.forma_pago,x.cant_pagos, x.suma_asegurada,x.prima,
                x.comision
                INTO _cod_no_renov,v_cod_ramo,v_nofactura,v_nodocumento,v_cod_contratante,
                     v_estatus,v_forma_pago,v_cant_pagos,v_suma_asegurada,
                     v_prima_suscrita,v_comision
                FROM temp_det x
               WHERE x.seleccionado = 1
               ORDER BY x.cod_no_renov,x.cod_ramo,x.no_factura
         ...
         RETURN v_no_renov_desc,v_cod_ramo,v_desc_ramo,v_nofactura,v_nodocumento,
                v_desc_nombre,v_suma_asegurada,v_prima_suscrita,
                v_comision,v_forma_pago,v_cant_pagos,v_descr_cia,
                v_filtros,v_estatus  WITH RESUME;
