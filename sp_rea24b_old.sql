...
drop procedure sp_rea24b;
create procedure "informix".sp_rea24b(a_compania char(3), a_sucursal char(3), a_periodo char(7), a_periodo2 char(7))
returning	char(3),                  --1
			varchar(50);              --2
...
foreach
	select no_documento
	  into _doc_poliza
	  from emipoliza
	 where no_documento in ('0101-00018-02','0215-00103-07')
...
--trace _doc_poliza;
	...
	-- Debo cambiarlo despues de la validacion del saldo
...
--trace on ;
	...
	foreach
		...
		if _no_cambio is null then
			return 1,'No Existe emireaco para la Póliza: ' || trim(_no_poliza) || ' y la Unidad: ' || trim(_no_unidad);
			commit work;
			continue foreach;
		end if
	...
	foreach
		...
		if _saldo_contrato      = 0 or
		   abs(_saldo_contrato) < 1 then
	...
	if _cod_ramo in ('002','020','023') then
		...
		foreach
			...
			foreach
				...
				update tmp_emireaco
				   set porc_partic_suma = porc_partic_suma * _porc_partic_suma
				 where no_poliza        = _no_poliza
				   and no_unidad        = _no_unidad
				   and cod_cober_reas   = _cod_cober_reas;				
			end foreach			
	...
	foreach
		...
		if _ult_no_cambio = _no_cambio and _no_cambio_orig <> _ult_no_cambio then
			let _no_cambio = 1;
		else
			let _no_cambio = 0;
		end if
