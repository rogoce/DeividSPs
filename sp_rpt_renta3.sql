...
DROP PROCEDURE sp_rpt_renta3;
CREATE PROCEDURE sp_rpt_renta3(a_cia CHAR(3),a_cod_agente CHAR(5) default "*",a_periodo char(7))
...
--	  and (pri_sus_pag_aa <> 0 ) or (pri_sus_pag_aa = 0 and sini_inc <> 0) or (pri_sus_pag_aa = 0 and pri_sus_pag_ap <> 0 ))
	...
	select estatus_licencia into _estatus_licencia from agtagent where cod_agente = _cod_agente;
			...
			let _nombre_tipo = 'VIDA INDIVIDUAL y SALUD INVIDIVUAL';   -- Caso Especial para rentabilidad3 para: 019 o (018 y concurso = 1) HGIRON
			--se separa los ramos 019 y 018
				if  _cod_tipo = '002' then 
				    let _nombre_tipo = 'SALUD INDIVIDUAL' ;
		        end if
				if  _cod_tipo = '004' then 
				    let _nombre_tipo = 'VIDA INDIVIDUAL' ;
		        end if
...
FOREACH
	...
	if _porcentaje <> 0 then	
		...
		RETURN  v_nombre_cia,
				...
				_siniestralidad/100,
				_porcentaje/100,
				_valor_prima,_periodo2 WITH RESUME;	
	end if
