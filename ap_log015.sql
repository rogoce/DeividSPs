...
-- SIS v.2.0 - d_cobr_sp_cob748h_dw1 - DEIVID, S.A.	 -- x acreedor 
 ...
 DROP PROCEDURE ap_log015; 
CREATE PROCEDURE "informix".ap_log015(a_compania CHAR(3),a_cobrador CHAR(3) DEFAULT '*',a_tipo_aviso SMALLINT,a_agente CHAR(5) DEFAULT '*',a_acreedor CHAR(5) DEFAULT '*', a_asegurado CHAR(10) DEFAULT '*',a_callcenter SMALLINT DEFAULT 0, a_referencia varchar(255) default "*")
...
--     and marcar_entrega <> 1  -- 30/5/2016 si se entrego no mostrarlo 
	 --and estatus <> 'Y'
	and ((estatus = 'Y' and clase = 2) or (estatus = 'I' and clase in (0,2))) 																															   
...
if a_tipo_aviso <> '6' then 
	...
	{	select valor_parametro 
		...
		let _siguiente = _reporte_certifica; 
			...
			into _no_documento --,_renglon
			  ...
			  where id_aviso = _no_aviso
			   --order by salto_pag,numero
			     ...
			     --AMORENO//27/03/2020//se repite en meses diferente
			   		let _buscarex = 0;
					select count(*)
					  into _buscarex
					  from cobccert1
					 where numero  = _siguiente_char 
					   and no_documento = _no_documento;
					   
						if _buscarex is null  then
							let _buscarex = 0;
						end if				   
		...
		into __numero, _no_documento  
		  ...
		  update avisocanc 
			  ...
			  --set marcar_certifica = 1,reporte_certifica = _siguiente, fecha_certifica = _fecha_actual, user_certifica = user_imp_aviso_log 
				set marcar_certifica = 1,reporte_certifica = _siguiente_char, fecha_certifica = _fecha_actual, user_certifica = user_imp_aviso_log 		  		  
