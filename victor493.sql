...
select saldo into v_saldo from emipomae
 where no_poliza = v_poliza;

let v_impto = 0.00;
select Sum(y.factor_impuesto) Into v_impto From emipolim x, prdimpue y
 where x.no_poliza    = v_poliza
   and x.cod_impuesto = y.cod_impuesto
   and y.pagado_por   = "C";
if v_impto is null then
   let v_impto = 0.00;
end if
let v_impto = v_impto + 100;
let v_saldo = ((v_saldo * 100) / v_impto);
let v_tot_saldo = v_saldo;

foreach 
   select no_unidad, cod_producto, prima
     Into v_unidad, v_producto, r_prima_unidad
     from emipouni
    where no_poliza   = v_poliza

   select Sum(prima) Into v_prima_total From emipouni
    where no_poliza  = v_poliza;

   let v_prima = (r_prima_unidad * v_saldo) / v_prima_total;

   let v_cober_total = 0.00;
   Select Sum(prima_anual) Into v_cober_total From emipocob
    Where no_poliza  = v_poliza
      and no_unidad  = v_unidad;
   if v_cober_total is null then
      let v_cober_total = 0.00;
   end if
   ...
   foreach
	  ...
	  If v_acepta = 1 Then
   	     let v_porc_descto = 0.00;
	     foreach
	  	    select x.porc_descuento Into v_porc_descto from emiunide x
	   	     where x.no_poliza  = v_poliza
			   and x.no_unidad  = v_unidad
      ...
      let v_recargo = 0.00;
      select sum(x.porc_recargo) into v_recargo from emiunire x
       where x.no_poliza  = v_poliza
         and x.no_unidad  = v_unidad;
      if v_recargo is null then
	     let v_recargo = 0.00;
      end if
      let v_recargo = (v_recargo * (v_prima_cob - v_tot_descto) / 100);
      let v_tot_saldo = v_tot_saldo - v_recargo + v_tot_descto;
   end foreach
end foreach

let v_impto   = 0.00;
let v_recargo = 0.00;
let v_saldo   = v_tot_saldo;
--let v_saldo   = v_prima;
