...
drop procedure sp_sis171;
create procedure sp_sis171(a_no_remesa char(10))
...
foreach
	...
	--***Programacion para arreglar emireaco si amerita, para ramo de automovil . AMM 28/01/2025
	if _cod_ramo in('002','023','020') then
		let _error = sp_arregla_emireaco_auto(_no_poliza, 1);
		if _error <> 0 then
			let _mensaje = 'Error en Distribucion de Reaseguro Auto, para Esta Poliza: '|| trim(_no_documento) ||', Por Favor Verifique ...';
	...
	if _cod_grupo = '77960' then
		call sp_sis171_banisi(_no_poliza,_no_cambio) returning _error, _mensaje;
	else
		call sp_sis171_3en1(_no_poliza,_cod_ramo,_no_cambio) returning _error, _mensaje;
	end if
	if _error <> 0 then
		return _error,_mensaje;
	end if
