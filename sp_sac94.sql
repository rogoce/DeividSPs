...
DEFINE ll_ciclo,_cnnt        INTEGER;
DEFINE ld_fecha_inicio date;
DEFINE ld_fecha_final,_fecha_valida_ano  date;
...
let _ano       = w_par_anofiscal;
LET mes        = w_par_mesfiscal;
--    LET ano      = w_par_mesfiscal;

IF w_par_mesfiscal = "14"  THEN

	let _fecha_valida_ano = MDY(12, 31, _ano);
	select count(*)
	  into _cnnt
      from cglresumen
     where res_tipcomp = '021'
       and res_fechatrx = _fecha_valida_ano;
	...
	if _cnnt = 0 THEN
		LET l_codigo = 1;
		LET mensaje_error = "NO PROCEDE EL CIERRE DE MES "||w_par_anofiscal ||"-"|| w_par_mesfiscal ||", NO EXISTE COMPROBANTE DE CIERRE.";
...
--****Caso 15215 Yasmin
--Agradezco evaluar que los saldos iniciales de las cuentas de resultados 4, 5, 6, 7 al mes de enero se mantengan en saldo 0.00,
--ya que mensualmente cuando se cierra un mes contable en el Deivid, los saldos iniciales de estas cuentas traen montos de diciembre
--del año previo lo cual no es correcto.

update cglsaldoaux
   set sld_incioano = 0
 where sld_cuenta like '7000%' 
   and sld_ano = w_par_anofiscal;
   
update cglsaldoaux
   set sld_incioano = 0
 where sld_cuenta like '6000%' 
   and sld_ano = w_par_anofiscal;
   
update cglsaldoaux
   set sld_incioano = 0
 where sld_cuenta like '5000%' 
   and sld_ano = w_par_anofiscal;
   
update cglsaldoaux
   set sld_incioano = 0
 where sld_cuenta like '4000%' 
   and sld_ano = w_par_anofiscal;
