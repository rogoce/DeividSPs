...
drop procedure sp_cob356b;
create procedure sp_cob356b(a_fecha_desde date, a_dias_vencida smallint)
returning	integer,
			varchar(100);
...
foreach
	select distinct l.no_documento,
	 ...
	 where e.no_poliza = l.no_poliza
	   ...
	   and l.monto_pen > 0.05   --SD#10166 ENILDA 16052024HG
	...
	if _cod_grupo in ('00000','1000','1090','1009') then --grupos del Estado, SCOTIA BANK y BAGATRAC
		continue foreach;
	end if
	...
	select cod_ramo,
	 ...
	 if _exigible <= 0 then
	     continue foreach;  --SD#10166 ENILDA 16052024HG
	 end if
