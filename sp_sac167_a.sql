...
DROP PROCEDURE sp_sac167_a;
CREATE PROCEDURE sp_sac167_a(a_tipo char(2),a_cuenta char(12),a_aux char(5),a_anio char(4),a_mes smallint,a_db CHAR(18)) 
RETURNING   CHAR(7),	--  Periodo
			CHAR(5),	--  Tercero
			CHAR(50),	--  Nombre
			DEC(15,2),  --  Monto Sld Inicial  4
			DEC(15,2),  --  Monto Debito 	5
			DEC(15,2),	--  Monto Credito 	 6
			DEC(15,2),	--  Monto Neto		  7
			DEC(15,2),	--  Monto Acumulado    8	
...
DEFINE _fecha    		  DATE;
DEFINE _c_agent			  CHAR(5);
DEFINE _c_licencia		  CHAR(10);
define v_nom_terc_lk      CHAR(60);
...
FOREACH
	...
	order by aux_tercero
	   ...
	   --	LET v_saldo = v_saldo_ant;
		...
		--IF v_saldo <> 0 and v_credito <> 0 and v_debito <> 0 THEN
			...
			VALUES(	v_periodo,
			     ...
			     v_credito,
				 v_debito + v_credito,
				 v_monto,    -- v_saldo + v_debito + v_credito,
				 l_cia_nom,
				 l_nombre ) ;
...
--	order by periodo,tercero,nombre


-- Cambios solicitado . Sr. Naranjo 30/04/2010
		let v_nom_terc_lk = upper(v_nom_terc)||"%";
		let _c_agent = "";
		let _c_licencia = "";

		FOREACH	
		  SELECT cod_agente,no_licencia				 
			INTO _c_agent,_c_licencia				 
		    FROM  deivid:agtagent
		    WHERE upper(nombre) like (v_nom_terc_lk)		     
			EXIT FOREACH;
		END FOREACH;
