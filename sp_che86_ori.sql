...
foreach
	...
	insert into fis_concurso(no_documento, pri_pag,tipo)
...
foreach
 select doc_remesa,
  ...
  where periodo     >= _per_ini_ap		--2015-10
	...
	insert into fis_concurso(no_documento, pri_pag_ap,tipo,pri_can_ap)
...
foreach
 select doc_remesa,
  ...
  where periodo     >= _per_siguiente
	...
	insert into fis_concurso(no_documento, pri_can_ap)
	values (_no_documento, _monto);
end foreach
--**********************************************************************************************
-- Prima Suscrita Actual periodo del concurso  2016-10 A 2017-09
--**********************************************************************************************
foreach
	select no_poliza,
		   no_endoso,
...
foreach
 select no_poliza,
  ...
  where actualizado  = 1
	...
	insert into fis_concurso(no_documento, pri_dev_ap)
	values (_no_documento, _prima_suscrita);
...
foreach
 select no_poliza,
  ...
  where actualizado  = 1
	...
	insert into fis_concurso(no_documento, pri_can)
	values (_no_documento, _prima_suscrita);
...
--**********************************************************************************************
INSERT INTO fis_concurso(no_documento, pri_dev) 
...
foreach
 select doc_poliza,
   ...
   from tmp_sinis
	...
	insert into fis_concurso(no_documento, sin_pag_aa)
...
foreach 
	 ...
	 having sum(variacion) > 0 
		...
		insert into fis_concurso(no_documento, sin_pen_ap)
...
foreach 
 ...
 having sum(variacion) > 0 
	...
	insert into fis_concurso(no_documento, sin_pen_aa)
...
foreach
	 ...
	 if _pri_sus_ap <= 0 then--ES AGENTE NUEVO, LO COLOCO EN RANGO 6
