...
DROP PROCEDURE sp_rec49b;

CREATE PROCEDURE "informix".sp_rec49b(
		a_compania  CHAR(3),
		a_agencia   CHAR(3),
		a_periodo1  CHAR(7),
		a_periodo2  CHAR(7),
...
FOREACH 
 SELECT x.no_reclamo,	
  ...
  WHERE x.cod_compania = a_compania
    AND y.no_reclamo   = x.no_reclamo
    AND x.actualizado  = 1
    AND x.cod_tipotran MATCHES '*' -- Para que incluya el indice creado
	AND y.no_poliza    =  a_poliza
	AND x.periodo      >= a_periodo1 
	AND x.periodo      <= a_periodo2
    AND x.variacion    <> 0
...
{UPDATE tmp_sinis
   SET seleccionado = 0
 WHERE seleccionado = 1
   AND no_poliza NOT IN (SELECT no_poliza FROM tmp_poliza);
