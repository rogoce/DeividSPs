...
if _es_medico = 1 then	--busca si hay uno pendiente para el medico.
	SELECT count(*)
	  INTO _cantidad
	  FROM emievalu
	 WHERE escaneado   = 1
	...
	if _cantidad > 0 then   --Si hay, se busca el mas viejo de ese usuario y se devuelve.
		...
		Return 0, _n_contratante, _no_evaluacion, _fecha, _no_recibo, _fecha_recibo, _monto, _cod_asegurado, _cod_producto;
	end if
