...
DROP PROCEDURE sp_che115_carga2016bk;
CREATE PROCEDURE sp_che115_carga2016bk(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
create index idx_tmp_che115a1 on fis_che115a(no_documento);}
...
{foreach	with hold
 ...
 select no_documento,
  ...
  select cod_ramo into _cod_ramo from emipoliza
  where no_documento = _no_documento;
  if _prima_suscrita is null then
	let _prima_suscrita = 0.00;
  end if
  if _cod_ramo = '001' then
	let _prima_suscrita = _prima_suscrita * 0.70;
  end if
...
foreach
	...
	if _cnt > 0 then		-- Facultativos, solo nuestra parte
		...
		if _cod_ramo = '001' then
			let _prima_ret_aa = _prima_ret_aa * 0.70;
			let _prima_ret_ap = _prima_ret_ap * 0.70;
		end if
		update fis_che115a
	...
	foreach			   --PRIMER EMIPOAGT
	 ...
	 if _flag in(1,2,3,4,5,6,7,8) then
	 	continue foreach;
	 end if
