--***************************************************************************************
-- Procedimiento que genera el Reporte para convencion a TOKIO 2023 para los corredores
--***************************************************************************************
--CONVENCION TOKYO 2023
...
let _per_ini      = "2022-01";

-- Periodo Final del Concurso

if a_periodo > "2022-12" then
	let a_periodo = "2022-12";
end if

-- Periodo Pasado
let _ano            = _per_ini[1,4];		  --2021
let _ano            = _ano - 1;				  --2020
let _per_ini_ap     = _ano || _per_ini[5,7];  --2020-01

let _ano            = a_periodo[1,4];		  --2021
let _ano            = _ano - 1;				  --2020
let _per_fin_ap     = _ano || a_periodo[5,7]; --2020-01
...
let _fecha_aa_ini = mdy(_per_ini[6,7], 1, _per_ini[1,4]);         --es del 01/01/2021
let _fecha_ap_ini = mdy(_per_ini_ap[6,7], 1, _per_ini_ap[1,4]);	  --es del 01/01/2020
let _fecha_aa     = sp_sis36(a_periodo);   --31/01/2021
let _fecha_ap     = sp_sis36(_per_fin_ap); --31/01/2020

SET ISOLATION TO DIRTY READ;

--delete from milan08;  se pasa al final
...
let _nombre_tipo = "";
--*************************************************************************************
-- Prima neta cobrada devengada periodo convencion 2022-01 Al periodo se esta evaluando
--*************************************************************************************
...
foreach
	...
	call sp_dev06f(_no_documento,_fecha_aa,'01/01/2022',_fecha_aa) returning _valor,_mensaje,_pri_dev_aa;
...
foreach
	select no_documento
	  into _no_documento
	  from emipoliza
	 where vigencia_fin >= '01/01/2022'
	   and vigencia_inic < '01/01/2022'
...
--**********************************************************************
-- Prima Pagada periodo convencion 2022-01 Al periodo se esta evaluando
--**********************************************************************
...
foreach
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/01/2022' and '31/12/2022';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/01/2022' and _fecha_anulado <= '31/12/2022' then
					let _monto_dev = 0;
				end if
			end if			
		else
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2022','31/12/2022',my_sessionid); --se cambia a tabla fisica 05/06/2018
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
--************************************************************************
call sp_bo077_2('01/01/2021', _fecha_dic_ap, _fecha_aa) returning _error, _error_desc; --01/01/2020, 31/01/2019, 31/01/2020 tomando como periodo de ejemplo 2020-01
...
----**********************************************************************************************
call sp_bo077_2('01/01/2022', _fecha_dic_aa, _fecha_aa) returning _error, _error_desc;	--01/10/2019, 31/01/2020, 31/01/2020 tomando como periodo de ejemplo 2020-01
...
foreach
	...
	if year(_date_added) = 2022 then --ES AGENTE NUEVO, LO COLOCO EN RANGO 6
