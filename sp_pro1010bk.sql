...
--			AND seleccionado = 1
         ...
         {SELECT min(vigencia_inic_pol),min( vigencia_final_pol)
		   INTO v_vig_ini, v_vig_fin
           FROM endedmae
          WHERE (periodo >= a_periodo1  
            AND periodo <= a_periodo2) 
...
--			 AND no_factura     = v_nofactura
		...
		foreach
		 select distinct month(vigencia_inic)
		  ...
		  where trim(no_documento) = trim(v_nodocumento)
		   ...
		   if v_contador = 0 and month(v_ult_vig) = 1 then
			   let v_mes  = 0;
		   else
      ...
      Trace off;		  		   
		   while v_diferencia >= 1				
				   ...
				   if v_anio = _anio_ant and v_mes = _mes_ant then 
					  LET v_diferencia = -1;
				   end if
