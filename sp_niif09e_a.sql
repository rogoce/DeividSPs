...
-- execute procedure sp_niif09e('2025-06','2025-06','001,002,003,004,005,006,007,008,009,010,011,012,013,014,015,016,017,018,019,020,021,022,023;')
-- SIS v.2.0 - DEIVID, S.A.

drop procedure sp_niif09e_a;
create procedure sp_niif09e_a(a_periodo_desde char(7),a_periodo_hasta char(7), a_cod_ramo varchar(255) default "*")
...
FOREACH
	select trx.no_reclamo,
	 ...
	 inner join tmp_sinis sen on sen.no_reclamo = rec.no_reclamo
--	 left join chqchrec trc on trx.no_tranrec = trc.no_tranrec
	 ...
	 where trx.actualizado  = 1
	   and trx.cod_tipotran not in ('004','005','006','007')
	...
	if _cod_ramo = '018' then
		select mae.fecha,
		  ...
		  into _fecha_bloque,
			   _fecha_factura
		  from rectrmae trx
		  left join atcdocde det on det.cod_asignacion = trx.cod_asignacion
		  left join atcdocma mae on mae.cod_entrada = det.cod_entrada
		 where trx.no_tranrec = _no_tranrec;
	else
		let _fecha_bloque = null;
		let _fecha_factura = null;
	end if
...
--set debug file to "sp_niff09e.trc";
--trace on;
