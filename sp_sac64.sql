...
CREATE PROCEDURE sp_sac64(v_compania char(3), v_notrx integer, a_usu_act char(8))
	...
	-----------------------------------------------#
	 SELECT cta_nivel, cta_tippartida, cta_recibe, cta_histmes,
	  ...
	  END IF
		...
		-------------------------------------------------------------------------------
		let w1_usuario = trim(w1_usuario);
		let a_usu_act  = trim(a_usu_act);
		
		if w1_origen = "CGL" and v_compania = "001" then
		
			if w1_usuario = a_usu_act THEN					--Colocado por AMM 22/05/2024
				return 1, "No puede actualizar el mismo usuario.";
			end if	
