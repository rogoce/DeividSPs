...
DROP PROCEDURE sp_pro77c;

CREATE PROCEDURE "informix".sp_pro77c(a_compania CHAR(03), a_terremoto SMALLINT, a_fecha DATE) 
...
if _error <> 0 then
	RETURN _error_desc,'',_error,0.00,0.00,0,0.00,0,0.00,0,0.00,'';
end if
  ...
  FOREACH WITH HOLD
	  SELECT e.cod_ramo,
	         e.cod_subramo,       
			 SUM(t.cantidad),        
			 SUM(t.suma_asegurada),  
			 SUM(t.retencion), 
			 SUM(t.cant_ret),      
			 SUM(t.primer_excedente),
			 SUM(t.cant_exe),
			 SUM(t.facultativo),
			 SUM(t.cant_fac),     
			 SUM(t.prima_terremoto)
		INTO _cod_ramo,      	  
		     _cod_subramo,
			 v_cnt_poliza,     
			 v_suma_asegurada, 
