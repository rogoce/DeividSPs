...
-- Creado    : 13/01/2022 - Autor: Armando Moreno M.
-- Modificado: 13/01/2022 - Autor: Armando Moreno M.
...
define _es_mensual,_fronting      smallint ;
...
FOREACH
	...
	if _cod_ramo = '001' and _cod_subramo = '006' then	  --No va poliza de Incendio subramo ZonaLibre  Incendio
		continue foreach;
	end if
	if _cod_ramo = '003' and _cod_subramo = '005' then	  --No va poliza de Incendio subramo ZonaLibre Multiriesgo
		continue foreach;
	end if
	...
	IF _tipo_prod in(4,3) THEN	-- excluir Reaseguro Asumido,excluir coas minoritario
	   CONTINUE FOREACH;
	END IF
	--Excluir grupo Traspaso Assa - Generali - Banisi correo 17/10/2019 Daivis F.
	if _cod_grupo in('77850',"00000","1000",'1122','77960','77982') then --excluir estado. excluir banisi ducruet segun caso 4356 fecha mod. 25/08/2022 -- SD#5708 23/02/2023 HG
		continue foreach;
    end if
	...
	FOREACH
		...
		if _cod_agente in('02569','02656','02111','02825','02531','02830','02831') then	--se excluyen estos corredores por instr. Analisa, correo del 09/07/2019
			continue foreach;
		end if
		...
		if _cod_agente in('02618','02532','02531','02904') then
			let _tipo_agente = 'A';
