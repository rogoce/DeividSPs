...
DROP PROCEDURE sp_che86_genesis2;
CREATE PROCEDURE sp_che86_genesis2(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
define _mes_pnd	,_vigaa, _renovaper		smallint;
...
foreach
	...
	if _renovaper = 1 then
		let _no_pol_ren_aa_per = 1;
	end if
	
		update milan08
		   set pri_sus_pag_aa = pri_sus_pag_aa + _pri_pag,
		       vigenteaa      = _no_pol_nue_aa,
			   pri_pag_ap     = _pri_dev_ap,
			   renovaa_per    = _no_pol_ren_aa_per
		 where no_documento = _no_documento
           and cod_agente   = _cod_agente;
