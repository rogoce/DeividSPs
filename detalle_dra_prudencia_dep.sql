select replace(ase.cedula,'-','') as _id ,ase.aseg_primer_ape,trim(nvl(ase.aseg_primer_nom,'')) || ' ' || trim(nvl(ase.aseg_segundo_nom,'')),trim(nvl(ase.aseg_primer_ape,'')) || ' ' || trim(nvl(ase.aseg_segundo_ape,'')),
       '-1' as  relacion,decode(ase.sexo,'F',0,'M',1) as genero,ase.fecha_aniversario,replace(ase.cedula,'-','') as di,min(uni.vigencia_inic) as fecha_inicio,max(uni.vigencia_final),ram.nombre as ramo,ase.e_mail,ase.celular,1 as operacion,
       ase.cod_cliente,emi.no_documento
  from emipomae emi
