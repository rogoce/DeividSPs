...
drop procedure sp_rec133;

create procedure "informix".sp_rec133()
returning integer,
		  char(20),
          char(10),
		  dec(16,2),
...
foreach
 select r.no_reclamo,
   ...
   into _no_reclamo,
		...
		_cod_recla
   from	recrcmae r
  where r.periodo    >= "2007-07"
    and r.actualizado = 1
    and r.no_reclamo  in (select no_reclamo from tmp_reclamos)
	...
	foreach
	 select no_tranrec,
      ...
      order by wf_apr_j_fh, wf_apr_jt_fh, wf_apr_jt_2_fh, wf_apr_g_fh, periodo, fecha, transaccion
		...
		if _reser_acum_neto < -0.05 then
			let _asterisco = "$";
		end if	

		if _reser_acum_exec < -0.05 then
			let _asterisco = "%";
		end if	
		...
		if _asterisco <> "" then
			...
			if _asterisco = "*" then
				call sp_rec151(_transaccion, _asterisco, _incurrido_bruto, _inc_neto_acum, _suma_max_ret) returning _error, _error_desc;
			end if

			if _asterisco = "$" then
				call sp_rec151(_transaccion, _asterisco, _variacion_tr, _reser_acum_neto, _suma_max_ret) returning _error, _error_desc;
			end if

			if _asterisco = "?" then
				call sp_rec151(_transaccion, _asterisco, _variacion_tr, _reser_acum_exec, _suma_max_ret) returning _error, _error_desc;
			end if
