...
drop procedure sp_cob101p;

create procedure sp_cob101p(
a_compania 		CHAR(3),
a_agencia  		CHAR(3),
a_cobrador 		CHAR(3),
...
--set debug file to "sp_cob101p.trc";
--trace on;
	   ...
	   select count(*)
		...
		foreach
			...
			if _cantidad = 0 and _existe = 0 then  --no existe
				insert into cobca90p(cod_cliente, fecha, procesado)
				values (_cod_cliente, _fecha_pago, 0);
			end if
