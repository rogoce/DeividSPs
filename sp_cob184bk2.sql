...
DROP PROCEDURE sp_cob184bk2;

CREATE PROCEDURE "informix".sp_cob184bk2(
a_compania		CHAR(3),
a_sucursal		CHAR(3),
a_user			CHAR(8),
a_turno			integer,
		...
		foreach
		   ...
		   if _flag2 = 0 then
			   ...
			   foreach
				   ...
				   {if _num_doc is not null then
					  for z = 1 to 30
						if _num_doc[z,z] in ("-", "'", "_", "*") then
							let _num_doc[z,z] = "";
						end if
...
{
trace "Remesa " || a_no_remesa; 
trace _id_det; 
trace _id_tipo_cobro;
trace _tipo_tarjeta;
trace _cod_banco;
trace _fecha_documento;
trace _num_doc;
trace _nombre_cte;
trace _monto_trancobro;
}
...
{
trace "id_cliente "      || _id_cliente;
trace "_cod_cobrador "   || _cod_cobrador;
trace "_dia_cobros1 "    || _dia_cobros1;
trace "_fecha_registro " || _fecha_registro;
trace "ld_fecha_hora " 	 || ld_fecha_hora;
}
