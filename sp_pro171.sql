-- Reporte de poliza con prima dif. al del producto 2006-2007 

-- Creado: 14/08/2006 - Autor: Amado Perez Mendoza 

-- SIS v.2.0 - d_prod_sp_pro171_dw1 - DEIVID, S.A.
-- SIS v.2.0 - d_prod_sp_pro76_crit - DEIVID, S.A.

drop procedure sp_pro171;
create procedure sp_pro171(a_compania char(3), a_sucursal char(3), a_fecha date, a_periodo char(7)) 
returning date,
		  char(100),
		  char(20),
		  varchar(100),
...
foreach
 ...
 --			VALUES(_no_documento, a_periodo, "POLIZA AUN NO CUMPLE ANIVERSARIO");
  ...
  {		if _prima_plan is null then
		...
		let _prima_nueva  = (_prima_nueva - _prima_vida) * _porc_impuesto;	--el impuesto no incluye la prima de vida
