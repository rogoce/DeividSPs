...
define _no_documento_old			char(20);
...
let _no_documento_old = v_no_documento;
...
--Forma de pago
if _responsable_cobro in ('Ducruet','DUCRUET','SEMUSA','ROLKAM') then
...
LET _porc_comision = sp_pro305(v_cod_agente, _cod_ramo,_cod_subramo);
...
let v_no_documento = sp_sis19(_cod_compania, _cod_sucursal, v_poliza_nuevo);

update emipomae
   set no_documento = v_no_documento
 where no_poliza = v_poliza_nuevo;
...
update prdemielctdet
 ...
 where cod_agente	= v_cod_agente
   and num_carga	= v_num_carga
   and proceso		= a_opcion
   and no_documento	= _no_documento_old;
