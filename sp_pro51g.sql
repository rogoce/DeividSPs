...
drop procedure sp_pro51g;

create procedure "informix".sp_pro51g(
a_poliza	char(10),
a_producto	char(5),
a_ramo		char(3),
a_unidad	char(5),
...
if a_ramo = '002' then
	...
	if _cod_subramo = '001' then
		if _opcion = "A" or _opcion is null or trim(_opcion) = "" then
			let _ld_tarifa = _ld_tarifa + _ld_tarifa * ld_rec_pri_a / 100;
		elif _opcion = "B" then
			let _ld_tarifa = _ld_tarifa + _ld_tarifa * ld_rec_pri_b / 100;
		elif _opcion = "C" then
			let _ld_tarifa = _ld_tarifa + _ld_tarifa * ld_rec_pri_c / 100;
		end if
	end if
