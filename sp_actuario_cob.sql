drop procedure sp_actuario_cob;
create procedure sp_actuario_cob(a_periodo_desde char(7), a_periodo_hasta char(7))
...
on exception set _error,_error_isam,_error_desc
	...
	rollback work;
	let _error_desc = trim(_error_desc) || '3no_poliza: ' || _id_poliza || 'no_endoso: ' ||  _no_endoso;
	return _error,_error_desc;
end exception

{set debug file to "sp_actuario_cob.trc";
trace on;}
...
--*****BORRAR MANUALMENTE ESTAS TABLAS ANTES DE EJECUTAR EL PROGRAMA
...
{foreach
	select distinct no_remesa,
	  ...
	  into _no_remesa_o,
		   _renglon_o
	  from tmp_ttco_dist
	 where no_remesa not in ('854019','847559')
	 {where id_mov_tecnico_anc in (6092917,6092918,6092920,6092921,6106135,6106136,6106137,6106138,6106868,6106869,6106870,6106871,6106894,6106895,6106896,6106897,6109348,6109349,6109354,
									6109355, 6109375,6109376,6109378,6109379,6109423,6109424,6109429,6109430,6111687,6111688,6120481,6120482,6120484,6120485,6120499,6120503,6120504,6120511,
									6120512,6125334,6125335,6131373,6131374,6133297,6133298,6133318,6133319,6138243,6142736,6145463,6145464,6145486,6145487,6145489,6145490,6145494,
									6145495,6145500,6145501,6165069,6165070,6166689,6166690,6166692,6166693)}
...
foreach with hold
	select no_poliza,
	 ...
	 --and no_documento = '0218-01697-03'
