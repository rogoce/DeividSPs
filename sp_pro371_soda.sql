-- Procedimiento que carga las el archivo de renovaciones.
-- creado    : 05/10/2012 - Autor: Roman Gordon--
-- sis v.2.0 - deivid, s.a.

drop procedure sp_pro371_soda;
create procedure "informix".sp_pro371_soda(a_periodo char(7))
returning   integer,
			char(100);   -- _error
...
let _periodo = a_periodo;
let _mes_periodo = a_periodo[6,7];
let _ano_periodo = a_periodo[1,4];

let _fecha_desde = mdy(_mes_periodo,1,_ano_periodo);
...
foreach
	select e.no_poliza,
		   ...
		   cod_contratante,
		   no_pagos,
		   to_char(vigencia_inic + 1 units year,"%m/%d/%Y"),
		   to_char(vigencia_final + 1 units year,"%m/%d/%Y"),
		   prima,
		   descuento,
		   prima_bruta,
		   impuesto,
	 ...
	 where a.no_poliza = e.no_poliza
	   ...
	   and vigencia_final between _fecha_desde and _fecha_hasta
	   and actualizado = 1
	...
	if _cod_ramo not in ('002','020') or _flag_agt = 0 then
