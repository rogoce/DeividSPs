...
drop procedure sp_che134;

create procedure sp_che134(a_fecha date, a_fecha2 date, a_cod_banco char(3), a_cod_chequera char(3))
 returning char(10),
		   char(10),
		   char(100),
		   dec(16,2),
...
foreach
 select	no_requis,
  ...
  where anulado         = 1
	...
	and cod_chequera    = a_cod_chequera
	and fecha_anulado between a_fecha and a_fecha2
	...
	foreach
	 select	a.no_requis,
	   ...
	   into	_no_requis,
			...
			_anulado_por,
			_fecha_impresion
	   from	chqchmae a, chqchrec b
	  where a.no_requis     = b.no_requis
	    and a.anulado         = 1
		and a.cod_banco       = '001'
		and a.cod_chequera    = '001'
		and a.fecha_anulado between a_fecha and a_fecha2
		and b.numrecla[1,2] in ('16','19')
