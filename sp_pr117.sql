...
-- 		FASCILIDAD PROVINCIAL -INCENDIO  
-- 		PRIMA SUSCRITA
-- execute PROCEDURE sp_pr117 ("001","001","2009-07","2009-09","*","*","*","*",
-- "001,003,002,010,011,012,013,014;","*","*","*")
---------------------------------------------------------------------------------
DROP PROCEDURE sp_pr117;
CREATE PROCEDURE sp_pr117(
		a_compania    CHAR(03),
		a_agencia     CHAR(03),
		a_periodo1    CHAR(07),
		a_periodo2    CHAR(07),
...
BEGIN
		...
		LET _borderaux = "05";	    -- REASEGURO PROVINCIAL
	    ...
	    DELETE FROM temphg1 where anio = _anio_reas and trimestre = _trim_reas and borderaux = _borderaux;
...
--		LET v_filtros = sp_rec61(
	 LET v_filtros = sp_rec708(	 a_compania,
	 a_agencia,
	 a_periodo1,
	 a_periodo2,
	 a_codsucursal,
...
--	  set debug file to "sp_pr117.trc";	 																						 
      ...
      FOREACH WITH HOLD
         ...
         FOREACH
				...
				if v_cod_contrato <>  "00585" and v_cod_contrato <>  "00576" then  -- Contrato Provincial 2009,2008
					CONTINUE FOREACH;
				end if
...
-- CARACTERISTICAS : del 2007 hasta la fecha, contrato = '00585','00576'
-- and cod_cobertura = '001' --and cod_ramo = '001' --and cod_coasegur = '036' 
-- trace on;


{FOREACH
   select serie,cod_cobertura,tipo_contrato,cod_ramo,sum(prima)
   into _serie,v_cobertura,v_tipo_contrato,v_cod_ramo,v_prima
   from temphg
   Where serie >= 2007 and seleccionado = 1
	and anio      = _anio_reas
	and trimestre = _trim_reas
	and borderaux = _borderaux 
   group by serie,cod_cobertura,tipo_contrato,cod_ramo}
...
--			and   t.cod_contrato in ("00585","00576")
			;  
