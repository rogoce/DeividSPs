...
DROP procedure sp_aud42b;
CREATE procedure "informix".sp_aud42b(a_periodo1 CHAR(07))
RETURNING int,
          int,
		  char(4),
		  char(2),
   ...
   BEGIN
	  ...
	  define v_prima_suscrita_agt      		 dec(16,2);
...
{	  let v_ano = a_periodo1[1,4];
 ...
 {     CALL sp_pro34(a_compania,a_agencia,a_periodo1,
      ...
      FOREACH WITH HOLD
         SELECT x.cod_ramo,
          ...
          select porc_partic_coas
           ...
           if _porc_partic_coas is null then
            ...
            SELECT porc_comis_agt, porc_partic_agt
              INTO v_porc_comis, _porc_partic_agt
              FROM endmoage
             WHERE no_poliza = v_nopoliza
  	           and no_endoso = v_noendoso
		 ...
		 let v_prima_suscrita_agt = v_prima_suscrita_agt * _porc_partic_coas / 100;

        
		 foreach 
		  select no_unidad, suma_asegurada, prima_suscrita
		    into _no_unidad, v_suma_asegurada, _prima_suscrita_uni
			from endeduni
		   where no_poliza = v_nopoliza
			 and no_endoso = v_noendoso	

           let _prima_suscrita_uni = _prima_suscrita_uni * _porc_partic_coas / 100;

           let _prima_suscrita_uni = _prima_suscrita_uni * _porc_partic_agt / 100;

		   let _diferencia = 0;

			if v_cod_ramo = '002' then
				call sp_sis188b(v_nopoliza,v_noendoso,_no_unidad) returning _error,_mensaje;
...
{           select count(*)
 ...
 --			 group by b.cod_cober_reas;
           ...
           select nombre
            ...
            LET _prima_suscrita = v_prima_suscrita * _porc_partic_agt / 100;

            LET v_comision     = _prima_suscrita * v_porc_comis / 100;

           let _diferencia = _diferencia + _prima_suscrita;
