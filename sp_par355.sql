...
drop procedure sp_par355;		
create procedure "informix".sp_par355(a_no_tranrec CHAR(10))
returning integer,
		  char(100);									  
...
--		end if
	...
	if _variacion_bru <> 0.00 then
		...
		let _cuenta = sp_sis15('RPRDSET', '04', _cod_origen,_cod_ramo,_cod_subramo);
	...
	if _monto2 <> 0.00 then
		...
		let _cuenta = sp_sis15('RPRDSMR', '04', _cod_origen,_cod_ramo,_cod_subramo);
	...
	if _variacion_net <> 0.00 then
		...
		let _cuenta = sp_sis15('RGADRST', '04', _cod_origen,_cod_ramo,_cod_subramo);
...
if _tipo_transaccion  = 4 and
   _monto            <> 0 then
	...
	let _cuenta = sp_sis15('RGSP', '04', _cod_origen,_cod_ramo,_cod_subramo);
