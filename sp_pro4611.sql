...
drop procedure sp_pro4611;

create procedure "informix".sp_pro4611(
v_poliza char(10), 
v_endoso char(5))
...
Select SUM(x.prima), SUM(x.descuento), SUM(x.recargo), SUM(x.prima_neta),
       SUM(x.impuesto), SUM(x.prima_bruta),
       SUM(suma_asegurada), SUM(x.prima_suscrita), SUM(x.prima_retenida)
  Into r_prima, r_descuento, r_recargo, r_prima_neta, r_impuesto, r_prima_bruta, 
       r_suma_aseg, v_prima_suscrita, v_prima_retenida
  From endeduni x
