...
let _per_fin_ap     = _ano || a_periodo[5,7]; --2015-04
...
foreach
	select doc_remesa,
	  ...
	  into _no_documento,
		   ...
		   _renglon
	  from cobredet
	 where periodo     >= _per_ini			--2015-10
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/10/2015' and '30/09/2016';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/10/2015' and _fecha_anulado <= '30/09/2016' then
					let _monto_dev = 0;
				end if
			end if			
		else
...
foreach
 select doc_remesa,
   ...
   into _no_documento,
		...
		_no_poliza
   from cobredet
  where periodo     >= _per_ini_ap		--2014-10
    and periodo     <= _per_fin_ap		--2015-07
	and actualizado = 1
	and tipo_mov    in ("P", "N")
...
foreach
 select no_poliza,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between '2014-10' And '2015-09'  --_per_ini_ap and _per_fin_ap		--2014-10    a     2015-09
...
foreach
 select no_poliza,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between '2014-10' And _per_fin_ap --   2014-10    A    2015-01
