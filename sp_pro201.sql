...
-- 
--
-- Creado    : 10/12/2010 - Autor: Amado Perez Mendoza
-- Modificado: 10/12/2010 - Autor: Amado Perez Mendoza
--
-- SIS v.2.0 - DEIVID, S.A.

DROP PROCEDURE sp_pro201;
CREATE PROCEDURE "informix".sp_pro201(a_cia CHAR(3), a_periodo CHAR(7), a_periodo2 CHAR(7))
...
FOREACH	WITH HOLD
    ...
    IF _fronting > 0 OR _facultativo = 100 THEN
	    CONTINUE FOREACH;
	ELIF _facultativo > 0 AND _facultativo < 100 THEN
	ELSE
	    CONTINUE FOREACH;
	END IF
		 ...
		 WHERE cod_agente = _cod_agente;
 {
		...
		FOREACH
			SELECT porc_partic_reas, porc_comis_fac, cod_contrato, prima
			  INTO _porc_partic_reas, _porc_comis_fac, _cod_contrato, _prima
			  FROM emifafac
			 WHERE no_poliza = _no_poliza
		       AND no_endoso = _no_endoso
	        ...
	        IF _tipo_contrato = 3 THEN
	            ...
	            LET _comis_fac = _comis_fac + _prima * _porc_partic_agt / 100 * _porc_comis_fac / 100;
				LET _porc_comis_fac_tot = _porc_comis_fac;
		    END IF
		END FOREACH
