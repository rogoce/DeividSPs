...
drop procedure sp_emite11_millenium;

create procedure "informix".sp_emite11_millenium() 
...
define _deduc_comprensivo		varchar(50);
	...
	foreach
		select first 5 
		  ...
		  into _no_unidad_int,
			   ...
			   _deduc_comprensivo,
			   _prima_comprensivo
		  from deivid_tmp:carga_millenium
		 where procesado = 0 
		   and motivo is null
		   --and no_unidad not in (248,249,250,267,293,302,303,304,305,306,307,308,309,310,311,387,390,391)
		...
		let _cod_cliente = '28084';
		let _no_poliza = '0003018821';
		...
		*/
		...
		if _no_unidad = '00001' then
			...
			select porc_descuento*100
			  into _desc_flota
			  from deivid_tmp:tmp_descuento
			 where unidad = _no_unidad_int;
		...
		if _ramo_sis = 1 then
			...
			if _cod_producto in ('11267','11268') then	--LIVIANO
				let _cod_tipoveh = '005';
				let _uso_auto_c = 'P';
			elif _cod_producto in ('11269','11270','11271') then
				let _cod_tipoveh = '008';
			elif _cod_producto in ('11272','11275') then
		...
		update emipocob
		 ...
		 where no_poliza = _no_poliza
		   and no_unidad = _no_unidad
		   and cod_cobertura = '01306'; --COMPRENSIVO
		...
		update emipocob
		   set limite_1 = _limite_asist1,
			   ...
			   prima_anual = _prima_asist,
			   prima = _prima_asist,
			   prima_neta = _prima_asist
		 where no_poliza = _no_poliza
		   and no_unidad = _no_unidad
		   and cod_cobertura = '01305'; --LESIONES
		...
		if li_return = 0 then
			call sp_proe01(_no_poliza, _no_unidad, '001') returning li_return;
		...
		if _error > 0 then
			...
			update deivid_tmp:carga_millenium
