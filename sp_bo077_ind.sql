...
foreach
	...
	if _nueva_renov = "N" then
		...
		if _cnt_cam > 0 then	   --tiene cambio de plan, debe colocarse como renovada
			insert into tmp_persis(no_documento, no_pol_renov, no_pol_renov_per,cambio_plan,vigencia_aniv,fecha_pago,pagada)
			values (_no_documento, 1, 1,1,_vigencia_aniv,_fecha_pago,_pagada);
		else		
			insert into tmp_persis(no_documento, no_pol_nueva, no_pol_nueva_per,cambio_plan,vigencia_aniv,fecha_pago,pagada)
			values (_no_documento, 1, _no_poliza_per,0,_vigencia_aniv,_fecha_pago,_pagada);
		end if	
		...
		if _cod_ramo <> "018" then
			...
			insert into tmp_persis(no_documento, no_pol_renov, no_pol_renov_per,cambio_plan,vigencia_aniv,fecha_pago,pagada)
			values (_no_documento, 1, _no_poliza_per,0,_vigencia_inic,_fecha_pago,_pagada);
...
foreach
	...
	values (_no_documento, 1, 1,0,_vigencia_aniv_nva,_fecha_pago,_pagada,_vigen_ini,_vigencia_act,'01/01/1900');

end foreach
return 0, _no_documento,_vigencia_ant,_vigencia_act,_vigencia_aniv;
end procedure