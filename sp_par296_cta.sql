...
drop procedure sp_par296_cta;
create procedure "informix".sp_par296_cta(a_no_registro char(10))
returning smallint,char(100),char(25),char(5),decimal(16,2),decimal(16,2),CHAR(10);
...
on exception set _error, _error_isam, _error_desc
	drop table tmp0_reas;
	return _error, _error_desc,'','',0,0,'';			
end exception

drop table if exists tmp0_reas;
create temp table tmp0_reas(
no_unidad			char(5),
...
create index idx_tmp0_reas_1 on tmp0_reas(no_unidad, cod_contrato, es_terremoto);
create index idx_tmp0_reas_2 on tmp0_reas(no_unidad, cod_contrato, cod_cober_reas);
create index idx_tmp0_reas_3 on tmp0_reas(bouquet);

drop table if exists tmp0_cta;
create temp table tmp0_cta(
 error        smallint,
 nom_cuenta   char(100),
 cuenta       char(25),
 cod_auxiliar char(5),
 ...
 referencia   CHAR(10)) with no log;
 create index idx_tmp0_cta_1 on tmp0_cta(cuenta,cod_auxiliar);  
 
drop table if exists tmp0_unidad;

create temp table tmp0_unidad(
no_unidad		char(5),
...
drop table if exists tmp0_cglcuentas;	
select *
  from sac:cglcuentas 
  into temp tmp0_cglcuentas;	
...
if _tipo_registro = 1 then -- Produccion
	...
	if _cant_reaseguro = 0     and
	   _prima_suscrita <> 0.00 Then
		...
		drop table tmp0_reas;
		return 1,"No Existe Distribucion de Reaseguro Para Factura: " || _no_factura,'','',0,0,'';				
	end if

	 delete from tmp0_reas;
	 delete from tmp0_unidad;
	...
	if _cod_ramo in ("001", "003") then
		...
		foreach
			...
			begin
				...
				insert into tmp0_unidad(no_unidad,prima_tot)
				values(	_no_unidad,_prima);
			end

			insert into tmp0_reas
...
--				and orden		   = _orden   se quito por que generaba error para los facultativos 22/04/2010 Armando/Demetrio				
				...
				if _monto <> 0.00 then
					...
					select cta_nombre
					  into _nom_cuenta
					  from tmp0_cglcuentas
					 where cta_cuenta = _cuenta; 
					 
					let _error_desc = "Exito Reaseguro Cedido" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
					insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
					values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PGRCSD');
					
					-- *return _error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PGRCSD' with resume;
				...
				if _monto2 <> 0.00 then
					...
					let _error_desc = "Exito Comision Ganada" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
					insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
					values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PICGRCSD');
					
					-- *return _error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PICGRCSD' with resume;
				end if
				...
				if _monto2 <> 0.00 then
					...
					let _error_desc = "Exito Impuesto Recuperado" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
					insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
					values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PIIRRCSD');					
					-- *return _error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PIIRRCSD' with resume;
				...
				if _monto3 <> 0.00 then
					...
					let _error_desc = "Exito Reaseguro por Pagar" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
					insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
					values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PPRXP');										
					-- *return _error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PPRXP' with resume;
		      ...
		      where cod_contrato   = _cod_contrato
				...
				if _monto3 <> 0.00 Then
					...
					if _bouquet = 1 then
						...
						let _error_desc = "Exito Reaseguro por Pagar" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
						insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
						values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PPPRXPB');																															
						-- *return _error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PPPRXPB' with resume;						
	...
	foreach
		...
		if _bouquet = 1 then
			if _porc_proporcion = 0 then		
				return 1, "% Proporcion es cero para la Rem: " || _no_remesa || " Rengl: " || _renglon,'','',0,0,'';						 
			end if
		 ...
		 select cod_coasegur,
	      ...
	      where cod_contrato   = _cod_contrato
			...
			if _monto <> 0.00 Then
				...
				let _error_desc = "Exito Provision por Reasegurador Bouquet" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
				insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
				values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PPPRXPB');																																									
				-- *return _error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PPPRXPB' with resume;
				...
				let _error_desc = "Exito Reaseguro por Pagar Bouquet" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
				insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
				values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'PPRXP');																																									
	...
	if _monto_reas <> _monto_reas_cob then
		...
		drop table tmp0_reas;
		return 1, "Sumatoria rectrmae vs rectrcob No Cuadra, no_tranrec = " || _no_tranrec,'','',0,0,'';			
...
--				else
					...
					return 1, "No hay Distribucion de Reaseguro para la Transaccion: " || _no_tranrec || " " || _cod_cober_reas,'','',0,0,'';			
			  ...
			  where no_tranrec     = _no_tranrec
				...
				if _tipo_contrato = 3 then -- Facultativos
					...
					if _cantidad = 0 then
						...
						drop table tmp0_reas;
						return 1, "No Existe Companias del Contrato: " || _cod_contrato || " Cobertura: " || _cod_cober_reas,'','',0,0,'';			
					 ...
					 select cod_coasegur,
					  ...
					  where cod_contrato   = _cod_contrato 
						...
						if _monto <> 0.00 Then
							...
							let _error_desc = "Exito Participacion de Reaseguradores en Siniestro" || " Poliza " || _no_poliza || " Endoso " || _no_endoso;
							insert into tmp0_cta(error, nom_cuenta, cuenta, cod_auxiliar, debito, credito, referencia)
							values(_error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'');																																																							
							-- *return _error,_nom_cuenta,_cuenta,_cod_auxiliar,_debito,_credito,'' with resume;
...
delete from tmp0_cta where cuenta not like ('231%');
--drop table tmp0_reas;
--drop table tmp0_unidad;

end

return 0, "Actualizacion Exitosa",'','',0,0,'';			
