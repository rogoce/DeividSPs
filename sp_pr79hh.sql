-- Listado de Totales de Vencimientos
--
-- Creado    : 26/10/2001 - Autor: Lic. Armando Moreno 
-- Modificado: 26/10/2001 - Autor: Lic. Armando Moreno
-- SIS v.2.0 - DEIVID, S.A.

DROP PROCEDURE sp_pro79hh;

CREATE PROCEDURE "informix".sp_pro79hh(
        a_compania  	CHAR(3), 
        a_agencia   	CHAR(3), 
        a_periodo1  	CHAR(7),
        a_periodo2  	CHAR(7),
	...
	-- Informacion de Poliza
		...
		SELECT no_poliza,
		 ...
		 WHERE vigencia_final >= _fecha1 
	       ...
	       AND incobrable  = 0
		   AND abierta     = 0
		   AND cod_ramo IN ('018','019')
		   AND estatus_poliza IN (1,3)

		IF MONTH(_vigencia_inic) <> MONTH(_fecha1) THEN
			CONTINUE FOREACH;	
		END IF
	   ...
	   CALL sp_cob74b(
		...
		IF a_saldo_cero = 0 OR a_saldo_cero = 3 OR a_saldo_cero = 4 THEN --saldo esta  entre 0-30%,31-60%,61% en adelante de la prima neta.
		   ...
		   IF _saldo_neto > 0 THEN
			   ...
			   IF _prima_porc >= 0 AND _prima_porc <= 30 THEN
				 LET _estatus = "1";
			   END IF

			   IF _prima_porc > 30 AND _prima_porc <= 60 THEN
				 LET _estatus = "2";
			   END IF
		   ...
		   END IF
		END IF
		IF a_saldo_cero = 5 THEN	--Saldo cero
		   IF _saldo_neto = 0 THEN
			 LET _estatus = "4";
		   ELSE
			 CONTINUE FOREACH;
		   END IF
		END IF
		IF a_saldo_cero = 1 THEN	--Saldo credito
		   IF _saldo_neto < 0 THEN
			 LET _estatus = "0";
		   ELSE
			 CONTINUE FOREACH;
...
ELSE
	FOREACH WITH HOLD
	   ...
	   IF a_saldo_cero = 0 OR a_saldo_cero = 3 OR a_saldo_cero = 4 THEN
		   IF _prima_neta = 0 THEN
			CONTINUE FOREACH;
		   END IF
		   IF _saldo_neto > 0 THEN
...
IF a_saldo_cero = 1 THEN --SALDO CREDITOS

   	LET v_filtros = " Saldo Credito;";
...
ELIF a_saldo_cero = 0 THEN --CON SALDO >0 - 30 %

   	LET v_filtros = " Con Saldo hasta 30%;";
...
ELIF a_saldo_cero = 3 THEN --CON SALDO 31 - 60 %

   	LET v_filtros = " Con Saldo de 31 A 60%;";
...
ELIF a_saldo_cero = 4 THEN --CON SALDO MAYOR A 60 %

   	LET v_filtros = " Con Saldo Mayor a 60%;";
...
ELIF a_saldo_cero = 5 THEN --CON SALDO 0

   	LET v_filtros = " Con Saldo Cero;";
...
ELSE --TODO
   	...
   	LET v_filtros = "Saldo:Todas las polizas;";
