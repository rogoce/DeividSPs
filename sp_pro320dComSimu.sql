...
drop procedure sp_pro320dComSimu;
create procedure sp_pro320dComSimu(v_usuario char(8), v_poliza char(10), v_poliza_nuevo char(10))
...
if _ramo_sis = 1 then
	if _cod_rammo <> '024' then
		if _cod_rammo = '020' then
			...
			if _cod_grupo = '77851' then  --77851 – VITRUVIO MOVILITY se incluirá VITRUVIO según correo del día 05-07-2025 como si fuera prima nivelada
				let _valor = sp_pro321eSimB(v_poliza_nuevo,v_poliza); -- Banisi o prima nivelada
			else
				let _valor = sp_pro321eComSimu(v_poliza_nuevo,v_poliza);
			end if
