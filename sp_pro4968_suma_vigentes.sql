DROP procedure sp_pro4968;
   CREATE procedure "informix".sp_pro4968(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
...
--------------------------------------------
	...
	DEFINE _tot_monto_tot   DECIMAL(16,2);
	DEFINE _tot_monto_tot1   DECIMAL(16,2);
...
IF a_periodo <> a_periodo2 THEN
	FOREACH                   
		SELECT cod_ramo, 0, no_poliza, '00000', cod_subramo, nueva_renov 
		  INTO _cod_ramo_ori, _total_pri_sus, _no_poliza, _no_endoso, _cod_subramo, _nueva_renov 
		  FROM estpolvih 
		...
		FOREACH 
			SELECT 	u.no_unidad,          		 
				...
				INTO _no_unidad,
					 ...
					 _suscrita_unidad,
					 _cod_producto				 
			   FROM endeduni u, endedmae v
			  WHERE u.no_poliza = v.no_poliza
			    and u.no_endoso = v.no_endoso
			    and v.no_poliza = _no_poliza
			    and v.periodo <= a_periodo2
	--			and u.no_endoso = _no_endoso			 
				order by 1
		...
		FOREACH WITH HOLD
			...
			IF _3ciclo = 1 THEN
             ...
             ELIF _3ciclo = 2 THEN
				  ...
				  let _primera_u = 1;
             ELIF _3ciclo = 3 THEN
                  LET _valor = _suma_asegurada; --_suma_asegurada ; -- 
				  LET _primera_u = 0;
             END IF 
			-- let _primera_u = 0;
			 let _orden = _3ciclo;
...
--SET DEBUG FILE TO "sp_pro4968.trc"; 
--trace on;
...
IF a_periodo <> a_periodo2 THEN
	...
	LET _tot_cant_sum = _cnt_prima_nva - _tot_cant_sum;
	LET _tot_cant_uni = _cnt_prima_ren - _tot_cant_uni;
ELSE
	...
	LET _tot_cant_sum = _cnt_prima_nva + _cnt_prima_ren - _tot_cant_sum;   
	LET _tot_monto_tot = 635778707.48 - _tot_monto_tot;
END IF
...
FOREACH WITH HOLD
   SELECT orden,descripcion,auto_co_p_cant,auto_co_p_monto,auto_co_c_cant,auto_co_c_monto,auto_rc_p_cant,auto_rc_p_monto,auto_rc_c_cant,
	...
	IF a_periodo = a_periodo2 THEN		
		IF _orden = 1 AND _grupo = '005' THEN
			IF _tot_cant_sum > _tot_cant THEN
				LET _auto_co_p_cant = _auto_co_p_cant + (_tot_cant_sum - _tot_cant);
				LET _sub_p_tot_cant = _sub_p_tot_cant + (_tot_cant_sum - _tot_cant);
				LET _tot_cant = _tot_cant_sum;
			ELIF _tot_cant_sum < _tot_cant AND _grupo = '005' THEN
				LET _auto_co_p_cant = _auto_co_p_cant + (_tot_cant_sum - _tot_cant);
				LET _sub_p_tot_cant = _sub_p_tot_cant + (_tot_cant_sum - _tot_cant);
				LET _tot_cant = _tot_cant_sum;
			END IF		
	...
	ELSE	 	 	 
		 IF _orden = 1 and _grupo = '005' THEN
			IF _tot_cant_sum  > _tot_cant THEN
				...
				LET _sub_p_tot_cant = _sub_p_tot_cant - (_tot_cant_sum - _tot_cant);
				LET _tot_cant = _tot_cant_sum;
			ELIF _tot_cant_sum < _tot_cant THEN
				LET _auto_co_p_cant = _auto_co_p_cant + (_tot_cant_sum - _tot_cant);
				LET _sub_p_tot_cant = _sub_p_tot_cant + (_tot_cant_sum - _tot_cant);
				LET _tot_cant = _tot_cant_sum ;
			END IF
		 ELIF _orden = 2 and _grupo = '005' THEN		
			IF _tot_cant_uni  > _tot_cant THEN
				LET _auto_co_p_cant = _auto_co_p_cant - (_tot_cant_uni - _tot_cant);
				LET _sub_p_tot_cant = _sub_p_tot_cant - (_tot_cant_uni - _tot_cant);
				LET _tot_cant = _tot_cant_uni;
			ELIF _tot_cant_uni < _tot_cant THEN
				LET _auto_co_p_cant = _auto_co_p_cant + (_tot_cant_uni - _tot_cant);
				LET _sub_p_tot_cant = _sub_p_tot_cant + (_tot_cant_uni - _tot_cant);
				LET _tot_cant = _tot_cant_uni ;
			END IF
		{ ELIF _orden = 3 and _grupo = '005' THEN
            IF a_periodo2 = '2023-11' THEN
				IF _tot_monto_tot > _tot_monto THEN
					LET _auto_co_p_monto = _auto_co_p_monto - (_tot_monto_tot - _tot_monto);
					LET _sub_p_tot_monto = _sub_p_tot_monto - (_tot_monto_tot - _tot_monto);
					LET _tot_cant = _tot_monto_tot;
				ELIF _tot_monto_tot < _tot_monto THEN
					LET _auto_co_p_monto = _auto_co_p_monto + (_tot_monto_tot - _tot_monto);
					LET _sub_p_tot_monto = _sub_p_tot_monto + (_tot_monto_tot - _tot_monto);
					LET _tot_cant = _tot_monto_tot ;
				END IF
