...
DROP PROCEDURE sp_rec308;
CREATE PROCEDURE informix.sp_rec308(
a_compania	char(3),
a_agencia	char(3),
a_periodo	char(7),
...
RETURNING   CHAR(20) as reclamo,
			...
			char(2) as es_pago_taller,
			CHAR(10) as user_added, 
			date as fecha_reserva_inicial,
			dec(16,2) as monto_reserva_inicial,
			date as fecha_cierre_reserva,
			dec(16,2) as monto_cierre_reserva,			
			date as fecha_reabrir_reserva,
			dec(16,2) as monto_reabrir_reserva,
			date as fecha_aumento_max,
			dec(16,2) as monto_aumento_max,
...
call sp_rec252a(a_periodo,'0') returning _error,_error_desc; 

if _error <> 0 then
	return '','',null,null,0.00,0.00,'',null,null,'',_error_desc,'','','',null,'','','',_error,'',0.00,'','','','',null,'','','','',null,0.00,null,0.00,null,0.00,null,0.00,'','','','','',0.00,'','';
...
FOREACH
 SELECT no_reclamo,
   ...
   INTO	_no_reclamo,
		...
		_ins_tipo
   FROM recrcmae
  WHERE  perd_total = 1	and no_documento[1,2] in ('02','20','23')
    and numrecla in (select distinct numrecla from tmp_26612)
  ...
  SELECT nombre
   ...
   foreach 
	 SELECT cod_tipotran,fecha,monto
	   INTO _cod_tipotran_b,_fecha_b,_monto_b
	   from rectrmae 
	  where no_reclamo = _no_reclamo
        and cod_tipotran in ('001','011','012','002')				 
		order by fecha desc, cod_tipotran
		
		
		 if _cod_tipotran_b in ('001') and _fecha_001 is null then
			let _fecha_001 = _fecha_b;
			let _monto_001 = _monto_b;
	    end if
		
		 if _cod_tipotran_b in ('011') and _fecha_011 is null then
			let _fecha_011 = _fecha_b;
			let _monto_011 = _monto_b;
	    end if
		
		 if _cod_tipotran_b in ('012') and _fecha_012 is null then
			let _fecha_012 = _fecha_b;
			let _monto_012 = _monto_b;
	    end if				

		 if _cod_tipotran_b in ('002')  then
			if _monto_002 < _monto_b then		 
				let _fecha_002 = _fecha_b;
				let _monto_002 = _monto_b;
			end if
	    end if													
