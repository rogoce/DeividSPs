...
foreach
	...
	if _cod_vendedor is null or trim(_cod_vendedor) = '' then
		select cod_vendedor
		  into _cod_vendedor
		  from agtagent
		 where cod_agente = _cod_agente;
	...
	select count(*)
	 ...
	 where cod_vendedor = _cod_vendedor
	   ...
	   and cod_subramo  = _cod_subra;

	if _cantidad = 0 then
		insert into deivid_bo:preventas
		values (_cod_vendedor, _cod_agente, _cod_ramo, _periodo, 0, 0, 0, 0,0,0,0,_cod_subra);
	end if
