...
let a_periodo   = '2017-12';  --****PONER COMENTARIO CUANDO CORRE ANUAL
...
foreach with hold
 select doc_remesa,
  ...
  where periodo     >= _per_ini
	...
	foreach
			...
			SELECT pagado,
			 ...
			 WHERE no_requis = _no_requis
			   and fecha_impresion between '01/01/2017' and '31/12/2017';
			IF _pagado = 1 THEN
				IF _fecha_anulado IS NOT NULL THEN
					IF _fecha_anulado >= '01/01/2017' and _fecha_anulado <= '31/01/2017' THEN
						LET _monto_dev = 0;
					END IF
				END IF			
			ELSE
	...
	insert into fis_renta2(no_documento, pri_sus_pag)
 ...
 select doc_remesa,
  ...
  where periodo     >= _per_ini_ap		
	...
	foreach
			...
			IF _pagado = 1 THEN
				IF _fecha_anulado IS NOT NULL THEN
					IF _fecha_anulado >= '01/01/2016' and _fecha_anulado <= '31/12/2016' THEN
						LET _monto_dev = 0;
					END IF
				END IF			
			ELSE
	...
	begin work;	
		insert into fis_renta2(no_documento, pri_sus_pag_ap)
		values (_no_documento, _monto);
...
foreach	with hold
 ...
 select doc_poliza,
  ...
  begin work;
	insert into fis_renta2(no_documento, sin_pag_aa)
...
foreach	with hold
 ...
 having sum(variacion) > 0 
   ...
   begin work;
	...
	insert into fis_renta2(no_documento, sin_pen_ap)
...
foreach	with hold
 ...
 having sum(variacion) > 0 
  ...
  begin work;
	...
	insert into fis_renta2(no_documento, sin_pen_aa)
...
foreach
 select no_documento,
  ...
  group by no_documento
	...
	insert into fis_renta2(no_documento, pri_dev)
