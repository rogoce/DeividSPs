CREATE PROCEDURE pr_cglu13(v_compania, v_comprobante CHAR(15))
--*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*--
...
DEFINE mensaje_error  char(100);
DEFINE l_codigo       INTEGER;
-- 1 Error
-- 0 Satisfactorio
...
--set debug file to "log";
--trace on;

 SELECT cia_bda_codigo INTO wdatabase FROM seguridad:sigman02
...
FOREACH
   SELECT trx1_notrx,trx1_tipo,trx1_comprobante,trx1_fecha,trx1_concepto,trx1_ccosto,
    ...
    WHERE trx1_comprobante = v_comprobante
      AND trx1_status      = "I"
	   ...
	   LET nivel1 = wcta_nivel;
		...
		FOREACH
		   SELECT sldet_debtop, sldet_cretop, sldet_saldop
                          pdebitos[J],pcreditos[J],psaldo[J]
		...
		IF ano_fiscal = work_ano THEN
		    ...
		    FOR J = 1 TO  14
		        UPDATE wdatabase:cglsaldodet
		   	   SET sldet_debtop  = pdebitos[J],
			       sldet_cretop  = pcreditos[J],
			       sldet_saldop  = psaldo[J]
			 WHERE sldet_cuenta  = work_cta
			   AND sldet_ano     = work_ano
	  		   AND sldet_ccosto  = w2_ccosto
			   AND sldet_tipo    = w1_tipo
		...
		ELSE
			 ...
			 ELSE
			    ...
			    IF work_cta < w_par_ingreso1 OR
				 ...
				 SELECT * FROM wdatabase:cglsaldoctrl
				 ...
				 FROM wdatabase:cglsaldoaux1
				  ...
				  END FOR
			       ...
			       FOREACH  
				    ...
				    INTO pdebitos[J],pcreditos[J],psaldo[J]
				   FROM wdatabase:cglsaldoaux1
				   WHERE sld1_tipo    = w1_tipo
				     AND sld1_cuenta  = work_cta
				     AND sld1_tercero = w3_auxiliar
				  ...
				  FOR J = 1 TO  14
					 ...
					 SET sld1_debitos  = pdebitos[J],
					     sld1_creditos = pcreditos[J],
					     sld1_saldo   = psaldo[J]
				       WHERE sld1_tipo    = w1_tipo
					 AND sld1_cuenta  = work_cta
					 AND sld1_tercero = w3_auxiliar
				  ...
				  IF ano_fiscal < work_ano THEN
				    ...
				    ELSE
					  ...
					  IF nvo_ano > ano_fiscal THEN
					     EXIT WHILE
					  END IF
					  SELECT * FROM wdatabase:cglsaldoaux
					     WHERE sld_tipo    = w1_tipo
					       AND sld_cuenta  = work_cta
