...
drop procedure sp_sis402;
create procedure sp_sis402(a_no_poliza char(10), a_fecha_hoy date, a_monto_pago	decimal(16,2) default 0, a_no_remesa char(10) default '00000')
returning	smallint,
			varchar(120),
			decimal(16,2);
...
{select prima_bruta
 ...
 where no_poliza = a_no_poliza
   and no_endoso = '00000';}
--Banisi - Lizzy Bernal, Se adiciona Felix Abadia solicitud AMORENO:17/09/2018  --  Banisi - Bac, Abadia, Ducruet 15/01/2019 CASO: 30140 USER: ASTANZIO  -- SD#3010 77960  11/04/2022 10:00
if _cod_grupo in('124','125','125','148','1122','77960') then   
...
if ABS(_existe_end - _existe_rev) > 0 then
...
if _tipo_produccion in (3) then --Coaseguro Minoritario
...
--Excepcion de Ramos
if _cod_ramo in ('004','008','016','018','019','023','001') then
	select nombre
	  into _nom_ramo
	  from prdramo
...
if _dias > 30 then
	let _error_desc = "La Póliza no cumple con las condiciones. Han pasado " || trim(cast(_dias as char(4))) || " días desde la Fecha de Suscripcion/Vigencia Inicial de la Póliza.";
	return 1,_error_desc,_descuento;
end if
...
if _cod_producto = '10602' then
	let _error_desc = 'Esta póliza no aplica para este descuento. Producto 10602';
	return 1,_error_desc,_descuento;
end if
