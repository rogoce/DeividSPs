...
Drop procedure sp_log011_ns1;  
CREATE procedure "informix".sp_log011_ns1(a_no_aviso varchar(255) default "*")  
...
drop table if exists tmp_codigos;
	...
	let a_no_aviso = '01679,01681,01682,01688,01692,01696,01700,01706,01710,01714,01718,01723,01724,01728,01730,01737;';
...
where a.no_aviso IN (SELECT codigo FROM tmp_codigos ) 
	   ...
	   or ( a.estatus in ('X','M')  and a.cancela = "0"  and a.date_imp_aviso_log is null))	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
