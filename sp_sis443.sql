--Creado 20/04/2016 Armando Moreno M.
--Procedimiento para realizar historico al momento del cierre de contabilidad para los reclamos pendientes(cuenta 221 y 222)

DROP PROCEDURE sp_sis443;
CREATE PROCEDURE "informix".sp_sis443(a_compania CHAR(3),a_agencia CHAR(3),a_periodo CHAR(7),a_sucursal	CHAR(255) DEFAULT "*", a_ajustador CHAR(255) DEFAULT "*", a_ramo CHAR(255) DEFAULT "*", a_agente CHAR(255) DEFAULT "*")
RETURNING smallint, CHAR(30);

--execute procedure sp_sis443('001','001','2016-07',"*", "*", "*","*")
...
DEFINE _variacion_bruta	  DECIMAL(16,2);
 ...
 SELECT no_reclamo,		
  ...
  SELECT recajust.nombre
   ...
   WHERE recajust.cod_ajustador =  v_ajustador_int  ;
    ...
    insert into hrecpend(
	numrecla,cliente,no_documento,fecha_siniestro,ultima_fecha,pagado_bruto,pagado_neto,reserva_bruta,reserva_neta,incurrido_bruto,incurrido_neto,ramo_nombre,
	ajustador_int,estatus_reclamo,estatus_audiencia,abogado,variacion_bruta,periodo,date_added,dif_222)
	values(v_doc_reclamo,v_cliente_nombre,v_doc_poliza,v_fecha_siniestro,v_ultima_fecha,v_pagado_bruto,v_pagado_neto,v_reserva_bruto,v_reserva_neto,v_incurrido_bruto,v_incurrido_neto,
	v_ramo_nombre,v_ajustador_int,_estatus_reclamo,_estat_aud,_abogado,_variacion_bruta,a_periodo,_fecha_time,v_reserva_bruto-v_reserva_neto);
