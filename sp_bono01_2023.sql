--**********************************************************************************************************************************************************
-- Procedimiento para determinar la prima suscrita nueva de los corredores del bono de producción de Ramos Generales 2023
--***********************************************************************************************************************************************************

-- Creado    : 31/03/2023 - Autor: Armando Moreno M.
...
if _periodo_actual[1,4] = '2023' then
...
foreach
	...
	if _cod_agente in('02111','02825','02531','02830','02831') then	--Se excluye cualquier poliza de este codigo de Javier Avila que es solo para talleres.
		continue foreach;
	end if
	foreach
			 select e.no_documento,e.no_poliza
			  ...
			  where e.no_poliza         = t.no_poliza
				...
				and e.actualizado       = 1
				and e.nueva_renov       = "N"
				and e.periodo >= "2023-01"
				and e.periodo <= "2023-12"
	...
	foreach
			select d.nueva_renov,
			 ...
			 where	d.no_remesa    = m.no_remesa
			   ...
			   and d.fecha        >= '01/01/2023'
			   and d.fecha        <= '31/12/2023'
