--***********************************************************************************
-- Procedimiento que genera el Bonificacion de Rentabilidad por corredores
--***********************************************************************************
-- Creado    : 03/03/2009 - Autor: Henry Giron
-- Modificado: 03/03/2009 - Autor: Henry Giron
-- SIS v.2.0 - DEIVID, S.A.
DROP PROCEDURE ver1;

CREATE PROCEDURE ver1(
a_compania          CHAR(3),
a_sucursal          CHAR(3),
v_periodo_aa        CHAR(7)
) RETURNING SMALLINT,
 ...
 select doc_remesa,
  ...
  where periodo     between _per_ini and a_periodo
    ...
    let _no_poliza = sp_sis21(_no_documento);
	  ...
	  INTO _cnt
	  FROM emipoagt
	WHERE no_poliza = _no_poliza and cod_agente = '01266';
 ...
 select no_documento,
  ...
  order by no_documento
	...
	{if _cnt = 0 then		   
	 ...
	 if _cod_grupo = "00000" then -- Excluir estado	  1
--		continue foreach;
		let _tc = '1';
	 end if	 
	 ...
	 IF _tipo_prod = 4 or _tipo_prod = 2 THEN 		-- Excluir Reaseguro Asumido y Coas. Mayoritario	2
--		continue foreach;
		let _tc = '2';
	 END IF	  
	 ...
	 if _cnt > 0 then		-- Excluir Facultativos	  3
--		continue foreach;
		let _tc = '3' ;
	 end if	
	 ...
	 foreach
		...
		SELECT nombre,
		 ...
		 WHERE cod_agente = _cod_agente;
			...
			if trim(_cedula_agt) = trim(_cedula_paga) then	--Contra pagador	  4
--				let _flag = 1;
				let _flag = 0;
				let _tc = '4';
				exit foreach;
			...
			if trim(_cedula_agt) = trim(_cedula_cont) then	--Contra Contratante	  5
--				let _flag = 1;
				let _flag = 0;
				let _tc = '5';
				exit foreach;
			...
			IF _tipo_agente <> "A" then	--solo agentes		   6
--				let _flag = 1;
				let _flag = 0;
				let _tc = '6';
				exit foreach;
			...
			IF _estatus_licencia <> "A" then  --El corredor debe estar activo		 7
--				let _flag = 1;
				let _flag = 0;
				let _tc = '7';
				exit foreach;
...
--/*CATEGORIZAR*/
--AUTOMOVIL                   = 002,020
--SALUD y HOSPITALIZACION     = 018
--PATRIMONIALES               = 001,003,005,006,015,011,013,010,009,014
--PERSONAS    				  = 019,004,016
--FIANZAS                     = 008,080
