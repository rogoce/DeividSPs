...
--execute procedure sp_rec02_cob('001','001','2016-03',"*","*","*","002,020,023;","*")
-- SIS v.2.0 - DEIVID, S.A.

drop procedure sp_rec02_cob;

create procedure "informix".sp_rec02_cob(
a_compania		char(3), 
a_agencia		char(3), 
a_periodo		char(7),
...
define _monto_cob_bruto		dec(16,2);
define _monto_cob_neto		dec(16,2);
...
foreach
	...
	foreach
		select trx.no_reclamo,
		 ...
		 where trx.cod_compania	= a_compania
		   and trx.periodo     	<= a_periodo 
		   and trx.actualizado  	= 1
		   and trx.no_reclamo = _no_reclamo
		 group by trx.no_reclamo,cob.cod_cobertura
--		having abs(sum(cob.variacion)) > 0 
   ...
   from tmp_sinis 
		...
		foreach
		 select cod_cobertura,
		  ...
		  where no_tranrec = _no_tranrec
			...
			let _monto_cob_bruto = 0.00;
			let _monto_cob_neto = 0.00;
			...
			let _monto_cob_bruto = _monto_cob   / 100 * _porc_coas;
			let _monto_cob_neto = _monto_cob   / 100 * _porc_reas;
			--let _monto_neto	= _monto_neto  + _monto_cob;

			update tmp_sinis
			   set reserva_total = reserva_total + _monto_cob,
				   reserva_bruto = reserva_bruto + _monto_cob_bruto,
				   reserva_neto  = reserva_neto  + _monto_cob_neto
			 where no_reclamo    = _no_reclamo
	...
	foreach
		...
		foreach
			...
			-- Actualizacion del Movimiento
			update tmp_sinis
			   set pagado_total = pagado_total + _monto_cob,
				   pagado_bruto = pagado_bruto + _monto_cob_bruto,
				   pagado_neto  = pagado_neto  + _monto_cob_neto
			 where no_reclamo   = _no_reclamo
			   and cod_cobertura = _cod_cobertura;
		end foreach
	end foreach	
