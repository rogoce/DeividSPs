-- Generacion del Archivo para Banco Hsbc

-- Creado    : 12/02/2007 - Autor: Armando Moreno M.
-- Modificado: 12/02/2007 - Autor: Armando Moreno M.

-- SIS v.2.0 - DEIVID, S.A.

DROP PROCEDURE sp_cob201;

CREATE PROCEDURE "informix".sp_cob201(
a_compania		CHAR(3),
a_sucursal		CHAR(3),
a_user			CHAR(8)
) RETURNING SMALLINT,
	...
	FOREACH
	 SELECT	renglon,
	  ...
	  WHERE no_tarjeta     = _no_tarjeta
		...
		LET _campo = '"1"' || ' ' ||
					 '"' || _no_tarjeta[1,4] || _no_tarjeta[6,9] || _no_tarjeta[11,14] ||
					 _no_tarjeta[16,19] || '"' || ' ' ||
