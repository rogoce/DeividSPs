...
DROP PROCEDURE sp_rwf46;
CREATE PROCEDURE "informix".sp_rwf46(a_no_reclamo CHAR(10), a_incidente INTEGER, a_user_name_ajust CHAR(8)) 
...
begin work;
 ...
 IF _estatus_reclamo = "C" THEN
        commit work;
	    RETURN 0, "Ya estaba cerrado","";
 END IF
 
 CALL sp_rec723(_numrecla) RETURNING _error, _descripcion; --VERIFICACIÓN DE ABOGADO ASIGNADO
...
--		   AND cod_cobertura = _cod_cobertura;
 ...
 BEGIN
	ON EXCEPTION SET _error 
		rollback work;
	 	RETURN _error, "Error al Actualizar Reclamo","";         
	END EXCEPTION 
