CREATE PROCEDURE "informix".sp_rwf42(a_no_tranrec CHAR(10), a_deducible DEC(16,2) DEFAULT 0.00) 
...
LET _hoy = CURRENT;
 ...
 IF _actualizado = 1 THEN
 	RETURN 0, "Esta transaccion ya esta actualizada", 0;
 END IF 
 ...
 IF _cod_tipopago <> "003" AND _cod_tipopago <> "004" THEN
		...
		BEGIN
			ON EXCEPTION SET _error 
				IF _error <> -239 AND _error <> -268 THEN
					rollback work;
				 	RETURN _error, "Error al INSERTAR PROVEEDORES", 0;
				ELSE
					UPDATE tmp_prov
	 ...
	 IF _cantidad = 0 THEN
	    DROP TABLE tmp_prov;
	    RETURN 1, "No tiene proveedores este comparativo, Verifique... ", 0;
	 END IF
 END IF
 ...
 BEGIN
	ON EXCEPTION SET _error 
		rollback work;
        DROP TABLE tmp_prov;
	 	RETURN _error, "Error al BUSCAR PARAMETROS", 0;         
	END EXCEPTION 
 ...
 IF _cod_cobertura IS NULL OR _cod_cobertura = "" THEN
    DROP TABLE tmp_prov;
 	RETURN 1, "Error buscando la cobertura", 0;
 END IF
 ...
 IF _cod_tipopago <> "003" AND _cod_tipopago <> "004" THEN
	...
	-- Coberturas
		 ...
		 FOREACH
		    ...
		    IF _tipo_transaccion = 4 THEN
				IF ABS(_variacion) > ABS(_reserva_actual) THEN
	                DROP TABLE tmp_prov;
					RETURN 1, "Variacion de Reserva es Mayor que Reserva Actual", 0;
				END IF
			END IF
		    ...
		    IF _tipo_transaccion = 4 THEN
		        ...
		        IF _error = 1 THEN
				   rollback work;
	               DROP TABLE tmp_prov;
				   RETURN  _error, "No se Actualizaron las Acumulaciones para el Ramo de Salud", 0;
				END IF
			ELSE
				UPDATE recrccob
	 ...
	 -- Creacion de Orden de compra o reparacion
		CALL sp_rwf43(_no_tranrec,_tipo_orden,a_deducible,_monto_orig,_hoy) returning _error, _descripcion;
		IF _error <> 0 THEN
			rollback work;
	        DROP TABLE tmp_prov;
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
	        DROP TABLE tmp_prov;
		 	RETURN _error, "Error al actualizar transaccion", 0;         
		END EXCEPTION 
 ...
 -- Genera requisicion si es generar cheque
	IF _generar_cheque = 1 THEN
		CALL sp_rwf39(a_no_tranrec) returning _error, _descripcion, _genera_incidente;
		IF _error <> 0 THEN
		   rollback work;
           DROP TABLE tmp_prov;
		   RETURN  _error, _descripcion, 0;
