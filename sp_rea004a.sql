...
-- Modificado por HENRY GIRON, solicitado por Arturo Naranjo FECHA: 10/04/2010
--								Muestra solo los contratos involucrados.
-- SIS v.2.0 - DEIVID, S.A.
  
drop procedure sp_rea004a;

create procedure sp_rea004a(a_periodo1 char(7), a_periodo2 char(7)) 
returning char(5),
		  char(3),
		  char(50),
		  smallint,
...
define _serie,_tipo_contrato         smallint;
		...
		foreach
		 select cod_coasegur,
	      ...
	      where cod_contrato   = _cod_contrato
			...
			values (_cod_coasegur, _cod_ramo, 0.00, 0.00, 000, 0.00, 0.00, _por_pagar,_cod_contrato,_cod_cober_reas);
		...
		foreach
		 select cod_coasegur,
	      ...
	      where cod_contrato   = _cod_contrato
			...
			insert into tmp_ramo
			values (_cod_coasegur, _cod_ramo, 0.00, 0.00, 0.00, 0.00, _monto_reas, 0.00,_cod_contrato,_cod_cober_reas);
			...
			foreach
			 select cod_coasegur,
		      ...
		      where cod_contrato   = _cod_contrato
				...
				insert into tmp_ramo
				values (_cod_coasegur, _cod_ramo, _monto_reas, _comision, _impuesto, _por_pagar, 0.00, 0.00,_cod_contrato,_cod_cober_reas);
...
foreach
 select distinct cod_contrato,cod_cober_reas
   into _cod_contrato,_cod_cober_reas
   from tmp_ramo
  order by 1, 2
