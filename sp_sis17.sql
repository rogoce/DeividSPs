-- Procedimiento que Crea el Historico de Polizas ENDOSOS
-- Creado    : 06/11/2000 - Autor: Demetrio Hurtado Almanza 
-- Modificado: 29/03/2001 - Autor: Demetrio Hurtado Almanza
-- Modificado: 25/09/2001 - Autor: Armando Moreno, Ach
-- Modificado: 11/07/2002 - Autor: Amado Perez M. Verifica si se cambio el tipo produccion de "Coaseg. Mayoritario"
-- Modificado: 27/06/2003 - Autor: Demetrio Hurtado Almanza
--                          Inclusion de Verificacion para Polizas Nuevas de la Forma de Cobro
...
define _no_p,_cod_asegurado	char(10);
...
define _cnt_vip,_tiene_prod,_holgura_vig,_anio_actual,_anio_evaluar,_anio_resul smallint;
define _sucursal_origen     char(3);
define _vig_f,_fecha_actual	date;
define _vig_anual,_vig_fin_resul           date;
...
-- if _no_doc_orig = '0223-00044-47' then
--	set debug file to "sp_sis17.trc";
--	trace on;
-- end if

-- Verifica el saldo renovación de prdpreren
if _nueva_renov = 'R' and _cod_ramo = '002' and _cod_subramo in ('002','005') then
	let _error = sp_pro381d(a_no_poliza);
	if _error <> 0 then
		return _error;
	end if
end if

--Control para vigencia inicial de emision
select holgura_vigencia into _holgura_vig from prdramo where cod_ramo = _cod_ramo;
let _anio_evaluar = year(_vig_i);
let _anio_actual  = year(_fecha_actual);
let _anio_resul = _anio_actual + _holgura_vig;
if _anio_evaluar > _anio_resul then
	return 343;
end if
--Control para vigencia final de emision
let _vig_fin_resul = _vig_i + _holgura_vig units year;
if _vig_f > _vig_fin_resul then
	return 343;
end if
if (_nueva_renov = 'N') And (_cod_ramo = '018' And _cod_subramo <> '012') then --Comisión Pólizas de Salud Suscritas por Cambio de Plan debe ser 10%, Analisa 26/06/2019
...
let _mes_val_i = month(_vig_i);
let _dia_val_i = day(_vig_i);
let _mes_val_f = month(_vig_f);
let _dia_val_f = day(_vig_f);
...
foreach
		...
		if _tipo_ramo = 1 And _vida = 0 Then --No puede emitir pólizas por el tipo de licencia
			let _error = 244;
		end if
...
if _cod_grupo = '00087' then --Grupo FINCAP
...
elif _cod_grupo in ('124','78020','125')  then --Grupo BANISI
	...
	let _par_grupo = "dias_banisi";   
	let _no_pagos = 12;
elif _cod_grupo in ('00068','77978','77974','77980','77973','77979') then  -- Caso  7665 -- Amado 30-08-2023
    let _vig_anual = _vig_i + 1 units year;
	if _vig_f = _vig_anual then
		let _par_grupo = "dias_treinta";  -- Se vuelve a cambiar de sesenta a treinta Caso 8314 -- Amado 02-11-2023  
		let _no_pagos = 10;
		if _cod_grupo in ('77974','77980','77973','77979') then	--caso 12182
			let _no_pagos = 12;
		end if
	else
		let _no_pagos = (year(_vig_f) - year(_vig_i)) * 12 + month(_vig_f) - month(_vig_i);
		if _no_pagos = 0 then
			let _no_pagos = 1;
		end if
	end if
elif _cod_grupo in ('77982','78015') then  -- CORPORACION DE CREDITO - PRIMA NIVELADA -- Caso 7667 -- Amado 30-08-2023  
	let _par_grupo = "dias_ccpn";   
	let _no_pagos = 12;
end if
...
-- 	Verificación de el campo de reemplaza poliza --Federico 06/02/2020
-- 	_error 327 el número de motor que desea emitir esta incluido en una póliza que fue cancelada para ser reemplazada debe llenar el campo reemplaza póliza.
...
if _cod_grupo = '77960' then	--grupo BANISI
	foreach
		select serie
		  into _serie
		  from rearumae
...
if _cod_formapag = "008" And _cnt = 1 then --and _cod_ramo = "020" then -- solo ramo soda solicitado por analisa.
	foreach
        ...
        if _cod_cobrador = "217" and _cod_agente not in ('03305') then
		    let _error = 1;
			exit foreach;
		end if
	end foreach
...
-- Excepción para el caso 2168 CODIGO 324 / EMISION POLIZA DE INCENDIO esta póliza nueva se va emitir bajo autorización previa de la vicepresidencia WE 30/12/2021 --Amado Perez M.
if a_no_poliza <> '0001730812' then
	if _cod_ramo in('001','003') And _nueva_renov = 'N' then	--Para incendio y Multiriesgo con vigencias >= 01/07/2018 en Z.L. y FF, no se puede emitir. 07/05/2018
		...
		select count(*)
		 ...
		 where u.cod_manzana = e.cod_manzana
		   ...
		   and e.cod_barrio in('0103','4400')
		   and cod_provincia = '03' 
		   and cod_distrito = '001'; --Barrio Zona L. y Barrio France Field 09/09/2025 Se agrego provincia y distrito que corresponden a Colón 
		if _cnt > 0 then
			return 324;
		end if
	...
	elif _cod_ramo = '019' then	--El asegurado debe ser mayor de edad 03/01/2025 AMM.
		foreach
		...
		select fecha_aniversario into _fecha_aniv from cliclien where cod_cliente = _cod_asegurado;
		if _fecha_aniv is null or _fecha_aniv = "" then
			return 336;
		end If
		let _edad = sp_sis78(_fecha_aniv,_vig_i);
		if _edad < 18 THEN
			return 340;
		end if
	end if
...
if _nueva_renov = 'R' then
	call sp_sis186(_no_doc_orig,_tiene_imp) returning _error;		--Verifica impuesto
	if _error <> 0 then
		return 3;
	end if
	...
	if _cod_grupo not in ('78015','78020') then
	
		if _cod_tipoprod <> '002' then -- Ajustar el bloqueo de la emisión de Pólizas por Perfil de Clientes # 7090 -- Amado 12/07/2023
			call sp_par380(_cod_contratante, _fecha_suscripcion, _sucursal_origen) returning _error, _cedula, _nombre_pagad, _ponderacion; -- Verificación del perfil del contratante # 6906 -- Amado 23/06/2023 -En espera dela fecha de implementacion
			if _error = 0 then
				return 11;
			end if
		end if
	...
	if _cod_ramo in ('002','020','023') and _cod_tipoprod <> '002' then	--se puso exclusion de coas. min por las cargas del estado, Jenniffer de F.
		foreach
			...
			if a_no_poliza = '0003290279' then
				exit foreach;
			end if	
...
if _ramo_sis = 1 then	--si es ramo auto
	if (_dia_val_i = _dia_val_f) And (_mes_val_i = _mes_val_f) then
	else
		if _cod_grupo in('1000','00068') then	--Excluye del control polizas de estado, caso 11695 Boni, Excluye del control Serafin Niño
		elif _cod_tipoprod in('001','002') then --Excluye del control ambos coaseguros, caso 11695 Boni		
		elif _cod_sucursal = '009' then
			foreach
			...
			if _cod_producto <> '01499' and _cod_producto <> '05409' and _cod_producto <> '05410' then -- Excluye del control Soda Frontera
				return 329;
			end if
		elif _cod_contratante = '11465' then --Excluye contratante Aseguradora Ancon
		else
			return 329;
		end if
	end if
...
if _nueva_renov = 'N' then
	...
	if _cod_ramo = '019' then --anos_pagador = 1 ramo vida individual cuando la poliza es nueva
		...
		let _error = sp_sis263(a_no_poliza);
		if _error <> 0 then
			return _error;
		end if
	...
	if _cod_ramo = '018' then -- Si se reemplaza una póliza de salud
...
if _cod_ramo <> '019' then
	...
	foreach
		select no_unidad,
			   sum(round(porc_partic_prima,4)),
			   sum(round(porc_partic_suma,4))
		  into _no_unidad,
			   _prima_sus_cal,
			   _prima_sus_sum
		  from emifacon
...
--Verificación de la serie de la vigencia de la póliza vs la serie de los contratos en la dist de reaseguro --24/04/2017
let _cnt_serie = 0;
...
-----------------------------------------------------------------------------------------                                                                   
-- Verificación del numero de factura que no este repetido. Federico Coronado 19/3/2019
select count(*) 
  into _canti 
  from emipomae 
 where actualizado = 1 
...
if _cod_formapag in ('003','005','092') then
	...
	if _cod_ramo = '002' and _cod_subramo = '001' then --Ramo=002 y Subramo=001 y Producto=10602, NO debe tener descuento electronico. caso 11648 AMM.28/09/2024
		let _tiene_prod = 0;
		let _tiene_prod = sp_emi04(a_no_poliza,'10602');
		if _tiene_prod = 1 then	--Tiene el producto en la unidad.
			let _desc_electr = 0;
		end if
	end if
	if _cod_ramo in('004','008','016','019','018','023') then  --Ramos Personales y SODA no aplican. -- Incendio ahora si aplica Amado 24-04-2025 SD 13432  
		let _flag_electronico = 1;
	end if
	
	if _cod_ramo = '003' and _cod_subramo = '005' then -- Incendio ahora si aplica todo Amado 24-04-2025 SD 13432 --> antes (_cod_ramo in ('001') and _cod_subramo = '006') or 
		let _flag_electronico = 1;
	end if
...
if _saldo_x_unidad = 1 then
	...
	IF _tipo_forma = 2 THEN -- Tarjetas de Credito
		...
		--Actualizar Letras a Pólizas con Vigencias Anteriores a la fecha de hoy	02/09/2014		
	...
	IF _tipo_forma = 4 THEN -- Ach
		...
		ELSE	--sumarle al monto del ach, el monto de la nueva poliza que se incorpora a la misma cuenta.
			...
			ELSE
				if _vig_i < today then --Actualizar Letras a Pólizas con Vigencias Anteriores a la fecha de hoy	02/09/2014		
					UPDATE cobcutas
					   SET monto        = _monto_visa
					 WHERE no_cuenta    = _no_cuenta
					   and no_documento = _no_documento;
...
-- Registros para el Comprobante de Reaseguro sac999:reacomp
...
-- Validar forma de pago para el Producto 10602 caso 11609 25/09/2024 FCORONAD
call sp_emi03(a_no_poliza) returning _error;
...
if _cod_producto <> '10602' then -- Producto 10602 creado para los corredores con la forma de pago tcr 003 no deben aplicar el descuento 5% ya que tienen un 20% de descuento en la cotizacion #Fcoronado 03/09/2024
	if _cod_formapag in('003','005','092') then	
		if _flag_electronico = 0 then 
...
CALL sp_sis253(a_no_poliza) returning _error, _error_desc; --Producto de Auto de Clientes sin Siniestros
if _error <> 0 then
	return _error;
end if
