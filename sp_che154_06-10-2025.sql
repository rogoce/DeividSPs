------------------------------------------------
--   Detalle de Autoriza ACH   Reclamos de salud    --
---  Amado - 21/06/2016 --
------------------------------------------------
drop procedure sp_che154;

create procedure sp_che154(a_origen_chk char(1) default "%")
...
ORDER BY a_nombre_de
	...
	foreach
		...
		if _cod_area = 1 and _cod_tipopago = '004' then -- Pago a tercero
			...
			select date_doc_comp
			  into _date_doc_comp -- Fecha de documentaciÃ³n completa Tercero de automÃ³vil
			  from recterce
			 where no_reclamo = _no_reclamo
			   and cod_tercero = _cod_cliente;
		end if
		  
		if _cod_area = 1 then -- AutomÃ³vil		  
			...
			select count(*) -- Verificando si es una requisiciÃ³n de pago por ajuste
			  into _cant_ajust			
			  from recordam
			 where no_requis = _no_requis;
			...
			if _cant_serv > 0 then
				if _fecha_factura is not null then
					...
					let _fecha_captura = _fecha_capt_ori; -- Fecha en que se capturÃ³ la requisicion
				end if
			end if
		    ...
		    select cod_entrada
			  into _cod_entrada -- NÃºmero del bloque
			  from atcdocde
			 where cod_asignacion = _cod_asignacion;
			 
		    select date(fecha) -- Fecha en que se creÃ³ el bloque
			  into _fecha_scan
			  from atcdocma
	...
	-- Prioridades por dÃ­as transcurridos
