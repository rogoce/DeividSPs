...
create procedure "informix".sp_cob274(a_cod_agente char(5))
					...
					if _cant_email_cobros = 0 then
						...
						if _email_agente = '' or _email_agente is null then
							if _email_reclamos = '' or _email_reclamos is null then 
								...
								update agtagent set _email_reclamos = _email_reclamos where cod_agente = a_cod_agente;	
							end if
						else
							update agtagent set email_cobros = _email_agente where cod_agente = a_cod_agente;
						end if
			 ...
			 where email = _email
			   ...
			   and tipo_correo = 'COB';
				...
				if _cant_email = 0 then
					...
					values (a_cod_agente,'COB',_renglon,_email);
