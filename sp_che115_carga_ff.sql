...
--Creado 15/03/2013	Armando Moreno M.


--DROP PROCEDURE sp_che115_carga_ff;
CREATE PROCEDURE sp_che115_carga_ff(
a_compania          CHAR(3),
a_sucursal          CHAR(3)
) RETURNING SMALLINT;
...
let _fec_aa_ini     = "01/01/2012";	--para sacar cancelada o anulada en el periodo
let _per_ini        = "2012-01";
let _per_ini_ap     = "2011-01";
let _ano            = a_periodo[1,4];
let _ano            = _ano - 1;
let _per_fin_ap     = _ano || a_periodo[5,7];

let _per_fin_dic    = "2011-12";         
let _fecha_aa_ini   = sp_sis36(_per_ini);	 --31/01/2012
let _fecha_aa       = sp_sis36(a_periodo);	 --31/12/2012
let _fecha_ap_ini   = sp_sis36(_per_ini_ap); --31/01/2011
let _fecha_ap       = sp_sis36(_per_fin_ap); --31/12/2011
...
pri_cob_max         dec(16,2) 	default 0) 
with no log;  }
...
{create temp table rentabilidad1(
periodo   			CHAR(7) , 
cod_agente     		CHAR(5) , 
no_documento   		CHAR(20), 
...
foreach
 select no_documento,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between _per_ini and a_periodo	   -- 2012-01 a 2012-12
...
--***********************
-- Prima  SUSCRITA  AP --	   falta restar el % por reserva de prima no devengada	  Anio Pasado
--***********************
...
foreach 
 select no_reclamo,		
   ...
   from rectrmae 
  where cod_compania = a_compania
    and periodo      <= _per_fin_dic     -- 2011-12
	and actualizado  = 1
  group by no_reclamo
...
{	 if _concurso = 0 then -- Excluir del Concurso
		continue foreach;
	 end if  	}  -- Esto es equivocado ya que no entra en la condicion.

     if _cod_ramo in("002","020","004","016","012","014","017","010","022","013","003","006","015","005","011","021","009","007") then
	 else
		continue foreach;
     end if			   
   ...
   if _cnt > 0 then
	...
	foreach
	 ...
	 select cod_agente
	  ...
	  where no_poliza = _no_poliza
        ...
        let _unificar = 0;	 --01068 FFseguros
	...
	--/*CATEGORIZAR*/
	--A-AUTOMOVIL                   ="002","020"
	--C-PATRIMONIALES               ="003","005","006","015","011","013","010","009","017","014","021","012","007","022"
	--D-PERSONAS    				="004","016"
	...
	if  _cod_ramo in ("003","005","006","015","011","013","010","009","017","014","021","012","007","022") then
	   let _cod_tipo = 'C' ;
	end if

	if  _cod_ramo in ("004","016") then
	   let _cod_tipo = 'D' ;
	end if}
	 ...
	 foreach
	  ...
	  SELECT cod_agente
	   ...
	   WHERE no_poliza = _no_poliza
           ...
           let _unificar = 0;	 -- 01068 Unificar FF seguros
