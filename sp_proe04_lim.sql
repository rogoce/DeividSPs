...
drop procedure sp_proe04_lim;
create procedure sp_proe04_lim(a_poliza	char(10),a_unidad char(5),a_suma dec(16,2),a_cia char(3))
returning integer;
...
define ld_impuesto1,_limite_2		   	dec(16,2);
define ld_imp_total,_limite_1       	dec(16,2);
...
define ld_suma,_suma_lim_cober dec(16,2); 
...
if ls_ramo in('002','023') then
	...
	call sp_sis188_uni(a_poliza,a_unidad) returning _error,_mensaje;
end if

let _suma_lim_cober = 0;
foreach
	...
	if _mult_plenos > 0 then
		...
		foreach
			...
			if ls_ramo in('002','023') then
				...
				select suma_cob_reas
				  into _suma_lim_cober
				  from tmp_dist_rea_u
				 where cod_cober_reas = ls_cober_reas;

				--let ld_suma = (a_suma * ld_porc_suma / 100) * _porc_proporcion / 100;
				let ld_suma = (_suma_lim_cober * ld_porc_suma / 100);
			end if
