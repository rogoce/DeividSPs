...
IF _cantidad <> 0 THEN
   FOREACH	
	SELECT no_unidad,cod_cober_reas,prima,orden
	  INTO _no_unidad,_cobertura,_prima_sus_cal,_orden
	  FROM emifacon	e, reacomae r
	 WHERE e.no_poliza     = a_no_poliza
	   AND e.no_endoso     = a_no_endoso
	   AND e.cod_contrato  = r.cod_contrato
	   ...
	   UPDATE emipomae
		...
		if _no_documento[1,2] = "18" or _no_documento[1,2] = "19" or _prima_bruta <= 0 then 
				...
				IF _tipo_forma = 2 and _no_tarjeta is not null THEN -- Tarjetas de Credito
				   --	let _monto_visa = sp_sis405(a_no_poliza);	6/08/2013 puesto en comentario Armando
...
CALL sp_cob314(a_no_poliza, a_no_endoso) returning _error, _mensaje; 
if _error <> 0 then
	return _error, _mensaje;
end if
