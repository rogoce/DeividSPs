...
drop procedure ap_rec177d;		

create procedure "informix".ap_rec177d(a_numrecla char(20))
...
foreach
	...
	if _no_reclamo is null or _no_reclamo = '' then
		continue foreach;
	end if
	...
	foreach
		...
		if _variacion_cob <> _res_actual_rcob then
			...
			return 1,a_numrecla,_no_reclamo,_res_actual_rcob,_variacion_cob with resume;
		end if
	end foreach
	...
	if _reserva_actual <> _variacion_tr then
		update recrcmae
		 ...
		 where no_reclamo = _no_reclamo;
		return 2,a_numrecla,_no_reclamo,_variacion_ac,_variacion_tr with resume;
	end if
end foreach
