...
-- SIS v.2.0 - DEIVID, S.A.	 execute procedure sp_cob753bk_new()

drop procedure sp_cob753bk_new;
create procedure sp_cob753bk_new()
returning integer,
          char(100);
...
{
foreach
select distinct trim(no_documento),fecha_proceso
  into _no_documento,_fecha_procesar
  from avisocanc where estatus not in ('Y','Z')   
   --and fecha_proceso >= _fecha_actual
   and year(fecha_proceso) >= 2016
 order by 2 desc 
 --select e.estatus_poliza,a.*
 }
 
 foreach
select a.no_documento,a.fecha_imprimir
from avisocanc a,emipomae e
where a.estatus = 'I'
and a.no_poliza = e.no_poliza
and e.estatus_poliza in (1)   -- (3,2,4)
and a.motivo is null  and ((a.dias_180 + a.dias_150 + a.dias_120 + a.dias_90 + a.dias_60) <= 0.00 )
and a.cod_ramo not in ("004","016","018","019")
and a.fecha_imprimir < '01/01/2017'
--and a.fecha_imprimir > e.fecha_
--and cod_acreedor <> ''
order by a.periodo desc,a.no_documento

 
    if _fecha_procesar is null then
		continue foreach;
	end if
...
foreach
	select b.fecha,
	 ...
	 where a.no_remesa = b.no_remesa
	   and b.actualizado  = 1
	   and trim(b.doc_remesa) = _no_documento   -- in (select distinct trim(no_documento) from avisocanc where estatus not in ('Y','Z'))
	   and b.tipo_mov     in ('P', 'N', 'X')	-- Pagos - Creditos
	   and a.date_posteo >= _fecha_procesar
	 group by 1,2
	 order by 1,2
