...
create procedure "informix".sp_pro77h(a_compania char(03), a_terremoto smallint, a_fecha date, a_fecha2 date) 
...
order by d.no_documento,d.no_poliza,e.no_endoso
	...
	foreach
		...
		if  _error <> 0 then
			--continue foreach;
			return  _error_desc, _error_desc,0,0,0,0,0,0,0,0,'',v_nodocumento, _no_unidad,null,null,'';		   
		end if}
		...
		if a_terremoto = 1 then
			foreach
				...
				foreach
					...
					if _porc_cober_reas is null then
						let _porc_cober_reas = 0;
					end if}
