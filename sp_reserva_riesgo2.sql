...
define _cod_ramo,_cod_ramo2			char(3);
...
with no log;
CREATE INDEX tmp_cod_ramos_1 ON tmp_ramos_acum(cod_ramo,n_ramo);
...
--CICLO PARA INVOLUCRAR A TODOS LOS RAMOS DE LA CARGA DEL MES.
foreach
	select cod_ramo
	  into _cod_ramo2
	...
	foreach
		...
		if _cod_ramo <> '017' then  --Para este ramo se hace mas abajo.
			select sum(rrc_100),
				   sum(rrc_cedida)
			  into _rrc_100,
				   _rrc_cedida
	...
	if _n_ramo is null then
	
		let _cod_ramo = _cod_ramo2;
		let _rrc_cedida      = 0.00;
		let _rrc_100         = 0.00;
		let _prima_suscrita  = 0.00;
		let _comision_corr   = 0.00;
