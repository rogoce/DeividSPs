   DROP procedure sp_pro284;
   CREATE procedure "informix".sp_pro284(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7))
...
---  Modificado por Ruben Dario Arnaez	 24 de Julio de 2007  							---
---  Ref. Power Builder 																---
-------------------------------------------------------------------------------------------
    ...
    DEFINE v_cant_polizas,_cnt_reclamo,v_cnt_pol_n          INTEGER;
...
{			   LET _prima_neta = 0;
			   ...
			   END IF
		  -- END IF
			   IF _no_endoso = '00000' AND _nueva_renov = 'N' THEN	--> Se agrega para contar las polizas nuevas del mes 
				   BEGIN
				      ON EXCEPTION IN(-239)
				         UPDATE temp_perfil1
				            SET cnt_pol_n      = cnt_pol_n + 1
				          WHERE cod_ramo       = _cod_ramo
				            AND cod_subramo    = _cod_subramo;
...
FOREACH WITH HOLD
 ...
 --	END IF
   ...
   END IF	
    ...
    IF v_cod_ramo = '002' OR v_cod_ramo = '020' THEN
		select count(*)
		  into _cantidad
		  from emipouni
		 where no_poliza = _no_poliza;
...
--   END IF
   ...
   SELECT SUM(cant_polizas), SUM(cnt_pol_n)
     INTO v_cant_polizas, v_cnt_pol_n
     FROM temp_perfil1
    WHERE cod_ramo = v_cod_ramo;
 ...
 {  ELIF	v_cod_ramo = "001" THEN
   ...
   ELIF	v_cod_ramo = "009" THEN
     UPDATE ramosubr2
      ...
      WHERE cod_ramo     = v_cod_ramo
        AND cod_subramo  = "001";
   ELIF	v_cod_ramo = "002" Or v_cod_ramo = "020" THEN
     UPDATE ramosubr2
        SET cnt_polizas  = v_cant_polizas,
     	    prima_suscrita = _prima_suscrita,
     	    incurrido_bruto = incurrido_bruto + v_incurrido_bruto,
