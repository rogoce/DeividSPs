...
DROP PROCEDURE sp_rec292;

CREATE PROCEDURE sp_rec292()
...
FOREACH	   
	select fecha_reclamo,
	  ...
	  into _fecha_reclamo,
		   ...
		   _numrecla
	  from recrcmae
	 where fecha_reclamo >= '01/03/2019'
...
--	 where no_reclamo = _no_reclamo
     ...
     select count(*)
	   ...
	   from recrccob
	  where no_reclamo = _no_reclamo
	    and cod_cobertura in (select cod_cobertura from prdcober where nombre like "LES%CO%" OR nombre like "DA%PR%");
