------------------------------------------------
--   Detalle de Autoriza ACH Reclamos que no usan chequera de salud          --
---  Amado - 22/04/2020 --
------------------------------------------------
drop procedure sp_che183;
create procedure sp_che183(a_origen_chk char(1) default "%")
...
define _cod_chequera, _cod_chequera_dist char(3);  
...
let _cod_chequera_dist = '006';

if a_origen_chk = "Z" or a_origen_chk = "S" then
	let a_origen_chk = "3";
end if

if a_origen_chk = 'M' then
    let _cod_chequera_dist = '001';
end if
...
foreach
  SELECT no_requis,   
   ...
   WHERE ( origen_cheque like a_origen_chk ) AND  
         ( cod_banco = '001') AND
         ( cod_chequera <> _cod_chequera_dist) AND 
         ( pagado = 0 ) AND  
         ( tipo_requis = 'A' ) AND  
         ( en_firma = 2 ) AND
...
ORDER BY a_nombre_de
	...
	let _cant = sp_rec315(_no_requis);	
  	...
  	elif _origen_orig = "M" then
  		if _numrecla[1,2] not in ('18','04') then
  			continue foreach;
  		end if
  		if _cant <> 0 then
  			continue foreach;
  ...
  --		end if
	...
	-- Prioridades por días transcurridos
		...
		elif _cod_area = 5 then -- Accidentes Personales
			let _prioridad = 100;
		end if				
