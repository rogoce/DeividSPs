...
drop procedure ap_par307;
create procedure ap_par307()
...
on exception set _error, _error_isam, _descripcion
	let _descripcion = 'Póliza: ' || trim(_no_documento) || ' ' || trim(_descripcion);
...
foreach with hold
	select no_documento
	  into _no_documento
	  from emipoliza
	 where cod_ramo is not null
	   and no_documento = '1921-00020-07'
	   --and year(vigencia_inic) >= 2017
	   --and cod_grupo = '01016'
	...
	{if _estatus_poliza = 1 then	--Solo a polizas vigentes. 04/03/2021
		call sp_pro545(_no_documento)returning _error, _descripcion; --puesto 03/03/2021
	end if}

   	call sp_cob245a("001","001",_no_documento, _periodo,_fecha)
