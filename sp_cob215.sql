...
DROP procedure sp_cob215;
CREATE procedure "informix".sp_cob215(a_agente CHAR(5), a_desde DATE, a_hasta DATE )
RETURNING   CHAR(3),   -- _cod_compania
	...
	Let _periodo1 = sp_sis39(a_desde);
	Let _periodo2 = sp_sis39(a_hasta);

	foreach  
		select cob.cod_compania,
				cob.no_poliza,
				cob.no_recibo,
				cob.prima_neta,
				cob.no_remesa,
		...
		where cob.periodo >= _periodo1
		  and cob.periodo <= _periodo2
  		  and cob.actualizado  = 1
  		  and cob.tipo_mov     in ("P", "N")
		  and agt.cod_agente = a_agente
		...
		RETURN  _cod_compania, _descr_cia, _cod_ramo, _ramo, _no_documento, _vigencia_inic, _vigencia_final,
		        _estatus_poliza, _cod_formapag, _no_recibo, _nombre, _prima_neta, _desc_formapag, _fecha_pago  WITH RESUME;
