-- Procedimiento que genera las cancelaciones Automatica
-- Creado     : 16/01/2008 - Autor: Demetrio Hurtado Almanza
-- Modificado : 01/10/2010 - Autor: Henry Giron
-- SIS v.2.0 - DEIVID, S.A.

drop procedure sp_par304;
create procedure sp_par304(
a_no_poliza		char(10),
a_usuario		char(8),
a_saldo			dec(16,2),
...
define _dias_cubiertos	dec(16,2);
...
--set debug file to "sp_par304.trc";
--trace on;
...
if _cnt = 0 then
	return 1, "Poliza "||_no_documento||" No tiene coberturas.",a_no_poliza;			
end if
...
if _flag_cambio = 1 then
	call sp_cob174(_no_documento) returning	_saldo;

	let _dias_cubiertos = (_saldo/_prima_bruta_e) * _dias1;	
	let _vig_fin_max = _vigen_fin - _dias_cubiertos units day;
	
	if _vig_fin_max < _vigen_ini then
		let _vig_fin_max = _vigen_ini;
	end if
end if
...
-- trace on;
call sp_pro493(a_no_poliza, _no_endoso, _factor_vig ) returning _error, _descripcion;
-- trace off;
...
if  _cod_tipocalc = "001"  and _cod_tiporamo = "002"  then --SD#7309 CMORAN
       let _saldo2 = 0;
      call sp_cob174(_no_documento) returning	_saldo2;	
	select sum(prima)
	  into v_prima
	  from endedmae 
	 where no_poliza = a_no_poliza
       and no_endoso  = _no_endoso;
		...
		if abs(v_prima) > abs(_saldo2) then
			set debug file to "sp_par304.trc";
			trace on;	
            let _saldo2	= _saldo2;
			let v_prima = v_prima;
			let _no_endoso = _no_endoso;
			return 1, "Poliza "||_no_documento||" con Prima Prorrata mayor al Saldo .",a_no_poliza;
			TRACE OFF;
		end if				
end if
