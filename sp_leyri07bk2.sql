...
drop procedure sp_leyri07bk2;
create procedure "informix".sp_leyri07bk2(a_fecha1 date, a_fecha2 date)
returning   char(10),
			varchar(50),
			char(20),
			char(10),
...
let	_res_cr	= 0.00;
 ...
 CREATE TEMP TABLE tmp_cgl
           (cod_agente       CHAR(5),
			...
			porc_partic_agt	 dec(5,2) not null,
			no_remesa        char(10),
			renglon          integer,
			tipo_mov         char(1)
			) WITH NO LOG;
...
foreach
   ...
   return _cod_agente,_n_agente,_no_documento,_no_poliza,_res_comprobante,_res_notrx,_res_db,_res_cr,_tipo_agente,_no_remesa,_renglon,_tipo_mov with resume;
