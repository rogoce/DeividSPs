--***************************************************************************************
-- Procedimiento que genera el Reporte para convencion 2027 para los corredores
--***************************************************************************************
--CONVENCION
...
--*************************************************************************************
-- Prima neta cobrada devengada periodo convencion 2026-01 Al periodo se esta evaluando
--*************************************************************************************
...
foreach
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2026','31/12/2026', my_sessionid);
	call sp_dev06f(_no_documento,_fecha_aa,'01/01/2026',_fecha_aa) returning _valor,_mensaje,_pri_dev_aa;
...
foreach
	select no_documento
	  into _no_documento
	  from emipoliza
	 where vigencia_fin >= '01/01/2026'
	   and vigencia_inic < '01/01/2026'
...
--**********************************************************************
-- Prima Cobrada periodo convencion 2026-01 Al periodo se esta evaluando
--**********************************************************************
...
foreach
	...
	if _estatus_poliza in (2,4) then
		continue foreach;	--POLIZA CANCELADA,ANULADA NO SE DEBE TOMAR EN CUENTA.
	end if
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/01/2026' and '31/12/2026';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/01/2026' and _fecha_anulado <= '31/12/2026' then
					let _monto_dev = 0;
				end if
			end if			
		else
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2026','31/12/2026',my_sessionid); --se cambia a tabla fisica 05/06/2018
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
call sp_bo077_2('01/01/2025', _fecha_ap, _fecha_aa) returning _error, _error_desc;
...
----**********************************************************************************************
call sp_bo077_2('01/01/2026', _fecha_aa, _fecha_aa) returning _error, _error_desc;
...
foreach
	...
	if year(_date_added) = 2026 then --ES AGENTE NUEVO, LO COLOCO EN RANGO 6
