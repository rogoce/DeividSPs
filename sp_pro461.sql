...
Select x.vigencia_inic, x.vigencia_final, cod_tipo_tar
  Into v_poliza_inic, v_poliza_fin, _cod_tipo_tar
  From emipouni x
...
foreach
  ...
  select x.acepta_desc, descuento_max, tipo_descuento 
  ...
  if v_poliza = '1499878' and v_endoso = '00005' then --Caso: 35550 USER: ARODRIGU PC: DAVID67
	 let _canti_2 = 0;
	  ...
	  if _canti_2 = 0 then
		  select acepta_desc, descuento_max, tipo_descuento 
			into v_acepta, _descuento_max, _tipo_descuento 
			from prdcobpd
		   where cod_producto  = '06082'
			 and cod_cobertura = v_cobertura;
	  end if
  ...
  If v_acepta = 1 Then
	  ...
	  let v_prima_descto = v_prima_cob;
   	   ...
   	   if _tipo_descuento in (1,2) and _cod_tipo_tar = '001' and _fecha_suscripcion >= "28/07/2014" and (_nueva_renov = "R" or v_poliza in ('0001058773'))then
			let _descuento_max = sp_proe78(v_poliza, v_unidad, v_producto, v_cobertura);
			
			if v_poliza not in ('0001058773',"970888") and v_unidad not in ("00030","00039") then
	   ...
	   if _cod_tipo_tar = '002' or _cod_tipo_tar = '004' or _cod_tipo_tar = '005' or _cod_tipo_tar = '006' or _cod_tipo_tar = '007' or _cod_tipo_tar = '008' then -- Descuento por modelo, Descuento por Siniestralidad, Descuento Sedan
         -- Esta calculando mal cuando hay 2 descuento por cobertura -- Amado 03-07-2025
  ...
  --     let v_tot_descto = _desc_cob;
	    ...
	    {if v_poliza = '2015270' and v_unidad = '00001' and v_endoso = '00001' and v_cobertura in('00118','00119')then
			let v_tot_descto   = 0.00;
			let v_prima_descto = v_prima_cob;
			foreach
