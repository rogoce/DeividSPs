...
--drop procedure sp_sac53;

create procedure sp_sac53(
a_periodo	char(7),
a_usuario	char(8),
a_origen	smallint
) returning integer,
...
if a_periodo = "2006-10" then
	let _desc_ajuste = "AJ7";
elif a_periodo = "2006-11" then
	let _desc_ajuste = "AJ8";
elif a_periodo = "2006-12" then
	let _desc_ajuste = "AJ9";
end if
  ...
  where m.no_requis = p.no_requis
	...
	if _impuesto <> 0.00 then
		...
		if _impuesto <> _suma_impuesto then
			...
			let _debito  = 0.00;
			let _credito = _impuesto - _suma_impuesto;
			
			update cgltrx2
			   set trx2_credito = trx2_credito + _credito
