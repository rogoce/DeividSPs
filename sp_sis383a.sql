-- Procedimiento que returna la cantidad de impresion original de renovacion
-- Corrige la cantidad de unidades que debe imprimir cada unidad por acreedor
-- Realizado por Henry
drop procedure sp_sis383a;

create procedure "informix".sp_sis383a(a_poliza CHAR(10)) 
returning 	  smallint;

define _cantidad		smallint;
define _copia			smallint;
define _leasing         smallint;
define _unidad			smallint;
define _unidad_acre     smallint;

let _leasing = 0;
LET _copia   = 1;	  -- contar del Asegurado
-- determinar si es leasing

select leasing
  into _leasing
  from emipomae
 where no_poliza = a_poliza;

-- Cuando el corredor es diferente a directo
select count(*)
into _cantidad
from  emipoagt
where no_poliza = a_poliza
and cod_agente <> '00099';

if _cantidad is null then 
	LET _cantidad = 0;
end if

LET _copia = _copia + _cantidad;
LET _cantidad = 0;
LET _unidad = 0;
LET _unidad_acre = 0;

-- Cuando el corredor tiene acreedor hipotecario
 select count(distinct no_unidad)
   into _unidad
   from emipouni
  where no_poliza = a_poliza;

 select count(distinct no_unidad)
   into _unidad_acre
   from emipoacr
  where no_poliza = a_poliza;

if _unidad = _unidad_acre then			-- Solo se Imprime si: Todas las unidades tienen el mismo acreedor
	select count(distinct cod_acreedor)								
	  into _cantidad												
	  from emipoacr
	 where no_poliza = a_poliza	;
	--   and no_unidad = a_unidad ;

		if _cantidad is null then 
			LET _cantidad = 0;
		end if

		if _cantidad = 1 then
			if _cantidad = 0 and _leasing = 1 then
				LET _copia = _copia + 1;
			end if
			LET _copia = _copia + _cantidad;
			LET _cantidad = 0 ;
		end if
end if

{-- Cuando el corredor tiene coaseguro mayoritario

select count(distinct cod_coasegur)
into _cantidad
from  emicoama
where no_poliza = a_poliza
and cod_coasegur <> '036';

if _cantidad is null then 
	LET _cantidad = 0;
end if}

LET _copia = _copia + _cantidad;
LET _cantidad = 0;


return _copia;

end procedure
				