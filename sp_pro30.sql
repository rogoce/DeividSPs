...
foreach
	...
	--*********************************Determinacion de la Ruta de reaseguro, se baja a nivel de la póliza para establecer la serie y contratos. SD Cesia 13194
	let _cod_ruta = null;
	let _error_desc = 'Contrato de Reaseguro. ';
	
	let _vigencia_ruta = MDY(month(_vigencia_inic), day(_vigencia_inic), _ano_contable);
	if _vigencia_ruta <= a_vigencia_hasta then	--Si es <= Se usa esa misma.
	else
		--Se resta 1 año y se arma la variable de la vigencia.
		let _ano = _ano_contable - 1;
		let _vigencia_ruta = MDY(month(_vigencia_inic), day(_vigencia_inic), _ano);	
	end if
	...
	foreach
		select cod_ruta
		 ...
		 where cod_ramo = _cod_ramo
		   and activo = 1
		   and _vigencia_ruta between vig_inic and vig_final
	...
	if _cod_ruta is null then
		...
		insert into emifacerr(no_documento, periodo, estatus,descripcion1,descripcion2,cerror)
		values (_no_documento,_periodo,0,_error_desc,'La Ruta No existe para la vigencia.',0);
		return 1, _error_desc; 
	end if
	...
	if _porc_partic_prima <> 100 then
		...
		insert into emifacerr(no_documento, periodo, estatus,descripcion1,descripcion2,cerror)
		values (_no_documento,_periodo,0,_error_desc,'porc_partic_prima',0);
		return 1, _error_desc; 
	end if
	...
	if _porc_partic_suma <> 100 then
		let _error_desc = 'La Distribucion No Es 100%, Verifique la Ruta ' || _cod_ruta;
		insert into emifacerr(no_documento, periodo, estatus, descripcion1,descripcion2,cerror)
		values (_no_documento, _periodo,0,_error_desc,'porc_partic_suma',0);
		return 1, _error_desc; 
	end if
...
--************************************************************************************************************************************
	...
	--****SI ESTA EN EMICARTASAL6 Y ES OPCION 2, SE INCREMENTA EL MES AL PERIODO Y LA OPCION A 4 EN EMICARTASAL 6.
