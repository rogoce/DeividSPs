...
-- se corrige condicion de listado y temporal de acreedor HGIRON
...
where a.no_aviso IN (SELECT codigo FROM tmp_codigos ) 
	   ...
	   or ( a.estatus in ('X','M')  and a.cancela = "0" ) and (a.user_marcar = 'DEIVID'))	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
...
foreach
	select distinct a.nombre_acreedor nombre,a.no_poliza poliza
	 ...
	 where a.no_aviso IN (SELECT codigo FROM tmp_codigos ) 
	   ...
	   or ( a.estatus in ('X','M')  and a.cancela = "0" ) and (a.user_marcar = 'DEIVID'))	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
       and ( trim(a.cod_acreedor) <> "" )
	   --and a.saldo > 0      -- and a.imp_aviso_log = '3'
