...
------------------------------------------------
--execute procedure sp_pr860c1_h('001','001','2013-07','2013-09',"*","*","*","*","001,003,006,008,010,011,012,013,014,021,022;","*","2013,2012,2011,2010,2009,2008;")

drop procedure sp_pr860c1_h;
create procedure sp_pr860c1_h(
	a_compania		char(03),
	a_agencia		char(03),
	a_periodo1		char(07),
	a_periodo2		char(07),
...
drop table if exists brx_reacoest;
drop table if exists brx_temphg;
...
select * 
  from temphg
  into temp brx_temphg;

select * 
  from reacoest
  into temp brx_reacoest;

if _borderaux = '01' then	--es bouquet y facilidad car  
	delete from brx_reacoest where anio = _anio_reas and trimestre = _trim_reas and borderaux = _borderaux;  -- Elimina borderaux del trimestre
	delete from brx_temphg   where anio = _anio_reas and trimestre = _trim_reas and borderaux = _borderaux;  -- Elimina borderaux datos

	if a_codramo = '*' then
		let a_codramo = "001,003,006,008,010,011,012,013,014,021,022;";
	end if
else
	delete from brx_reacoest
	 where anio = _anio_reas 
	   and trimestre = _trim_reas
	   and borderaux = _borderaux;  -- Elimina borderaux del trimestre
	   
	delete from brx_temphg   
	 where anio = _anio_reas 
	   and trimestre = _trim_reas 
...
call sp_che143_h(a_compania,a_agencia,a_periodo1,a_periodo2,a_codsucursal,a_codgrupo,   --crea tabla temp_det (temporal)
...
drop table brx_reacoest;
drop table brx_temphg;
