...
drop procedure sp_sis462;
create procedure sp_sis462(a_no_documento char(20), a_periodo CHAR(7))
returning	smallint,
			varchar(100);
...
define _min_fecha_pago		date;
define _max_fecha_pago		date;
...
foreach
	...
	select count(*)
	 ...
	 inner join polcovid cov
			...
			and cov.no_documento = a_no_documento;   --endoso de descuento covid 19
	...
	select count(*)
	 ...
	 inner join polcovid cov
			...
			and cov.procesado = 1
			and endo.actualizado = 1
			and cov.no_documento = a_no_documento;   --reversion endoso de descuento covid 19
...
if (_monto_60 + _monto_90) > 5 then
	select min(fecha) + 1 units day,
		   max(fecha) + 1 units day
	  into _min_fecha_pago,
		   _max_fecha_pago
	  from cobredet
	 where no_poliza = _no_poliza
	   and tipo_mov in ('P','N')
	...
	let _periodo_pago = sp_sis39(_max_fecha_pago);
	
	if _periodo_pago <= a_periodo then
		let _fecha_morosidad = _max_fecha_pago;
	else
		let _periodo_pago = sp_sis39(_min_fecha_pago);
		if _periodo_pago <= a_periodo then
			let _fecha_morosidad = _min_fecha_pago;
		else
			insert into tmp_poliza
			values(	_no_poliza,
					0,
