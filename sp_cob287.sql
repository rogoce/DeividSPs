...
drop procedure sp_cob287;
create procedure sp_cob287()
returning	integer,
			char(50);
...
--------------------Para cuando el año es bisiesto  AMM 29/02/2024
let _dos_anos = current;
let _dia = day(_dos_anos);
let _mes = month(_dos_anos);
let _ano = year(_dos_anos);

if _dia = 29 and _mes = 2 then
	let _dos_anos = MDY(2, 28, _ano);
end if
let _dos_anos = _dos_anos - 2 units year;
----------------------------------------------------
foreach
	select no_poliza,
		   cod_formapag,
