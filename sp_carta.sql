...
DROP PROCEDURE sp_carta;
CREATE PROCEDURE sp_carta(a_compania CHAR(3),a_sucursal CHAR(3),a_ano integer, a_usuario CHAR(10), a_agente VARCHAR(255), a_membrete SMALLINT DEFAULT 0)
RETURNING   CHAR(5)      as cod_corredor,
            VARCHAR(100) as nombre,
			CHAR(10)     as no_requis,
			CHAR(10)     as no_remesa,
			DEC(16,2)    as com_desc,
			DEC(16,2)    as comision,
			DEC(16,2)    as cinco_seis,
			DEC(16,2)    as cinco_dos,
			DATE         as fecha,
...
DEFINE _fecha,_fecha_impresion  DATE;     
...
FOREACH	WITH HOLD
 ...
 ORDER BY cod_agente
    ...
    if _no_requis is null then
		select no_requis,fecha_impresion
		  into _no_requis,_fecha_impresion
		  from chqchmae
		 where no_cheque = v_no_cheque
		   and monto     = _monto;
	else
