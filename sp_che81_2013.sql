--************************************************************
-- Procedimiento que Carga las Bonificaciones de cobranza 2013
--************************************************************
...
FOREACH
	...
	if _prima_cobrada >= 25000 or _retro = 1 then	--si es >= a 25000, se toma en cuenta al corredor para el pago.
	else
		continue foreach;
	end if
	...
	if _retro = 0 then  --Le voy a pagar retroactivo
		...
		FOREACH
			 SELECT	d.no_poliza,
			  ...
			  if _cod_ramo = '019' or _cod_ramo = '018' then	  --No va poliza de vida individual ni Salud, ni colectivas de salud
				INSERT INTO bonibita(periodo,poliza,descripcion)	VALUES (a_periodo,_no_documento,'Se excluye Pol. Fianza,Vida Ind.,Salud y Col.');
				continue foreach;
			  end if

			  if _cod_ramo = '001' and _cod_subramo = '006' then	  --No va poliza de Incendio subramo ZonaLibre
					INSERT INTO bonibita(periodo,poliza,descripcion)	VALUES (a_periodo,_no_documento,'Se excluye Pol.Inc. Zona Libre.');
					continue foreach;
			  end if
...
FOREACH
	...
	let v_monto_30bk = v_monto_30;
  	...
  	if v_monto_30 > 0 then	 --Morosidad > 30 no se debe tomar en cuenta
	    INSERT INTO bonibita(periodo,poliza,descripcion)	VALUES (a_periodo,_no_documento,'Se excluye Morosidad a mas de 30.');
		continue foreach;
	end if
