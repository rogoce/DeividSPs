...
a_dia				smallint,
a_fecha_promesa		date,
a_monto_promesa		dec(16,2) default 0,
...
--TEMPORAL RGC 26/11/2015
if a_cod_gestion = '013' then
	let _tipo_accion = 3;
elif a_cod_gestion = '078' then
	let _tipo_accion = 1;
end if
...
--	   and cod_campana	  	= _cod_campana;
	...
	foreach
		...
		call sp_cob245a('*','*',v_documento,_periodo,_fecha_hoy)
	...
	if a_hora = '00:00' or a_hora is null then
		let a_hora = '08:00';
	end if
	select cant_call
	  into _cant_call
	...
	let _cod_cobrador_ant_gestor = null;
	...
	if a_cod_gestion = '032' then
		let _cod_cobrador_ant_gestor = a_cobrador;
	end if
	update cascliente
	   set cod_cobrador     = null,
		   cod_cobrador_ant = _cod_cobrador_ant_gestor,
		   dia_cobros3		= 0,
		   nuevo			= 0,
		   procesado 		= _procesado,
		   cant_call		= _cant_call
...
elif _tipo_accion = 13 then	--No Anulación
	if a_dia is null then
		let a_dia = 0;
	end if
...
elif _tipo_accion = 11 then -- Promesa de Pago
	update cascliente
	   set fecha_ult_pro	= _fecha_hoy,
		   cod_gestion		= a_cod_gestion,
		   ultima_gestion	= a_ultima_gestion,
		   ...
		   nuevo			= 0,
		   --cod_cobrador		= null,
		   fecha_promesa	= a_fecha_promesa,
		   monto_promesa	= a_monto_promesa
	 where cod_cliente		= a_cod_cliente;
end if
end
