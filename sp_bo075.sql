-- Procedure que elimina los registros de ef_cglresumen1

-- Creado: 17/11/2010 - Autor: Demetrio Hurtado Almanza

drop procedure sp_bo075;

create procedure "informix".sp_bo075()
returning integer,
          char(100);
...
foreach
 select res1_noregistro,
   ...
   into _res_noregistro,
		_res_cia_comp
   from sac999:ef_cglresumen1
  group by res1_noregistro, res1_cia_comp

	let _cant_reg = _cant_reg + 1;
	
    delete from sac999:ef_cglresumen1
     where res1_noregistro = _res_noregistro
       and res1_cia_comp   = _res_cia_comp;
