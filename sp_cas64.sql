...
drop procedure sp_cas64;

create procedure sp_cas64(a_compania CHAR(3),a_agencia CHAR(3),a_cobrador CHAR(3),a_detalle smallint)
returning char(10),char(100),char(3),smallint,smallint,date,char(50),CHAR(3),CHAR(3),CHAR(3),smallint,CHAR(100);
...
if a_detalle = 1 then	--detalle de con reg. en el rutero
...
elif a_detalle = 2 then	 --detalle de con saldo <= 0
foreach
	select cod_cliente,
	       nuevo
	  into _cod_cliente,
...
elif a_detalle = 3 then	 --Reg. nuevos para procesar hoy
foreach
	select cod_cliente,
	       nuevo
	  into _cod_cliente,
...
elif a_detalle = 4 then	 --Atrazados con gestion para hoy
foreach
	select cod_cliente,
	       nuevo
	  into _cod_cliente,
...
elif a_detalle = 5 then	 --Atrazados con gestion para otro dia
foreach
	select cod_cliente,
	       nuevo
	  into _cod_cliente,
   ...
   if _prioridad = 0 then
	if _cod_gestion_cascliente = "002" or 	--llamar en nva fecha
	   _cod_gestion_cascliente = "006" or	--pagara en oficina
	   _cod_gestion_cascliente = "005" or	--se le dejo mensaje a
	   _cod_gestion_cascliente = "013" or	--Otros
	   _cod_gestion_cascliente = "008" then	--pagara en banco
	   let _dia_actual = day(_fecha_actual);
...
elif a_detalle = 6 then	 --Atrazados sin gestion
foreach
	select cod_cliente,
	       nuevo
	  into _cod_cliente,
