...
drop procedure sp_pro848;

create procedure "informix".sp_pro848(a_cod_manzana char(255), a_poliza char(20) DEFAULT '*', a_estatus_poliza smallint)
   ...
   if a_cod_manzana	<> '' and a_cod_manzana <> "*" Then
	if a_estatus_poliza <> 4 then
	   FOREACH WITH HOLD
			...
			FOREACH
			   SELECT cod_contrato,cod_cober_reas,porc_partic_suma,sum(suma_asegurada)
			     INTO v_cod_contrato,_cod_cober_reas,_porc_partic_suma,_suma
			     FROM emifacon
			    WHERE no_poliza = _no_poliza
			    --  AND no_endoso = '00000'
				  AND no_unidad = _no_unidad
				  AND cod_cober_reas = "001"    -- INCENDIO
			      AND prima <> 0
			   group by cod_contrato, cod_cober_reas,porc_partic_suma
