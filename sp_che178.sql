...
drop procedure sp_che178;		

create procedure "informix".sp_che178(a_compania char(3), a_sucursal char(3), a_fecha_desde date, a_fecha_hasta date, a_cod_agente char(255))
...
define v_fecha				date;
define v_comision_simulada	dec(16,2);
...
foreach
	...
	let	_no_recibo			= 0.00;
	let _cnt_existe			= 0;
	let v_comision_simulada = v_comision;
	
	{if v_no_documento = '0215-00302-01' then
		set debug file to "sp_che137.trc";
		trace on;
	end if}
		...
		if v_fecha >=  '03/01/2013' then		
			if _cnt_existe <> 0 then
				...
				if v_no_recibo = _no_recibo then
					...
					if v_no_documento = '0112-00180-01' then
						select comision
						  into v_comision
						  from chqcomis
						 where cod_agente = v_cod_agente
				...
				if v_no_documento <> '0213-00946-04' then
					if _cnt_existe > 0 or  v_no_documento in ('0215-00865-01','0403-00191-01','0207-01988-01','0214-03842-01','0212-02539-01','1808-00589-01') then
						foreach
						...
						if  v_no_documento in ('0215-00865-01','0403-00191-01','0207-01988-01','0214-03842-01','0212-02539-01','1808-00589-01') then
							if v_no_recibo = _no_recibo then
								let _comision_devengada	= v_comision;
								let v_comision			= _comision_adelanto;
							else
								let _comision_devengada	= v_comision;
								let v_comision			= 0.00;
								let _comision_saldo		= _comision_saldo;
