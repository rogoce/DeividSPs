...
-- Modificado: 22/01/2013 - Autor: Amado Perez Mendoza -- Se modifico para cuando sean pagos de tipo legal - nueva ley de seguro

DROP PROCEDURE sp_cob115;

CREATE PROCEDURE "informix".sp_cob115(
a_compania CHAR(3),
a_agencia  CHAR(3),
a_remesa   CHAR(10),
a_fecha    DATE
...
--set debug file to "sp_cob115.trc";
--trace on;
...
Foreach
 ...
 If _tipo_mov = "L" Then -- Cobranza Legal
    Select saldo Into _saldo from coboutleg Where no_documento = _documento;
 Else
...
-- Poliza	se cambia para que salga el pagador, antes estaba el cod_contratante.  05/05/2015
...
--	  Let _nombre_agente = trim(_coaseguro)||'/'||trim(_ramo);
      ...
      Let _movimiento = 'Cobranza Legal';	 
   else
      continue foreach;
   End if	   
