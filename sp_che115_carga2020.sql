--Procedimiento para realizar la carga en Rentabilidad1 para bono de RENTABILIDAD 2020
--Usando prima neta cobrada devengada
--Creado 18/03/2020	Armando Moreno M.

DROP PROCEDURE sp_che115_carga2020;
CREATE PROCEDURE sp_che115_carga2020(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
let _fec_aa_ini     = "01/01/2020";	--para sacar cancelada o anulada en el periodo
let _per_ini        = "2020-01";
let _per_ini_ap     = "2019-01";
let _ano            = a_periodo[1,4];
let _ano            = _ano - 1;
let _per_fin_ap     = _ano || a_periodo[5,7];

let _per_fin_dic    = "2019-12";         
let _fecha_aa_ini   = sp_sis36(_per_ini);    --31/01/2020	 
let _fecha_aa       = sp_sis36(a_periodo);	 --31/12/2020 
let _fecha_ap_ini   = sp_sis36(_per_ini_ap); --31/01/2019 
let _fecha_ap       = sp_sis36(_per_fin_ap); --31/12/2019
...
foreach
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2020',_fecha_aa, my_sessionid);
	call sp_dev06f_renta(_no_documento,_fecha_aa,'01/01/2020',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2019',_fecha_ap,my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2019','31/12/2019',my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
foreach
	...
	--************************************************
	--   Calculos para incremeto de PSP 2020 vs 2019
	--************************************************
		let _pri_sus_aa_tmp = 0;
		let _pri_sus_ap_tmp = 0;		
