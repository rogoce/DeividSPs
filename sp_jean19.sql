DROP procedure sp_jean19;
CREATE procedure sp_jean19(a_reporte smallint default 0)
...
define _prima_neta_an,_saldo_exi_an,_prima_neta_vi,_saldo_exi_vi,_prima_neta	     decimal(16,2);
...
if a_reporte = 0 then
	...
	foreach
		select distinct e.prima_neta * g.porc_partic_agt /100,e.cod_tipoprod,e.cod_ramo,e.no_poliza,g.cod_agente,e.estatus_poliza,e.cod_no_renov
		  into _prima_neta,_cod_tipoprod,_cod_ramo,_no_poliza,_cod_agente,_estatus_poliza,_cod_no_renov
		  from emipomae e, emipoagt g, agtagent a
		 where e.no_poliza = g.no_poliza
		   and g.cod_agente = a.cod_agente
		   and e.actualizado = 1
		   and e.vigencia_inic >= '01/01/2024'
		   and a.estatus_licencia = 'A'
		 order by e.no_poliza,g.cod_agente
		...
		if _estatus_poliza = 1 then	--polizas vigentes
			if _cod_no_renov = '039' then	--polizas vigentes y que tenga no_renovacion 039 - Vigentes Cese
				...
				if _cnt = 0 then
					...
					values(_cod_agente,1,_prima_neta,_exigible);
					continue foreach;
				else
					update deivid_tmp:cnt_polizas_agente
					   set prima_neta_ce = prima_neta_ce + _prima_neta,
						   saldo_exi_ce = saldo_exi_ce + _exigible,
			...
			if _cnt = 0 then
				...
				update deivid_tmp:cnt_polizas_agente
				   set prima_neta_vi = prima_neta_vi + _prima_neta,
					   saldo_exi_vi = saldo_exi_vi + _exigible,
		...
		if _estatus_poliza = 3 then	--polizas vencida
			if _cod_ramo = '018' And _cod_no_renov = '027' then	--Ramo 018 y que tenga no_renovacion 027 - Vencida Salud
				...
				if _cnt = 0 then
					...
					update deivid_tmp:cnt_polizas_agente
					   set prima_neta_sa = prima_neta_sa + _prima_neta,
						   saldo_exi_sa = saldo_exi_sa + _exigible,
			...
			if _cod_ramo = '019' And _cod_no_renov = '041' then	--Ramo 019 y que tenga no_renovacion 041 - Vencidas Vida
				...
				if _cnt = 0 then
					...
					update deivid_tmp:cnt_polizas_agente
					   set prima_neta_ve_vi = prima_neta_ve_vi + _prima_neta,
						   saldo_exi_ve_vi = saldo_exi_ve_vi + _exigible,
			...
			if _cnt = 0 then
				...
				update deivid_tmp:cnt_polizas_agente
				   set prima_neta_ve = prima_neta_ve + _prima_neta,
					   saldo_exi_ve = saldo_exi_ve + _exigible,
		...
		if _estatus_poliza = 2 then	--polizas Canceladas
			...
			if _cnt = 0 then
				...
				update deivid_tmp:cnt_polizas_agente
				   set prima_neta_ca = prima_neta_ca + _prima_neta,
					   saldo_exi_ca = saldo_exi_ca + _exigible,
		...
		if _estatus_poliza = 4 then	--polizas Anuladas
			...
			if _cnt = 0 then
				...
				update deivid_tmp:cnt_polizas_agente
				   set prima_neta_an = prima_neta_an + _prima_neta,
					   saldo_exi_an = saldo_exi_an + _exigible,
