...
DROP procedure sp_pro63b_b;
CREATE procedure "informix".sp_pro63b_b(a_cia CHAR(3),a_agencia CHAR(3),a_codsucursal CHAR(255) DEFAULT "*",a_periodo DATE, a_cod_ramo CHAR(255) DEFAULT "*", a_codcliente CHAR(255) DEFAULT "*" ,a_subramo CHAR(255) DEFAULT "*", a_no_documento CHAR(255) DEFAULT "*")
...
ORDER BY vigencia_final
    ...
    select cod_grupo,fecha_cancelacion
	  into _cod_grupo,_fecha_cancelacion
	  from emipomae
   ...
   select nombre
	...
	FOREACH
	 SELECT	c.cod_contrato,
	  ...
	  order by e.fecha_emision,c.cod_contrato,c.no_unidad
		  ...
		  END IF
			...
			if _cod_endomov = '005' and _fecha_emision < a_periodo then			   
			        let _cnt = 0;
			     SELECT count(*)
				   INTO _cnt
			       from tmp_contratos					
			...
			if _cod_endomov = '004' and _fecha_emision < a_periodo then
				CONTINUE FOREACH;
			end if					
	    ...
	    if _cod_endomov = '002' and _fecha_emision > a_periodo then
				CONTINUE FOREACH;
		end if
