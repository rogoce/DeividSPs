--------------------------------------------
---            POLIZAS VIGENTES            ---
---  Reporte de Rangos de Sobre Suma Retenida --
---  Modificado por Henry Giron  Solicitud: Omar Wong 31/05/2011
---  Ref. Power Builder - d_prod_sp_pro02n_dw1	 execute procedure sp_pro02n("001","*",'31/03/2011',"*","001;", "2008,2009,2010,2011;","*" )
--------------------------------------------
DROP procedure sp_pro02n;
CREATE procedure "informix".sp_pro02n(a_compania CHAR(3),a_agencia  CHAR(03) DEFAULT "*",a_periodo DATE,a_codsucursal CHAR(255) DEFAULT "*",a_codramo CHAR(255) DEFAULT "*", a_serie char(255) default "*", a_subramo CHAR(255) DEFAULT "*" )
RETURNING DEC(16,2),
		  DEC(16,2),
		  SMALLINT,
		  DEC(16,2),
...
IF a_subramo <> "*" THEN
	...
	LET _tipo = sp_sis387(a_subramo); -- Separa los valores del String

	IF _tipo <> "E" THEN -- Incluir los Registros
		UPDATE temp_det
		       SET seleccionado = 0
		     WHERE seleccionado = 1
		       AND cod_subramo NOT IN(SELECT codigo FROM tmp_codigos2);
	ELSE
		UPDATE temp_det
		       SET seleccionado = 0
		     WHERE seleccionado = 1
		       AND cod_subramo IN(SELECT codigo FROM tmp_codigos2);
		END IF
	DROP TABLE tmp_codigos;
END IF
...
--				   and b.no_documento = "0109-00401-01"
...
--	let _limite_max = _suma_unidad + _limite_2_a + _limite_1_b + _limite_2_c;
	let _limite_max = _prima_retenida + _limite_2_a + _limite_1_b + _limite_2_c;	
 	...
 	INSERT INTO temp_conteo
	  ...
	  SELECT parinfra.rango1, 
	   ...
	   WHERE parinfra.cod_ramo = v_codramo
	     AND parinfra.rango1 <= _sum_ret  -- v_suma_asegurada	   
	     AND parinfra.rango2 >= _sum_ret; -- v_suma_asegurada;
  ...
  SELECT no_poliza,
   ...
   ORDER BY cod_ramo
	...
	SELECT parinfra.rango1, 
	  ...
	  FROM parinfra
	 WHERE parinfra.cod_ramo = v_codramo
	   AND parinfra.rango1 <= _prima_retenida --v_suma_asegurada	   
	   AND parinfra.rango2 >= _prima_retenida; --v_suma_asegurada;
...
-- Procesos v_filtros
  ...
  LET v_filtros ="";
  IF a_serie <> "*" THEN
  	LET v_filtros = TRIM(v_filtros) || "Serie "||TRIM(a_serie); --_fecha_serie_ini || " - " || _fecha_serie_fin || ";";
  END IF
