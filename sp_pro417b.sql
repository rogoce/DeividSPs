...
-- Procedimiento para insertar la anulacion de los endosos de descuento apoyo COVID-19
-- Amado Perez M - 27/04/2020
-- execute procedure sp_pro417a('2020-04','',100,'033')

drop procedure sp_pro417b;
create procedure sp_pro417b(
	a_periodo			char(7),
	a_no_poliza			char(10),
	a_prima_bruta_end	dec(16,2),
	a_cod_endomov		char(3),
	a_fecha_gestion     datetime year to second) 
returning	smallint,
		    char(5),
			char(100);

define _desc_gestion		char(80);
...
--set debug file to "sp_sis417a.trc";
--trace on;
...
{select count(*)
  into v_existe_end
  from endedmae endo
 inner join polcovid cov
		 on cov.no_poliza   = endo.no_poliza
		and cod_endomov = a_cod_endomov
		and cov.periodo = a_periodo
		and cov.procesado = 1
		and endo.actualizado = 1
		and cov.no_poliza = a_no_poliza;   --endoso de descuento covid 19

if a_cod_endomov = '033' then
	let _cod_endomov_r = '034';
else
	let _cod_endomov_r = '033';
end if
...
select count(*)
 ...
 inner join polcovid cov
		 on cov.no_poliza   = endo.no_poliza
		and cod_endomov = _cod_endomov_r
		and cov.periodo = a_periodo
		and cov.procesado = 1
		and endo.actualizado = 1
		and cov.no_documento = a_no_poliza;   --reversion endoso de descuento covid 19
...
let _cod_endomov   = a_cod_endomov;
...
if a_cod_endomov = '033' then
	let _desc_gestion = 'APOYO ANCON PLUS AUTOMOVIL APLICADO. PERIODO: ' || a_periodo;
else
	let _desc_gestion = 'REVERSION APOYO ANCON PLUS AUTOMOVIL APLICADO. PERIODO: ' || a_periodo;
end if
