...
drop procedure sp_cob310b;

create procedure "informix".sp_cob310b()
returning	smallint,
			char(100);
...
foreach
	{select no_documento,
	  ...
	  into _no_documento,
		   _comision_saldo
	  from tmp_det
	 where no_documento in ('0214-00486-01','0213-02927-01')
	 group by no_documento--no_documento = _no_documento	
	 having sum(comis_pagada)- sum(comis_devengada) <> 0.00}
	...
	select no_documento,
	  ...
	  into _no_documento,
		   _comision_saldo
	  from cobadeco
	 where no_documento in ('1909-00013-01')
	{select no_poliza 
	  into a_no_poliza
	  from endedmae
	...
	let _catalago_afect = _comision_saldo;
	let _comision_saldo	= _comision_saldo * -1;
	let _descripcion	= 'Devolucion de Comision Descontada. Pago adelantado de Comisión.';
	let _no_factura = 'ContAdeco';
	...
	insert into cobredet(
		...
		values	(
				...
				_no_factura || _renglon,		--_no_recqibo_ancon,
...
return 0,'Creación exitosa de la remesa. # ' || _no_remesa;
