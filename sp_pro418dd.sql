...
drop procedure sp_pro418dd;
create procedure sp_pro418dd(a_periodo char(7)) 
returning	{smallint,
			char(100)}
			char(21)		as Poliza,
...
FOREACH
	select distinct mae.no_documento
	  into _no_documento
	  from emipoliza mae
	inner join cobredet cob
         on cob.doc_remesa = mae.no_documento
        and mae.cod_ramo in ('002')
        and mae.vigencia_fin >= '01/04/2020'
        and mae.cod_status = '1'
        and (mae.cod_subramo = '001' )
        and cob.periodo >= '2020-04'
        and cob.actualizado = 1
        and cob.tipo_mov in ('P','N')
        and mae.no_documento not in ( select distinct no_documento from endedmae where cod_endomov = '033' and actualizado = 1)-- or (mae.cod_subramo = '005' and prd.cod_producto in ('03322','03324')))
