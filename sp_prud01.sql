-- Listado de Asegurados Dra. Prudencia
-- Creado: 29/08/2025 - Autor: Federico Coronado 

drop procedure sp_prud01;
create procedure sp_prud01()
...
set isolation to dirty read;
    ...
    FOREACH
        ...
        FROM (
            ...
            INNER JOIN prdramo ram ON ram.cod_ramo = emi.cod_ramo
            INNER JOIN emidepen dep ON dep.no_poliza = uni.no_poliza
                                    AND dep.no_unidad = uni.no_unidad
                                    AND dep.activo = 1
    ...
    FOREACH
        ...
        FROM (
            ...
            SELECT replace(ase.cedula,'-','') AS v_id,
                   ...
                   ase.fecha_aniversario AS v_fecha_aniversario,
                   trim(replace(ase.cedula,'-','')) AS v_di,
                   min(emi.vigencia_inic) AS v_fecha_inicio,
                   max(emi.vigencia_final) AS v_vigencia_final,
                   ram.nombre AS v_nombre,
                   ase.e_mail AS v_e_mail,
                   trim(replace(ase.celular,'-','')) AS v_celular,
                   1 AS v_operacion,
            ...
            INNER JOIN cliclien ase ON ase.cod_cliente = uni.cod_asegurado
            INNER JOIN prdramo ram ON ram.cod_ramo = emi.cod_ramo
            WHERE emi.cod_ramo in('001', '002', '003', '004', '016', '017','019', '020', '021')
          --AND NOT ((emi.cod_ramo = '004' AND emi.cod_subramo IN ('006','007'))
           --    OR (emi.cod_ramo = '016' AND emi.cod_subramo = '007'))
		      AND NOT(emi.cod_ramo = '016' AND emi.cod_subramo = '007')
              AND emi.actualizado = 1
              AND emi.estatus_poliza = 1
              AND ase.tipo_persona = 'N'
			  AND emi.cod_tipoprod IN('001','005')
			  AND emi.vigencia_inic <= TODAY
			  AND (emi.saldo < emi.prima_bruta OR emi.saldo = 0)
			  AND emi.cod_grupo <> '1000'
            GROUP BY 1,2,3,4,5,6,7,8,11,12,13,ase.cod_cliente,emi.no_documento
        ) AS union_result_1
        ORDER BY v_fecha_aniversario
