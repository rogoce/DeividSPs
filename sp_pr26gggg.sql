...
-- 02/05/2017 Procedimiento para cargar ramosubrbase de enero a marzo 2017
--DROP PROCEDURE sp_pr26bk;
DROP PROCEDURE sp_pr26ggg;

CREATE PROCEDURE "informix".sp_pr26ggg(
		a_compania  CHAR(3),
		a_agencia   CHAR(3),
		a_periodo1  CHAR(7),
		a_periodo2  CHAR(7),
	...
	ELIF _tipo_mov = 11 THEN
	  ...
	  IF _nueva_renov = "N" THEN
		if _cod_ramo = '019' then
		   ...
		   let li_dia = day(_vigencia_inic_pol);
		   let li_mes = month(_vigencia_inic_pol);
		   let li_anio = year(_vigencia_inic_pol);

			If li_mes = 2 Then
				If li_dia > 28 Then
					let li_dia = 28;
					let _vigencia_inic_pol = mdy(li_mes, li_dia, li_anio);
				else
					let _vigencia_inic_pol = mdy(li_mes, li_dia, li_anio);
				End If
			else
			...
			LET _vig_fin_vida = _vigencia_inic_pol + 1 UNITS YEAR;
			...
			IF _vig_fin_vida > _vig_ini_end and _nueva_renov = "N" THEN
				LET _total_prima_nva = _total_prima_sus;		
				LET _cnt_prima_nva   = 1;		
            END IF		
			
			IF _vig_fin_vida <= _vig_ini_end or _nueva_renov = "R" THEN
