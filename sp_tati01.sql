   ...
   DROP procedure sp_tati01;
   CREATE procedure sp_tati01(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
   RETURNING varchar(255);
	...
	define _nombre_cte   varchar(100);
...
{foreach
	select cod_contratante,
	 ...
	 group by cod_contratante,no_poliza
	 
	select count(*)
	  into _cnt
	  from emipoagt
	 where no_poliza = _no_poliza
	   and cod_agente = '00141';
	...
	if _cnt > 0 then
		...
		select nombre
		  ...
		  from cliclien
		 where cod_cliente =  _cod_contratante;
		return 'Clientes: '|| _cod_contratante || ' ' || _nombre_cte with resume;
	end if	
end foreach
}
...
foreach
	...
	if _cliente_pep = 1 then
		return 'Asegurado: '|| _cod_asegurado || ' ' || _nombre_cte with resume;
	end if
end foreach
