...
Drop procedure sp_par357eebk; 

CREATE PROCEDURE "informix".sp_par357eebk(
a_secuencia	INTEGER,
a_url CHAR(500)   
...
Let _html_body1 = trim(_html_body1) ||'Hemos renovado su p&oacute;liza de '||v_ramo||' No. '||v_poliza||' con vigencia desde '||cast(v_vigen_ini as varchar(10))||' hasta '||cast(v_vigen_fin as varchar(10))||'.<br>'; 
 ...
 if trim(v_periodo_pago) = 'INMEDIATA' then -- SD#7084 JEPEREZ 12/07/2023 HG
    Let _html_body1 = trim(_html_body1) ||'Su prima Total a pagar es B/.'||cast(v_prima_total as varchar(10))||', en '||cast(v_n_pagos as varchar(3))||' pagos de B/. '||cast(v_prima_mes as varchar(8))||'.<br>'; 
 else
...
--Let _html_body1 = trim(_html_body1) ||'Su prima Total a pagar es B/.'||cast(v_prima_total as varchar(10))||', en '||cast(v_n_pagos as varchar(3))||' pagos de B/. '||cast(v_prima_mes as varchar(8))||' '||v_periodo_pago||'.<br>'; 
Let _html_body1 = trim(_html_body1) ||'<br><b>Descargue su p&oacute;liza <a target="_blank" href="https://app.asegurancon.com/poliza_web/print_pol.php?'||v_url||'">aqu&iacute;.</a></b><br>';
...
Let _html_body2 = trim(_html_body2) ||'Celular: '||v_telefono2||' <br>';
Let _html_body2 = trim(_html_body2) ||'Direcci&oacute;n: '||v_dir_cobro||'';                   
...
--Let _html_body2 = trim(_html_body2) ||'</p>';
Let _html_body2 = trim(_html_body2) ||'<p>';
Let _html_body2 = trim(_html_body2) ||'<br>Recuerde que debe realizar el primer pago de su renovaci&oacute;n durante los 10 primeros d&iacute;as de su vigencia.';
Let _html_body2 = trim(_html_body2) ||'<br>Si desea realizar el pago de inmediato ingrese al siguiente enlace <a target="_blank" href="https://app.asegurancon.com/pago_online/">pagos online</a> y env&iacute;e su comprobante de pago al correo cobrosancon01@asegurancon.com.';
Let _html_body2 = trim(_html_body2) ||'</p>';
Let _html_body2 = trim(_html_body2) ||'Si usted no desea la renovaci&oacute;n de su p&oacute;liza, o tiene alguna duda o consulta; puede escribirnos al correo electr&oacute;nico:<br>';
Let _html_body2 = trim(_html_body2) ||'conservacioncliente@asegurancon.com. <br>';
Let _html_body2 = trim(_html_body2) ||'<br>Puede contactarnos al (+507) 6631-1369 o visitar nuestra p&aacute;gina web www.asegurancon.com para consultar su saldo, realizar un pago, cotizar, ver estatus de reclamos y acceder a otros servicios en l&iacute;nea.<br>';
--Let _html_body2 = trim(_html_body2) ||'<p>';

--Let _html_body3 = trim(_html_body3) ||'<br>&iexcl;Valoramos tu opini&oacute;n! Cu&eacute;ntanos que tal fue tu experiencia con nuestros productos y servicios en la siguiente <a target="_blank" href="https://forms.office.com/Pages/ShareFormPage.aspx?id=iEoE-rfND0GbV6CST6EclfF01golxm1JhPH2pT-E0AFUM0xaNlNDNDFMWUVXNVFGS1ZEQTJLOThJTC4u&sharetoken=m9nz03JBaCtzxDbabNFz">encuesta</a> y<br>';  
--Let _html_body3 = trim(_html_body3) ||'ay&uacute;danos a seguir mejorando. ';
...
Let _html_body3 = trim(_html_body3) ||'</p>';
Let _html_body3 = trim(_html_body3) ||'<img style="width:100%;" src="https://app.asegurancon.com/imagenes/formaDePago.jpg">';
Let _html_body3 = trim(_html_body3) ||'</td>';
Let _html_body3 = trim(_html_body3) ||'</tr>';
--Let _html_body3 = trim(_html_body3) ||'<tr>';
--Let _html_body3 = trim(_html_body3) ||'<td colspan="2">';
