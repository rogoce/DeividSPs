...
DROP PROCEDURE sp_pro26_018;

CREATE PROCEDURE "informix".sp_pro26_018(
		a_compania  CHAR(3),
		a_agencia   CHAR(3),
		a_periodo1  CHAR(7),
		a_periodo2  CHAR(7),
...
CREATE INDEX iend0_tmp_prod ON tmp_prod(cod_subramo);
  ...
  WHERE e.periodo >= a_periodo1 
    ...
    SELECT count(*)
	 ...
	 WHERE no_poliza = _no_poliza
	   and cod_ramo = '018'      -- SALUD
	   and cod_subramo <> '012';	 -- NO SON COLECTIVO	   
	   --and cod_subramo = '012';	 -- COLECTIVO
   ...
   END FOREACH
	...
	IF _tipo_mov = 2 THEN --Cancelación

		if _cod_tipocan = "009" then --Para se Reemplazada
	...
	ELIF _tipo_mov = 8 THEN --Reversar

		LET _total_prima_rev = _total_prima_sus;		
		LET _cnt_prima_rev   = 1;		

	ELIF _tipo_mov = 11 THEN --Factura Original
