-- Cambio del proceso automatico que solo marque en estatus de cancelada y permita en Deivid seleccionar las polizas a cancelar 
-- Creado    : 27/10/2010 - Autor: Henry Giron 
-- SIS v.2.0 - DEIVID, S.A.	 execute procedure sp_cob753() 

drop procedure sp_cob753; 
create procedure sp_cob753() 
returning integer, 
          char(100);
...
foreach
	select p.no_documento,
		   f.no_factura,
		   m.nombre,
		   f.fecha_emision --,p.fecha_cancelacion
	  into _no_documento,
		   _no_factura,
		   _descripcion,
		   _fecha_proceso
	...
	if _cantidad > 0 then
		...
		if _estatus_poliza = 2 then
			let _bitacora = 'CANCELACION';
		else
			let _bitacora = 'ANULACION';
		end if
		
		let _bitacora = trim(_bitacora) || ' MOTIVOS VARIOS, FACTURA: '||trim(_no_factura)|| ' MOTIVO: ' ||trim(_descripcion)||' El DIA: ' ||_fecha_proceso|| ' REF.: ' ||trim(_no_aviso);
...
---------------
--- PROCESAR PAGOS, se quito de este procedure y se puso aparte en el sp_cob753bk
{
foreach
	select b.fecha,b.doc_remesa,sum(b.monto)
	 ...
	 where a.no_remesa = b.no_remesa
	   and b.actualizado  = 1
	   and trim(b.doc_remesa) in (select distinct trim(no_documento) from avisocanc where estatus not in ('Y'))
...
foreach	
	select no_poliza,
	  ...
	  from tmp_pol_gestion
	  	...
	  	if _fecha_ult_gestion is null then
			let _fecha_ult_gestion = _fecha_actual + 10 units day; --AMORENO correo 07/08/2020
		end if
...
foreach
	...
	--No se debe cambiar el estatus a las pólizas de salud en los avisos no procesados. 13/02/2016
...
foreach	
	...
	if _estatus_poliza <> 1 then
		...
		update emipomae  
		   set carta_vencida_sal = 1,
		       fecha_vencida_sal = today   --,carta_prima_gan = 0
	 	 where no_poliza = _no_poliza ;
	...
	if _saldo_con_mora <= 5 then	 -- Y: Desmarca por pago
...
--- PROCESAR PRONTO PAGOS	 Armando, puesto en produccion 13/08/2013	   
let _return_trace = 'E';
foreach
	select no_factura,
		   fecha_emision,
	  ...
	  from endedmae
	 where actualizado   = 1
	   and no_documento  in (select distinct trim(no_documento) from avisocanc where estatus not in ('Y','Z'))
	   and cod_endomov   = '024' --Endoso de Pronto Pago
	   and fecha_emision = _fecha_actual
...
--	       and no_poliza    = _no_poliza
		...
		if _saldo_con_mora <= 5 then
			let _descripcion = "CON ENDOSO DE PRONTO PAGO";
			let _estatus     = "Y";
		    let _cancelada   = 0;
		    let _fecha_canc  = _fecha_ult_pago;
			...
			let _fecha_gestion2 = _fecha_gestion;	
			let _bitacora = "ENDOSO DE PRONTO PAGO REALIZADO "||_fecha_ult_pago||" FACTURA: "||trim(_no_recibo)|| ", MONTO: "||_monto_rec||" Y REF.: "||trim(_no_aviso);
...
-------------- PROCESAR AVISOS HECHOS POR EL CALLCENTER	 Armando, puesto en produccion 14/08/2013
let _return_trace = 'F';
foreach
	select b.fecha,
		   b.doc_remesa,
	 ...
	 where a.no_remesa = b.no_remesa
	   and b.actualizado  = 1
	   and trim(b.doc_remesa) in (select distinct trim(no_poliza) from cobavica)
	   and b.tipo_mov     in ('P', 'N', 'X')	-- Pagos - Creditos
	   and a.date_posteo >= _fecha_actual
	 group by 1,2,3
	 order by 1,2,3
	...
	if _saldo_con_mora <= 5 then
		...
		let _fecha_gestion2 = _fecha_gestion;	
		let _bitacora = "PAGO EFECTUADO "||_fecha_ult_pago||" RECIBO: "||trim(_no_recibo)|| ", MONTO: "||_monto_rec;
