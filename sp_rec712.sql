...
create procedure sp_rec712(a_compania char(3), a_fecha1 date, a_fecha2 date, a_ramo varchar(255) DEFAULT "*", a_cliente varchar(255) DEFAULT "*")
...
DEFINE _cust_qry, _cust_qry2 CHAR(500);
define _cod_icd         char(10);
define _diagnostico     char(500);
...
--set debug file to "sp_rec712.trc";      
--trace on;

-- Procesos v_filtros
LET v_filtros = "";
LET a_cliente = a_cliente;
...
let _cust_qry = "select no_requis, cod_cliente, monto, a_nombre_de, periodo_pago, firma1, firma2, anulado, pagado, autorizado, fecha_impresion, fecha_cobrado";
let _cust_qry = trim(_cust_qry) || " from chqchmae where en_firma in (0,2) and origen_cheque = '3' and fecha_impresion >= ? and fecha_impresion <= ?"; 

if trim(a_cliente) <> "*" then
	let _cust_qry = trim(_cust_qry) || " and cod_cliente in (" || a_cliente || ")";
end if

PREPARE stmt_id FROM trim(_cust_qry);

DECLARE cust_cur cursor FOR stmt_id;

OPEN cust_cur USING a_fecha1, a_fecha2;

WHILE (1 = 1)

	FETCH cust_cur INTO _no_requis,	_cliente_ck, _monto_tran, _a_nombre_de,	_periodo_pago, _firma1,	_firma2, _anular_nt, _pagado, _autorizado, _fecha, _fecha_cobrado;
...
let _cust_qry2 = "select no_requis, cod_cliente, monto, a_nombre_de, periodo_pago, firma1, firma2, anulado, pagado, autorizado, fecha_anulado, fecha_cobrado";
let _cust_qry2 = trim(_cust_qry2) || " from chqchmae where en_firma in (0,2) and origen_cheque = '3' and fecha_anulado >= ? and fecha_anulado <= ? "; 

if trim(a_cliente) <> "*" then
	let _cust_qry2 = trim(_cust_qry2) || " and cod_cliente in (" || a_cliente || ")";
end if

PREPARE stmt_id FROM trim(_cust_qry2);
