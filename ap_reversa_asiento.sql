...
drop procedure ap_reversa_asiento;

create procedure "informix".ap_reversa_asiento()
...
foreach
	select sac_asiento
	  into _sac_notrx
	  from tmp_rever_asiento
	 where procesado = 0
	   and sac_asiento = 1426571
	order by 1
	
	call sp_sac257(_sac_notrx) returning _error, _error_desc;
	
	if _error = 0 then
		update tmp_rever_asiento
		   set procesado = 1
