...
let _top5 = 1;   -- ANEXAR COD_AGENTE con NO_LICENCIA Donde las 3 primeras letras indica “OAL”- canal alternativo. AMORENO 06/02/2019
foreach             -- COLOCADO 26/07/2019
	select e.cod_agente, sum(e.prima_suscrita),count(e.no_poliza)
	  into _cod_agente, _prima_suscrita, _cnt_pol
      from temp_agente e, agtagent a
	 where e.cod_agente   = a.cod_agente	   
	   and a.no_licencia[1,3] = 'OAL' 
      -- and e.tipo_persona = 'J'     
	 group by 1
     order by sum(prima_suscrita) desc
	 
	select count(distinct e.cod_contratante)
	  into _cnt
	  from temp_agente e, agtagent a
	 where e.cod_agente   = a.cod_agente	   
	   and a.no_licencia[1,3] = 'OAL' 
	   and e.cod_agente = _cod_agente
	   and e.tipo_per_cte = 'N';
	   
	select count(distinct e.no_poliza)
	  into _cantidad
	  from temp_agente e, agtagent a
	  where e.cod_agente   = a.cod_agente
	   and a.no_licencia[1,3] = 'OAL' 	   
	...
	select nombre,no_licencia
	 ...
	 return 5,_n_agente,_cnt,_cantidad,_cnt_j,_cantidad_j,_prima_suscrita,_no_licencia with resume;
	 let _top5 = _top5 + 1;
end foreach
