...
-- MODIFICADO: 24/12/2009 - Autor: Henry Giron
-- SIS v.2.0 - DEIVID, S.A.	 execute procedure sp_che94("001","001","2009-12","HGIRON")

--DROP PROCEDURE sp_che112;

CREATE PROCEDURE sp_che112(
a_compania          CHAR(3),
a_sucursal          CHAR(3),
...
foreach
 select no_documento,
  ...
  order by no_documento
	 ...
	 if	_cod_ramo not in ("002","020","004","018","001","003","013","011","005","006","017","009","014","016","008","080") then
		 continue foreach;
	 end if
...
-- basado en la prima neta
	...
	-- 2009 No considerar si tiene saldo a 90 dias
	if _monto_90_ap > 0 then
		continue foreach;
	end if
	...
	select nombre
	 ...
	 foreach
	  SELECT cod_agente
        ...
        --/*CATEGORIZAR*/
        --A-AUTOMOVIL                   ="002","020"
        --B-ACCIDENTES PERSONALES,SALUD ="004","018"
        --C-PATRIMONIALES               ="001","003","013","011","005","006","017","009","014" 
        --D-PERSONAS(COLECTIVOS DE VIDA, ACCIDENTES,SALUD > 1)  ="016","004","018"
        --E-FIANZAS                     ="008","080"

		{Ramos a considerar
		C001-INCENDIO y lineas aliadas
		*-lucro sesante
		C003-multiriesgo residencial y comercial
		C014-car
		C013-montaje
		*-equipo pesado
		C011-rotura de maquinaria
		C005-robo
		*-fidelidad
		C006-responsabilidad civil
		C017-casco maritimo
		BD004-accidente personales
		D016-colectivo de vida
		BD018-salud
		A002-020-automovil
		E008-080fianzas
		C009-transporte maritimo,aereo y terrestre}
		...
		if  _cod_ramo in ("018","004")  then
		   let _tipo = 'B'	;
		end if

		if  _cod_ramo in ("001","003","013","011","005","006","017","009","014")  then
		   let _tipo = 'C'	;
		end if


		if  _cod_ramo in ("016","004","018")  then
		   let _tipo = 'D'	;
		end if
		...
		-- 2009, TOMAR EN CUENTA INDIVIDUAL :
...
foreach
   ...
   foreach
		...
		if _pri_sus_pag_ap = 0  then	 -- se quito por no tener movimiento el año anterior y no representa rentabilidad
			continue foreach;
		end if
		...
		if _porcentaje <> 0 then
				...
				if  _cod_tipo = 'B' then 
				  let _nombre_tipo_g = 'ACCIDENTES PERSONALES,SALUD (INDIVIDUAL)'  ;
                end if
				if  _cod_tipo = 'C' then 
				  let _nombre_tipo_g = 'PATRIMONIALES'  ;
                end if
				if _cod_tipo = 'D' then 
				  let _nombre_tipo_g = 'PERSONAS(COLECTIVOS DE VIDA,ACCIDENTES,SALUD)'  ;
                end if
				if  _cod_tipo = 'E' then 
