...
drop procedure sp_wun01bk;		

create procedure "informix".sp_wun01bk() 
returning integer,
          char(50);
...
-- let _contador = 0;
-- SET DEBUG FILE TO "sp_wun01bk.trc";      
-- TRACE ON;     
...
foreach
 select no_documento,
  ...
  where saldo > 0
    ...
    if _estatus_poliza = 2 then	--> incluir la opcion de que no pasen las polizas canceladas 10/03/2015
	    {let _cod_tipocan = "";
		foreach
			select cod_tipocan
			  into _cod_tipocan
	...
	call sp_wun04(_cedula_contratante,_tipo_persona_contratante)returning _cedula_contratante;
	...
	if _cod_pagador <> '' AND _cod_pagador IS NOT NULL then
		...
		--grupo rey
	call sp_wun04(_cedula_pagador,_tipo_persona_pagador)returning _cedula_pagador;		
			insert into deivid_cob:gr_clientes(
			cuenta,
			cliente,
			saldocorriente,
	...
	if _cod_asegurado <> '' AND _cod_asegurado IS NOT NULL then
		...
		if _cod_contratante <> _cod_asegurado then
			...
			--grupo rey
			call sp_wun04(_cedula_asegurado,_tipo_persona_asegurado)returning _cedula_asegurado;		
			insert into deivid_cob:gr_clientes(
			cuenta,
			cliente,
			saldocorriente,
