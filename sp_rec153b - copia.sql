...
foreach
 select	a.fecha_reclamo,
  ...
  where a.no_poliza = b.no_poliza
	...
	if _cnt_dias > 46 then
		let _n_ajustador = 'SCASTILL';
		select e_mail into _e_mail from insuser where usuario = _n_ajustador;
	elif _cnt_dias >= 31 AND _cnt_dias <= 46 then
		let _n_ajustador = 'DULCE';
		select e_mail into _e_mail from insuser where usuario = _n_ajustador;
	elif _cnt_dias >= 21 AND _cnt_dias <= 30 then
	...
	if _cnt_dias > a_dias and _cnt_dias <= a_dias2 then
	else
		continue foreach;
	end if
