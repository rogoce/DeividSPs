...
ELIF  a_tipo_dato = '05' THEN -- Usar Referencias (Origen Aseguradora, Ramo, Subramo)
	...
	SELECT cod_tiporamo
	  ...
	  FROM prdramo
	 WHERE cod_ramo = _cod_ramo;
ELIF  a_tipo_dato = '06' THEN -- Usar Referencias (Origen Aseguradora, Ramo, Subramo) de acuerdo a la Cobertura de reaseguro de la unidad
	SELECT cod_tipoprod,
		   cod_contratante,
		   cod_origen
	  INTO _cod_tipoprod,
	...
	select cod_ramo
	  ...
	  from emipouni
	 where no_poliza = a_codigo_ref
	   and no_unidad = a_codigo_ref2;

	let _cod_subramo = '001'; --Pendiente de confirmación de la creación del Subramo Condominio Comercial.
