...
create procedure sp_pro493(v_poliza char(10), v_endoso char(5), v_factor dec(9,6))
...
define _cant,_valor			smallint;
define _error				smallint;
define _fecha_suscripcion,_v_ini_p_vida	date;
...
if v_poliza = '1443702' then
	return r_error, r_descripcion;
end if

if v_poliza = '2811875' then
	set debug file to "sp_pro493.trc";
	trace on;
end if
...
--drop table if exists tmp_endmoaut;
select *
  from endmoaut
 where no_poliza = v_poliza
   and no_endoso = v_endoso
...
if v_tipocalc = "004" then -- POR SALDO
	...
	let _prima_neta_cob = _prima_neta_cob * (1 + v_impuesto/100);
	...
	if _ramo_sis = 5 then
		...
		if v_prima_bruta <> 0 then
			if _prima_neta_cob <> v_prima_bruta then
				let v_prima_bruta = _prima_neta_cob;
			end if
	...
	if v_tipo_mov in ("2","24","1","25","3","19","33","34") then
		select sum(prima),
			   sum(prima_neta)
		  into v_prima_bruta,
		       _prima_neta
...
--		if v_poliza = "0001444242" and v_endoso = "00009" then
--			LET v_prima_bruta = 8156.61;
--		end if
...
foreach
	...
	{if _cod_ramo = "" And _v_ini_p_vida >= "01/07/2024" then
		let _valor = sp_proe04b_vida(v_poliza, v_unidad,v_suma_asegurada, v_endoso);
    else}
   ...
   --  Cargar Reaseguros Facultativos
	...
	if _ramo_sis = 5 then
		if v_tipocalc = "004" then
			...
			call sp_sis235(v_poliza) returning _error,_error_desc;
			
			select nvl(sum(porc_proporcion),0)
			...
			update endedcob
			   set prima_anual = _prima_salud *_porc_proporcion,
			       prima_neta  = _prima_salud *_porc_proporcion,
				   prima       = _prima_salud *_porc_proporcion
			 where no_poliza   = v_poliza
			   and no_endoso   = v_endoso
			   and no_unidad   = v_unidad
			   and prima_neta  <> 0.00;
	...
	foreach
		...
		if v_tipocalc = "005" or v_tipocalc = "006" then
			if v_tipo_mov = "24" or v_tipo_mov = "25" then
				update endedcob
				   set endedcob.prima_anual   = endedcob.prima_anual * r_signo, -- * v_factor,
					   ...
					   endedcob.recargo       = endedcob.recargo * v_factor * r_signo,	 -- -1
				...
				update endedcob
				   set endedcob.prima_anual   = endedcob.prima_anual * r_signo, -- * v_factor,
					   ...
					   endedcob.recargo       = endedcob.recargo     * v_factor * r_signo, -- -1 -- Amado 21/05/2012 se estaba haciendo mal el calculo del recargo
			...
			let _desc_cob    = v_prima * _desc_porc;
			let r_prima_neta = v_prima - _desc_cob;}
			...
			if _acepta_descuento = 1 then
			   ...
			   if _tipo_descuento = 1 and _cod_tipo_tar = '002' then	--> Descuento RC, solo polizas nuevas  
					...
					let _desc_cob    = v_prima * _desc_porc;
					let r_prima_neta = v_prima - _desc_cob;
			   elif _tipo_descuento = 2 and _cod_tipo_tar = '002' then --> Descuento Combinado Casco, solo polizas nuevas and _cod_tipo_tar = '002'
			   ...
			   if _cod_tipo_tar = '004' or _cod_tipo_tar = '005' or _cod_tipo_tar = '006' or _cod_tipo_tar = '007' or _cod_tipo_tar = '008' then -- Descuentos por modelo y siniestralidad
					--let _descuento_max = sp_sis430(v_poliza, v_endoso, v_unidad, v_cobertura); -- Busqueda del descuento forma vieja poner en comentario
					--let _desc_porc     = _descuento_max / 100;
					--let _desc_cob    = v_prima * _desc_porc;
					--let r_prima_neta = v_prima - _desc_cob;
					call sp_sis513(v_poliza, v_endoso, v_unidad, v_cobertura, v_prima) returning r_prima_neta, _desc_cob; -- Busqueda del descuento como en polizas nuevas -- Amado Perez 30-09-2022
			   end if
			   --end if
               let r_descuento = _desc_cob;
			...
			update endedcob
			   set endedcob.prima_anual   = endedcob.prima_anual * r_signo,
			       endedcob.prima         = v_prima,						--endedcob.prima         = endedcob.prima * v_factor,        
  ...
  --		let v_tot_descto = 0.00;
	...
	foreach
		...
		foreach
			...
			if _porc_comis_fac is null then
				let _porc_comis_fac = 0.00;
			end if
			...
			update emifafac
			   set emifafac.prima          = v_prima_reas,
				   ...
				   monto_comision          = v_prima_reas * _porc_comis_fac / 100,
				   monto_impuesto          = v_prima_reas * _porc_impuesto / 100
	...
	if _tot_reaseguro <> _neta Then
		...
		if r_signo < 0.00 and v_prima_suscrita > 0.00 and v_tipo_mov <> '025' and v_tipo_mov <> '034' Then
...
-- Actualizando endedmae -- Amado 04/06/2014 Caso 17426 USER: JUDISSA PC: CMEMIS23
if (v_tipocalc = "004" or v_tipocalc = "006") and v_poliza <> '58337' then -- POR SALDO o MANUAL
	...
	update endedmae
	   set prima_suscrita = v_prima_suscrita,
		   ...
		   suma_asegurada = v_suma_asegurada,
	  	   prima_neta     = r_prima_neta,  
	  	   impuesto 	  = v_impuesto,	  
...
if ((v_tipocalc = "006" or v_tipocalc = "004") and _ramo_sis <> 5) or v_poliza = '338188' Then  --
	...
	if r_prima_anual <> 0 then
		--if v_poliza <> '411237' and v_endoso <> '00019' then
		--	if r_prima_anual > 0 then
		--		let v_descto = -0.01;
		--	else
		--		let v_descto = +0.01;
	    ...
	    --	else
			...
			if _cnt_cober = 0 then
				...
				let v_descto = r_prima_anual / _cnt_cober;
				if v_descto = 0.00 then
					let v_descto = 0.01;
				end if
			end if
			
			if v_poliza <> '802216' And v_poliza <> '729057' And v_poliza <> '791631' And v_poliza <> '895800' then
				if r_prima_anual > 0 then
					let v_descto = v_descto * -1 * r_signo;
				else
					let v_descto = v_descto * 1 * r_signo;
				end if 
			else
				if r_prima_anual > 0 then
					let v_descto = v_descto * -1;
				else
					let v_descto = v_descto * 1;
				end if 
			end if
		...
		foreach
			select no_unidad,
			 ...
			 where no_poliza  = v_poliza
		   	...
		   	if abs(r_prima_anual) < abs(v_descto) then
				let v_descto = r_prima_anual * -1;
			end if
