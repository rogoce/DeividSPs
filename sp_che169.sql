...
-- execute procedure sp_che169('2017-10')

drop procedure sp_che169;
create procedure sp_che169(a_periodo char(7))
returning smallint,
          char(100);
...
define _monto_queda	        dec(16,2);
define _monto_recupera		dec(16,2); 
...
define _cantidad_canc smallint;
...
let	_monto_queda	    = 0.00;
let	_monto_recupera	    = 0.00;
let _null				= null;

set debug file to "sp_cob169.trc";
trace on;
--Busqueda de Poliza que aplican para el pago del bono y no ha sido recuperado
select count(*)	 
   into _cant
...
-- Descripción del periodo del pago del Bono para el detalle de remesa
let _descripcion	= 'Recupero de Bono Vida Individual(Nuevas) '||a_periodo;		
let _no_recibo = 'RBV'||a_periodo[1,4]||a_periodo[6,7];
...
foreach 
 select r.cod_agente,sum(r.prima_sus_nva)
   into _cod_agente,_pri_sus_canc
   from emipomae p, chqbono019 r
  where p.no_poliza = r.no_poliza 
	and r.periodo = a_periodo
	and p.estatus_poliza in (2,4)	
  ...
  group by r.cod_agente
	...
	Let _monto_bono = 0;	   
	select bono_queda,pri_sus_act
	  into _monto_queda,_pri_sus_act
	  from chqbono019e   
	...
	let _prima_sus_nva = _pri_sus_act - _pri_sus_canc;
	...
	if _monto_bono is null then  	
	     Let _monto_bono = 0.00;
	end if			   
	   
	let _monto_recupera = _monto_queda - _monto_bono;

	if _monto_recupera = 0 then
		continue foreach;
	end if
		
	let _renglon	= _renglon + 1;		
	let _monto_bono	= _monto_recupera * -1;
	...
	-- Afectacion de la catalago a la cuenta de Gastos de Honorarios Profesionales Corredores
	let _cuenta	= sp_sis15('PGCOMCO','04','001','019','001');
	...
	values	(_no_remesa,
	 ...
	 update chqbono019
	  ...
	  where no_poliza in ( select r.no_poliza   
							 from emipomae p, chqbono019 r
							where p.no_poliza = r.no_poliza 
							  and r.periodo = a_periodo
							  and p.estatus_poliza in (2,4)
	...
	if _cantidad_canc is null then
		Let _cantidad_canc = 0;
	end if	  		
	
    update chqbono019e
	   set bono_queda = bono_queda - _monto_recupera,
		   bono_recupero = bono_recupero + _monto_recupera,
		   saldo_canc = saldo_canc + _pri_sus_canc, 		    
		   cantidad_canc = cantidad_canc + _cantidad_canc, 	
		   ult_fecha_recupero = current,
           no_remesa_rec = _no_remesa
	 where periodo = a_periodo 
	   and cod_agente = _cod_agente 		
	   and aplica = 1; 		
	...
	--llamar  al sp_cob163 cuando _renglon = 0 y no llamar al sp_cob29
	if _renglon = 0 then 
		call sp_cob163(_no_remesa)  returning _error, _error_desc;
	else
