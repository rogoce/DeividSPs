...
------------------------------------------------
--execute procedure sp_pr860('001','001','2013-07','2013-09',"*","*","*","*","001,003,006,008,010,011,012,013,014,021,022;","*","2013,2012,2011,2010,2009,2008;")

drop procedure sp_pr860prueba;

create procedure sp_pr860prueba(
a_compania		char(03),
a_agencia		char(03),
a_periodo1		char(07),
a_periodo2		char(07),
				...
				select distinct cod_coasegur,porc_cont_partic,porc_comision,porc_impuesto
				 ...
				 Where serie         = _serie
				   ...
				   and borderaux     = _borderaux 
					...
					if v_cod_ramo in('002','023') then --Automovil
						 let v_clase = '013';
					end if
					...
					let _porc_inun = 0.00;
			  		...
			  		if v_clase = '002' then
						...
						if v_cobertura = '021' or v_cobertura = '022' then
							foreach
								select distinct porc_comision,porc_cont_partic,contrato_xl
								  into _porc_comision4,_porc_cont_partic,_contrato_xl
								  from reacoase
								 where cod_contrato   = v_cod_contrato
...
foreach
	select sum(t.pagado_neto),t.cod_ramo,r.serie,p.relac_inundacion
	  into _pagado_neto,_cod_ramo,_serie,_relac_inund
...
FOREACH
       SELECT r.serie,t.cod_ramo,t.cod_contrato,sum(t.pagado_neto) 
        ...
        where r.cod_contrato = t.cod_contrato 
          ...
          and t.tipo_contrato not in ('3','1') 
          and r.serie >= 2008 
          and t.cod_ramo in ("001","003","006","010","011","012","013","014","008","080","004","019","016","021","022","002","023")
