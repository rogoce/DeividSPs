-- Reporte Pool de Endoso - Logistica 
-- Creado    : 12/04/2017 - Autor: Henry Giron
DROP PROCEDURE sp_log025;
CREATE PROCEDURE "informix".sp_log025(a_desde DATE,a_hasta DATE,a_sucursal CHAR(255))
...
drop table if exists tmp_log025;

Create Temp Table tmp_log025(
...
foreach
 SELECT no_documento,
   ...
   into _no_documento,	
		...
		_cod_endomov		
   FROM endpool0
  WHERE (fecha_elimino >= a_desde ) AND
        ( fecha_elimino <= a_hasta ) --AND     
