...
DEFINE _wf_firma_esp	CHAR(20);
DEFINE _wf_firmas      SMALLINT;
define _usuario2       char(8);
define _emis_firma_aut smallint;

--if a_poliza = '2855578' then
...
if _cod_grupo = '991' then
	let _ls_autoriza = "WESPINOZ";
	RETURN '',_ls_autoriza,0,'';
end if 
...
--		LET _li_cotizacion = _ls_cotizacion;
	...
	-->**<--
	    ...
	    IF _ls_autoriza IS NULL OR _wf_firmas = 0 THEN   --si no encuentra la firma, se pone la de csa matriz
        ...
        IF _wf_firmas = 0 THEN
			LET _ls_autoriza = NULL;
		END IF
		...
		if _wf_firma_aprob <> "" then
		    select usuario,
			  ...
			  into _usuario2,
				   _emis_firma_aut
			  from insuser
			 where usuario = _wf_firma_aprob;

			if _emis_firma_aut = 1 then
				let _wf_firma_aprob = _usuario2;
			    LET _ls_autoriza = _wf_firma_aprob;
			end if
		end if
		
		--- SD 11473 LSALDANA aprobó la emisión sin embargo no tiene firma capturada en parámetros Amado 07-09-2024
        LET _wf_firmas = 0;
