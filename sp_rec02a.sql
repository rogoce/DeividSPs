drop procedure sp_rec02a;
create procedure "informix".sp_rec02a(
a_compania	char(3),
a_agencia	char(3),
a_periodo	char(7),
...
FOREACH 
 SELECT no_reclamo,		
  ...
  ORDER BY cod_ramo,numrecla
	...
	SELECT cod_reclamante,fecha_siniestro, ajust_interno, estatus_reclamo, estatus_audiencia, cod_abogado,no_tramite,fecha_reclamo,perd_total,no_unidad,ajust_externo,cod_evento
	  INTO _cod_cliente, v_fecha_siniestro, v_ajustador_int, _estatus_reclamo, _estatus_audiencia, _cod_abogado,_no_tramite,_fecha_reclamo,_es_perdida,_no_unidad,_ajust_externo,_cod_evento
	  FROM recrcmae
	...
	SELECT recajust.nombre
	  INTO v_ajustador_externo
	  FROM recajust  
	 WHERE recajust.cod_ajustador =  _ajust_externo;	 
	...
	ELIF _estatus_audiencia = 9 THEN
	   LET _estat_aud = "Pend. de Audiencia";
	ELIF _estatus_audiencia = 10 THEN
	   LET _estat_aud = "Pend. de Resolucion";
    ELIF _estatus_audiencia = 11 THEN
	   LET _estat_aud = "Ganado-Pend. Res.";
	ELIF _estatus_audiencia = 12 THEN
	   LET _estat_aud = "Perdido-Pend. Res.";   
	ELSE
	   LET _estat_aud = NULL;
	END IF
    ...
    FOREACH	
        ...
        LET v_descripcion = trim(v_descripcion) || " " || trim(_desc_transaccion);
    END FOREACH		
