...
DROP PROCEDURE sp_che94_ex;
--drop table tmp_concurso;  
CREATE PROCEDURE sp_che94_ex(
a_compania          CHAR(3),
a_sucursal          CHAR(3),
v_periodo_aa        CHAR(7),
a_usuario           CHAR(8)
...
SET DEBUG FILE TO "sp_che94_ex.trc";
...
let a_periodo       = "2008-12";
let _per_ini        = '2008-01';

let _ano            = a_periodo[1,4];
let _ano            = _ano - 1;
let _per_fin_ap     = '2007-12';
let _per_ini_ap 	= '2007-01';
let _per_fin_ap 	= '2007-12';
...
foreach
 select doc_remesa,
  ...
  where periodo     between _per_ini and a_periodo
	...
	if _cod_tipoprod = "004" then	-- sin REASEGURO ASUMIDO
		continue foreach;
	end if
...
foreach
 select no_documento,
  ...
  group by no_documento
	...
	if _prima_can > 0 then		  -- tiene prima cancelada
		continue foreach;
	end if 
...
foreach
 select no_documento,
  ...
  where cod_endomov  in ("002", "003") 
    and actualizado  = 1
	and periodo between '2007-01' and '2007-12'
  group by no_documento
...
call sp_rec01(a_compania, a_sucursal, '2008-01', '2008-12') returning _filtros;
...
foreach
 select no_documento,
  ...
  order by no_documento
	 ...
	 if _cod_grupo = "00000" then -- Excluir estado	 GRUPO DEL ESTADO
	   	INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza esta grupo del estado.',2);
		continue foreach;
	 end if
	...
	select beneficios
	 ...
	 if _beneficios = 0 then -- Excluir sucursales sin estos beneficios
	   	INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza esta en sucursales sin estos beneficios.',2);
		continue foreach;
	 end if
	 ...
	 IF _tipo_prod = 4 or _tipo_prod = 2 THEN 		-- Excluir Reaseguro Asumido y Coas. Mayoritario
	   	INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza tiene Reaseguro Asumido y Coas. Mayoritario.',2);
		CONTINUE FOREACH;
	 END IF
	 ...
	 if _cnt > 0 then		-- Excluir Facultativos
	   	 INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza esta en facultativo.',2);
		 continue foreach;
	 end if
		...
		SELECT nombre,
		 ...
		 WHERE cod_agente = _cod_agente;
			...
			if trim(_cedula_agt) = trim(_cedula_paga) then	--Contra pagador
			   	 INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza es contra pagador.',2);
				 let _flag = 1;
				exit foreach;
			end if
			
			if trim(_cedula_agt) = trim(_cedula_cont) then	--Contra Contratante
			   	 INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza es contra contratante.',2);
				 let _flag = 1;
				exit foreach;
			end if

			IF _tipo_agente <> "A" then	--solo agentes
				 INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza no es tipo agentes.',2);
				 let _flag = 1;
				exit foreach;
			END IF

			IF _estatus_licencia <> "A" then  --El corredor debe estar activo
				 INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,'La Poliza no es de corredor activo.',2);
				 let _flag = 1;
				exit foreach;
			END IF
...
--	if _pri_pag <= 0 then
	if _pri_pag = 0 then
		...
		CALL sp_par78b(a_compania, a_sucursal, _no_documento, '2008-12', _fecha_aa, _no_poliza)
	...
	select nombre
	 ...
	 foreach
	  SELECT cod_agente
        ...
        --C-PATRIMONIALES               ="012","014","017","010","001","013","003","006","015","005","011","009","007","021"
		...
		if  _cod_ramo in ("012","014","017","010","001","013","003","006","015","005","011","009","007","021")  then
		   let _tipo = 'C'	;
		end if
...
foreach
   ...
   foreach
		select tipo,
			   ...
			   sum(sini_inc) 
		  into _cod_tipo1,
			   _cod_agente1,           -- _no_documento,
			   _prima_neta,            -- _no_poliza,
			   _pri_sus_pag, 
			   _pri_sus_pag_ap, 
			   _sini_incu
		  from chqrenta
		...
		--************************************************				
		if _pri_sus_pag = 0 or _prima_neta = 0   then	 --se quito de la condicion el menor porque todos cumplen la condicion
			continue foreach;
		end if
		...
		if _cod_tipo = 'A' then	  --   automovil
			...
			let s_psp_c = _pri_sus_pag;
			let s_cre_c = _crecimiento;
			let s_sin_c = _siniestralidad;

			let _psp_c = 'PSP : '||trim(s_psp_c)||'-NO, ';
			let _cre_c = 'CREC: '||trim(s_cre_c)||'-NO, ';
			let _sin_c = 'SIN : '||trim(s_sin_c)||'-NO, ';

			if _pri_sus_pag >= 25000 then 
				let _psp_c = 'PSP 25000-SI, ';
				--let _min_prima_pag_aplica = 1; 
				if _crecimiento >= 40 then
				    let _cre_c = 'CREC 40%-SI, ';
			...
			if _porcentaje = 0 then
				let _descrip = _psp_c || _cre_c || _sin_c   ;
			end if
		...
		if _cod_tipo = 'E' then	  --  Fianzas
			...
			if _pri_sus_pag >= 50000 then 
				let _psp_c = 'PSP 50000-SI, ';
				--let _min_prima_pag_aplica = 1; 
				if _crecimiento >= 30 then
				    let _cre_c = 'CREC 30%-SI, ';
		...
		let _descrip = 'Poliza de '||trim(_nombre_tipo_g)||', '||trim(_descrip) ;
				  ...
				  let _nombre_tipo_g = 'FIANZAS'  ;
   				...
   				foreach
					...
					group by cod_ramo
						...
						--   Calculos de rentabilidad
						--************************************************				
						if _pri_sus_pag <= 0 or _prima_neta <= 0   then
							continue foreach;
						end if
						...
						let _valor_prima = _prima_neta * ( _porcentaje_g / 100);

						let _descrip = 'Poliza de '||trim(_cod_agente)||', Ramo:'||trim(_cod_ramo)||' '||trim(_nombre_tipo_g)||', '||trim(_descrip) ;
							...
							VALUES (_cod_agente,_no_poliza,_prima_neta,_valor_prima,_nombre,_no_documento,_no_licencia,0,a_periodo,current,_cod_ramo,
								...
								foreach
									...
									and cod_ramo = _cod_ramo
									 ...
									 INSERT INTO bonibita(periodo,poliza,descripcion,tipo)	VALUES (a_periodo,_no_documento,_descrip,2);
