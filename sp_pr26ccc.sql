...
--DROP PROCEDURE sp_pro26ccc;
CREATE PROCEDURE "informix".sp_pro26ccc(a_compania CHAR(3),a_agencia  CHAR(3),a_periodo1 CHAR(7),a_periodo2 CHAR(7),a_sucursal CHAR(255) DEFAULT "*",a_ramo CHAR(255) DEFAULT "*",a_grupo CHAR(255) DEFAULT "*", a_usuario  CHAR(255) DEFAULT "*", a_reaseguro CHAR(255) DEFAULT "*", a_agente CHAR(255) DEFAULT "*",a_zona CHAR(255) DEFAULT "*")
		RETURNING   CHAR(50),
		            DECIMAL(16,2),
					INTEGER,
		            DECIMAL(16,2), 
...
define _area_tecnica     varchar(15);
...
FOREACH WITH HOLD
  ...
  if _cod_ramo in('002','020','023') then
	let _area_tecnica = 'AUTOMOVIL';
  elif _cod_ramo in('004','016','019') then
	let _area_tecnica = 'PERSONAS';
  elif _cod_ramo in('008') then
	let _area_tecnica = 'FIANZAS';
  elif _cod_ramo in('018') then
	let _area_tecnica = 'SALUD';
  else
	let _area_tecnica = 'PATRIMONIALES';	
  end if
