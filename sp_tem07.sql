-- Procedimiento para anular una transaccion
-- 
-- creado: 20/12/2004 - Autor: Amado Perez.

DROP PROCEDURE sp_tem07;
CREATE PROCEDURE "informix".sp_tem07(a_no_tranrec CHAR(10)) 
...
LET _transaccion = null;
 ...
 IF _actualizado = 1 THEN
    rollback work;
 	RETURN 0, "Esta transaccion ya esta actualizada";
 END IF 
...
END
 ...
 IF _cantidad = 0 THEN
	rollback work;
    RETURN 1, "Esta transaccion no Tiene Coberturas Registradas, Verifique... ";
 END IF
 ...
 IF _tipo_transaccion = 4 AND _anular_nt <> "" THEN
	 ...
	 IF _no_tranrec_nt = "" OR _no_tranrec_nt IS NULL THEN
		rollback work;
	    RETURN 1, "La transaccion no existe, verifique...";
	 END IF

	 IF ABS(_monto) <> ABS(_monto_nt) THEN
		rollback work;
	    RETURN 1, "Los montos son diferentes,verifique...";
	 END IF
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
	        rollback work;
		 	RETURN _error, "Error al actualizar transaccion anulada";         
		END EXCEPTION 
		UPDATE rectrmae 
		   SET pagado         = 1,
 ...
 IF _monto_cob_tot <> _monto_con_tot And _tipo_transaccion = 4 THEN
 	rollback work;
	RETURN 1, "Existen Diferencias Entre las Coberturas y los Conceptos de Pagos";
 END IF
...
-- Actualizando en recrcmae cuando es cerrar reclamo
 IF _cerrar_rec = 1 THEN
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar reclamo - cerrar reclamo";         
		END EXCEPTION 
		UPDATE recrcmae 
...
IF _perd_total = 1 THEN
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar reclamo - perdida total";         
		END EXCEPTION 
		UPDATE recrcmae 
		   SET perd_total = 1
		 WHERE no_reclamo = _no_reclamo;
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar emipomae - cerrar reclamo";         
		END EXCEPTION 
		UPDATE emipomae 
		   SET perd_total = 1
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar emipouni - cerrar reclamo";         
		END EXCEPTION 
		UPDATE emipouni 
		   SET perd_total = 1
		 WHERE no_poliza  = _no_poliza 
...
-- Actualizando coberturas
 BEGIN
	...
	--		 WHERE no_tranrec = a_no_tranrec;
	        ...
	        UPDATE rectrmae
			   SET variacion = _variacion_sum
			 WHERE no_tranrec = a_no_tranrec;
	    ELIF _tipo_transaccion = 2 or _tipo_transaccion = 12 THEN
			LET _variacion_sum = 0;
 ...
 BEGIN
	 ON EXCEPTION SET _error 
		rollback work;
	 	RETURN _error, "Error al actualizar recrcmae - lo ultimo";         
	 END EXCEPTION 
...
FOREACH
     ...
     IF _no_tranrec_otro IS NOT NULL AND _no_tranrec_otro <> "" THEN
		IF _no_tranrec_otro <> a_no_tranrec THEN
			rollback work;
		 	RETURN _error, "Transaccion Duplicada";         
		END IF
	 END IF
END FOREACH
