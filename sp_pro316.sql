...
drop procedure sp_pro316;
create procedure sp_pro316(a_poliza char(10), a_periodo char(7) default '*')
returning integer;
...
-- Solicitud del 02/09/2014	Puesta en produccion 02/09/2014 correo de Juan Silva
-- habilitar Excepcion de Responsabilidad Civil
...
if _cod_grupo = '1091' then
  		let _usuario = sp_pro322(_centro_costo,'2',65);	--Traspaso de cartera Seguros Confianza, Juan Silva F9.
  		let _gerarquia = sp_pro327(_centro_costo,'2',65,_usuario);

		INSERT INTO tmp_reaut(usuario,no_poliza,renglon,tipo_ramo,gerarquia) VALUES (_usuario,a_poliza,65,'2',_gerarquia);
end if 
 ...
 --***********************
  ...
  if _cnt > 0 then
		if _cod_ramo in('002','023') then --AUTO
  ...
  if _cod_ramo in('001','003') then
	  ...
	  foreach
		  ...
		  elif _cod_manzana in('030010020000001','030010020000002') then  --se incluye 15/12/2014 Armando

		  		let _usuario = sp_pro322(_centro_costo,'2',63);				--polizas con ubicacion BarrioNorte - BarrioSur
		  		let _gerarquia = sp_pro327(_centro_costo,'2',63,_usuario);

				INSERT INTO tmp_reaut(usuario,no_poliza,renglon,tipo_ramo,gerarquia) VALUES (_usuario,a_poliza,63,'2',_gerarquia);
				exit foreach;
		  elif _cod_subramo in('005','006') then
		  		let _usuario = sp_pro322(_centro_costo,'2',1);--polizas con ubicacion zona libre - francefield
		  		let _gerarquia = sp_pro327(_centro_costo,'2',1,_usuario);
...
--Polizas con Reclamos
  ...
  if v_cantidad > 0 then
   	...
   	if _cod_ramo in('008') then	  --FIANZAS
...
{ if _cod_ramo = '020' then	--Ramo SODA
 ...
 {if _cod_ramo in('023') then	--Ramo AUTOMOVIL FLOTA
 ...
 {if _cod_ramo in('002') then	--Ramo AUTOMOVIL
	  ...
	  if _bandera > 0 then
			...
			let _usuario = sp_pro322(_centro_costo,'1',62); --cobertura de transporte AUTOMOVIL
			let _gerarquia = sp_pro327(_centro_costo,'1',62,_usuario);

			INSERT INTO tmp_reaut(usuario,no_poliza,renglon,tipo_ramo,gerarquia) VALUES (_usuario,a_poliza,62,'1',_gerarquia);
	  end if
    if _cod_subramo = '016' then 	--Póliza auto certificado
		let _usuario = sp_pro322(_centro_costo,'1',64);--Flota del ramo AUTOMOVIL
		let _gerarquia = sp_pro327(_centro_costo,'1',64,_usuario);

		INSERT INTO tmp_reaut(usuario,no_poliza,renglon,tipo_ramo,gerarquia) VALUES (_usuario,a_poliza,64,'1',_gerarquia);
	end if
 end if}
...
{		end if
 ...
 --Excepcion de Cobros	 *
 --***********************
--Si es Vida Individual no debe tener excepcion de cobros. segun sol. de Georgina 26/12/2012
--Si es grupo COLECTIVO SCOTIABANK no debe tener excepcion de cobros 18/01/2017

if _cod_ramo not in("019","020","002","023") then
 ...
 --***********************
if _cod_ramo not in("020","002","023") then
    --Poliza con Notas
	select count(*)
	  into v_cantidad
	  from eminotas
...
-- Excepcion por notas relevantes
if _cod_ramo in("002","020") then
	...
	if _mala_referencia = 1 then
		--let _usuario = sp_pro322(_centro_costo,'1',66);
		--let _gerarquia = sp_pro327(_centro_costo,'1',66,_usuario);
		--INSERT INTO tmp_reaut(usuario,no_poliza,renglon,tipo_ramo,gerarquia) VALUES (_usuario,a_poliza,66,'1',_gerarquia);
	end if
end if
