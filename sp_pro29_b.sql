...
-- Modificado: 15/08/2001 - Marquelda Valdelamar (inclusion de filtro de cliente)
--			   05/09/2001 -                       inclusion de filtro de poliza
--             29/06/2009 -  Henry Giron tomar comision e impuesto de emifacon
-- SIS v.2.0 - DEIVID, S.A.

DROP procedure sp_pro29_b;
CREATE procedure "informix".sp_pro29_b(a_compania CHAR(03),a_agencia CHAR(3),a_periodo1 CHAR(07),a_periodo2 CHAR(7),a_codramo CHAR(255) DEFAULT "*",a_codagente CHAR(255) DEFAULT "*",a_codsucursal CHAR(255) DEFAULT "*",a_codgrupo CHAR(255), a_codcoasegur CHAR(255),a_useradd CHAR(255) DEFAULT "*",a_reaseguro CHAR(255) DEFAULT "*", a_cod_cliente CHAR(255) DEFAULT "*", a_no_documento CHAR(255) DEFAULT "*", a_fronting SMALLINT DEFAULT 0 )
...
RETURNING CHAR(50),
   ...
   BEGIN
      ...
      DEFINE v_m_comision							DECIMAL(9,2);
      DEFINE v_m_impuesto              				DECIMAL(9,2);
      ...
      CREATE INDEX iend6_temp_endoso ON temp_endoso(cod_coasegur,
                                        cod_ramo,no_documento);
		  ...
		  FOREACH
             ...
             SELECT e.prima,
               ...
               INTO v_prima_suscrita,
					...
					v_m_comision,
					v_m_impuesto
               FROM emifafac e, endeduni i
              WHERE e.no_poliza = i.no_poliza
			    AND e.no_endoso = i.no_endoso
				AND e.no_unidad = i.no_unidad
                AND e.no_poliza = s_nopoliza
                AND e.no_endoso = s_noendoso
			...
			END IF;
			if (v_prima_suscrita + v_m_comision + v_m_impuesto) = 0 then
				--continue foreach;
			end if
            ...
            IF v_m_comision IS NULL THEN
              LET v_m_comision = 0;
            END IF;

            IF v_m_impuesto IS NULL THEN
              LET v_m_impuesto = 0;
            END IF;
...
--            LET v_impuesto = (v_prima_suscrita * _porc_impuesto/100);
			...
			LET v_comision = v_m_comision;
			LET v_impuesto = v_m_impuesto;
...
--Filtro de poliza
   ...
   END IF
   --- Especifico para ALLIED INSURANSE COMPAÑY , SOLICITADO AMORENO 24/07/2019  016 no va
   UPDATE temp_endoso
      SET seleccionado = 0
    WHERE seleccionado = 1 
   	  AND cod_coasegur= '034' 
      AND cod_ramo = '016';
      ...
      FOREACH
       ...
       ORDER BY x.cod_coasegur,x.cod_ramo,x.no_factura
         ...
         SELECT cliclien.nombre, cedula
           INTO v_descclte, _cedula
           FROM cliclien
