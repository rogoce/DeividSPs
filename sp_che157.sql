...
---  Amado - 21/06/2016 --
------------------------------------------------
drop procedure sp_che157;
create procedure sp_che157(a_no_requis char(10))
returning	date;
...
if a_no_requis = '1216973' then
	set debug file to "sp_che157.trc";
	trace on;
end if 
...
foreach
  SELECT autorizado,   
   ...
   WHERE no_requis = a_no_requis
	...
	if _origen_cheque not in ('3','M') then
		return null;
	end if
	...
	if _cantt = 0 or _origen_cheque not in ('3','M') then
		return null;
	end if
...
return _fecha_captura + _dias_compara + 5;
end
