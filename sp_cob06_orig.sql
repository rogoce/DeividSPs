-- Procedimiento que Genera la Morosidad Especial para una Poliza
-- 
...
--SET DEBUG FILE TO "\\NEMESIS\Ancon\Store Procedures\Debug\sp_cob01.trc";
  ...
  WHERE	no_poliza      = a_no_poliza -- Facturas de la Poliza
	...
	AND prima_bruta    > 0           -- Procesa la Facturas de Aumento de Prima
		...
		FOR _ciclo = 1 TO _no_pagos
			...
			ELIF _dias >= 0 AND _dias <= 100 THEN
		        LET v_corriente = v_corriente   + _prima_bruta;
			ELIF _dias > 100 AND _dias <= 200 THEN
			    LET v_monto_30 = v_monto_30     + _prima_bruta;
			ELIF _dias > 200 AND _dias <= 300 THEN
