...
--Armando Moreno
--09/04/2012
--execute procedure sp_rec193('001','001','2011-07','2011-07','018;')

DROP procedure sp_rec193;
CREATE PROCEDURE "informix".sp_rec193(
a_compania  CHAR(3), 
a_agencia   CHAR(3), 
a_periodo1  CHAR(7), 
a_periodo2  CHAR(7), 
...
{FOREACH 
 SELECT no_reclamo		
  ...
  where actualizado   = 1
	...
	LET v_filtros = sp_rec193a(a_compania,a_agencia,a_periodo1,'2011-07','*','*',a_ramo,'*','*','*','*','*',_no_reclamo);

END FOREACH}

--LET v_filtros = sp_rec193b(a_compania,a_agencia);

call sp_rec193c(a_compania,a_agencia,a_periodo1) returning v_filtros;

--set debug file to "sp_rec193.trc";
--trace on;
...
foreach
	 ...
	 select no_reclamo,
			pagado_jul,
			pagado_jul + pagado_ago,
			pagado_jul + pagado_ago + pagado_sep,
			pagado_jul + pagado_ago + pagado_sep + pagado_oct,
			pagado_jul + pagado_ago + pagado_sep + pagado_oct + pagado_nov,
			pagado_jul + pagado_ago + pagado_sep + pagado_oct + pagado_nov + pagado_dic
	   into	_no_reclamo,
	        _pag_jul,
			_pag_ago,
			_pag_sep,
