...
--------------------------------------------
DROP PROCEDURE sp_pro855;

CREATE PROCEDURE sp_pro855(a_compania CHAR(03), a_agencia CHAR(03),	a_periodo1 CHAR(07), a_periodo2 CHAR(07), a_codsucursal CHAR(255) DEFAULT "*", a_codgrupo CHAR(255) DEFAULT "*", a_codagente CHAR(255) DEFAULT "*",	a_codusuario CHAR(255) DEFAULT "*",	a_codramo CHAR(255) DEFAULT "*",	a_reaseguro CHAR(255) DEFAULT "*")
   ...
   BEGIN
     ...
     CREATE TEMP TABLE temp_produccion
               (cod_coasegur     CHAR(3),
				...
				por_pagar        DEC(16,2),
				desc_cob         CHAR(100),
				porc_comision    DEC(5,2),
				porc_impuesto    DEC(5,2),
				porc_cont_partic DEC(5,2), 
            PRIMARY KEY(cod_coasegur, cod_ramo, cod_contrato, cod_cobertura)) WITH NO LOG;

CREATE INDEX idx1_temp_produccion ON temp_produccion(cod_coasegur);
CREATE INDEX idx2_temp_produccion ON temp_produccion(cod_ramo);
CREATE INDEX idx3_temp_produccion ON temp_produccion(cod_contrato);
CREATE INDEX idx4_temp_produccion ON temp_produccion(cod_cobertura);
CREATE INDEX idx5_temp_produccion ON temp_produccion(desc_cob);
   ...
   set debug file to "sp_pro855.trc";
