...
CREATE PROCEDURE sp_pr77tc(a_compania CHAR(03), a_terremoto SMALLINT, a_fecha DATE, a_fecha2 DATE, a_ubica CHAR(255) DEFAULT "*") 
...
DEFINE _no_cambio, _es_terremoto,_estatus_poliza,_cnt   SMALLINT;
...
PRIMARY KEY (no_poliza,no_unidad,cod_manzana,referencia)) WITH NO LOG;
...
FOREACH
	SELECT d.no_poliza, e.no_endoso, d.no_documento, d.cod_contratante, d.cod_ramo, d.cod_subramo, d.vigencia_final,  d.fecha_cancelacion, d.estatus_poliza, e.cod_endomov
	 ...
	 WHERE e.no_poliza = d.no_poliza
	   ...
	   AND d.actualizado = 1
	   AND e.actualizado = 1
	--   AND d.no_documento = '0121-00044-01'
	...
	foreach
		...
		if _no_poliza in ('2743704','2982759','2956271') and _no_unidad = '00001' then
...
foreach
	select no_poliza,
	  ...
	  into _no_poliza,
		   ...
		   _suma_aseg_total
	  from temp_ubica
	 where no_documento in ('0123-00205-01','0121-00560-01','0124-05879-07','0123-00060-01','0123-00060-01')  --'0123-02256-01', Excluido Amado 09-02-2026

	if _no_poliza = '2975498' then
		update temp_ubica 
		   set suma_aseg_total = 4500000
		 where no_poliza = _no_poliza
		   and no_unidad = _no_unidad;
		
		if _no_unidad = '00002' then
			update temp_ubica
			   set suma_asegurada = 3000000, retencion = 3000000 * (16.665/100), primer_excedente = 3000000 * (83.335/100)
			 where no_poliza = _no_poliza
			   and no_unidad = _no_unidad;
		end if
	else
		update temp_ubica
		   set retencion = 500000, primer_excedente = suma_asegurada - 500000
		 where no_poliza = _no_poliza
		   and no_unidad = _no_unidad;
	end if
end foreach
