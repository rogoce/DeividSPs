...
drop procedure sp_aud35;		

create procedure "informix".sp_aud35() 
...
foreach 
	select no_reclamo,
	 ...
	 where numrecla[1,2] in ("02", "20")
	   ...
	   and fecha_documento <= "31-05-2013"
	   and actualizado = 1
   ...
   if _cant_uni2 > 0 then
	...
	if _no_orden is not null then
		...
		If _cod_tipopago = "001" Then  -- Proveedor
			let _nom_proveedor = _nom_cliente;
		elif _cod_tipopago = "002" Then  -- Taller
			let _nom_taller = _nom_cliente;
		elif _cod_tipopago = "004" Then  -- Tercero
			let _nom_tercero = _nom_cliente;
		end if
	end if
