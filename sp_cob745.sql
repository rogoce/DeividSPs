...
DROP PROCEDURE sp_cob745;

CREATE PROCEDURE sp_cob745(a_compania CHAR(3),a_remesa char(10)) 
...
FOREACH 
  SELECT cobredet.doc_remesa,
    ...
    FROM cobreagt,   
         cobredet  
   WHERE ( cobredet.no_remesa = cobreagt.no_remesa ) and  
         ( cobredet.renglon = cobreagt.renglon )  and  
         ( cobredet.no_remesa = a_remesa )   and
		 (cobreagt.monto_calc <>  cobreagt.monto_man )
	order by cobredet.renglon
