...
DEFINE _codaseguradora, _renglon                SMALLINT;
...
DEFINE _cod_conductor_ter						CHAR(10);
DEFINE _perd_total_ter							CHAR(2);
DEFINE _descripcion_ter							CHAR(255);
DEFINE _contador_tot, _contador, _error			INTEGER;
DEFINE _tipopago                                CHAR(2);
...
ON EXCEPTION SET _error 
    IF _error NOT IN(-239, -268) THEN 
 		RETURN _error, "Error al generar registros";     
	END IF
END EXCEPTION
...
--set debug file to "sp_sis33.trc";
--trace on;
...
FOREACH
	...
	-- Se pondra en blanco el parte por mientras 04/10/2005
	...
	-- Estatus del Reclamo
	IF _estatus_reclamo = 'A' THEN   --_estatus_reclamo = 'T' OR _estatus_reclamo = 'R' THEN	 
	...
	LET _descripcion = '';
	LET _tamano = 0;
	LET _no_reclamo = trim(_no_reclamo);
	...
	BEGIN
	  ON EXCEPTION SET _error 
	    IF _error NOT IN(-239, -268) THEN 
	 		RETURN _error, "Error al Insertar en cib_reclamos";     
		END IF
	  END EXCEPTION
		...
		VALUES (_codaseguradora,
		        trim(_numrecla),
		        _fecharegistro,
				trim(_usuarioregistro),
				_fecha_reclamo,
				trim(_user_added),
				_fecha_siniestro,
				trim(_tiposiniestro),
				_descripcion,
				trim(_parte_policivo),
				trim(_no_resolucion),
				trim(_no_denuncia),
				_no_oficio,
				_nombrecorredor,
...
-- Asegurados
   ...
   SELECT tipo_persona,
    ...
    IF _tipo_persona_cond = "G" THEN
		LET _tipo_persona_cond = "J";
	END IF
   ...
   SELECT cod_marca,
    ...
    IF _monto_tot = 0 THEN
		LET _tipopago = '';
	ELSE
		LET _tipopago = 'PD';
	END IF

	IF _estatus_reclamo IN ('D','N') THEN
		LET _estadorecl = 'DE';
	ELIF _estatus_reclamo = 'A' THEN
		LET _estadorecl = 'NP';
	ELSE
	  	LET _estadorecl = 'PA';
	END IF
	...
	IF _cod_asegurado is not null AND _cod_asegurado <> '' THEN
		BEGIN
		    ON EXCEPTION SET _error 
			    IF _error NOT IN(-239, -268) THEN 
			 		RETURN _error, "Error al Insertar en cib_reclamantes AS";     
				END IF
		    END EXCEPTION
	...
	FOREACH
		...
		END IF
		  ...
		  -- Conductor
           ...
           IF _cant > 0 THEN
			   -- Auto
			   SELECT cod_marca,	  -->Oportunidad de mejoras Amado 13/10/2011
			          cod_modelo,
					  cod_color,
					  ano_auto,
					  no_chasis,
		   ...
		   END IF
			...
			IF _cod_tercero is not null AND _cod_tercero <> '' THEN
				BEGIN
					ON EXCEPTION SET _error 
					    IF _error NOT IN(-239, -268) THEN 
					 		RETURN _error, "Error al Insertar en cib_reclamantes AF";     
						END IF
					END EXCEPTION
					INSERT INTO cib_reclamantes
					VALUES (_codaseguradora,
				...
				END
			END IF
	    LET _contador_tot = _contador_tot + 1;    
	END FOREACH	
    LET _contador_tot = _contador_tot + 1;    
END FOREACH
