DROP procedure sp_pro4971;
   CREATE procedure "informix".sp_pro4971(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
...
--trace on;
DELETE FROM esttrpxtipo WHERE periodo = a_periodo2;
...
FOREACH                   
 	SELECT cod_ramo, no_poliza, no_endoso, cod_endomov, nueva_renov, cod_subramo
      INTO _cod_ramo_ori, _no_poliza, _no_endoso, _cod_endomov, _nueva_renov, _cod_subramo
     FROM estpolenh 
     WHERE cod_ramo in ('002','020','023')
       AND periodo = a_periodo2	
       ...
       FOREACH 
		SELECT 	u.no_unidad,          		 
		  ...
		  WHERE u.no_poliza = _no_poliza
			...
			if _cnt_prima_nva = 0 and _cnt_prima_ren = 0 then
				LET _suma_asegurada = 0;
			end if
			  ...
			  ON EXCEPTION IN(-239)
				 UPDATE tmp_unidad
					SET prima_suscrita = prima_suscrita + _suscrita_unidad,
						...
						emipouni_sa = emipouni_sa + _emipouni_sa
...
{IF _cnt_prima_nva + _cnt_prima_ren > _tot_cant THEN
	UPDATE tmp_rpt
	   SET auto_co_p_cant = auto_co_p_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant),
	       sub_p_tot_cant = sub_p_tot_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant),
		   tot_cant = tot_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant)
	 WHERE orden = 1
       AND grupo = '005';
ELIF _cnt_prima_nva + _cnt_prima_ren < _tot_cant THEN	   
	UPDATE tmp_rpt
	   SET auto_co_p_cant = auto_co_p_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant),
	       sub_p_tot_cant = sub_p_tot_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant),
		   tot_cant = tot_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant)
...
END IF	}
	...
	IF _cnt_prima_nva_p + _cnt_prima_ren_p > _sub_p_tot_cant THEN
		UPDATE tmp_rpt
		   SET auto_co_p_cant = auto_co_p_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant),
			   sub_p_tot_cant = sub_p_tot_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant)
	     WHERE orden = 1
           AND grupo = '005';
		...
		UPDATE tmp_rpt
	     ...
	     WHERE orden = 1
           AND grupo = '005';
	ELIF _cnt_prima_nva_p + _cnt_prima_ren_p < _sub_p_tot_cant THEN 
		UPDATE tmp_rpt
		   SET auto_co_p_cant = auto_co_p_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant),
			   sub_p_tot_cant = sub_p_tot_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant)
	     WHERE orden = 1
		...
		UPDATE tmp_rpt
	     ...
	     WHERE orden = 1
           AND grupo = '005';
	END IF		
	IF _cnt_prima_nva_c + _cnt_prima_ren_c > _sub_c_tot_cant THEN
		UPDATE tmp_rpt
		   SET auto_co_c_cant = auto_co_c_cant + (_cnt_prima_nva_c + _cnt_prima_ren_c - _sub_c_tot_cant),
			   sub_c_tot_cant = sub_c_tot_cant + (_cnt_prima_nva_c + _cnt_prima_ren_c - _sub_c_tot_cant)
	     WHERE orden = 1
           AND grupo = '005';
		...
		UPDATE tmp_rpt
	     ...
	     WHERE orden = 1
           AND grupo = '005';
	ELIF _cnt_prima_nva_c + _cnt_prima_ren_c < _sub_c_tot_cant THEN --aqui
		UPDATE tmp_rpt
		   SET auto_co_c_cant = auto_co_c_cant + (_cnt_prima_nva_c + _cnt_prima_ren_c - _sub_c_tot_cant),
			   sub_c_tot_cant = sub_c_tot_cant + (_cnt_prima_nva_c + _cnt_prima_ren_c - _sub_c_tot_cant)
	     WHERE orden = 1
...
DROP TABLE  if exists tmp_rpt_dif;
