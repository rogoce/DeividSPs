-- Procedimiento que Genera la Remesa de Creditos de Cuentas del Grupo Shahani
-- 
-- Creado    : 24/11/2006 - Autor: Demetrio Hurtado Almanza
-- modificado: 24/11/2006 - Autor: Demetrio Hurtado Almanza
-- 
-- SIS v.2.0 - DEIVID, S.A.

DROP PROCEDURE sp_cob199;

CREATE PROCEDURE "informix".sp_cob199()
RETURNING INTEGER,
		  CHAR(50);
...
'005',
_null,
"GRUPO SHAHANI - REMESA ANULACION SALDOS",
'C',
_fecha,
0,
3,
...
FOREACH
 SELECT poliza,
   ...
   FROM deivid_tmp:shahani
	...
	-- Rebajar la provision de comision
	...
	let _prima_suscrita = _prima * _porc_partic_coas / 100;
	 ...
	 Select	porc_comis_agt,
	  ...
	  Where	no_poliza = _no_poliza
		...
		if _tipo_agente = "A" then -- Agentes Normales
			...
			Let _comis_monto = _prima_suscrita * (_porc_partic_agt / 100) * (_porc_comis_agt / 100);
			let _comis_acum  = _comis_acum + _comis_monto;
...
-- Primas por Cobrar Shahani

let _no_documento = "1805001";	
LET _renglon      = 1;
LET _tipo_mov     = 'M';
