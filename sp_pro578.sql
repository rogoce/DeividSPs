...
drop procedure sp_pro578;
create procedure sp_pro578 (a_no_poliza char(10), a_user char(8), a_prima_bruta_end dec(16,2)) 
returning	smallint,
			char(100);
...
--set debug file to "sp_pro578.trc";
--trace on ;
...
if v_existe_end = 0 then
	return 1, 'No Existe endoso a reversar para esta póliza.';
end if
...
select count(*)
  ...
  from endedmae
 where endedmae.no_poliza   = a_no_poliza
   and endedmae.cod_endomov = '025'
   and actualizado          = 1;

if v_existe_rev is null then
   let v_existe_rev = 0;
end if

if v_existe_rev > 0 then
	return 1, 'El Endoso de Pronto Pago ya fue Reversado.';
end if
