drop procedure sp_rec206;	
create procedure "informix".sp_rec206()
returning	char(3),						--1  g_no_CodAjustador 		_cod_ajustador,		
			char(3),						--2  g_no_CodSucursal		_sucursal_origen,	
			char(3),						--3  g_no_CodEvento		_cod_evento,		
			char(10),						--4  g_no_CodReclamante	_cod_reclamante,	
			char(10),						--5  g_no_NumPoliza		_no_poliza,			
			smallint,						--6  g_no_PosibleRecobro	_posible_recobro,	
			date,							--7  g_fe_Documento		_fecha_documento,	
			char(1),						--8  g_no_TieneAudiencia	_tiene_audiencia,	
			smallint,						--9  g_no_Actualizado		_actualizado,		
			char(1),						--10 g_no_StatusReclamo	_status_reclamo,	
			char(7),						--11 g_no_Periodo			_periodo,			
			char(1),						--12 g_no_PerdTotal			_perdida_total,		
			dec(16,2),						--13 g_nu_SumaAsegurada	_suma_asegurada,	
			smallint,						--14 g_no_CordBeneficios	_cord_beneficios,	
			smallint,						--15 g_no_TieneSaldo		_tiene_saldo,		
			char(1),						--16 g_no_AsisLegal		_asist_legal,		
			char(1),						--17 g_no_ConsLegal		_cons_legal,		
			datetime hour to second,		--18 g_no_HoraSiniestro		_hora_siniestro,	
			char(8),						--19 g_no_useradded		_user_added,		
			date,							--20 g_fe_Siniestro			_fecha_siniestro,	
			date,							--21 g_fe_FechaReclamo		_fecha_reclamo,		
			char(5),						--22 g_nu_Unidad			_no_unidad,			
			char(10),						--23 g_nu_Reclamo			_no_reclamo,		
			char(3),						--24 g_no_CodCompania		_cod_compania,		
			char(21),						--25 g_no_Documento		_no_documento,		
			date,							--26 u_date				_fecha_tramite,		
			char(10),						--27 g_nu_Tramite			_no_tramite,		
			datetime hour to second,		--28 g_no_HoraTramite		_hora_tramite,		
			char(30),						--29 g_no_NoMotor			_no_motor,			
			varchar(255),					--30 g_no_Narracion			_narracion,			
			char(8),						--31 g_no_UserActualiza		_user_ajustador,		
			char(10),						--32 g_no_reclamante		_bandera,			
			integer,						--33 SYS_INCIDENT			_incidente,			
			char(18),						--34 g_no_reclamoexterno	_numrecla,			
			varchar(50),					--35 g_no_EmailAjust		_email_ajust,		
			varchar(100),					--36 g_no_NomCompleto		_reclamante,		
			date,							--37 g_fe_Audiencia			_fecha_audiencia,	
			char(3),						--38 g_no_CodLugCi		_cod_lugci,			
			char(10),						--39 g_no_PartePolicivo		_parte_policivo,	
			datetime hour to second,		--40 g_no_HoraAudiencia		_hora_audiencia,	
			varchar(20),					--41 g_no_Resolucion		_no_resolucion,		
			smallint,						--42 g_no_EstatusAudiencia	_status_audiencia,
			char(10),						--43 g_no_CodConductor		_cod_conductor,		
			varchar(200),						--44 g_lst_coberturas		_cod_cobertura,		
			char(10),						--45 g_no_CodAsegurado	_cod_asegurado,		
			char(1),						--46 g_nu_FormatoUnico		_formato_unico,		
			char(10),						--47 g_no_t_tercero			_cod_tercero,		
			char(30),						--48 g_no_t_motor			_no_motor_t,		
			char(10),						--49 g_no_t_conductor		_cod_conductor_t,	
			varchar(255),					--50 g_no_t_descripcion		_descripcion_t,		
			char(3),						--51 g_no_t_marca			_cod_marca_t,		
			char(5),						--52 g_no_t_modelo			_cod_modelo,		
			char(10),						--53 g_no_t_placa			_placa,				
			varchar(50),					--54 g_nu_t_agno			_ano_auto,			
			char(30),						--55 g_no_t_chasis			_no_chasis	
			varchar(20),					--56 g_no_NomRamo
			char(3),                        --57 g_no_codRamo
			varchar(10);                    --58 g_no_corredor
...
foreach
	select no_documento,                  --1
	 ...
	 where procesado = 0
	   ...
	   --and no_documento <> '0213-00140-01'
	...
	/*a Partir del lunes 5 de mayo por orden del Sr. demetrio   Federico Coronado*/
	let _tipo = 1;
	foreach
	...
	CALL sp_rec206a(_tipo_siniestro, _cod_ramo, _cod_producto, _no_poliza, _no_unidad) RETURNING _cod_evento, _cod_cobertura, _nom_ramo;
