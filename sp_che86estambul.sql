--***************************************************************************************
-- Procedimiento que genera el Reporte para convencion a EUROPA 2022 para los corredores
--***************************************************************************************
--CONVENCION EUROPA 2022
...
-- Diciembre
let _per_fin_dic  = _per_ini[1,4] || "-12";   --2021-12
...
let _nombre_tipo = "";
--*************************************************************************************
-- Prima neta cobrada devengada periodo convencion 2021-01 Al periodo se esta evaluando
--*************************************************************************************
...
foreach
	...
	call sp_dev06f(_no_documento,_fecha_aa,'01/01/2021',_fecha_aa) returning _valor,_mensaje,_pri_dev_aa;
...
--**********************************************************************
-- Prima Pagada periodo convencion 2021-01 Al periodo se esta evaluando
--**********************************************************************
...
foreach
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/01/2021' and '31/12/2021';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/01/2021' and _fecha_anulado <= '31/12/2021' then
					let _monto_dev = 0;
				end if
			end if			
		else
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2021','31/12/2021',my_sessionid); --se cambia a tabla fisica 05/06/2018
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
--************************************************************************
call sp_bo077_2('01/01/2020', _fecha_ap, _fecha_aa) returning _error, _error_desc; --01/01/2020, 31/01/2019, 31/01/2020 tomando como periodo de ejemplo 2020-01
...
foreach
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2020','31/12/2020',my_sessionid);
	foreach
		select cod_agente
		  into _cod_agente
		  from con_corr
...
----**********************************************************************************************
call sp_bo077_2('01/01/2021', _fecha_aa, _fecha_aa) returning _error, _error_desc;	--01/10/2019, 31/01/2020, 31/01/2020 tomando como periodo de ejemplo 2020-01
