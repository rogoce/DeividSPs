drop procedure ap_pr1008a;
create procedure ap_pr1008a(
a_compania		char(3),
a_agencia		char(3),
a_periodo1		char(7),
a_periodo2		char(7),
...
foreach with hold
		...
		if _flag = 1 then
			if _facilidad_car = 1 then
				...
				if v_cod_ramo in ('002','020','023') then
					if v_cobertura in ('025','002','033','044','046') then
				...
				if _tipo_cont = 1 then		--	retencion
					if v_cod_ramo in ('002','020','023') then
						...
						if v_cobertura in ('025','002','033','044','046') then
							...
							let v_prima_1 = v_prima_1 + v_prima_tipo; 
							let _ret_varios = _ret_varios + v_prima2;
						else
	...
	--end foreach
		...
		let _prima_total = v_prima_1 + _ret_casco + v_prima_ot + _sum_fac_car + v_prima_bq + v_prima_3 + _cob_otros + _cont_cob_otros + _cont_casco + _ret_varios + _cont_varios;
		select nombre
		  into v_desc_contrato
		  from reacomae
		 where cod_contrato = v_cod_contrato;
