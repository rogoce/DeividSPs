--***************************************************************--
-- Procedimiento que categoriza al corredor para la mini convencion Punta Cana 2012
-- Prima cobraada total
--***************************************************************--

-- Creado    : 24/03/2011 - Autor: Armando Moreno M.
-- Modificado: 04/07/2012 - Autor: Armando Moreno M.

-- SIS v.2.0 - DEIVID, S.A.

DROP PROCEDURE sp_che206_verifica;

CREATE PROCEDURE sp_che206_verifica()
RETURNING char(5);
...
FOREACH
	...
	if _cod_ramo in("002","001","018","009","014","020","003") then	--SOLO AUTO, INC, SALUD, TRANSPORTE SODA Y CAR
	else
		continue foreach;
	end if

	if _cod_grupo = "00000" then --excluir estado		YA
		continue foreach;
	end if  	
	...
	if _cod_ramo = '009' and _cod_subramo in('001','002','006') then --Excluir Transporte terrestre	 YA
		continue foreach;
	end if
   ...
   let _prima = (_porc_coas_ancon * _prima) / 100;
	...
	FOREACH
	 SELECT	cod_agente,
	  ...
	  let _prima_cob_nva = 0;
	  if _nueva_renov = 'N' then  --Si la poliza era Nva tomo en cuenta esa prima
		let _prima_cob_nva = _prima_neta;
	  end if
	...
	END FOREACH
	if _bandera = 1 then
		return _cod_agente with resume;
	end if
