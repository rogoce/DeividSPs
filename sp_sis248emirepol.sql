...
drop procedure sp_sis248emirepol;

create procedure sp_sis248emirepol()
...
{	select distinct a.no_poliza,
	  ...
	  into _no_poliza,
		   _no_documento,
		   _user_added
      from emirepol a, emipomae c, hemirepo d
	 where a.no_poliza = c.no_poliza
       and a.no_poliza = d.no_poliza
	   and a.estatus = 4
       and c.renovada = 1
   --    and d.estatus_final = 2	   
	...
	if _cant_mov > 0 then	   
		...
		let _return = sp_pro326(_no_poliza, _user_added); -- actualiza o inserta emirepo
	end if
	...
	return 0,0,'',_no_poliza with resume;
		...
		select distinct a.no_poliza,
			  ...
			  into _no_poliza,
				   _no_documento,
				   _user_added
			  from emirepol a, emipomae c, emipomae d
			 where a.no_poliza = c.no_poliza
			   and c.no_documento = d.no_documento
			   and year(d.vigencia_final) = 2025
			   and d.actualizado = 1
			   and a.estatus in (1, 4)
			   and c.renovada = 1
			   and a.vigencia_final <= '31-10-2024'
			   and c.cod_ramo in ('002')
