...
RETURNING CHAR(50),CHAR(100),CHAR(5),CHAR(50),CHAR(1),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,4),DEC(16,2),char(7);
 -- v_nombre_cia,
 -- nombre_agente,
 -- cod_agente,
 -- nombre_tipo,
...
DEFINE _pri_susc_dev_ap,_comision  	DEC(16,2);
			...
			if a_periodo >= "2011-12" then
				...
				select trim(name_tipo)
			     ...
			     where periodo  = a_periodo
				  ...
				  -- let _reserva = _pri_susc_dev_aa + _pri_susc_dev_ap;
			else
...
select agt_per_fidel into _periodo2 from parparam; --Esta variable es para que en el reporte de rentabilidad en curso, salga el mes que se esta ejecutando.
...
FOREACH
	select cia,
		   ...
		   categoria,
		   cod_tipo,
		   sum(prima_suscrita),sum(monto_90),sum(prima_exc_monto90),sum(reserva),sum(prima_exc_reserva),sum(siniestro),sum(prima_ant)
	  into v_nombre_cia,
	       _nombre_agente,
		   _cod_agente,
		   _nombre_tipo,
		   _cod_tipo,
		   _prima_aa,_monto_90,_prima_exc_m90,_reserva,_prima_aplica,_sini,_prima_ant
	  from tmpche95c 
	 group by 1,2,3,4,5
	...
	if _cod_agente = '02311' and _cod_tipo = '003' then --Solicitud de GS -- Román --30/03/2023
