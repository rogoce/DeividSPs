...
create procedure sp_pro43(a_no_poliza char(10), a_no_endoso char(5)) 
...
DEFINE _vigencia_inic,_vigencia_final,_fecha_viaje,_fecha_indicador,_vig_f				DATE;
DEFINE _prima_bruta,_impuesto,_prima_neta,_descuento,_recargo,_prima,_prima_suscrita,_prima_retenida,_suma_f,_suma_e	DEC(16,2);
...
if a_no_poliza = '3018294' then set debug file to "sp_sis43.trc";trace on; end if
SELECT cod_compania,cod_sucursal,cod_endomov,periodo,vigencia_inic,vigencia_final,cod_tipocan,prima_bruta,impuesto,prima_neta,descuento,recargo,prima,prima_suscrita,
	   prima_retenida,tiene_impuesto,no_factura,user_added,no_documento
  INTO _cod_compania,_cod_sucursal,_cod_endomov,_periodo_end,_vigencia_inic,_vigencia_final,_cod_tipocan,_prima_bruta,_impuesto,_prima_neta,_descuento,_recargo,_prima,_prima_suscrita,
	   _prima_retenida,_tiene_impuesto,_no_fac_orig,_user_added,_no_doc
  FROM endedmae
...
if _cnt > 0 and _cod_endomov not in('015','024','032','033','034','031') and a_no_poliza <> '001187729' and _no_doc <> '0225-00845-01' and _no_doc <> '2322-00027-01' and _no_doc <> '0215-00284-06'
            and _no_doc <> '2322-00028-01' and _no_doc <> '2323-00004-03 ' and _no_doc <> '0213-04424-01' and _no_doc <> '2322-00017-01' and _no_doc <> '2319-00077-01' and _no_doc <> '0213-91790-47' then --015=endoso descriptivo
...
IF _tipo_mov <> 17 AND  -- Cambio de Reaseguro Individual
   _tipo_mov <> 15 THEN	-- Cambio de Coaseguro
	...
	IF abs(_prima_sus_sum - _prima_suscrita) > 1.55 THEN
	...
	END IF
END IF
SELECT SUM(prima) INTO _prima_sus_sum FROM emifacon WHERE no_poliza = a_no_poliza AND no_endoso = a_no_endoso;
IF _prima_sus_sum IS NULL THEN
	LET _prima_sus_sum = 0;
END IF
LET _prima_suscrita = _prima_suscrita * 1;
IF _tipo_mov <> 17 then
...
IF _prima_sus_sum IS NULL THEN 	LET _prima_sus_sum = 0; END IF
IF _tipo_mov <> 17 then
...
IF _tipo_mov <> 24 AND _tipo_mov <> 25 THEN
	IF ABS(_prima_neta - _prima_sus_sum) > 0.08 THEN
		LET _mensaje = 'Sumatoria de Primas de Coberturas Diferente de Prima Neta, Por Favor Verifique ...';
		RETURN 1, _mensaje;
	END IF
END IF
FOREACH
 SELECT no_unidad,cod_cober_reas,SUM(porc_partic_prima),SUM(porc_partic_suma)
   INTO _no_unidad,_cobertura,_prima_sus_cal,_prima_sus_sum
   FROM emifacon
  WHERE no_poliza     = a_no_poliza
    AND no_endoso     = a_no_endoso
...
IF _cantidad <> 0 THEN
   FOREACH
	SELECT no_unidad,cod_cober_reas,prima,orden,suma_asegurada
	  INTO _no_unidad,_cobertura,_prima_sus_cal,_orden,_suma_e
	  FROM emifacon	e, reacomae r
	 WHERE e.no_poliza     = a_no_poliza
	   AND e.no_endoso     = a_no_endoso
	   AND e.cod_contrato  = r.cod_contrato
	   ...
	   AND r.tipo_contrato = 3
		...
		IF abs(_prima_sus_cal - _prima_sus_sum) > 0.03 THEN
			...
			RETURN 1, _mensaje;
		END IF
		IF abs(_suma_e - _suma_f) > 0.03 THEN
			LET _mensaje = 'Sumatoria de Suma Aseg. de Facultativos Diferente a Suma Aseg. del Contrato Para la Unidad ' || _no_unidad;
			RETURN 1, _mensaje;
		END IF
...
IF _tipo_mov = 1 THEN -- Aumento de Vigencia
	BEGIN
		...
		DEFINE _renglon,_holgura 	   SMALLINT;
		...
		DEFINE _no_doc         CHAR(20);
		define _vig_f,_vig_fin_end          date;
		call sp_sis57(a_no_poliza, a_no_endoso); -- Informacion Necesaria para BO
		select cod_ramo,vigencia_final into _cod_r,_vig_f from emipomae where no_poliza = a_no_poliza;
		select holgura_vigencia into _holgura from prdramo where cod_ramo = _cod_r;
		let _vig_fin_end = _vig_f + _holgura units year;
		if _vigencia_final > _vig_fin_end then
			RETURN 1, "Año de la Vigencia Final es mayor a lo permitido, Verifique...";
		end if
		FOREACH 
		 SELECT	no_unidad,suma_asegurada,prima,prima_neta,descuento,recargo,impuesto,prima_bruta
		   INTO	_no_unidad,_suma_asegurada,_prima,_prima_neta,_descuento,_recargo,_impuesto,_prima_bruta
		   FROM	endeduni
		  WHERE no_poliza = a_no_poliza
		    AND no_endoso = a_no_endoso
		 UPDATE emipouni
		  ...
		  WHERE no_poliza      = a_no_poliza
			...
			FOREACH
			 SELECT	cod_cobertura,prima,prima_neta,descuento,recargo,prima_anual,limite_1,limite_2,deducible
			   INTO _cod_cobertura,_prima,_prima_neta,_descuento,_recargo,_prima_anual,_limite_1,_limite_2,_deducible
			   FROM	endedcob
			  WHERE no_poliza = a_no_poliza
			    AND no_endoso = a_no_endoso
			    AND no_unidad = _no_unidad
...
ELIF _tipo_mov = 2 THEN		-- Cancelacion
	BEGIN
		...
		SELECT accion,cod_no_renov
		  INTO _accion,_cod_no_renov
		  FROM endtican
...
ELIF _tipo_mov = 3 THEN	-- Rehabilitacion
	BEGIN
		...
		IF _estatus_p not in(2,4) THEN
			LET _mensaje = 'La poliza no necesita ser rehabilitada, verifique!';
			RETURN 1, _mensaje;
		END IF 
		call sp_cas068(a_no_poliza) returning _error, _error_desc;
		if _error <> 0 then return _error,_error_desc; end if
		...
		ELSE
			LET _accion = 1;
		END IF
		UPDATE emipomae SET estatus_poliza = _accion, fecha_cancelacion = NULL, no_renovar = 0, cod_no_renov = NULL,fecha_no_renov = NULL,carta_aviso_canc = 0,fecha_aviso_canc = null WHERE no_poliza = a_no_poliza;
		SELECT tipo_forma
		  INTO _tipo_forma
		  FROM cobforpa
		 WHERE cod_formapag = _cod_formapag;
		if _accion = 1 and (_tipo_forma = 5 or _tipo_forma = 3) then LET _return	= sp_cas022(a_no_poliza); end if
		if _ramo_sis <> 5 then let _valor = sp_pro28c(_periodo_pro,_no_documento); end if
		let _valor = sp_act_cumulo(a_no_poliza,a_no_endoso);
	END
ELIF _tipo_mov = 4 THEN	-- Inclusion de Unidades
	BEGIN
		...
		let _retorno = sp_sis505(a_no_poliza, a_no_endoso);
		INSERT INTO emipouni(no_poliza,no_unidad,cod_ruta,cod_producto,cod_asegurado,suma_asegurada,prima,descuento,recargo,prima_neta,impuesto,prima_bruta,reasegurada,
	    vigencia_inic,vigencia_final,beneficio_max,desc_unidad,activo,prima_asegurado,prima_total,no_activo_desde,facturado,user_no_activo,perd_total,impreso,fecha_emision,
	    prima_suscrita,prima_retenida,suma_aseg_adic,tipo_incendio,cod_manzana,cod_ramo)
		SELECT no_poliza,no_unidad,cod_ruta,cod_producto,cod_cliente,suma_asegurada,prima,descuento,recargo,prima_neta,impuesto,prima_bruta,reasegurada,vigencia_inic,
			   vigencia_final,beneficio_max,desc_unidad,1,0,0,_null,1,_null,0,1,CURRENT,prima_suscrita,prima_retenida,suma_aseg_adic,tipo_incendio,cod_manzana,cod_ramo
		  FROM endeduni
	 	 WHERE no_poliza = a_no_poliza
	 	   AND no_endoso = a_no_endoso;
		LET _retorno = 0;
		...
		END FOREACH
 		If _retorno = 1 Then
			...
			RETURN 1, _mensaje;
		End If
		INSERT INTO emiauto(no_poliza, no_unidad, cod_tipoveh, no_motor, uso_auto, ano_tarifa,opcion)
		SELECT no_poliza, no_unidad, cod_tipoveh, no_motor, uso_auto, ano_tarifa,opcion FROM endmoaut WHERE no_poliza = a_no_poliza AND no_endoso = a_no_endoso;
		...
		INSERT INTO emicobde(no_poliza,no_unidad,cod_cobertura,cod_descuen,porc_descuento)
		SELECT no_poliza,no_unidad,cod_cobertura,cod_descuen,porc_descuento
		  FROM endcobde
		 WHERE no_poliza = a_no_poliza
		   AND no_endoso = a_no_endoso;
		INSERT INTO emicobre(no_poliza,no_unidad,cod_cobertura,cod_recargo,porc_recargo)
		SELECT no_poliza,no_unidad,cod_cobertura,cod_recargo,porc_recargo
		  FROM endcobre
		 WHERE no_poliza = a_no_poliza
		   AND no_endoso = a_no_endoso;
		FOREACH
		  ...
		  GROUP BY no_unidad, cod_cober_reas
			...
			DELETE FROM emireama
			 WHERE no_poliza      = a_no_poliza
			   ...
			   AND no_cambio      = _no_cambio
			   AND cod_cober_reas = _cod_cober_reas;
			INSERT INTO emireama(no_poliza,no_unidad,no_cambio,cod_cober_reas,vigencia_inic,vigencia_final)
			VALUES(a_no_poliza,_no_unidad,_no_cambio,_cod_cober_reas,_vigencia_inic,_vigencia_final);
		END FOREACH
		INSERT INTO emireaco(no_poliza,no_unidad,no_cambio,cod_cober_reas,orden,cod_contrato,porc_partic_suma,porc_partic_prima)
		SELECT a_no_poliza,no_unidad,_no_cambio,cod_cober_reas,orden,cod_contrato,porc_partic_suma,porc_partic_prima
		FROM emifacon
		WHERE no_poliza = a_no_poliza
		  AND no_endoso = a_no_endoso;
		INSERT INTO emireafa(no_poliza,no_unidad,no_cambio,cod_cober_reas,orden,cod_contrato,cod_coasegur,porc_partic_reas,porc_comis_fac,porc_impuesto)
		SELECT a_no_poliza,no_unidad,_no_cambio,cod_cober_reas,orden,cod_contrato,cod_coasegur,porc_partic_reas,porc_comis_fac,porc_impuesto
		FROM emifafac
		WHERE no_poliza = a_no_poliza
		  AND no_endoso = a_no_endoso;
		INSERT INTO emibenef(no_poliza,no_unidad,cod_cliente,cod_parentesco,benef_desde,porc_partic_ben,nombre)
		SELECT no_poliza,no_unidad,cod_cliente,cod_parentesco,benef_desde,porc_partic_ben,nombre
		  FROM endbenef
		 WHERE no_poliza = a_no_poliza
		   AND no_endoso = a_no_endoso;
		INSERT INTO emicupol(no_poliza,no_unidad,cod_ubica,suma_incendio,suma_terremoto,prima_incendio,prima_terremoto)
		SELECT no_poliza,no_unidad,cod_ubica,suma_incendio,suma_terremoto,prima_incendio,prima_terremoto
		  FROM endcuend
		 WHERE no_poliza = a_no_poliza
		   AND no_endoso = a_no_endoso;	
		...
		UPDATE emipomae
		 ...
		 WHERE no_poliza      = a_no_poliza;
		INSERT INTO emitrans(no_poliza,no_unidad,cod_nave,consignado,tipo_embarque,clausulas,contenedor,sello,fecha_viaje,viaje_desde,viaje_hasta,sobre)
		SELECT no_poliza,no_unidad,cod_nave,consignado,tipo_embarque,clausulas,contenedor,sello,fecha_viaje,viaje_desde,viaje_hasta,sobre
		  FROM endmotra
		 WHERE no_poliza = a_no_poliza
		   AND no_endoso = a_no_endoso;
		LET _return	= sp_sis417(a_no_poliza, a_no_endoso);  -- Para marcar el campo cod_tipo_tar, renovaciones 
	END 
	...
	BEGIN
		...
		FOREACH 
		 SELECT	no_unidad INTO _no_unidad FROM endeduni WHERE no_poliza = a_no_poliza AND no_endoso = a_no_endoso
			SELECT COUNT(*)	INTO _cant_uni FROM emipouni WHERE no_poliza = a_no_poliza AND no_unidad = _no_unidad;
			IF _cant_uni = 0 OR _cant_uni IS NULL THEN
				LET _mensaje = 'La unidad ' || _no_unidad || ', ya fue eliminada verifique...';
				LET _cant_uni = -1;
				EXIT FOREACH;
			END IF
			INSERT INTO endmoaut(no_poliza,no_endoso,no_unidad,cod_tipoveh,no_motor,uso_auto,ano_tarifa)
			SELECT no_poliza,a_no_endoso,no_unidad,cod_tipoveh,no_motor,uso_auto,ano_tarifa
			  FROM emiauto
			 WHERE no_poliza = a_no_poliza
			   AND no_unidad = _no_unidad;	
			INSERT INTO endbenef(no_poliza,no_endoso,no_unidad,cod_cliente,cod_parentesco,benef_desde,porc_partic_ben,nombre)
			SELECT no_poliza,a_no_endoso,no_unidad,cod_cliente,cod_parentesco,benef_desde,porc_partic_ben,nombre
			  FROM emibenef
			 WHERE no_poliza = a_no_poliza
			   AND no_unidad = _no_unidad;
			...
			INSERT INTO endcuend(no_poliza,no_endoso,no_unidad,cod_ubica,suma_incendio,suma_terremoto,prima_incendio,prima_terremoto)
			SELECT no_poliza,a_no_endoso,no_unidad,cod_ubica,suma_incendio,suma_terremoto,prima_incendio,prima_terremoto
			  FROM emicupol
			 WHERE no_poliza = a_no_poliza
			   AND no_unidad = _no_unidad;
			END IF
		...
		UPDATE emipomae
		 ...
		 WHERE no_poliza = a_no_poliza;
END
ELIF _tipo_mov = 6 THEN	-- Modificacion de Unidades
	BEGIN
		DEFINE _no_unidad,_cod_cobertura      CHAR(5);
		...
		if _cod_ramo <> "018" then
			FOREACH 
				SELECT	no_unidad,suma_asegurada,prima, prima_neta,descuento, recargo, impuesto, prima_bruta,suma_aseg_adic
				  INTO	_no_unidad,_suma_asegurada,_prima,_prima_neta,_descuento,_recargo,_impuesto,_prima_bruta,_suma_aseg_adic
				  FROM	endeduni
				 WHERE no_poliza = a_no_poliza
				   AND no_endoso = a_no_endoso
			    UPDATE emipouni
				...
				FOREACH
					...
					SELECT COUNT(*)
					 ...
					 WHERE no_poliza     = a_no_poliza
					   ...
					   AND cod_cobertura = _cod_cobertura;
					IF r_cant = 0 THEN
					   INSERT INTO emipocob(no_poliza,no_unidad,cod_cobertura,orden,tarifa,deducible,limite_1,limite_2,prima_anual,prima,descuento,recargo,prima_neta,
					   date_added,date_changed,factor_vigencia,desc_limite1,desc_limite2)
					   SELECT no_poliza,no_unidad,cod_cobertura,orden,tarifa,deducible,limite_1,limite_2,prima_anual,prima,descuento,recargo,prima_neta,
					   date_added,date_changed,factor_vigencia,desc_limite1,desc_limite2
						 FROM endedcob
					    WHERE no_poliza     = a_no_poliza
					      AND no_endoso     = a_no_endoso
		...
		CALL sp_sis417o(a_no_poliza, a_no_endoso) returning _error, _mensaje;
		if _error <> 0 then		return _error, _mensaje;		end if	
	END 
	...
	BEGIN
		...
		LET	r_porc = 0.00;
		 ...
		 IF r_porc <> 100.00 THEN
		 	...
		 	RETURN 1, _mensaje;
		 END IF
		DELETE FROM emipoagt WHERE no_poliza = a_no_poliza;
		FOREACH
		 SELECT	cod_agente, 
				porc_partic_agt,
				porc_comis_agt, 
		...
		END FOREACH
		call sp_pro4956a(a_no_poliza);
		CALL sp_pro4965(a_no_poliza, a_no_endoso) returning _error, _mensaje; --valid endoso agente tipo O
		if _error <> 0 then
			return _error, _mensaje;
		end if
	END 
ELIF _tipo_mov = 13 THEN		-- Modificacion de Asegurado
	BEGIN
		...
		SELECT cod_cliente,
		 ...
		 WHERE no_poliza = a_no_poliza
		   AND no_endoso = a_no_endoso;
		if _cod_tipoprod <> '002' then -- Ajustar el bloqueo de la emisiÃ³n de PÃ³lizas por Perfil de Clientes # 7090 -- Amado 12/07/2023
			--if _tipo = 1 or _tipo = 3 then -- Se comenta por caso enviado # 14293 Tatiana Sanchez 
				call sp_par380(_cod_cliente, CURRENT,'*') returning _error, _cedula, _nombre, _ponderacion; -- VerificaciÃ³n del perfil del contratante # 6906 -- Amado 23/06/2023
				if _error = 0 then
					return 1, "Para continuar con esta emision, debe perfilar el cliente " || _cedula || " " || _nombre;
				end if	
			--end if
		end if	
	...
	BEGIN
		...
		ELSE
			...
			ELSE					   -- Cambio en Coaseguro Mayoritario
				SELECT cod_tipoprod INTO _cod_tipoprod1 FROM emitipro WHERE tipo_produccion = 2;
				SELECT MAX(no_cambio) 
				  INTO _cambio
				  FROM emihcmm
				 WHERE no_poliza = a_no_poliza;
				...
				END IF
				DELETE FROM emicoama WHERE no_poliza = a_no_poliza;
				UPDATE emihcmm
				   SET vigencia_final = _vigencia_inic
				 WHERE no_poliza      = a_no_poliza
				   AND no_cambio      = _cambio;
			...
			END IF
		END IF
		UPDATE emipomae SET cod_tipoprod = _cod_tipoprod1  WHERE no_poliza = a_no_poliza;
	END
...
ELIF _tipo_mov = 26 THEN		-- Cambio de Tipo de Vehic.	 Henry-16/03/2011
	CALL sp_pro4955(a_no_poliza, a_no_endoso) returning _error, _mensaje; -- 6/4/16 Henry :Se adiconÃƒÂ³ cambio Uso Auto y Tipo Vehiculo
	if _error <> 0 then
		return _error, _mensaje;
	end if
	...
	BEGIN
		DEFINE _cod_manzana   CHAR(50);
		  ...
		  AND no_endoso = a_no_endoso;
			...
			END IF	
	END
ELIF _tipo_mov = 29 THEN		-- Cambio de producto salud.	Federico 26/03/2014
	CALL sp_end07(a_no_poliza, a_no_endoso) returning _error, _mensaje;
	if _error <> 0 then
		return _error, _mensaje;
	end if 
END IF
...
BEGIN
	...
	END IF
	SELECT COUNT(*)	INTO _cant_fact FROM endedmae WHERE no_factura = _no_factura AND actualizado = 1;
	...
	if _tipo_mov <> 15 then	-- El proceso de cambio de coaseguro crea la tabla con los valores
		select suma_asegurada into _suma_aseg from endedmae where no_poliza = a_no_poliza and no_endoso = a_no_endoso;
	...
	select count(*)	into _cantidad_uni from emipouni where no_poliza = a_no_poliza;
	if _cantidad_uni > 1 then
		update emipomae	set colectiva = "C" where no_poliza = a_no_poliza;
	end if
    UPDATE emipomae SET saldo = saldo + _prima_bruta
    WHERE no_poliza = a_no_poliza;
	IF _tipo_mov <> 2 AND _tipo_mov <> 3 then -- Cancelacion / Rehabilitacion
	   UPDATE emipomae
	    ...
	    WHERE no_poliza      = a_no_poliza;
		select cod_formapag,prima_bruta,no_pagos,no_tarjeta,no_documento,no_cuenta
		  into _cod_formapag,_prima_bruta,_no_pagos,_no_tarjeta,_no_documento,_no_cuenta from emipomae where no_poliza = a_no_poliza;
		SELECT tipo_forma
		  INTO _tipo_forma
		  FROM cobforpa
		 WHERE cod_formapag = _cod_formapag;
		if _no_documento[1,2] = "18" or _no_documento[1,2] = "19" or _prima_bruta <= 0 then
		else
			LET _no_pol_ele = sp_sis21(_no_documento);
			if _tipo_mov  not in (24,25,33,34) then
				IF _tipo_forma = 2 and _no_tarjeta is not null THEN -- Tarjetas de Credito
				    LET _monto_visa = _prima_bruta / _no_pagos;
				    UPDATE emipomae	SET monto_visa = _monto_visa WHERE no_poliza = a_no_poliza;
					if trim(_no_pol_ele) = trim(a_no_poliza) then
					    UPDATE cobtacre SET monto = _monto_visa WHERE no_tarjeta = _no_tarjeta and no_documento = _no_documento;
					end if
				END IF
				IF _tipo_forma = 4 and _no_cuenta is not null THEN -- Ach	3/05/2013 implementado Armando
				    ...
				    UPDATE emipomae	SET monto_visa = _monto_visa WHERE no_poliza  = a_no_poliza;
					if trim(_no_pol_ele) = trim(a_no_poliza) then
					    UPDATE cobcutas SET monto = _monto_visa WHERE no_cuenta = _no_cuenta and no_documento = _no_documento;
					end if
				END IF
			end if
		end if
	END IF
END
if _tipo_mov = 30 then let _error = sp_pro1017(a_no_poliza, a_no_endoso); end if
if _tipo_mov = 31 then call sp_pro4956a(a_no_poliza); end if
if _tipo_mov = 2 then call sp_cob310(a_no_poliza, a_no_endoso) returning _error, _mensaje; if _error <> 0 then 	return _error, _mensaje; end if end if
CALL sp_pro517(a_no_poliza, a_no_endoso) returning _error, _mensaje; --Nueva ley de seguro
if _error <> 0 then return _error, _mensaje; end if
CALL sp_cob314(a_no_poliza, a_no_endoso) returning _error, _mensaje;  --Ajuste de Adelanto de Comisiones 
if _error <> 0 then return _error, _mensaje; end if
CALL sp_pro541(a_no_poliza, a_no_endoso) returning _error, _mensaje;	--emiletra
if _error <> 0 then return _error, _mensaje; end if
