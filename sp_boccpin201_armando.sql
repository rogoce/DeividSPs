...
DROP PROCEDURE sp_boccpin201_armando;
CREATE PROCEDURE sp_boccpin201_armando(a_ano smallint, a_trimestre smallint)
RETURNING smallint,char(5),varchar(50),char(3),varchar(50),dec(16,2),integer,dec(16,2),smallint,smallint,dec(16,2);

DEFINE _cod_agente,_cod_agente_uni,_cod_agente2      CHAR(5);
...
foreach
	...
	--SABER SI EL CORREDOR PRINCIPAL TIENE OTROS CODIGOS.
	call sp_che168a(_cod_agente) returning _error,_cod_agente_u;
	
	SELECT COUNT(*)
	  into _cnt
	  FROM systables
	 WHERE tabname = 'tmp_codigos';
	...
	if _cnt > 0 then
		...
		if _cnt2 > 0 then	--SE RECORRE TODOS LOS CODIGOS DEL CORREDOR PRINCIPAL PARA BUSCAR LA PRIMA SUSCRITA Y COBRADA
			FOREACH
				select codigo
				  into _cod_agente_uni
				  from tmp_codigos
				...
				foreach
					select e.no_documento,
					 ...
					 where e.no_poliza         = t.no_poliza
					   ...
					   and t.cod_agente         = _cod_agente_uni
