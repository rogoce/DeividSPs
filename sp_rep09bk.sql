...
DROP PROCEDURE "informix".sp_rep09bk;

CREATE PROCEDURE "informix".sp_rep09bk(
a_compania      CHAR(50),
a_periodo       char(7),
...
let _ano = a_periodo[1,4] - 1;
let a_periodo = _ano ||"-"|| a_periodo[6,7];

 call sp_sis40b(a_periodo)returning v_fecha_ini;
 call sp_sis36(a_periodo)returning v_fecha_fin;
...
FOREACH    
  SELECT no_poliza,
   ...
   WHERE cod_ramo        = _cod_ramo 	 
	...
	-- todas las polizas que vencen el año siguiente		  
		if YEAR(_fecha_ani)  > a_periodo[1,4] then
			if month(_fecha_aniversario) >= month(_fecha_ani) then
				if month(_fecha_aniversario) = month(_fecha_ani) then
				/*	if day(_fecha_aniversario) >= day(_fecha_ani) then*/
						let _inserta = 0;
				/*	else
						let _inserta = 1;
					end if
		...
		if year(_fecha_ani) = a_periodo[1,4] + 1 then 
			if month(_fecha_aniversario) <= month(_fecha_ani) then
				if month(_fecha_aniversario) = month(_fecha_ani) then
					/*if day(_fecha_aniversario) < day(_fecha_ani) then*/
						let _inserta = 0;
				...
				if _inserta = 0 then
					IF a_periodo[1,4] + 1 - YEAR(_fecha_aniversario) = _edad_desde THEN            
						LET _edad = (a_periodo[1,4] + 1) - YEAR(_fecha_aniversario); 
						let _nombre_depen = _nombre_cliente;
	...
	-- fin de las polizas que vencen el año siguiente
	
	-- pólizas que vencen el año actual
		if YEAR(_fecha_ani) = a_periodo[1,4] then
			if month(_fecha_aniversario) <= month(_fecha_ani) then
				if month(_fecha_aniversario) = month(_fecha_ani) then
					/*if day(_fecha_aniversario) <= day(_fecha_ani) then*/
		...
		if year(_fecha_ani) > a_periodo[1,4] - 1 and year(_fecha_ani) <> a_periodo[1,4] + 1 /*and month(_fecha_aniversario) <= month(_fecha_ani)*/ then 
			if month(_fecha_aniversario) >= month(_fecha_ani) then
				if month(_fecha_aniversario) = month(_fecha_ani) then
					/*if day(_fecha_aniversario) > day(_fecha_ani) then*/
						let _inserta = 0;
	...
	--fin pólizas que vencen el año actual
	end foreach
...
-- polizas que vencen el año siguiente		  
			if YEAR(_fecha_ani)  > a_periodo[1,4] then
				if month(_fecha_aniversario_depen) >= month(_fecha_ani) then
					if month(_fecha_aniversario_depen) = month(_fecha_ani) then
					/*	if day(_fecha_aniversario_depen) >= day(_fecha_ani) then*/
							let _inserta = 0;
					/*	else
							let _inserta = 1;
						end if
			...
			if year(_fecha_ani) = a_periodo[1,4] + 1  then	
				if month(_fecha_aniversario_depen) <= month(_fecha_ani) then
					...
					if _inserta = 0 then
						IF (a_periodo[1,4] + 1) - YEAR(_fecha_aniversario_depen) = _edad_desde THEN
							let _prima_total 	= _prima_total - _prima_depen;

							LET _edad_depen = (a_periodo[1,4] + 1) - YEAR(_fecha_aniversario_depen); 
			...
			/*end if*/
-- fin de las polizas que vencen el año que siguiente 2016

-- polizas que vencen el año actual	  
			if YEAR(_fecha_ani) = a_periodo[1,4] then
				if month(_fecha_aniversario_depen) <= month(_fecha_ani) then
					if month(_fecha_aniversario_depen) = month(_fecha_ani) then
					/*	if day(_fecha_aniversario_depen) <= day(_fecha_ani) then*/
			...
			-- evaluo el año pasado	
			if year(_fecha_ani) > a_periodo[1,4] - 1 and year(_fecha_ani) <> a_periodo[1,4] + 1 /*and month(_fecha_aniversario_depen) <= month(_fecha_ani)*/ then	
				if month(_fecha_aniversario_depen) >= month(_fecha_ani) then
					if month(_fecha_aniversario_depen) = month(_fecha_ani) then
					/*	if day(_fecha_aniversario_depen) > day(_fecha_ani) then*/
							let _inserta = 0;
			...
			/*end if*/
-- fin de las polizas que vencen el año actual
		end foreach
