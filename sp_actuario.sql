 DROP procedure sp_actuario;

 CREATE procedure "informix".sp_actuario(a_serie smallint, a_vig_ini date, a_vig_fin date, a_ramo char(3))
   RETURNING integer;
...
FOREACH WITH HOLD
       ...
       SELECT no_poliza,
        ...
        WHERE cod_ramo = a_ramo
		  AND actualizado = 1
		  AND ((vigencia_inic >= a_vig_ini AND vigencia_inic   <= a_vig_fin)   
		   OR (vigencia_final >= a_vig_ini AND vigencia_final  <= a_vig_fin))	--> Verificar el cambio que hice

	 --	  AND (vigencia_inic  between a_vig_ini and a_vig_fin   
	 --	   OR  vigencia_final between a_vig_ini and a_vig_fin)	--> Verificar el cambio que hice


     {  IF v_fecha_cancel IS NOT NULL THEN
	    FOREACH
			SELECT fecha_emision INTO v_fecha_cancel FROM endedmae WHERE no_poliza = v_no_poliza AND cod_endomov = '002' ORDER BY no_poliza, no_endoso desc
			EXIT FOREACH;
		END FOREACH
	   END IF }
       ...
       FOREACH 
		  ...
		  if v_nom_marca is null then
		   ...
		   SELECT sum(d.monto)			--> Verificar
		     INTO v_prima_cobrada
		     FROM cobredet d, cobremae m
		    WHERE d.cod_compania = '001'
		      AND d.actualizado  = 1
		      AND d.periodo[1,4] = _serie
		      AND d.tipo_mov     IN ('P','N')
		      AND d.doc_remesa   = v_no_documento
