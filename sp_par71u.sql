...
drop procedure sp_par71u;
create procedure "informix".sp_par71u(a_no_tranrec CHAR(10))
returning integer,
		  char(100);									  
...
--		end if
	...
	if _variacion_bru <> 0.00 then
		...
		let _cuenta    = sp_sis15('RPRDSET','06', _no_poliza,_no_unidad);
		call sp_par73(a_no_tranrec, _cuenta, _debito, _credito, _tipo_comp, _periodo, _centro_costo, _fecha);

	end if

	-- Reserva de Siniestros Monto Recuperable(222)
	...
	if _monto2 <> 0.00 then
		...
		let _cuenta    = sp_sis15('RPRDSMR','06', _no_poliza,_no_unidad);
		call sp_par73(a_no_tranrec, _cuenta, _debito, _credito, _tipo_comp, _periodo, _centro_costo, _fecha);
	...
	if _variacion_net <> 0.00 then
		...
		let _cuenta    = sp_sis15('RGADRST','06', _no_poliza,_no_unidad);
		call sp_par73(a_no_tranrec, _cuenta, _debito, _credito, _tipo_comp, _periodo, _centro_costo, _fecha);
...
if _tipo_transaccion  = 4 and
   _monto            <> 0 then
	...
	let _cuenta    = sp_sis15('RGSP','06', _no_poliza,_no_unidad);
	call sp_par73(a_no_tranrec, _cuenta, _debito, _credito, _tipo_comp, _periodo, _centro_costo, _fecha);
