...
DROP PROCEDURE sp_pr77ic;
CREATE PROCEDURE "informix".sp_pr77ic(a_compania CHAR(03), a_terremoto SMALLINT, a_fecha DATE, a_fecha2 DATE, a_ubica CHAR(255) DEFAULT "*")
...
DEFINE _cant_ret, _cant_exe, _cant_fac, _cnt INT;
...
DEFINE _3no_unidad        CHAR(5);
DEFINE _3tipo_incendio    INT;          
DEFINE _3cod_manzana      CHAR(15);
DEFINE _3referencia       CHAR(50);
DEFINE _3nombre_barrio    CHAR(50);	
define _encontro int;
...
cod_manzana		CHAR(15),
referencia		CHAR(50),
nombre_barrio	CHAR(50)) WITH NO LOG;
...
on exception set _error,_error_isam,_error_desc	
	return '','','','',null,_error_desc,_error_isam,0.00,0.00,0,0.00,0,0.00,0,0.00,'',_no_unidad,0,_no_poliza,_no_endoso,'';
end exception 
}
...
FOREACH
	SELECT d.no_poliza, e.no_endoso, d.no_documento, d.cod_contratante, d.cod_ramo, d.cod_subramo, d.vigencia_final,  d.fecha_cancelacion
	 ...
	 WHERE d.cod_compania = a_compania
	   ...
	   AND e.actualizado = 1
	  --and d.no_documento in ('0110-00334-01','0109-00519-01')
	  
	LET _encontro = 0;

	SELECT count(*)
	  into _encontro
	  FROM endeduni LEFT OUTER JOIN emiman05 ON endeduni.cod_manzana = emiman05.cod_manzana , emipomae
	 WHERE  endeduni.no_poliza = _no_poliza
	   and  endeduni.no_endoso = _no_endoso
	   and emipomae.no_poliza = endeduni.no_poliza
	...
	if _encontro = 0 or _encontro is null then
		CONTINUE FOREACH;
	end if
...
FOREACH
		SELECT no_unidad, m.cod_barrio						 
		  INTO _no_unidad, _cod_barrio
		  FROM endeduni u, emiman05 m
		 WHERE u.cod_manzana = m.cod_manzana
				...
				SELECT cod_ubica, 
				 ...
				 WHERE no_poliza = _no_poliza
				   ...
				   and no_unidad = _no_unidad
					...
					if _cod_barrio not in ('0103','4400') then
					  let _suma = 0; 
					  let _prima = 0; 					    
					end if
					...
					BEGIN
						...
						END EXCEPTION
						 ...
						 INSERT INTO temp_unidad
							...
							referencia,
							nombre_barrio)
							SELECT DISTINCT endeduni.no_poliza,endeduni.no_endoso,
							endeduni.no_unidad,
							endeduni.tipo_incendio,
							endeduni.cod_manzana,
							emiman05.referencia AS referencia,
							(  SELECT emiman04.nombre
								FROM emiman04
								WHERE ( emiman04.cod_provincia = emiman05.cod_provincia ) AND
										( emiman04.cod_distrito = emiman05.cod_distrito ) AND
										( emiman04.cod_correg = emiman05.cod_correg ) AND
										( emiman04.cod_barrio = emiman05.cod_barrio )  ) AS nombre_barrio
						FROM endeduni LEFT OUTER JOIN emiman05 ON endeduni.cod_manzana = emiman05.cod_manzana ,   emipomae
					   WHERE  endeduni.no_poliza = _no_poliza and endeduni.no_endoso = _no_endoso and endeduni.no_unidad = _no_unidad
						 and emipomae.no_poliza = endeduni.no_poliza
						 --and emiman05.cod_barrio in ('0103','4400')
						 AND emipomae.vigencia_final >= a_fecha --OR emipomae.vigencia_final IS NULL
						 AND emipomae.fecha_suscripcion <= a_fecha
...
if _error <> 0 then 
	RETURN _error_desc,'','','',null,'',_error,0.00,0.00,0,0.00,0,0.00,0,0.00,'','',0,'','',''; 		
end if
