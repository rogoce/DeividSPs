DROP PROCEDURE sp_sis227;

CREATE PROCEDURE sp_sis227(a_requis char(10))
RETURNING smallint;
...
let _monto_c = _monto;
  ...
  if _origen_cheque = '3' and _en_firma = 2 then
	...
	if _numrecla[1,2] in ('02','20','23') then
		update cheprereq
		   set preautorizado = preautorizado - _monto,
		       saldo = saldo + _monto,
		       saldo_real = saldo_real + _monto,
	...
	if _periodo_requis < _periodo_act then
		...
		update cheprereq
		   set preautorizado = preautorizado - _monto,
		       saldo = saldo + _monto,
		       saldo_real = saldo_real + _monto_c,
			   pagado_real = pagado_real - _monto_c
		 where anio = year(today)
		   and mes = month(today)
		   and opc = 2;
	end if	
