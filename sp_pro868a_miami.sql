...
--Periodo de Clasificacion: Del 01 de febrero de 2019 al  31 de mayo del 2019
--*****************************
-- Polizas Nuevas
--*****************************
foreach
	...
	if _pagada > 0 then	--Debe tener al menos un pago
		...
		values (_no_poliza,_no_documento,'Debe estar pagada la primera letra.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
		continue foreach;
    end if
	if _cod_ramo = '018' And _cod_subramo = '012' then --Se excluye colectivo de Miami 07/06/2018
		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
		values (_no_poliza,_no_documento,'Se excluye Col. de salud.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
		continue foreach;
	end if
	...
	if _estatus_poliza = 1 then  --solo polizas vigentes
		...
		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
		values (_no_poliza,_no_documento,'Pol. No esta vigente.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
		continue foreach;
	end if
	if _cod_grupo in("00000","1000") then -- Excluir Estado
		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
		values (_no_poliza,_no_documento,'Grupo de Estado No aplica.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
		continue foreach;
	end if
	...
	if _cod_ramo = '018' then	--Para salud, debe ser la prima anualizada. si la poliza tiene cambio de producto, NO debe ser considerada como nueva.
		...
		IF _reemplaza_pol <> "" or _reemplaza_pol is not null then
			...
			if _cnt_cam > 0 then
	    		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
				values (_no_poliza,_no_documento,'Pol. Salud con cambio de plan No aplica.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
				continue foreach;
			end if
		END IF
		select meses
    ...
    if _tipo_prod in(3, 4) THEN   -- Excluir Coaseguro Minoritario y Reaseguro Asumido
		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
		values (_no_poliza,_no_documento,'Pol. Coas.Min y Reas. Asumido No aplica.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
	   CONTINUE FOREACH;
	end if
	...
	if _cnt > 0 then			--no polizas rehabilitadas
		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
		values (_no_poliza,_no_documento,'Pol. Rehabilitada No aplica.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
		continue foreach;
	end if 
...
--**********************************************************************************************
-- PRIMA SUSCRITA AÑO PASADO TODO EL AÑO 2018	sp_sis421_mini2019				****************
--**********************************************************************************************
	INSERT INTO tmp_caribe(no_documento, pri_sus_ap, cod_agente)         -- Adicione prima 2018 en tabla TMP
	SELECT no_documento, prima_suscrita, cod_agente FROM prisusapmini;
--**********************************************************************************************
foreach
	select no_documento,
	       cod_agente,
		   sum(pri_sus),
		   sum(pri_sus_ap)   -- lectura de prima 2018 y utilizo el campo prima_cobrada de tabla punta_cana que no se esta utilizando
	  into _no_documento,
	       _cod_agente,
		   _prima_suscrita,
		   _pri_pag
	...
	if _cod_ramo = '018' then
		let _prima_salud = _prima_suscrita;
	elif _cod_ramo = '018' And _cod_subramo = '019' then
		let _prima_cancer = _prima_suscrita;
	elif _cod_ramo = '002' then
		let _prima_auto = _prima_suscrita;
	elif _cod_ramo = '003' then
		let _prima_mr = _prima_suscrita;
	elif _cod_ramo = '019' then
		let _prima_vida = _prima_suscrita;
	end if
...
--Clasificacion del corredor dependiendo de su prima suscrita año pasado a los grupos
foreach
	select cod_agente,
	       sum(prima_cobrada)
	  into _cod_agente,
  ...
  group by cod_agente
	...
	if _pri_sus_ap > 500000 then
		let _nombre_tipo = "Grupo I";
	elif (_pri_sus_ap >= 250000 and _pri_sus_ap <= 500000) then
			let _nombre_tipo = "Grupo II";
	elif (_pri_sus_ap >= 150000 and _pri_sus_ap <= 250000) then
			let _nombre_tipo = "Grupo III";
	elif (_pri_sus_ap >= 100000 and _pri_sus_ap <= 150000) then
			let _nombre_tipo = "Grupo IV";
	elif _pri_sus_ap <= 100000 then
...
foreach
	...
	order by tipo_agente,sum(prima_sus_nva) desc
    
	let _grupo_agente = trim(_grupo_agente);
	
	if _grupo_agente = 'Grupo I' then
		if _pri_sus >= 50000 then
			let _nombre_tipo = "Grupo I Opc. A";
		else
			let _nombre_tipo = "No ha llegado Opc. A";
		end if	
	elif _grupo_agente = 'Grupo II' then
		if _pri_sus >= 40000 then
			let _nombre_tipo = "Grupo II Opc. A";
		else
			let _nombre_tipo = "No ha llegado Opc. A";
		end if
	elif _grupo_agente = 'Grupo III' then
		if _pri_sus >= 25000 then
			let _nombre_tipo = "Grupo III Opc. A";
		else
			let _nombre_tipo = "No ha llegado Opc. A";
		end if
	elif _grupo_agente = 'Grupo IV' then
		if _pri_sus >= 20000 then
			let _nombre_tipo = "Grupo IV Opc. A";
		else
			let _nombre_tipo = "No ha llegado Opc. A";
		end if
	elif _grupo_agente = 'Grupo V' then
...
foreach
	...
	if _grupo_agente = 'Grupo I' then
		if _pri_sus >= 15000 then
			...
			let _nombre_tipo = "No ha llegado Opc. B";
		end if
	elif _grupo_agente = 'Grupo II' then
		if _pri_sus >= 12000 then
			let _nombre_tipo = "Grupo II Opc. B";
		else
			let _nombre_tipo = "No ha llegado Opc. B";
		end if
	elif _grupo_agente = 'Grupo III' then
		if _pri_sus >= 9000 then
			let _nombre_tipo = "Grupo III Opc. B";
		else
			let _nombre_tipo = "No ha llegado Opc. B";
		end if
	elif _grupo_agente = 'Grupo IV' then
		if _pri_sus >= 7000 then
			let _nombre_tipo = "Grupo IV Opc. B";
		else
			let _nombre_tipo = "No ha llegado Opc. B";
		end if
	elif _grupo_agente = 'Grupo V' then
		if _pri_sus >= 5000 then
