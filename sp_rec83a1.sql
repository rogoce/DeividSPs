...
drop procedure sp_rec83a1;

create procedure sp_rec83a1(
a_compania		char(3),
...
define _coaseguro_hasta dec(16,2);
define _coaseguro_fuera dec(16,2);
define _stoplosslocal   dec(16,2);  
define _stoplossfuera   dec(16,2);
 ...
 select	numrecla,
  ...
  where	numrecla = a_no_reclamo
	...
	if _cod_producto is not null and trim(_cod_producto) <> "" then
		SELECT p.deducible_local,p.deducible_fuera,p.tipo_acum_deduc, p.coaseguro_hasta, p.coaseguro_fuera
		  into _ded_local,_ded_fuera,_tipo_acum_deduc, _coaseguro_hasta, _coaseguro_fuera
		  FROM prdprod p
...
--		let _fecha_hasta  = mdy(a_fecha_hasta[4,5], a_fecha_hasta[1,2], a_fecha_hasta[7,10]);
	...
	----------------------------
	select monto_deducible,monto_deducible2, monto_coaseguro, monto_coaseguro2
	  into _ded_local_monto,_ded_fuera_monto, _stoplosslocal, _stoplossfuera
	  from recacuan
	...
	foreach
	 select cod_proveedor,
	  ...
	  where no_reclamo   = _no_reclamo
		...
		if trim(_no_requis) <> '' then
			select no_cheque into _no_cheque from chqchmae where no_requis = _no_requis;
		end if
