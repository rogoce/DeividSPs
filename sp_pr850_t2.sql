--execute procedure sp_pr850_tamm('001','001','2024-08','2024-08','*','*','*','*','*','*','*')

drop procedure sp_pr850_t2;
create procedure sp_pr850_t2(
a_compania    char(03),
a_agencia     char(03),
a_periodo1    char(07),
a_periodo2    char(07),
...
define _porc_inun,v_prima_casco	dec(16,2);
...
define v_tipo_contrato_01   smallint;

set isolation to dirty read;

let _borderaux = a_tipo_bx;	    -- bouquet -- prima suscrita
let _nom_contrato = '';
select tipo 
  into _tipo2
  from reacontr
...
if _borderaux = '11' then	--auto casco prima suscrita
	let a_codramo = "002,020,023;";
elif _borderaux = '12' then	--vida prima suscrita
	let a_codramo = "019;";
else
...
foreach 
	...
	if _bouquet <> 1 then	--Si no es bouquet NO VA
		continue foreach;
	end if
	...
	if v_tipo_contrato = 1 then --Retencion NO VA
		continue foreach;
	end if
...
foreach
	select serie,cod_ramo,cod_contrato,cod_cobertura,sum(prima) 
	  ...
	  from temphg1
	 where cod_coasegur in ('173','036','042','050','063','076','089','128','117','134','136','141','146','147','149','153','034','164') --SD#6343 add 164 27/04/2023-HGIRON --agregué el 153 porque es nuevo APM 12-01-2022   ---SD#4159 CECHAVAR adicion 034-ALLIED 03/08/2022
	   and anio      = _anio_reas
	   and trimestre = _trim_reas
	...
	foreach 
		select distinct cod_coasegur,porc_cont_partic,porc_comision,porc_impuesto
		  ...
		  from temphg1
		 where serie          = _serie
		   and cod_coasegur   in ('173','050','063','076','042','036','089','128','117','134','136','141','146','147','149','153','034','164') --SD#6343 add 164 27/04/2023-HGIRON --agregué el 153 porque es nuevo APM 12-01-2022   ---SD#4159 CECHAVAR adicion 034-ALLIED 03/08/2022
		...
		if v_cod_ramo = '006' then 
			...
			--Caso Josue # 14351
			select tipo_contrato
			  into v_tipo_contrato_01
			  from reacomae
			 where cod_contrato = v_cod_contrato; 
			if v_tipo_contrato_01 = 7 then --EXCEDENTE
				let v_clase = '016';
			else
				let v_clase = '007'; --CUOTA PARTE
			end if
			--Fin # 14351
