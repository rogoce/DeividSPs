   ...
   DROP procedure sp_super01;
   CREATE procedure sp_super01(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
...
{
   select decode(origen,'001','LOCAL','002','EXTERIOR'),tipo_persona,nacionalidad,count(*) from temp_contratantep
	where es_pep = 1
	group by origen,tipo_persona,nacionalidad
	order by tipo_persona
	
   select decode(origen,'001','LOCAL','002','EXTERIOR'),tipo_persona,nacionalidad,sum(cant_aseg) from temp_asegurado2
	where es_pep = 1
	group by origen,tipo_persona,nacionalidad
	order by tipo_persona
	
   select decode(origen,'001','LOCAL','002','EXTERIOR'),tipo_persona,nacionalidad,count(*) from temp_pagadorp
	where es_pep = 1
	group by origen,tipo_persona,nacionalidad
	order by tipo_persona

   select decode(origen,'001','LOCAL','002','EXTERIOR'),tipo_persona,nacionalidad,count(*) from temp_benef2
	where es_pep = 1
	group by origen,tipo_persona,nacionalidad
	order by tipo_persona
}	
