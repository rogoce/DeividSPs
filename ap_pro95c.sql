...
DROP procedure ap_pro95c;
CREATE procedure "informix".ap_pro95c(a_cia CHAR(03),a_agencia CHAR(3),a_periodo DATE,a_codramo CHAR(255), a_reaseguro CHAR(255)  DEFAULT "*", a_origen CHAR(3) default "%")
	   ...
	   FOREACH WITH HOLD
          SELECT  d.no_poliza,
			...
			--  AND d.cod_origen like a_origen
              ...
              AND d.actualizado = 1
              AND d.cod_ramo in ('002','020','023')		
              AND d.cod_subramo = '005' -- Caso especial quitar después Amado 29-09-2023			  
			...
			ELSE
				...
				SELECT count(*),
				 ...
				 WHERE a.no_poliza = b.no_poliza
				   ...
				   AND d.cod_tipoauto not in (
				   select cod_tipoauto from emitiaut WHERE nombre like '%BUS%'
				   );
