...
drop procedure sp_pro371a;
create procedure "informix".sp_pro371a(a_periodo char(7), a_usuario char(8))
...
foreach
	select trim(no_documento),   
	 ...
	 where periodo = a_periodo
	  ...
	  and cod_grupo not in ('1122','77850','77857','77960') -- Grupo Ducruet Banisi   -- SD#3010 77960  11/04/2022 10:00  
	  
	let _dia = SUBSTR(_vig_inic,4,2);
	let _mes = SUBSTR(_vig_inic,1,2);
	let _ano = SUBSTR(_vig_inic,7,4);
	
	let _vigencia_inic = mdy(_mes,_dia,_ano);

	let _dia = SUBSTR(_vig_fin,4,2);
	let _mes = SUBSTR(_vig_fin,1,2);
	let _ano = SUBSTR(_vig_fin,7,4);
	
	let _vigencia_fin = mdy(_mes,_dia,_ano);
	...
	select count(*)
	 ...
	 where no_documento = _no_documento
       ...
       and vigencia_final = _vigencia_fin	   
	   and actualizado = 1  and nueva_renov = 'R';  --SD#7659JAQUELIN 31/08/2023
	   
	if _renovada is null then
		let _renovada = 0;
	end if
    ...
    -- Excluir Cese de Cobertura
	
	if _cod_no_renov = '039' then
		continue foreach;
	end if

    -- Solo los que tengan permiso de asignar renovacion
    select supervisor_ren,
	       codigo_perfil
      into _supervisor_ren,
	       _codigo_perfil
      from insuser
     where usuario = a_usuario;	 
	 
	if _renovada > 0 and _supervisor_ren = 1 then --DRN-TBD106 Amado 10-02-2022
		update emirenduc
		   set enviado = 1
		 where no_documento = _no_documento
