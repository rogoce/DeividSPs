...
DROP procedure sp_proe27amm;
CREATE PROCEDURE sp_proe27amm(a_cia CHAR(3),a_agencia CHAR(3),a_codsucursal CHAR(255) DEFAULT "*",a_corredor char(255) DEFAULT "*",a_codramo CHAR(255) DEFAULT "*",a_subramo CHAR(255) DEFAULT "*",a_periodo1 DATE, a_cod_cliente CHAR(255) DEFAULT "*", a_no_documento CHAR(255) DEFAULT "*") 
     RETURNING CHAR(03),CHAR(50),CHAR(20),INTEGER,INTEGER;
 BEGIN
    ...
    FOREACH
	   ...
	   SELECT COUNT(*)
		...
		IF _unidades IS NULL THEN
			LET _unidades = 0;
		END IF
        if v_codramo in ('002','020') then
			let _unidades = 0;
        end if			

         RETURN v_codramo,v_desc_ramo,v_documento,_unidades,_dependientes WITH RESUME;
