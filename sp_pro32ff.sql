...
drop procedure sp_pro32ff;
create procedure "informix".sp_pro32ff(
a_cia			char(3),
a_agencia		char(3),
a_fecha			date,
	...
	foreach with hold
		select y.no_documento,
		 ...
		 where y.seleccionado = 1
		   and y.cod_contratante = _cod_contratante
		...
		if v_cod_ramo = '018' then
			...
			if _cnt = 1 then
				...
				let _desc_producto = "Ver Detalle";
			end if
		else
			select count(*) 
			  into _cnt
