...
-------------------------------------------------
   BEGIN
	  ...
	  DEFINE _sanio_ant                      char(4);
	  DEFINE _smes_ant                       char(2);
	  DEFINE _anio_ant   				     integer; 
	  DEFINE _mes_ant					     integer;	  
	  DEFINE _bandera                        char(7);
	  DEFINE _smes_ini                       char(2);
	  DEFINE _sanio_ini	  			         char(4);
	  DEFINE _mes_ini   				     integer; 
	  DEFINE _anio_ini					     integer; 

	  DEFINE _smes_ini1                      char(2);
	  DEFINE _sanio_ini1  			         char(4);
	  DEFINE _mes_ini1   				     integer; 
	  DEFINE _anio_ini1					     integer; 

	  DEFINE _smes_ini2                      char(2);
	  DEFINE _sanio_ini2	  			     char(4);
	  DEFINE _mes_ini2   				     integer; 
	  DEFINE _anio_ini2					     integer; 
	  ...
	  define v_cargo           				varchar(50);
		...
		CREATE TEMP TABLE tmp_correcto (
				anio		    integer,
				mes			    integer, 
				actual		    integer) WITH NO LOG;
		CREATE INDEX xie01_tmp_correcto ON tmp_correcto(anio,mes);
      ...
      CALL sp_pro69(a_compania,a_agencia,a_periodo1,
                    ...
                    "*",a_codramo,a_cliente, a_no_documento)
                    RETURNING v_filtros;
--	  a_codusuario
--	  Set debug file to "sp_pro1010.trc";
--      Trace on;

	  let _smes_ini1  = a_periodo1[6,7];
	  let _sanio_ini1 = a_periodo1[1,4];
	  let _anio_ini1  = _sanio_ini1;
	  let _mes_ini1   = _smes_ini1;

	  let _smes_ini2  = a_periodo2[6,7];
	  let _sanio_ini2 = a_periodo2[1,4];
	  let _anio_ini2  = _sanio_ini2;
	  let _mes_ini2   = _smes_ini2;

	select par_periodo_ant[1,4],par_periodo_ant[6,7]
	  into _sanio_ant,_smes_ant
	  from parparam;

		let _anio_ant = _sanio_ant;
		let _mes_ant  =	_smes_ant;

		let v_mes = _smes_ini1;
		let v_anio = _anio_ini1;
        LET _fecha_cierre = MDY(_mes_ant, 1,_anio_ant);


	while  v_anio <= _anio_ini2	

		while v_mes < 13

			  if _anio_ini1 <= _anio_ant then

		          LET _fecha_proceso = MDY(v_mes, 1,v_anio);
				  if _fecha_proceso < _fecha_cierre then
					  insert into tmp_correcto(anio,mes,actual)
					  values (v_anio,v_mes,0);
				  end if
	...
	SELECT valor_parametro 
	  INTO v_firma_cartas
	  FROM inspaag
	 WHERE codigo_parametro = "firma_k_rta";
	 
	SELECT descripcion,cargo
	  INTO v_nombre_completo,v_cargo
	  FROM insuser
...
--      Trace on;
		...
		FOREACH
		 SELECT distinct month(vigencia_inic)
		   INTO v_cant_realizo
		   FROM endedmae 
		...
		foreach
		 select year(vigencia_inic),month(vigencia_inic)
		   into v_anio,v_mes
		   from endedmae 
		  where trim(no_documento) = trim(v_nodocumento)
		    and vigencia_inic >= v_vig_ini
		    and vigencia_final <= v_vig_fin
			and vigencia_inic <= _fecha_cierre
		    and cod_endomov = "023"
			order by 1,2   
