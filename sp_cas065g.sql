-- Sacar del callcenter a clientes con todas sus poliza vencidas o canceladas y asignarlas al cobrador vencidas y canceladas.

-- Creado    : 23/01/2004 - Autor: Armando Moreno M.

-- SIS v.2.0 - d_cobr_cobros_x_dia_cte - DEIVID, S.A.

--drop procedure sp_cas065g;

create procedure sp_cas065g()
returning char(3),
          char(3),
          char(10),
          char(50),
...
foreach
	...
	foreach
        ...
        if _estatus_poliza = 2 or _estatus_poliza = 3 then
			let _cant2 = _cant2 + 1;
		end if

		if _cant = _cant2 then  --todas son vencidas o canceladas
			foreach
				select no_documento
			      into _no_documento
			      from caspoliza
