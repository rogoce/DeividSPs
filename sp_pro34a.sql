...
if a_codagente <> "*" then
	...
	drop table tmp_codigos;
      ...
      FOREACH WITH HOLD
         SELECT x.cod_grupo,x.no_factura,x.no_documento,x.cod_contratante,
                x.suma_asegurada,x.prima
                INTO v_cod_grupo,v_nofactura,v_nodocumento,v_cod_contratante,
                     v_suma_asegurada,v_prima_suscrita
                FROM temp_det x
               WHERE x.seleccionado = 1
               ORDER BY x.cod_grupo,x.no_factura
         ...
         RETURN v_cod_grupo,v_desc_grupo,v_nofactura,v_nodocumento,
