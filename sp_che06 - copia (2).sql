...
--             Se modifica la 
-- Modificado: 21/08/2019 - Autor: Amado Perez
--             Se modifica para que actualice el saldo en agtagen y los montos en agtsalra, inserte en agtsalhi utilizando la temporal de arrastre tmp_arrastre 
    ...
    IF _tipo_pago = 1 THEN -- Pago por ACH
        ...
        SELECT cod_chequera
		 ...
		 WHERE cod_banco = _banco_ach
		   AND enlace_cat = '03'; --> que no sea firma electronica de salud
...
{		SELECT monto
	...
	IF _comision_a IS NULL THEN
		LET _comision_a = 0;
	END IF

	UPDATE agtagent
	   SET saldo      = saldo - _comision_a
	 WHERE cod_agente = a_cod_agente;
