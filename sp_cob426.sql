...
-- Creado    : 17/09/2020 - Autor: Amado Perez -- Copia del sp_cob80

-- SIS v.2.0 - DEIVID, S.A.

drop procedure sp_cob426;
create procedure "informix".sp_cob426(
a_compania	char(3),
a_sucursal	char(3),
a_fecha		date,
a_user		char(8))
...
define _campo2              varchar(113);
define _adenda				char(80);
define _mensaje				char(50);
define _aseg_resultado		varchar(22);
define _aseg_resultado2		varchar(35);
define _no_documento		char(20);
define _no_cuenta			varchar(17);
define _no_cuenta2			varchar(12);
define _monto_char			varchar(11);
define _monto_char2			varchar(17);
define _cod_cliente			char(10);
define _cod_pagador			char(10);
define _no_poliza			char(10);
define _ruta_char			char(4);
define _ruta_char2			char(4);
define _fecha_char			char(8);
define v_periodo			char(7);
define _orden				char(6);
define _no_lote_char		char(5);
define _cedula				char(5);
define _cant_tran_char		varchar(4);
...
define v_fecha				date;
define _concepto            varchar(45);


--SET DEBUG FILE TO "sp_cob426.trc";
--trace on;
...
--if _no_cuenta = '0445992839403' then	 
--SET DEBUG FILE TO "sp_cob426.trc";
--trace on;
--end if
	...
	let _aseg_resultado2 = trim(_nombre_pagador);
	let _aseg_resultado2 = fun_quitar_acentos(_aseg_resultado2);
	...
	-- Nuevo formato de texto para Multibank 2020

	let _monto_char2 = _monto;
	let _monto_char2 = replace(_monto_char2, ',', '');
	...
	if _ruta_numero = 372 then
		let _ruta_char = '0';
	else
		if _ruta_numero > 999 then
			let _ruta_char = _ruta_numero;
		elif _ruta_numero > 99 then
			let _ruta_char[2,4] = _ruta_numero;
		elif _ruta_numero > 9 then
			let _ruta_char[3,4] = _ruta_numero;
		else
			let _ruta_char[4,4] = _ruta_numero;
		end if
	end if
...
--	values (_campo,_orden);
	...
	-- Nuevo formato Multibank 2020
	let _cant_tran_char = _cant_tran;
    let _no_cuenta2 = _no_cuenta;
	let _concepto = trim(_cant_tran_char) || '-' || trim(_no_documento);

    let _campo2 = '';
	let _campo2 = trim(_campo2) || trim(_aseg_resultado2) || '@';
	let _campo2 = trim(_campo2) || trim(_no_cuenta) || '@';
	if _tipo_cuenta = 'S' then
		let _campo2 = trim(_campo2) || 'CA' || '@';
	else
		let _campo2 = trim(_campo2) || 'CC' || '@';
	end if
	let _campo2 = trim(_campo2) || trim(_ruta_char) || '@';
	let _campo2 = trim(_campo2) || trim(_monto_char2) || '@';
	let _campo2 = trim(_campo2) || trim(_concepto);

	insert into cobcuban2 
	values (_campo, _orden, _campo2);

	let _campo = 'A' || trim(_adenda);
	let _orden = _cedula || 'A';

	insert into cobcuban2 
	values (_campo,_orden,null);
