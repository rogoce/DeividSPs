--***********************************************************************************
-- Procedimiento que genera Mini Convencion MEXICO DF 2023
--***********************************************************************************
-- Creado    : 14/03/2023 - Autor: Armando Moreno M.
-- Creado    : 14/03/2023 - Autor: Armando Moreno M.

DROP PROCEDURE sp_minic2023;
CREATE PROCEDURE sp_minic2023(a_compania CHAR(3),a_sucursal CHAR(3))
RETURNING SMALLINT;
...
let _fecha_aa_ini = "01/02/2023";
let _fecha_aa_fin = "15/05/2023";
let _fecha_tope   = '15/05/2023';
LET _reemplaza_pol = "";
...
foreach
    ...
    if _cod_ramo in('002','023') then  --Ramo de auto y auto flota
		...
		if _cnt > 0 then
			...
			values (_no_poliza,_no_documento,'Excluye polizas Auto RC.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
			continue foreach;
		end if
	end if
	...
	if _cod_formapag = '091' then --Se excluye forma de pago GOB
		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
		values (_no_poliza,_no_documento,'Excluye forma de pago GOB.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
		continue foreach;
	end if
	...
	if _cnt > 0 then
		insert into miamiexc(no_poliza,poliza,descripcion,vig_ini,vig_fin,monto_pag,prima_suscrita)
		values (_no_poliza,_no_documento,'Excluye poliza facultativa.',_vigen_ini,_vigencia_final,_monto_p,_prima_suscrita);
		continue foreach;
	end if
...
foreach
	...
	let _puntos_tiene  = 0;
	let _flag          = 0;
	if _cod_ramo in('004','018','019','016') then
		let _puntos = 1.20;
	elif _cod_ramo in('001','017','009','006','015','003','014','022','005','010','011','013') then
		let _puntos = 1.00;
