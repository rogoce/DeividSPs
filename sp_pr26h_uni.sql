...
DROP PROCEDURE sp_pr26h_uni;

CREATE PROCEDURE "informix".sp_pr26h_uni(
		a_compania  CHAR(3),
		a_agencia   CHAR(3),
		a_periodo1  CHAR(7),
		a_periodo2  CHAR(7),
...
CREATE TEMP TABLE tmp_prod_uni(
...
CREATE INDEX iend1_tmp_prod_uni ON tmp_prod_uni(cod_ramo);
CREATE INDEX iend2_tmp_prod_uni ON tmp_prod_uni(cod_grupo);
CREATE INDEX iend3_tmp_prod_uni ON tmp_prod_uni(cod_sucursal);
CREATE INDEX iend4_tmp_prod_uni ON tmp_prod_uni(tipo_produccion);
CREATE INDEX iend5_tmp_prod_uni ON tmp_prod_uni(cod_agente);
CREATE INDEX iend6_tmp_prod_uni ON tmp_prod_uni(user_added);
CREATE INDEX iend7_tmp_prod_uni ON tmp_prod_uni(cod_vendedor);
...
FOREACH 
 SELECT e.no_poliza,	
   ...
   INTO _no_poliza, 	
		...
		_suma_asegurada,
		_no_unidad
   FROM endedmae e, emipomae f, endeduni g
  WHERE e.no_poliza = f.no_poliza
     AND e.no_poliza = g.no_poliza
