...
-- Modificado: 20/04/2011 - Autor: Roman Gordon
--			   Refacturacion de Polizas de Salud con saldo a 61+ dias

-- Modificado: 02/11/2011 - Autor: Roman Gordon 
--			   Se agrego control para que no permita aplicar pagos en suspenso cuando la remesa que creo el pago en
--			   suspenso no ha sido actualizada

-- Modificado: 11/01/2013 - Autor: Amado Perez 
--			   Se agrego procedure sp_cob315 que actualiza los saldos de los cobros externos legales
--			   nueva ley de seguro

-- Modificado: 24/04/2013 - Autor: Roman Gordon
--			   Se agrego procedure sp_cob326 que genera la información del reportes de movimientos
--			    de pólizas de General Rerpresentatives (Banco Delta)
...
create procedure sp_cob29(a_no_remesa char(10), a_usuario char(8))
returning integer,
		  char(150);

define _desc_renglon		varchar(100);
define _mensaje				char(106);
define _error_desc			char(50);
define _no_comp				char(10);
define _user_posteo			char(8);
define _periodo_par			char(7);
define _periodo,_per		char(7);
...
define _fecha,_fec_pos  	date;
define _Tipo_B              smallint;

if a_no_remesa = '2073512' then
...
begin
	...
	foreach	
		...
		if _no_documento in("0503-00026-01","0000-00000-00") then
			let _mensaje = "El Numero de Poliza " ||_no_documento || " esta bloqueado para recibir movimientos";
			return 1, _mensaje;
		end if

		if _no_documento in ('0114-00344-01','0514-00004-01','0614-00049-01') then --'1112-00002-01' se quita 14/08/20 F9 de gisela
			let _mensaje = 'La(s) Póliza(s) no puede recibir pagos por estar Cancelada(s)/Anulada(s). Verifique...';
			return 1, _mensaje;
		end if
		if _no_documento in ('0216-00890-03') then -- Correo Roman 24/08/2020
			let _mensaje = 'CASO ESPECIAL,LLAMAR A ERIC FANOVICH ANTES DE RECIBIR DINERO O HACER TRÁMITE A PÓLIZA';
			return 1, _mensaje;
		end if
...
--*
		 if a_no_remesa in ('1425614','1425632') then --BANISI
...
call sp_web51() returning _per, _fec_pos;

update cobremae
   set user_posteo = a_usuario,
       date_posteo = _fec_pos,
	   actualizado = 1
 where no_remesa   = a_no_remesa;
...
if _tipo_remesa in ('A', 'M') then
	...
	if _cnt > 0 then
  		LET _mensaje = "No Puede Actualizar la remesa, La Caja ya esta Cerrada, Por favor Verifique.";
  		RETURN 1, _mensaje;
  	end if
