...
-- BALANCEAR LA CARTERA DE INCENDIO , incluye observacion
-- Creado    : 10/04/2019      -- Autor: Henry Giron 
-- Execute Procedure sp_pro1023a("001","001",today,"*","001,003;","*","*","*","*","*","*")
DROP procedure sp_pro1023a;
CREATE procedure "informix".sp_pro1023a(a_cia CHAR(03),a_agencia CHAR(3),a_fecha DATE,a_codsucursal CHAR(255) DEFAULT "*",a_codramo CHAR(255) DEFAULT "*",a_codgrupo CHAR(255) DEFAULT "*",a_agente CHAR(255) DEFAULT "*",a_usuario CHAR(255) DEFAULT "*", a_cod_cliente CHAR(255) DEFAULT "*", a_acreedor CHAR(255) DEFAULT "*", a_no_documento CHAR(1255) DEFAULT "*")
RETURNING 
CHAR(3) as cia, 
...
CHAR(15) as cod_manzana,
CHAR(50) as referencia,
CHAR(15) as cod_manzana2,
CHAR(50) as referencia2,
CHAR(50) as categoria,
...
REFERENCES TEXT as descripcion;
    ...
    DEFINE u_cod_manzana2                        CHAR(15);
    DEFINE u_tipo_asegurado                      CHAR(50); 
    DEFINE u_referencia                          CHAR(50); 
    DEFINE u_referencia2                          CHAR(50); 
    DEFINE _categoria                          CHAR(50); 
       ...
       FOREACH 
          SELECT no_unidad,
           ...
           WHERE no_poliza = v_no_poliza
			...
			if u_cod_manzana2 is null then
				let u_cod_manzana2 = '';
			end if
			...
			SELECT referencia
	       	  INTO u_referencia2 
	       	  FROM emiman05 
	       	 WHERE cod_manzana = u_cod_manzana2; 
			  
			  IF u_referencia2 is null THEN
				 LET u_referencia2  = '';
			 END IF
...
--				END EXCEPTION
  ...
  --			END			   
    ...
    --SET DEBUG FILE TO "sp_pro1023.trc";
    --TRACE ON;
    ...
    FOREACH 
     ...
     ORDER BY nombre_agente,cod_ramo,zona_libre, no_documento, no_unidad	 	 	  	 	 
		...
		foreach
		  ...
		  if _cod_manzana is null then
		   ...
		   SELECT cod_provincia,
			...
			if _cod_categoria is null then
				let _cod_categoria = '';
			end if
		...
		-- LET v_descripcion = "";
		 ...
		 call sp_blob_emipode2(v_no_poliza ,u_no_unidad) returning v_descripcion;			
