...
--execute procedure sp_niif012a('2023-01','2023-12','001,002,003,004,005,006,007,008,009,010,011,012,013,014,015,016,017,018,019,020,021,022,023;')
drop procedure sp_niif012a;
create procedure sp_niif012a(a_periodo_desde char(7),a_periodo_hasta char(7), a_cod_ramo varchar(255) default "*")
...
FOREACH
	/*select cob.no_poliza,
		   ...
		   emi.cod_grupo,
		   grp.nombre,
		   case con.tipo_contrato when 1 then 'No Cedido' else 'Cediddo'end, 
		   sum(rea.prima)
	  into _no_poliza,
		   _no_documento,
		   _vigencia_inic,
	 ...
	 inner join emifacon rea on rea.no_poliza = cob.no_poliza and rea.no_endoso = cob.no_endoso
	 ...
	 inner join emifacon rea.
	 where cob.periodo between a_periodo_desde and a_periodo_hasta
	...
	select cob.no_poliza,
	 ...
	 where cob.periodo between a_periodo_desde and a_periodo_hasta
	   and cob.actualizado = 1
	   --and cob.no_documento in ('0223-00155-01')--,'1621-00024-01','1621-00025-01','1621-00026-01','1621-00027-01','1621-00028-01','1621-00029-01','1621-00030-01')
	...
	if _tipo_contrato = 1 then -- Retencion
		if _cod_ramo = '018' then
			let _monto_pagado = _pagado_bruto * 0.70; --Forzar el contrato de cesiÃ³n
			let _comision_agt = _monto_pagado * (_porc_comis_agt/100);
			let _impuesto_rea = _monto_pagado * (_porc_impuesto_rea/100);
			
			let _pagado_bruto = _pagado_bruto * 0.30;
			let _comision_agt = _pagado_bruto * (_porc_comis_agt/100);
			let _impuesto_seg = _pagado_bruto * (_porc_impuesto_seg/100);
			...
			on exception in (-239)
				update fichero_cobros_niif
				 ...
				 where no_poliza = _no_poliza
				   and no_endoso = _no_endoso
				   and tipo_reas = 'Cedido C'
				   and reasegurador = 'MAPFRE RE';
			end exception
