...
--Ejecutado desde Sac-Online
-- Creado    : 22/09/2004 - Autor: Demetrio Hurtado Almanza 
--
-- SIS v.2.0 - DEIVID, S.A.
  
drop procedure sp_sac135a;
create procedure sp_sac135a(
a_usuario	char(8),
a_origen	smallint
) returning integer,
			integer,
...
--set debug file to "sp_sac135a.trc";
--trace on;
...
if _cantidad <> 0 then
	...
	--return 1,_cnt_reg_acum,'1' with resume;
	end foreach
	...
	foreach
	 select	x.centro_costo,
	  ...
	  order by x.centro_costo, y.fecha, x.cuenta
		...
		--return 1,_cnt_reg_acum,'2' with resume;
	...
	foreach
	 select	x.centro_costo,
	  ...
	  order by x.centro_costo, y.fecha, x.cuenta
		...
		--return 1,_cnt_reg_acum,'3' with resume;
	end foreach
	...
	foreach
	 select	x.centro_costo,
	  ...
	  order by x.centro_costo, y.fecha, x.cuenta
		...
		--return 1,_cnt_reg_acum,'4' with resume;
	end foreach
