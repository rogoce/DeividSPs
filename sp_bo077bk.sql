...
--drop procedure sp_bo077bk;
create procedure "informix".sp_bo077bk(
a_fecha_ini	date,
a_fecha_fin	date
) returning integer,
            char(50);
	...
	if _nueva_renov = "N" then
		...
		if _estatus_poliza = 2 then  --esta cancelada
			...
			if _fecha_canc is not null then
				if _fecha_canc > a_fecha_fin then --Si la fecha de cancelada es anterior a la fecha final dada, no se toma en cuenta.
				else
					continue foreach;
				end if
			end if	
