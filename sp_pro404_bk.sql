...
DROP PROCEDURE sp_pro404_bk;
create procedure sp_pro404_bk()
returning 
		  VARCHAR(20), -- 1. Cedula
...
FOREACH
  SELECT a.no_documento,
   ...
   WHERE a.cod_contratante = b.cod_cliente
	 ...
	 AND g.cod_producto not in('04561','04562')
  ...
  SELECT a.no_documento,
   ...
   WHERE a.cod_contratante = b.cod_cliente
	...
	ELIF _cod_tipoveh in('010') then
		let v_asistencia = 'EP'; --let v_asistencia = 'FEP';
	ELSE	
		LET v_asistencia = "FL";
	END IF
...
FOREACH WITH HOLD
  ...
  ORDER BY 11, 12, 14
	...
	if _fecha_suspension < _fecha_hoy and v_poliza not in ('0210-01288-01', '2315-000107-01') and _cod_grupo <> "1090" then
	    if _cod_grupo in('77850','1090','1009','01016','124','00087','125','148','1122') then  --Correo Jesus 18/02/2020 autorizando a estos grupos para que siempre vayan a pma asistencia.
		else
			continue foreach;
		end if
	end if
