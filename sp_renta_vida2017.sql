--Procedimiento para realizar la carga en Rentabilidad3 para simular el bono de RENTABILIDAD especial de 019 y 018 sin colectivo
--Creado 22/03/2017	Armando Moreno M.
--Modificado 04/01/2018 Henry Girón adicion especial para rentabilidad3
-- execute procedure  sp_renta_vida2017('001','001');

DROP PROCEDURE sp_renta_vida2017;
CREATE PROCEDURE sp_renta_vida2017(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
--delete from rentabilidad3 where periodo = a_periodo;
...
foreach
	 select no_documento,
	   ...
	   into _no_documento,
			...
			_pri_susc_ap
	   from fis_che115a
	   where no_documento[1,2] in ( '18','19')
	  group by no_documento
    ...
    if _cod_ramo in("019") Or (_cod_ramo = '018' and _concurso = 1) then
	else
		continue foreach;
    end if
	...
	select cod_tipo 
	 ...
	 where periodo  = a_periodo
	   ...
	   let _cod_tipo = '002';  -- caso especial para 019 o (018 y concurso = 1) --HGIRON
	   --se realiza la correccion para separar ambos ramos, 018 = 2, 019 = 3
	...
	foreach
	  SELECT cod_agente,
	   ...
	   WHERE no_poliza = _no_poliza
		...
		let _aplica = 0;
		let _n_cod_tipo = 'VIDA INDIVIDUAL y SALUD INVIDIVUAL';   --CAso especial para rentabilidad3 para: 019 o (018 y concurso  = 1) HGIRON
		insert into rentabilidad3(
		periodo,   			
		cod_agente,     		
		no_documento,   		
		pri_susc_aa,  		
...
foreach
	select tipo,
	 ...
	 if _cod_tipo = '004' then
	    let _cod_tipo_aux = '004';
	else
		let _cod_tipo_aux = '002';
	end if
	let _cod_tipo = '002';
				 ...
				 where periodo  = a_periodo
				   ...
				   if _porcentaje <> 0 then
					   ...
					   if _prima_beneficio > _prima_maxima then
						 ...
						 update rentabilidad3
							set bono       = _bono,
							    beneficio  = _porcentaje,
							    aplica     = 1 
						  where periodo    = a_periodo
							and cod_agente = _cod_agente
							and tipo       = _cod_tipo_aux --_cod_tipo
							and monto_90   = 0
							and aplica     = 0;	
