...
---  Amado - 21/06/2016 --
------------------------------------------------
drop procedure sp_che150;
create procedure sp_che150(a_area smallint, a_anio integer, a_mes smallint)
returning	int,
	        varchar(50);
...
on exception set _error_cod, _error_isam, _error_desc
	return _error_cod, "- Req. " || _no_requis || "- Tr. " || _transaccion;
end exception
   ...
   WHERE origen_cheque in ('3','M')
	...
	foreach
		...
		let _dias_compara_tmp = 0;
         ...
         -- Prioridad por Poliza -- Solo debe aplicarse para pagos a asegurados -- 27-10-2025
		...
		if a_area = 1 then -- Automóvil		  	
			...
			if _cod_tipopago = '003' then --Pago a Asegurado
				if _dias >= _dias_compara and _monto <= 5000.00 then
				...
				elif _dias >= _dias_compara and _monto > 5000.00 then
					let _prioridad = 99 + _por_prioridad;
				end if	
