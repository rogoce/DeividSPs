...
DROP PROCEDURE sp_pro586d;

CREATE PROCEDURE "informix".sp_pro586d(
		a_compania  CHAR(3),
		a_agencia   CHAR(3),
		a_periodo1  CHAR(7),
		a_periodo2  CHAR(7),
...
FOREACH 
 SELECT e.no_poliza,	
   ...
   INTO _no_poliza, 	
		...
		_cod_tipocan,
		_suma_asegurada
   FROM endedmae e, emipomae f, emipoagt g
  WHERE e.no_poliza = f.no_poliza 
    ...
    AND e.actualizado = 1
	AND g.cod_agente in (select cod_agente from agtagent where no_licencia[1,3] = "OAL")
