-- Procedimiento que Genera el Cheque de bonificacion de 1% web para Un Corredor

-- Creado    : 24/02/2015 - Autor: Armando Moreno

DROP PROCEDURE sp_che215;
CREATE PROCEDURE sp_che215(
a_compania       CHAR(3),
a_sucursal       CHAR(3),
a_cod_agente     CHAR(5),
a_usuario        CHAR(8),
...
if _comision = 0 then  --No debe generar Requis, si el monto a pagar es cero. 05/04/2023
	RETURN 0;
end if
...
SELECT cod_origen,
 ...
 WHERE cod_banco = a_banco;
	...
	LET _descripcion = 'PAGO DE BONIFICACION DE 1% WEB DE: ' || _nombre_mes || " DEL " || a_periodo[1,4];
	...
	call sp_par276(_no_requis, "F") returning _error, _error_desc;
