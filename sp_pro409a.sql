...
drop procedure sp_pro409a;
create procedure sp_pro409a()
returning	integer,
			varchar(200);
...
foreach with hold
	...
	select count(*)
	  ...
	  from emipoagt
	 where no_poliza = _no_poliza
	   and cod_agente in (_cod_agente_old);
	...
	if _cnt_agt = 0 then
		...
		return 1, 'No pertenece al antiguo corredor: ' || trim(_no_documento) with resume;
		commit work;
		continue foreach;
	end if
	...
	if _cnt_agt_25 > 0 and trim(_cod_agente_old) = trim(_cod_agente_new) and trim(_cod_agente_new) = '02569' then  -- 23/08/2019,  AMORENO
		...
		call sp_pro409(_no_poliza,'DEIVID','001',_cod_agente_old,_cod_agente_new,_fecha_efectiva) returning _error,_error_desc,_no_endoso;
	--end if
