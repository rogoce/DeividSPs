...
drop procedure sp_seg004;
create procedure "informix".sp_seg004(a_usuario CHAR(8), a_status CHAR(1), a_fecha_status date, a_chg_user  char(8), a_registro integer)
...
DEFINE _h_hora_inicio			datetime hour to fraction(5);	
DEFINE _h_hora_final			datetime hour to fraction(5);	
...
DEFINE _h_recl_firma_perdida    SMALLINT ;							  
DEFINE _registro            	INTEGER  ;
DEFINE s_usuario 				CHAR(8)  ;
DEFINE s_status,_tipo_usuario	CHAR(1)  ;
...
define _cod_carnet_ca           integer;
define _cod_telefono			char(10);
define _nom_cargo				char(100);
define _cnt 	                smallint;
define _ubi_prt1,_ubi_prt2      char(3);
...
--LET a_usuario = a_usuario;
--LET a_registro = a_registro;
   ...
   where usuario  = a_usuario
	...
	select  usuario				   ,
	  ...
	  into	_h_usuario			   ,
			...
			_h_internet			   ,
			_h_business_obj		   ,
			_h_recl_firma_perdida,
			_ubi_prt1,
			_ubi_prt2
	   from segv05:insuser
	  where usuario = a_usuario;

let _cargo = trim(_cargo);
...
-- Backup de historico.
if _crear_cliente is null then
	let _crear_cliente = 0;
end if
if _h_crear_cliente is null then
	let _h_crear_cliente = 0;
end if
if _h_aut_endoso is null then
	let _h_aut_endoso = 0;
end if

if a_status = "C" then

	if _h_hora_inicio is null then
		let _h_hora_inicio = CURRENT;
	end if

	if _h_hora_final is null then
		let _h_hora_final = CURRENT;
	end if

    if _h_no_login_permitido is null then
		let _h_no_login_permitido = 0;
	end if

	select count(*)
	  into _cnt
	  from hisuser
	 where usuario  = _h_usuario
	   and f_cambio	= _fecha_proceso
	   and u_cambio	= a_chg_user
	   and r_cambio	= a_registro;
    ...
    if a_fecha_status = _fecha_cambio then
	    if _tipo_usuario = 'R' then			--Es rehabilitacion, hay que actualizar la nueva fecha de inicio y blanquear la fecha final. Sol Maque caso 32712. AMM.
			update segv05:insuser
		       set fecha_inicio	= a_fecha_status,
			       fecha_final  = null
		     where usuario = a_usuario;
		end if
		update segv05:insuser
		   set cargo			 = 	_nom_cargo			,
			   ...
			   recl_firma_perdida = _recl_firma_perdida	
		 where usuario = a_usuario;
	end if
...
if _codigo_agencia <> _h_codigo_agencia  then
   ...
   --	Delete from insusco where usuario = _h_usuario and codigo_compania = _h_codigo_compania  and codigo_agencia = _h_codigo_agencia;

	update insusco
	   set status = "I"
	 where usuario = _h_usuario and codigo_compania = _h_codigo_compania  and codigo_agencia = _h_codigo_agencia;

	CALL sp_seg006(a_usuario,"A",_fecha_proceso) RETURNING	_agt_error, _agt_mensaje;

	if _agt_error <> 0 then
	   RETURN _agt_error, _agt_mensaje;
	end if
