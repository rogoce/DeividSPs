--************************************************************************************
-- Procedimiento que genera el Reporte para consurso a Grecia 2012 para los corredores
--************************************************************************************
...
-- Se desactivo la carga de Madrid  
-- Era hasta el 30 de Septiembre de 2012
-- Demetrio Hurtado	(03/10/2012)
...
--let a_periodo = "2011-08";


if a_periodo > "2012-09" then
	let a_periodo = "2012-09";
end if

--***********************

let _per_ini        = "2011-10";
let _per_ini_ap     = "2010-10";
let _ano            = a_periodo[1,4];
let _ano            = _ano - 1;
let _per_fin_ap     = _ano || a_periodo[5,7];
let _per_fin_dic    = "2011-12";

let _fecha_aa_ini = mdy(_per_ini[6,7], 1, _per_ini[1,4]);        --es del 01/10/2010

let _fecha_ap_ini = mdy(_per_ini_ap[6,7], 1, _per_ini_ap[1,4]);	 --es del 01/10/2009
  ...
  let _no_poliza = sp_sis21(_no_documento);
   ...
   select count(*)
    ...
    where no_poliza     = _no_poliza
      ...
      and fecha_emision >= '01/10/2011'
      and fecha_emision <= '30/09/2012';    --'21/10/2012'
	 ...
	 select cod_agente
	  ...
	  where no_poliza = _no_poliza
	   ...
	   --3 Doulos Insurance Consultants  (DICSA)(01048,01837) ,Logos Insurance(01569,01838), Juan Carlos Sanchez(01315,01834), Chung Wai Chun(00623,01836), Katia Mariza Dam de Spagnuolo(01575,01835)
		...
		-- Solicitud de Leticia del 01/10/2012
		-- Unificar todos los KAM
		-- Demetrio Hurtado (02/102/2012)

		if _cod_agente IN ("00133","01746","01749","01852","02004","02075") then  
		...
		if _cod_agente IN("00874","00620","00226","00119","00124","00030","00081") then  -- Ser Company correo 7/10/2011 leticia. cambiar a individual
