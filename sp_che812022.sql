--************************************************************
-- Procedimiento que Carga las Bonificaciones de cobranza 2022
--************************************************************
...
define _monto_fac_ac,_prima_cob12,_prima_cob13		dec(16,2);
...
select sum(prima_cobrada)
 ...
 select sum(prima_cobrada)
  into _prima_cob9
  from chqboagt							
 where cod_agente in('00035','02618','02904'); --Agrupar Directo Ducruet Banisi 02618 con Ducruet 00035
--8
select sum(prima_cobrada)
  into _prima_cob10
  from chqboagt							
...
foreach
	...
	if _cod_agente in('00035','02618') then
	...
	if _prima_cobrada >= 40000 or _retro = 1 then	--si es >= a 40000, se toma en cuenta al corredor para el pago.
	else
		continue foreach;
	end if
	...
	foreach
		...
		if _cod_grupo in('77850','1122','77960','77982') then  -- SD#5708 23/02/2023 HG
			continue foreach;
		end if
		let _contado = 0;
		let _pago    = 0;
...
foreach
	...
	if _forma_pag = 1 then --electronico
		...
		if _contado = 3 then
			let _porc_comis2 = 3;  --Boton de pago
		end if
	end if

	{if _cod_agente = '02569' then --Acuerdo especial con el corredor Javier Avila
		let _porc_comis2 = 4;
	end if}
