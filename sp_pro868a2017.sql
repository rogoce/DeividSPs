...
--**********************************************************************************************
-- PRIMA SUSCRITA AÑO PASADO TODO EL AÑO 2016	SP_SIS421						****************
--**********************************************************************************************
	INSERT INTO tmp_caribe(no_documento, pri_sus_ap)         -- Adicione prima 2016 en tabla TMP
	SELECT no_documento, prima_suscrita FROM prisusap;
--**********************************************************************************************
foreach
	select no_documento,
		   sum(pri_sus),
		   sum(pri_sus_ap)   -- lectura de prima 2016 y utilizo el campo prima_cobrada de tabla punta_cana q no se esta utilizando
	  into _no_documento,
		   _prima_suscrita,
		   _pri_pag
	  from tmp_caribe
...
foreach
  ...
  group by cod_agente
	...
	if _pri_sus_ap > 500001 then
		let _nombre_tipo = "Grupo I";
	elif (_pri_sus_ap >= 250001 and _pri_sus_ap <= 500000) then
			let _nombre_tipo = "Grupo II";
	elif (_pri_sus_ap >= 150001 and _pri_sus_ap <= 250000) then
			let _nombre_tipo = "Grupo III";
	elif (_pri_sus_ap >= 100001 and _pri_sus_ap <= 150000) then
