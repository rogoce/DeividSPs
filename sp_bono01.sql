--**********************************************************************************************************************************************************
-- Procedimiento para determinar la prima suscrita nueva de los corredores del bono de producción de Ramos Generales 2025
--***********************************************************************************************************************************************************

-- Creado    : 07/05/2025 - Autor: Armando Moreno M.
...
define _monto_fac,_prima_neta_acum       dec(16,2);
...
define _prima,_prima_sus  decimal(16,2);
...
{if _periodo_actual[1,4] = '2024' then
...
foreach
	...
	--se excluye '02825','02531','02830','02831' caso 3679
	--se excluye 02442 caso 11579, 19/09/24 9:45 am
	if _cod_agente in('02111','02442') then	--Se excluye cualquier poliza de este codigo de Javier Avila que es solo para talleres.
		continue foreach;
	end if
	foreach
			...
			--***********************************************
			--Se Excluye poliza rehabilitada, o traspaso de cartera o cambio de corredor en el periodo del concurso
			select count(*)
			  into _cnt
			  from endedmae
			 where no_poliza     = _no_poliza
			   and actualizado   = 1
			   and cod_endomov in ('003','012','031')  	
			...
			if _cnt > 0 then
				...
				if _cnt1 > 0 then  --Quiere decir que tiene tipo de cancelacion por duplicidad(si se debe tomar en cuenta)
				else
					continue foreach;
				end if	
			end if
			...
			let _prima_suscrita = _prima_suscrita;
...
foreach
	...
	let _prima_neta_acum = 0.00;
	
	foreach
			select d.nueva_renov,
			       d.no_poliza,
				   d.no_remesa,
				   d.renglon,
				   d.no_recibo,
			 ...
			 order by d.fecha,d.no_poliza,d.nueva_renov
			...
			let _prima_neta_acum = _prima_neta_acum + _prima;
			
			if _prima_neta_acum >= _prima_sus then
				update bono_prod_d
