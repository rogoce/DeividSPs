...
drop procedure sp_emite11_delta;

create procedure "informix".sp_emite11_delta() 
...
define _tarifa_comprensivo	dec(16,2);                      
...
define _error				smallint;
	...
	--set debug file to "sp_emite11_delta.trc"; 
	--trace on;
...
--	end if
	...
	let _cod_ramo = '002';
	let _cod_subramo = '012';
	let _cod_compania = '001';
	let _cod_sucursal = '001';
	let _poliza_maestra_auto = '0226-01520-01';
	let _cod_contratante = '102309';
	let _null = null;
	...
	foreach
		...
		if _existe = 0 then
			...
			call sp_sis372(_cod_cliente,			--ls_valor_nuevo char(10),				    
						   ...
						   'ldeconte@pnspanama.com',		   			--ls_email char(50),         			 
		...
		if _ramo_sis = 1 then
			...
			if _cnt_auto = 0 then
				...
				update emivehic
				   set valor_auto = _suma_asegurada, placa = _placa,capacidad = _puertas
				 where no_motor = _no_motor;
			end if 
			...
			elif _cod_producto in ('11964','11963') then --PARTICULAR
		...
		values(_no_poliza,'00001','02039',_suma_asegurada);
		...
		select grupo,
		  ...
		  into _grupo,
			   _tipo_auto
		  from emimodel mdl
		 inner join emitiaut aut on aut.cod_tipoauto = mdl.cod_tipoauto 
		 where cod_marca = _cod_marca
		   and cod_modelo = _cod_modelo;

		if _grupo is null then
			let _grupo = '';
		end if
		...
		if _ano_auso > 10 then
			let _ano_auso = 10;
		end if

		if _cod_producto in ('11964','11963') then
			call sp_pro589(_cod_producto,'00119',_ano_auso,_suma_asegurada,_grupo) returning _error,_error_desc,_tarifa_colision;
			call sp_pro589(_cod_producto,'00118',_ano_auso,_suma_asegurada,_grupo) returning _error,_error_desc,_tarifa_comprensivo;
			
			let _prima_incendio = 0.00;
			let _prima_robo = 0.00;
			let _prima_colision = _suma_asegurada * (_tarifa_colision/100);
			let _prima_comprensivo = _suma_asegurada * (_tarifa_comprensivo/100);			
		else
			call sp_pro51g(_no_poliza, _cod_producto, _cod_ramo, '00001','00119',_suma_asegurada) returning _prima_colision;
			call sp_pro51g(_no_poliza, _cod_producto, _cod_ramo, '00001','00120',_suma_asegurada) returning _prima_incendio;
			let _prima_robo = _prima_incendio;
			--call sp_pro51g(_no_poliza, _cod_producto, _cod_ramo, '00001','00120',_suma_asegurada) returning _prima_comprensivo;
		...
		update emipocob
		 ...
		 where no_poliza = _no_poliza
		   and no_unidad = '00001'
		   and cod_cobertura = '00107'; --GASTOS MED
		...
		update emipocob
		 ...
		 where no_poliza = _no_poliza
		   and no_unidad = '00001'
		   and cod_cobertura in ('00117','00907'); --ASISTENCIA
