...
drop procedure sp_sis421_mini2019;
create procedure sp_sis421_mini2019()
returning integer;
...
{select e.no_poliza,
  ...
  where e.no_poliza = t.no_poliza
    ...
    and e.actualizado  = 1
	and e.periodo between '2018-01' and '2018-12'
	and t.cod_agente in('00845','02337')
}
foreach
 select no_poliza,
		no_endoso,
   ...
   into _no_poliza,
		...
		_prima_suscrita
   from endedmae
  where no_poliza = t.no_poliza
    and no_endoso = t.no_endoso
    and actualizado  = 1
	and periodo between '2018-01' and '2018-12'
    ...
    if _cod_ramo = '001' and _cod_subramo = '006' then  -- Se excluye Zona L.,France F. y Cocosolito.
		continue foreach;
	end if
   ...
   select count(*)
	...
	if _cnt = 0 then
		foreach
			...
			insert into prisusapmini(no_documento, prima_suscrita, cod_agente)
