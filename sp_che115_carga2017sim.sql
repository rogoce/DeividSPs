...
DROP PROCEDURE sp_che115_carga2017sim;
CREATE PROCEDURE sp_che115_carga2017sim(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
	...
	-- Excluir Reaseguro Asumido y Coas. Minoritario
	 ...
	 if _cnt > 0 then		-- Facultativos, solo nuestra parte
		...
		if _prima_ret_ap is null then
		 ...
		 update fis_che115a_2017
		   set pri_sus_pag    = _prima_ret_aa,
			   pri_sus_pag_ap = _prima_ret_ap
		 where no_documento   = _no_documento;
		let _pri_susc_aa = _prima_ret_aa;
		let	_pri_susc_ap = _prima_ret_ap;
	 end if
	--70% Incendio 
	...
	foreach
	  SELECT cod_agente,
	   ...
	   WHERE no_poliza = _no_poliza
		...
		let _aplica = 0;
		insert into rentabilidad1_2017(
		periodo,   			
		cod_agente,     		
		no_documento,   		
		pri_susc_aa,  		
