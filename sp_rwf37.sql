...
DROP PROCEDURE sp_rwf37;
CREATE PROCEDURE sp_rwf37(a_no_tranrec CHAR(10)) 
			RETURNING SMALLINT, VARCHAR(50), SMALLINT, SMALLINT, CHAR(10);  
...
LET _transaccion = null;
 ...
 IF _numrecla IS NULL OR TRIM(_numrecla) = "" THEN
    rollback work;
 	RETURN 1, "Numero de reclamo es nulo", 0, 0, NULL;
 END IF

 IF _wf_incidente_str IS NULL THEN
	LET _wf_incidente_str = "";
 END IF
 
 IF _actualizado = 1 THEN
    rollback work;
 	RETURN 0, "Esta transaccion ya esta actualizada", 0, 0, NULL;
 END IF 

BEGIN
	ON EXCEPTION SET _error 
		rollback work;
	 	RETURN _error, "Error al BUSCAR PARAMETROS", 0, 0, NULL;         
	END EXCEPTION 
...
END
 ...
 IF _cantidad = 0 THEN
	rollback work;
    RETURN 1, "Esta transaccion no Tiene Coberturas Registradas, Verifique... ", 0, 0, NULL;
 END IF
 ...
 IF trim(_transaccion) = "" OR _transaccion IS NULL THEN
	rollback work;
	RETURN 1, "Error generando # de transaccion", 0, 0, NULL;
 END IF
 ...
 IF _tipo_transaccion = 4 AND _anular_nt <> "" THEN
	 ...
	 IF _no_tranrec_nt = "" OR _no_tranrec_nt IS NULL THEN
		rollback work;
	    RETURN 1, "La transaccion no existe, verifique...", 0, 0, NULL;
	 END IF

	 IF ABS(_monto) <> ABS(_monto_nt) THEN
		rollback work;
	    RETURN 1, "Los montos son diferentes,verifique...", 0, 0, NULL;
	 END IF
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
	        rollback work;
		 	RETURN _error, "Error al actualizar transaccion anulada", 0, 0, NULL;         
		END EXCEPTION 
		UPDATE rectrmae 
		   SET pagado         = 1,
 ...
 IF _monto_cob_tot <> _monto_con_tot And _tipo_transaccion = 4 THEN
 	rollback work;
	RETURN 1, "Existen Diferencias Entre las Coberturas y los Conceptos de Pagos", 0, 0, NULL;
 END IF
 ...
 BEGIN
	ON EXCEPTION SET _error 
		rollback work;
	 	RETURN _error, "Error al actualizar transaccion", 0, 0, NULL;         
	END EXCEPTION 
...
-- Actualizando en recrcmae cuando es cerrar reclamo
 IF _cerrar_rec = 1 THEN
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar reclamo - cerrar reclamo", 0, 0, NULL;         
		END EXCEPTION 
		UPDATE recrcmae 
...
IF _perd_total = 1 THEN
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar reclamo - perdida total", 0, 0, NULL;         
		END EXCEPTION 
		UPDATE recrcmae 
		   SET perd_total = 1
		 WHERE no_reclamo = _no_reclamo;
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar emipomae - cerrar reclamo", 0, 0, NULL;         
		END EXCEPTION 
		UPDATE emipomae 
		   SET perd_total = 1
	 ...
	 BEGIN
		ON EXCEPTION SET _error 
			rollback work;
		 	RETURN _error, "Error al actualizar emipouni - cerrar reclamo", 0, 0, NULL;         
		END EXCEPTION 
		UPDATE emipouni 
		   SET perd_total = 1
		 WHERE no_poliza  = _no_poliza 
	 ...
	 BEGIN
	    ...
	    IF _error <> 0 THEN
		   rollback work;
		   RETURN  _error, _descripcion, 0, 0, NULL;
		END IF
	 END

	 BEGIN
		CALL sp_rec347(_no_motor, _user_added) returning _error, _descripcion; --Bloqueo de motores
	    IF _error <> 0 THEN
		   rollback work;
		   RETURN  _error, _descripcion, 0, 0, NULL;
		END IF
...
-- Actualizando coberturas
 BEGIN
	 ON EXCEPTION SET _error 
	 	rollback work;
		RETURN _error, "Error al actualizar las coberturas del reclamo", 0, 0, NULL;         
	 END EXCEPTION
	...
	ELSE 	-- Verificar		-- Se modifica para que haga el recalculo de la variacion A.P.M. 24/10/2008
	    IF _tipo_transaccion = 4 OR _tipo_transaccion = 3 THEN	  -->Pago y Disminucion
			...
			IF _numrecla[1,2] = "02" OR _numrecla[1,2] = "20" OR _numrecla[1,2] = "23" THEN -- 7.26
			...
			FOREACH
				...
				FOREACH
					...
					IF a_no_tranrec = '2993105' THEN --Caso de Reclamos con reservas mal actualizadas ACARDENAS 29-10-2024
						IF _reserva_actual = 0 THEN
							LET _variacion = 0;
						ELIF _monto_tr > _reserva_actual THEN
							LET _variacion = _reserva_actual * -1;
						ELIF _monto_tr < 0 and _reserva_actual = 0 THEN
							LET _variacion = 0;
						ELSE
	...
	IF _numrecla[1,2] = "18" THEN
        ...
        IF _tipo_transaccion = 4 OR _tipo_transaccion = 3 THEN
			IF ABS(_variacion_sum) > ABS(_reserva_actual) THEN
		   		rollback work;
				RETURN 1, "Variacion de Reserva es Mayor que Reserva Actual", 0, 0, NULL;
			END IF
		END IF
 ...
 IF _error <> 0 THEN
 	rollback work;
	RETURN  _error, "No se creo el Reaseguro a Nivel de Transaccion", 0, 0, NULL;
 END IF

 BEGIN
	 ON EXCEPTION SET _error 
		rollback work;
	 	RETURN _error, "Error al actualizar recrcmae - lo ultimo", 0, 0, NULL;         
	 END EXCEPTION 
...
FOREACH
     ...
     IF _no_tranrec_otro IS NOT NULL AND _no_tranrec_otro <> "" THEN
		IF _no_tranrec_otro <> a_no_tranrec THEN
			rollback work;
		 	RETURN _error, "Transaccion Duplicada", 0, 0, NULL;         
		END IF
	 END IF
END FOREACH
 ...
 IF _error <> 0 THEN
 	rollback work;
	RETURN  _error, "Error al generar Recibo de Pago de Primas Pend.", 0, 0, NULL;
 END IF
 
-- Procedimiento que crea los registros para el envío de los finiquitos parmailsend y parmailcomp
 CALL sp_rec310(a_no_tranrec) returning _error, _descripcion;
 IF _error <> 0 THEN
 	rollback work;
	RETURN  _error, _descripcion, 0, 0, NULL;
 END IF
...
-- rollback work;
 ...
 RETURN 0, "Actualizacion Exitosa", _genera_incidente, _envia_correo, _no_requis;
END PROCEDURE