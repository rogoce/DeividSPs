...
------------------------------------------------
drop procedure sp_pro575;
create procedure sp_pro575(a_periodo char(7),
                            a_opcion smallint,
...
DEFINE _cust_qry, _cust_qry2, _cust_qry3, _cust_qry4, _cust_qry5 VARCHAR(250);
...
if a_poliza in ('1800-00502-01') then
	set debug file to "sp_pro575.trc";
	trace on;
end if	
drop table if exists temp_carta2012;
     ...
     create temp table temp_carta2012
			  (nombre_cliente	char(100), 
			   ...
			   inf_suf_prima    dec(16,2),
			   cambio_edad      dec(16,2)
            ) with no log;
...
if a_opcion = 1 then
 let _plan = "and a.cod_subramo in ('007','009')"; --Panama Plus, Global
else
 let _plan = "and a.cod_subramo in ('008','018','016')"; --Panama, Salud vital, Hospitalizacion Plus
end if	
...
let _cust_qry4 = " and a.no_documento like ?) and (a.tipo_cambio = ?) and (b.estatus_poliza not in (2,4)) and (b.renovada = 0) and (a.fecha_proceso is not null)";
let _cust_qry5 = "order by a.nombre_agente asc, a.no_documento asc";  --and a.user_added in ('AMADO','FANY') 	 
...
OPEN cust_cur USING _fecha_desde, _fecha_hasta, a_imp1, a_imp2, a_envi1, a_envi2, a_envi3, a_envi4, a_poliza, a_tipo;
...
{	   let _cnt_rec = 0;
	   ...
	   select cod_vendedor
		...
		select resolucion
		 ...
		 let _inf_suf_prima = _diferencia;
		 }
		 -- Nuevo cálculo
		 select siniestralidad
		   into _siniestralidad
		   from emicartasal2
		  where no_documento = _no_documento;
		  
		 if _fecha_desde >= date('01-11-2018') then
			let _siniestralidad = 101;
		 end if
...
--									trunc(_porc_aumento,0),
--									trunc(_inf_suf_prima,0),
--									trunc(_cambio_edad_t,0)
ELSE
