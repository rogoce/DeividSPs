...
drop procedure sp_rep12b;
create procedure sp_rep12b(a_fecha_desde date, a_dias_vencida smallint)
returning	integer,
			varchar(100);
...
foreach
	select distinct l.no_documento,
	 ...
	 where e.no_poliza = l.no_poliza
	   ...
	   and l.no_letra = 1
	   and l.monto_letra <> 0
	   and abs(l.monto_pag)  between 0 and 1	--antes decia cero cambiado 04/03/2021 --and l.monto_pag = 0
...
--		continue foreach;
		...
		{select fecha_emision
		  into _fecha_emision_ren
		  from endedmae
		 where no_poliza = _no_poliza
		   and no_endoso = '00000'
		   and actualizado = 1;
		
		let _fecha_emision_ren = _fecha_emision_ren - 1 units day;
		let _periodo_ren = sp_sis39(_fecha_emision_ren);

		call sp_cob33d(_cod_compania,'001',_no_documento,_periodo_ren,_fecha_emision_ren)
