...
DROP PROCEDURE sp_pro77j;
CREATE PROCEDURE "informix".sp_pro77j(a_compania CHAR(03), a_terremoto SMALLINT, a_fecha DATE, a_fecha2 DATE, a_ubica CHAR(255) DEFAULT "*", a_serie char(255) default "*")
					...
					BEGIN
						...
						INSERT INTO temp_ubica
						   VALUES(_cod_ubica,
								  ...
								  _suma_facultativo,
								  _cant_fac,
								_prima_cobrada_uni, --_prima_cobrada,  --_prima);								
								_no_unidad,
								0);
					end								  
					--END
...
if _error <> 0 then
	return 	_error_desc,'',_error,0.00,0.00,0,0.00,0,0.00,0,0.00,'','','',v_filtros;		   
end if
-- HG:CASO: 31242
-- Filtro por Serie
IF a_serie <> "*" THEN
...
END IF			 
  ...
  FOREACH WITH HOLD
	  SELECT no_documento,
	   ...
	   FROM temp_ubica
      WHERE seleccionado = 1	   
	 order by cod_ramo,cod_subramo,no_documento, no_unidad   
--	  GROUP BY cod_ramo,cod_subramo
	...
	RETURN v_ramo,
		   ...
		   v_compania_nombre,	
	       v_nodocumento,
	       _no_unidad,v_filtros        with resume;
