...
drop procedure sp_cob385;
create procedure sp_cob385(a_no_remesa char(10)
...
--set debug file to "sp_cob385.trc";
--trace on;
...
let _mes = month(_date_posteo);
let _anio = year(_date_posteo);
...
foreach
	...
	if _tipo_forma = 2 then
		...
		let _fecha_cobro = mdy(_mes,_dia_cobro,_anio);

		if _fecha_cobro >= _date_posteo then
			if _pagada = 0 then
				update cobtacre
				   set excepcion = 1,
					   excep_ini = today,
					   excep_fin = _fecha_cobro - 1 units day
					   user_excepcion = 'DEIVID'
