...
drop procedure sp_pro466;
create procedure sp_pro466(
...
if a_no_documento = '1817-00018-01' then --Poliza pote no entra en el proceso
	...
	if _error = -268 or _error = -239 then 
 		update emicartasal2
 		   set  nombre_cliente = a_nom_cliente,
                ...
                enviado_email    = null,
                fecha_email      = null,
                prima_devengada  = _prima_devengada,
                incurrido_bruto  = _incurrido_bruto,
                siniestralidad   = _siniestralidad,
                fecha_proceso    = current,
                cod_prod_sav 	 = _producto_sav			
...
If _no_poliza is not null then 
	...
	-- Buscando la siniestralidad histórica

    call sp_rec278(a_no_documento) returning _prima_devengada, _incurrido_bruto, _siniestralidad;
	...
	let _sinies = 0;
	let _ducruet = 0;
	let _producto_sav = null;
	let _producto_cav = null;
	
	select nombre
	  into _nom_producto_ant
	  from prdprod
	 where cod_producto = _cod_producto_ant;
	
	if a_fecha_aniv < date('01-11-2018') then
		if _cnt_ducruet > 0 then
			let _ducruet = 1;
		end if
	end if
	
	if _siniestralidad > 100 or a_fecha_aniv >= date('01-11-2018') then
		let _sinies = 1;	
	end if

	let _cod_cob_viaje = null;
	
	select cod_cobertura  --Se busca si el producto actual tiene la cobertura asistencia viaje Casos de Fany 
	  into _cod_cob_viaje
	  from prdcobpd
	 where cod_producto = _cod_producto_ant
	   and cod_cobertura in (select cod_cobertura from prdcober where nombre like "%ASIS%VIAJ%");
	...
	select producto_nuevo,
	 ...
	 where cod_producto = _cod_producto
		and mayor_cien = _sinies
		and ducruet = _ducruet;
		
	if _cod_cob_viaje is not null then
		let _cod_producto_new = _producto_cav;
	end if
