...
--Creado 16/07/2014	Armando Moreno M.


DROP PROCEDURE sp_che115_carga2014;
CREATE PROCEDURE sp_che115_carga2014(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
let a_periodo   = '2014-12';  --****PONER COMENTARIO CUANDO CORRE ANUAL

update parparam
   set agt_per_fidel = a_periodo;

let _fec_aa_ini     = "01/01/2014";	--para sacar cancelada o anulada en el periodo
let _per_ini        = "2014-01";
let _per_ini_ap     = "2013-01";
let _ano            = a_periodo[1,4];  --2014
let _ano            = _ano - 1;		   --2013
let _per_fin_ap     = _ano || a_periodo[5,7]; --2013-12

let _per_fin_dic    = "2013-12";         
...
foreach	with hold
 ...
 select no_documento,
  ...
  begin work;
	...
	insert into tmp_che115a(no_documento, pri_sus_pag, tipo)
...
foreach	with hold
 ...
 select no_documento,
  ...
  begin work;
	...
	insert into tmp_che115a(no_documento, pri_sus_pag_ap, tipo)
...
foreach	with hold
 ...
 select doc_poliza,
  ...
  begin work;
	insert into tmp_che115a(no_documento, sin_pag_aa, tipo)
...
foreach	with hold
 ...
 having sum(variacion) > 0 
   ...
   begin work;
	...
	insert into tmp_che115a(no_documento, sin_pen_ap, tipo)
...
foreach	with hold
 ...
 having sum(variacion) > 0 
  ...
  begin work;
	...
	insert into tmp_che115a(no_documento, sin_pen_aa, tipo)
...
foreach
	...
	if _cod_ramo = '023' then
     ...
     if _cod_ramo in("001","002","020","004","016","012","014","017","010","022","013","003","006","015","005","011","021","009","007",'023') then
	 else
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'RAMO NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
		continue foreach;
     end if
     			   
	 if _cod_ramo = '001' and _cod_subramo = '006' then	 --Incendio y zona libre no va
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'INCENDIO ZONA L. NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
		continue foreach;
	 end if
	...
	-- Excluir Reaseguro Asumido y Coas. Minoritario
	 ...
	 if _cnt > 0 then		-- Excluir Facultativos
		 INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'FACULTATIVOS NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
		 continue foreach;
	 end if
   ...
   if _cnt > 0 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'REHAB. O CANCELADA NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
		continue foreach;
   end if
	...
	foreach			   --PRIMER EMIPOAGT
	  ...
	  where no_poliza = _no_poliza
		...
		SELECT nombre,
		 ...
		 WHERE cod_agente = _cod_agente;
		   ...
		   {	if _agente_agrupado = "01068" then -- Grupo FF Seguros	se quita por inst. Omayra 18/12/2014
			    let _flag = 5;
				exit foreach;
			end if }
	 ...
	 if _flag = 1 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'CED AGENTE = CED PAGADOR NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	 	continue foreach;
	 end if

	 if _flag = 2 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'CED AGENTE = CED CONTRATANTE NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	 	continue foreach;
	 end if

	 if _flag = 3 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'SOLO APLICA TIPO AGENTES',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	 	continue foreach;
	 end if

	 if _flag = 4 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'LICENCIA AGENTE NO ACTIVA NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	 	continue foreach;
	 end if

	 if _flag = 5 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'GRUPO FF SEGUROS NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	 	continue foreach;
	 end if

	 if _flag = 6 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'TECNICA COL VIDA GRUPO SUNCT NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	 	continue foreach;
	 end if

	 if _flag = 7 then
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'DUCRUET AGENCIA 75 RAMO SODA NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	 	continue foreach;
	 end if
	...
	if v_monto_90 > 0 then	 
		let _pri_susc_dev_aa = 0;
		let _pri_susc_aa     = 0;
		INSERT INTO bonibita2(periodo,poliza,descripcion,tipo,cod_agente,cod_ramo,prima_suscrita) VALUES (a_periodo,_no_documento,'MOROSIDAD A 90 NO APLICA',2,_cod_agente,_cod_ramo,_pri_susc_aa);
	end if

	let _pri_susc_dev_aa	 = 0;
	let _pri_susc_dev_ap	 = 0;

	-- Prima Suscrita Devengada	Año Actual 2012
	...
	-- Prima Suscrita Devengada	Año Pasado 2011
