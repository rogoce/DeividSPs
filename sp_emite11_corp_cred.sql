...
drop procedure sp_emite11_corp_cred;

create procedure "informix".sp_emite11_corp_cred() 
...
--	end if
	...
	let _poliza_maestra_auto = '0225-00843-01';
	let _null = null;
	...
	foreach
		select first 20 aseg_primer_nom,
		...
		if _existe = 0 then
			...
			call sp_sis372(_cod_cliente,			--ls_valor_nuevo char(10),				    
						   ...
						   'atencionclientes@corporaciondecredito.com',		   			--ls_email char(50),         			 
			...
			if _error <> 0 then
				...
				update cliclien
				   set --cod_ocupacion	= _cod_ocupacion,
					   direccion_cob	= 'CORPORACION DE CREDITO',
					   digito_ver		= '00'
			...
			update cliclien
			   set --cod_ocupacion	= _cod_ocupacion,
				   direccion_cob	= 'CORPORACION DE CREDITO',
				   e_mail = 'atencionclientes@corporaciondecredito.com'
			 where cod_cliente		= _cod_cliente;
		end if
		...
		if _ramo_sis = 1 then
			...
			elif _cod_producto in ('11250') then --PARTICULAR
		...
		values(_no_poliza,'00001','02260',_suma_asegurada);
		...
		let _prima_comprensi vo = (_subtotal - (_prima_lesiones / 1.55) - (_prima_dpa/ 1.55) - (_prima_asistencia/1.55) - _prima_naviera)/2;
