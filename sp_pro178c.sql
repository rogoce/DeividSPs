...
DROP PROCEDURE sp_pro178c;

CREATE PROCEDURE "informix".sp_pro178c(
		a_compania  CHAR(3),
		a_agencia   CHAR(3),
		a_periodo1  CHAR(7),
		a_periodo2  CHAR(7),
...
DEFINE _monto_total, _monto_total_c     DECIMAL(16,2);
DEFINE _monto_bruto, _monto_bruto_c     DECIMAL(16,2);
...
CREATE INDEX xie01_tmp_inc_cob ON tmp_inc_cob(no_reclamo);
CREATE INDEX xie02_tmp_inc_cob ON tmp_inc_cob(no_reclamo,periodo);
CREATE INDEX xie03_tmp_inc_cob ON tmp_inc_cob(no_poliza);
CREATE INDEX xie04_tmp_inc_cob ON tmp_inc_cob(cod_cobertura);
...
{	foreach
    ...
    FOREACH
		...
		LET _monto_bruto_c = _monto_total_c / 100 * _porc_coas;
...
UPDATE tmp_inc_cob
   SET incurrido_abierto = reserva_bruto + pagado_bruto;
