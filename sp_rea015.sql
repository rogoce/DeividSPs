-- Procedure que genera el saldo por pagar por reasegurador de todas las polizas con saldo al 31/12/2009 para Bouquet.
-- Creado    : 11/03/2010 - Autor: Henry Giron 
-- CON VIGENCIA A LA FECHA INGRESADA
-- SIS v.2.0 - DEIVID, S.A.
  
drop procedure sp_rea015;
create procedure sp_rea015()
...
foreach
 ...
 select no_documento
   ...
   from emipoliza
	...
	foreach
	 ...
	 SELECT d.no_poliza
	  ...
	  WHERE d.cod_compania     = '001'
	    AND d.no_documento     = _no_documento
	    AND (d.vigencia_final  >= '31/03/2010'
	     OR d.vigencia_final   IS NULL)
	    AND d.fecha_suscripcion <= '31/03/2010'
	...
	CALL sp_cob223("001","001", _no_documento, "2010-03", "31/03/2010")

	RETURNING _saldo, _prima_orig;
		...
		foreach
		 select cod_coasegur,
	      ...
	      where cod_contrato   = _cod_contrato
			...
			insert into tmp_ramo
			values (_cod_coasegur, _cod_ramo, 0.00, 0.00, 0.00, 0.00, _monto_reas, 0.00, _no_documento);
			...
			foreach
			 select cod_coasegur,
		      ...
		      where cod_contrato   = _cod_contrato
				...
				insert into tmp_ramo
				values (_cod_coasegur, _cod_ramo, _monto_reas, _comision, _impuesto, _por_pagar, 0.00, 0.00, _no_documento);
