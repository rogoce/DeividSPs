...
-- Creado		: 17/01/2011 - Autor : Henry Giron
-- Modificado	: 21/01/2013 - Autor: Roman Gordon -Redireccionar los montos de los tipo de cuentas '0012','0013','0014','0015','0016','0017','0018','0020','0021','0022','0023','0024' al auxiliar G0001

drop procedure sp_sac218c;
create procedure "informix".sp_sac218c(
a_ano 	  char(4), 
a_mes 	  smallint,
a_nivel	  smallint,
a_db	  char(18),
	 ...
	 select	cta_cuenta,
	  ...
	  if _det_ccosto is null then
		...
		foreach
			...
			if _debito = 0 and _credito = 0 then
				continue foreach;
			end if
			  ...
			  select sld1_tercero,sum(sld1_debitos),sum(sld1_creditos)
			   ...
			   where sld1_cuenta    = _cuenta
				...
				order by 1
					...
					if _tipo in ('0012','0013','0014','0015','0016','0017','0018','0020','0021','0022','0023','0024') then
						let _auxiliar = 'G0001';
					end if
					...
					if _hay = 0 then
						...
						insert into tmp_auxsac218b(cuenta,auxiliar,name_aux,ene,feb,mar,abr,may,jun,jul,ago,sep,oct,nov,dic,total,ter_cedula,rubro,tipo,ccosto)
						values (_cuenta,_auxiliar,_name_aux,_ene,_feb,_mar,_abr,_may,_jun,_jul,_ago,_sep,_oct,_nov,_dic,_total,_ter_cedula,_rubro,_tipo,_det_ccosto);
					end if 

					if _periodo = 1 then
						update tmp_auxsac218b
