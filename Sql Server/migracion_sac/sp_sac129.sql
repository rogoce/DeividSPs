...
drop procedure sp_sac129;

create procedure "informix".sp_sac129(a_ano smallint)
returning integer,
          char(50);

define _cuenta		char(25);
define _riesgo		char(10);
...
foreach
 select cuenta,
   ...
   into _cuenta,
		...
		_monto_nov,
		_monto_dic
  from deivid_tmp:presupue1

	let _cuenta = trim(_cuenta) || trim(_riesgo);
	...
	if _cantidad = 0 then
		return 1, "No Existe la cuenta " || _cuenta;
	end if
