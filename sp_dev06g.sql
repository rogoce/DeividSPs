...
drop procedure sp_dev06g;
create procedure sp_dev06g(a_no_documento char(20),a_fecha_corte date, a_fecha_desde date, a_fecha_hasta date)
returning	smallint		as cod_error,
			varchar(100)	as error,
			decimal(16,2)   as prima_neta_dev,
...
begin
	...
	if _error <> 0 then
		let _mensaje = _mensaje || ' ' || trim(a_no_documento);
		return _error,_mensaje,0,0 with resume;
	end if
	...
	if _tiene_impuesto = 1 then
		...
		let _prima_dev_neta = _prima_devengada / (1 + (_factor_impuesto/100));
	else
		let _prima_cob_dev_neta = _prim_cobrada_bruta;
		let _prima_dev_neta = _prima_devengada;
	end if
drop table if exists tmp_consumo_prima;
return 0,'Actualización Exitosa',_prima_dev_neta,_prima_cob_dev_neta;
end
end procedure;