...
DROP PROCEDURE sp_rwf61;
--DROP TABLE tmp_arreglo;
CREATE PROCEDURE "informix".sp_rwf61(a_incidente int) 
...
DEFINE v_firma            CHAR(25);
DEFINE v_firma2           CHAR(25);
DEFINE v_firma3           CHAR(25);
DEFINE v_descripcion      CHAR(30);
DEFINE v_descripcion2     CHAR(30);
DEFINE v_descripcion3     CHAR(30);
...
CREATE TEMP TABLE tmp_arreglo(
 ...
 SELECT a.cod_ajustador,
  ...
  WHERE a.actualizado = 1
	AND a.tipo_ord_comp = "R"
	AND b.no_tranrec = a.no_tranrec
	AND wf_incidente = a_incidente
 ...
 {SELECT no_tranrec
  ...
  WHERE no_orden = _no_orden
    AND actualizado = 1;}
 	...
 	SELECT no_tramite,	--> Se indica el numero de tramite en vez del numero de reclamo Sabish 17/02/2011
           no_poliza,
		   cod_reclamante,
		   no_motor
   	  INTO v_reclamo,
...
FOREACH WITH HOLD
 SELECT no_reclamo,   
  ...
  ORDER BY cod_proveedor
    ...
    If v_dias_entrega Is null Then
		let v_dias_entrega = 0;
	End If
...
--	 END IF
	CALL sp_rwf60(_no_tranrec) returning v_firma, v_firma2, v_firma3, v_descripcion, v_descripcion2, v_descripcion3;
