...
DEFINE _pagado_ancon, _pagado_bruto, _pagado_contrato, _pagado_otro  DECIMAL(16,2);
...
DEFINE _no_reclamo,_transaccion, _no_tranrec CHAR(10);
...
DEFINE _tipo_transaccion, _cuota_parte SMALLINT;
...
CREATE TEMP TABLE tmp_sinis(
		...
		pagado_bruto         DEC(16,2) NOT NULL,
	  	pagado_ancon         DEC(16,2) NOT NULL,
	  	pagado_contrato      DEC(16,2) NOT NULL,
	  	pagado_otro          DEC(16,2) NOT NULL,
...
FOREACH
 SELECT a.no_reclamo,
  ...
  WHERE a.cod_compania = a_compania
	...
	LET _pagado_bruto   = _monto_bruto;						   -- Esto es el 100%
   ...
   FOREACH
		SELECT recreaco.cod_contrato,recreaco.porc_partic_suma,reacomae.nombre,
		  ...
		  INTO _cod_contrato,_porc_reas,_nombre_contrato,_tipo_contrato
		...
		IF _tipo_contrato = 1 THEN
			LET _pagado_ancon  = _monto_bruto  / 100 * _porc_reas; -- Esto es lo cedido
		ELIF _tipo_contrato = 4 OR _tipo_contrato = 5 THEN
			IF _nombre_contrato = 'CUOTA PARTE' THEN
				LET _pagado_contrato  = _monto_bruto  / 100 * _porc_reas; -- Esto es lo cedido
				LET _cuota_parte = 1;
			ELSE
				LET _pagado_otro  = _monto_bruto  / 100 * _porc_reas; -- Esto es lo cedido
			END IF
		ELIF _tipo_contrato = 5 THEN
			LET _pagado_contrato  = _monto_bruto  / 100 * _porc_reas; -- Esto es lo cedido
		ELIF  _tipo_contrato = 3 OR _tipo_contrato = 2 THEN
			LET _pagado_otro  = _monto_bruto  / 100 * _porc_reas; -- Esto es lo cedido
		END IF
	...
	IF _cuota_parte = 1 THEN
		BEGIN
		ON EXCEPTION IN(-239)

			UPDATE tmp_sinis
			   SET pagado_ancon    = pagado_ancon   + _pagado_ancon,
				   pagado_contrato = pagado_contrato + _pagado_contrato,
				   pagado_otro     = pagado_otro + _pagado_otro,
