...
--3. En el procedimiento sp_che86 que es el que corre diariamente, se borra la tabla hawaii y la tabla fis_concurso. Se carga fis_concurso a partir de fis_concurso1 y sigue el proceso de carga de datos,
...
DROP PROCEDURE sp_che86_sim;
CREATE PROCEDURE sp_che86_sim(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
let _per_fin_ap     = _ano || a_periodo[5,7]; --2017-09
...
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando a Ciclo Prima Pagada este anio...1.', CURRENT);
foreach
	select doc_remesa,
		   sum(prima_neta)
	  into _no_documento,
	...
	foreach
		...
		insert into fis_concursoha(no_documento, pri_pag,tipo,cod_agente)
		values (_no_documento, _monto2, 1, _cod_agente);
	end foreach
end foreach
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Prima Pagada este anio...1.', CURRENT);
--**********************************************************************************************
-- Prima Pagada AÑO PASADO a la fecha
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando a Ciclo Prima Pagada anio pasado...2.', CURRENT);
foreach
	 select no_poliza,
			sum(prima_neta)
	   into _no_poliza,
	  ...
	  where periodo     >= _per_ini_ap		--2016-10
		and periodo     <= _per_fin_ap		--2017-09
		and actualizado = 1
		and tipo_mov    in ("P", "N")
	  group by no_poliza
	...
	let _valor = sp_sis101a(_no_documento,'01/10/2016','30/09/2017',my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
		...
		insert into fis_concursoha(no_documento, pri_pag_ap, cod_agente,pri_can_ap,tipo)
		values (_no_documento, _monto2, _cod_agente,0,2);
	end foreach
end foreach

INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Prima Pagada anio pasado...2.', CURRENT);
--**********************************************************************************************
-- Prima Suscrita ACTUAL periodo del concurso  2017-10 A 2018-09
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Prima Suscrita Actual del concurso...3.', CURRENT);
foreach
	select no_poliza,
		   no_endoso,
		   no_documento,
	...
	foreach
		...
		insert into fis_concursoha(no_documento, pri_sus_pag, cod_agente)
		values (_no_documento, _prima_suscrita2, _cod_agente);
			
	end foreach
end foreach

INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Prima Suscrita Actual del concurso...3.', CURRENT);
--*************************************************************************************************
-- PRIMA SUSCRITA AÑO PASADO PERIODO DEL CONCURSO AL MES ACTUAL
--*************************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Prima Suscrita anio pasado...4.', CURRENT);
foreach
 select no_poliza,
		no_endoso,
		no_documento,
  ...
  where actualizado  = 1
	...
	foreach
		    ...
		    let _prima_suscrita2 = 0.00;
			let _prima_suscrita2 = _prima_suscrita * _porcentaje /100;  
			insert into fis_concursoha(no_documento, pri_can, cod_agente)
			values (_no_documento, _prima_suscrita2, _cod_agente);
			
	end foreach
end foreach

INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Prima Suscrita anio pasado...4.', CURRENT);
--**********************************************************************************************
-- Siniestros Pagados AÑO ACTUAL
--**********************************************************************************************
call sp_rec01(a_compania, a_sucursal, _per_ini, a_periodo) returning _filtros;

INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Siniestros pagados anio actual...5.', CURRENT);
 ...
 select doc_poliza,
   ...
   from tmp_sinis
	...
	foreach
		...
		insert into fis_concursoha(no_documento, sin_pag_aa, cod_agente)
		values (_no_documento, _monto2, _cod_agente);
	end foreach
end foreach
drop table tmp_sinis;

INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Siniestros pagados anio actual...5.', CURRENT);
--**********************************************************************************************
-- Siniestros Pendientes AÑO ACTUAL
--**********************************************************************************************
INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Siniestros pendientes anio actual...6.', CURRENT);
...
foreach 
 ...
 having sum(variacion) > 0 
	...
	foreach
		...
		insert into fis_concursoha(no_documento, sin_pen_aa, cod_agente)
		values (_no_documento, _monto2, _cod_agente);
	end foreach
end foreach

INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Siniestros pendientes anio actual...6.', CURRENT);
---**********************************************************************************************
-- Polizas Nuevas y Renovadas AÑO PASADO inicio del concurso a la fecha
---**********************************************************************************************
call sp_bo077_sim(_fecha_ap_ini, _fecha_ap) returning _error, _error_desc;	-- 01/10/2016   A 30/09/2017	Se pone como estaba, 02/08/2018

INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Nuevas y Renovadas anio pasado...7.', CURRENT);
foreach
		select no_documento,
			   sum(no_pol_nueva),
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Nuevas y Renovadas anio pasado...7.', CURRENT);
----**********************************************************************************************
-- Polizas Nuevas y Renovadas AÑO ACTUAL a la fecha
----**********************************************************************************************
call sp_bo077_sim(_fecha_aa_ini, _fecha_aa) returning _error, _error_desc;	--01/10/2017    -    31/01/2018

INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Nuevas y Renovadas anio actual...8.', CURRENT);
foreach
		select no_documento,
			   sum(no_pol_nueva),
			   sum(no_pol_renov),
...
drop table tmp_persis;
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Nuevas y Renovadas anio actual...8.', CURRENT);
...
INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo Group by Fis_Concurso...9.', CURRENT);
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo Group by Fis_Concurso...9.', CURRENT);
--Aplicar Rango a los corredores tomando como base la Primas Suscrita año pasado periodo del concurso. (2016-10 a 2017-09)

INSERT INTO log_jb (mensaje, hora) values ('Entrando al Ciclo ubicacion de agentes...10.', CURRENT);
...
INSERT INTO log_jb (mensaje, hora) values ('Saliendo del Ciclo ubicacion de agentes...10.', CURRENT);
COMMIT work;
end
return 0;
END PROCEDURE; 