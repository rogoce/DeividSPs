...
DROP PROCEDURE sp_rec709;
CREATE PROCEDURE sp_rec709(a_compania  CHAR(3),a_agencia   CHAR(3),a_periodo1  CHAR(7),a_periodo2  CHAR(7),a_sucursal  CHAR(255) DEFAULT "*",a_grupo CHAR(255) DEFAULT "*",a_ramo CHAR(255) DEFAULT "*",a_agente CHAR(255) DEFAULT "*",a_ajustador CHAR(255) DEFAULT "*",a_evento CHAR(255) DEFAULT "*",a_suceso CHAR(255) DEFAULT "*") RETURNING CHAR(255);
...
FOREACH
 SELECT a.no_reclamo,
  ...
  WHERE a.cod_compania = a_compania
    ...
    AND b.tipo_transaccion IN (4,7)      -- 4 Pago de Reclamo y 7 Pago a Deducible
	...
	LET _monto_facultativo = _monto_bruto   / 100 * _porc_reas_fac;
	LET _monto_neto    = _monto_bruto   / 100 * _porc_reas;
	LET _reserva_bruto = _reserva_total / 100 * _porc_coas;
	LET _reserva_facultativo = _reserva_bruto   / 100 * _porc_reas_fac;
...
UPDATE tmp_sinis
   SET incurrido_total = reserva_total + pagado_total,
       ...
       incurrido_fac = reserva_fac + pagado_fac,
