...
drop procedure sp_bo077_cambio;
create procedure "informix".sp_bo077_cambio(a_fecha_ini date, a_fecha_fin date)
...
foreach
	...
	let _prima_pagada = 0;
	--Determinar si la póliza tiene prima pagada en el ranogo de feha enviado
	select sum(monto)
	  into _prima_pagada
	  from cobredet
	...
	if _prima_pagada is null then
		let _prima_pagada = 0.00;
	end if
	if _prima_pagada > 0 then
	else
		continue foreach;
	end if
