...
-- Modificado: 02/08/2015 - Autor: Federico Coronado

DROP PROCEDURE "informix".sp_pro76c1;

CREATE PROCEDURE "informix".sp_pro76c1(
a_compania      CHAR(50),
a_sucursal      CHAR(50),
a_mes           varchar(20),
a_ano           smallint,
...
define _prima_con_imp			dec(16,2);
define _prima_sin_imp			dec(16,2);
...
WHILE _c_ano <= _tope_ano
	...
	LET _c_ano = _c_ano + 1;
END WHILE;
--set debug file to "sp_pro76c1.trc";
--trace on;

{if a_ano < year(_fecha_hoy)then
	let a_ano = year(_fecha_hoy);
end if
}

delete from enviocartadet where date_added = today;
delete from enviocarta where fecha = _fecha_hoy and enviado = 0;
...
FOREACH    
	...
	--and no_documento    in('1803-00197-01','1804-00412-01')
	...
	--fin pÃ³lizas que vencen el aÃ±o actual
	end foreach
   ...
   --- let _prima_total  = _prima;
	let ld_recargo = 0; -- si el asegurado tenia recargo ya fue insertado en tmp_carta
--Dependientes	
...
-- inicio	  
			IF _edad = _edad_desde THEN 
				LET _prima_depen_r = _prima_depen;
	...
	foreach
		...
		select count(*)
		 ...
		 where no_documento = trim(_no_documento)
		   and fecha = _fecha_hoy;
			if _cnt = 0 then
			  insert into enviocarta(no_documento,
									 fecha,
									 user_added,
									 enviado,
									 periodo)
		...
		foreach	 
			...
			Select sum(factor_impuesto)
			  ...
			  From prdimsub a inner join prdimpue b on a.cod_impuesto = b.cod_impuesto
			 Where cod_subramo = _cod_subramo
			   And cod_ramo = _cod_ramo;
	
			let _factor_impuesto = _factor_impuesto / 100;
			let _prima_sin_imp	= _prima_bruta + ld_recargo;
			let _impuesto		= _prima_sin_imp * _factor_impuesto;
			let _prima_con_imp	= _prima_sin_imp + _impuesto;
...
/*			 --Seleccion de los Asegurados
				...
				if _email is null or trim(_email) = "" then
					let _email_vacio = 0; -- correo vacio
				else 
					let _email_vacio = 1; -- correo lleno
				end if
*/
			let _email_vacio = sp_pro76c31(_no_documento);
			-- _email_vacio 0 = email vacio asegurado
			-- _email_vacio 1 = Correcto
			-- _email_vacio 2 = email mal escrito cliente 
			-- _email_vacio 3 = email mal escrito corredor 
			-- _email_vacio 4 = ambos mal escritos
		/*se modifica la fecha_ani porque es la fecha de renovacion de la poliza y si generan una carta del aÃ±o pasado le coloca la fecha actual de renovacion*/
			if year(_fecha_ani) < year(_fecha_hoy)then
				let _ano_carta = year(_fecha_hoy);
				LET _dia2      		= DAY(_fecha_ani);
				LET _mes2           = MONTH(_fecha_ani);
				LET _fecha_ani = mdy(_mes2,_dia2,_ano_carta);
			end if		
				INSERT INTO enviocartadet(
										no_documento,    
										nombre_cliente,  
										direccion1,      
										direccion2,      
