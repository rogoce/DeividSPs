...
--DROP PROCEDURE sp_pro312bk3;

create procedure sp_pro312bk3()
...
FOREACH
  SELECT a.no_documento,
   ...
   WHERE b.cod_cliente = a.cod_contratante
	...
	if v_cod_subramo in("001","012") then
	else
		continue foreach;
	end if
...
FOREACH
	...
	if v_cod_subramo in("002","012","004","005","003") then
  ...
  SELECT a.no_documento, 
   ...
   WHERE b.cod_cliente = a.cod_contratante
    ...
    IF v_ano_auto IS NULL THEN
		LET v_ano_auto = "";
	END IF
    IF v_cod_subramo in('002','004','003','005') then
		LET v_asistencia = "L";
	ELSE
	    if v_cod_subramo = '001' and v_cod_tipoveh = '005' then
			LET v_asistencia = "C";
		end if
	END IF
