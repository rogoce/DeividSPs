...
DROP PROCEDURE verif_comision;

CREATE PROCEDURE verif_comision(
a_compania 		 CHAR(3), 
a_sucursal 		 CHAR(3),
a_fecha_desde    DATE,
a_fecha_hasta    DATE
...
DEFINE _monto_minimo, _comision, _monto    DEC(16,2);
DEFINE _tipo_pago       SMALLINT; 
DEFINE _primera_q     	SMALLINT;
...
--SET DEBUG FILE TO "verif_comision.trc";
--TRACE ON;
...
FOREACH
 SELECT SUM(comision),
  ...
  GROUP BY cod_agente
	...
	IF v_comis_arrastre = 0 THEN
		CONTINUE FOREACH;
	END IF

	IF v_comis_periodo <> v_comis_arrastre THEN
		CONTINUE FOREACH;
	ELSE
		select sum(monto)
		  into _monto
     	...
     	If _monto <> v_comis_arrastre Then
			delete from agtsalra WHERE cod_agente = _cod_agente;
		Else
			CONTINUE FOREACH;
		End If
	END IF
