...
let _per_ini = "2014-10";

-- Periodo Final del Concurso

if a_periodo > "2015-09" then    --"2015-10"
	let a_periodo = "2015-09";
end if

--let a_periodo = "2014-10"; -- Demetrio Borrar

-- Periodo Pasado
let _ano            = _per_ini[1,4];		  --2014
let _ano            = _ano - 1;				  --2013
let _per_ini_ap     = _ano || _per_ini[5,7];  --2013-10

let _ano            = a_periodo[1,4];		  --2015
let _ano            = _ano - 1;				  --2014
let _per_fin_ap     = _ano || a_periodo[5,7]; --2014-09

-- Diciembre

let _per_fin_dic    = _per_ini[1,4] || "-12"; --2014-12
...
foreach
 select doc_remesa,
   ...
   into _no_documento,
		...
		_renglon
   from cobredet
  where periodo     >= _per_ini				--2014-10
    and periodo     <= a_periodo			--2015-09
	and actualizado = 1
	and tipo_mov    in ("P", "N")
	...
	foreach
			...
			SELECT pagado,
			 ...
			 WHERE no_requis = _no_requis
			   and fecha_impresion between '01/10/2014' and '30/09/2015';
			IF _pagado = 1 THEN
				IF _fecha_anulado IS NOT NULL THEN
					IF _fecha_anulado >= '01/10/2014' and _fecha_anulado <= '30/09/2015' THEN
						LET _monto_dev = 0;
					END IF
				END IF			
			ELSE
...
foreach
 select doc_remesa,
   ...
   into _no_documento,
		...
		_renglon
   from cobredet
  where periodo     >= _per_ini_ap		--2013-10
    and periodo     <= _per_fin_ap		--2014-09
	and actualizado = 1
	and tipo_mov    in ("P", "N")
	...
	insert into tmp_concurso(no_documento, pri_pag_ap,tipo)
	values (_no_documento, _monto,2);
end foreach
...
foreach
	select no_poliza,
	  ...
	  from endedmae
	 where actualizado  = 1
	   and periodo between _per_ini and a_periodo		--2014-10   a   2015-09
...
--**********************************************************************************************
-- Prima Suscrita Anno Pasado fecha del concurso
--**********************************************************************************************
foreach
 select no_poliza,
		no_endoso,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between _per_ini_ap and _per_fin_ap		--2013-10    a     2014-09
  --group by no_poliza,no_endoso,no_documento
...
--**********************************************************************************************
-- Prima Suscrita Anno Pasado todo el año								***************************************************
--**********************************************************************************************
...
--let _fecha_proceso = sp_sis40();
--return 5,_fecha_proceso with resume;
...
{foreach
 select no_poliza,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between '2014-01' and '2014-12'
  --group by no_poliza,no_endoso,no_documento
...
--let _fecha_proceso = sp_sis40();
--return 6,_fecha_proceso with resume;
--**********************************************************************************************
...
foreach 
	 select no_reclamo,		
	  ...
	  where cod_compania = a_compania
		and periodo      <= _per_fin_dic 	--2014-12
		and actualizado  = 1
	  group by no_reclamo
...
call sp_bo077(_fecha_ap_ini, _fecha_ap) returning _error, _error_desc;	--01/10-2013  -  30/09/2014
  ...
  order by no_documento
   ...
   select count(*)
    ...
    where no_poliza     = _no_poliza
      ...
      and fecha_emision >= '01/10/2014'
      and fecha_emision <= '30/09/2015';
...
foreach
	  select cod_agente,
	   ...
	   order by 1
		 ...
		 elif _pri_dev_ap >= 200000 then
			let _nombre_tipo = "Rango 2";
		 elif _pri_dev_ap >= 100000 then
			let _nombre_tipo = "Rango 3";
		 else
			let _nombre_tipo = "Rango 4";
			if _prima_suscrita >= 100000 then	--Corredor de rango 4 y tiene prima suscrita > a 100000 es Rango 3 se puso 14/05/2015
				let _nombre_tipo = "Rango 3";
			end if			
		 end if
...
--let _fecha_proceso = sp_sis40();
return 0;
--fecha_proceso with resume;
{if _pri_dev_ap = 0 and _prima_suscrita >= 100000 then	--Corredor nuevo y tiene prima suscrita > a 100000 es Rango 3
		let _pri_dev_ap = 100000;
end if}	--se pone en comentario 14/05/2015
END PROCEDURE; 