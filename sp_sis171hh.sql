...
drop PROCEDURE sp_sis171hh;

CREATE PROCEDURE "informix".sp_sis171hh(a_no_remesa CHAR(10), _renglon smallint)
...
DELETE FROM camcobreaco WHERE no_remesa = a_no_remesa and renglon = _renglon;
...
FOREACH
   ...
   {	delete from emireaco
     where no_poliza    = _no_poliza
       and cod_contrato = '00631'
       and porc_partic_prima = 50
 	...
 	FOREACH
	  ...
	  GROUP BY no_remesa,renglon,cod_cober_reas
		...
		IF _porcentaje <> 100 THEN
			LET _mensaje = a_no_remesa || '/ ' || _renglon; --'Distribucion de Reaseguro de Prima No Suma 100%, Verifique ... ' || a_no_remesa;
			RETURN 1, _mensaje;
		END IF
	END FOREACH
	...
	IF _contador_ret <> 0 THEN
	   ...
	   FOREACH
		  ...
		  GROUP BY no_remesa,renglon
			...
			IF _porcentaje < 100 THEN
				LET _mensaje = _no_poliza || ' Distribucion de Reaseguro de Facultativos No Suma 100%, Por Favor Verifique ...' || a_no_remesa;
				RETURN 1, _mensaje;
			END IF
	   END FOREACH
