...
-- Modificado	: 20/08/2019	- Autor: Henry Giron  --- Adicion Fecha Cese
-- SIS v.2.0 - DEIVID, S.A.

drop procedure sp_cob103bk_1;
create procedure sp_cob103bk_1(
a_compania 	   char(3),
a_agencia      char(3),
...
returning char(20),  --1  _no_documento 
		  ...
		  smallint,	 --32 saber si la póliza es facultativa
		  char(5),	 --33 cod_grupo
		  char(50),	 --34 nombre del grupo
		  date,		 --35 fecha_cubierto
		  date,		 --36 fecha_cese
		  char(100); --37 contratante
...
{if a_cod_cliente = '27139' then --133' then
...
--if _consulta = 0 then
	foreach						-- Se consulto una poliza por el boton por Llamada y no esta en ninguna camapaña
	   ...
	   --	if _tipo_mov <> "R" then
		...
		let _fecha_cese = sp_sis460(_no_documento);  -- trae fecha de cese
	...
	let _asegurado = null;	 
	foreach
		select first 1 trim(n.nombre)
		  into _asegurado
		  from  emipouni e, cliclien n
		 where e.cod_asegurado = n.cod_cliente
		   and e.no_poliza = _no_poliza
		 order by e.no_unidad
		exit foreach;
	end foreach
	if _asegurado is null then
		select nombre
		  into _asegurado
		  from cliclien
		 where cod_cliente = a_cod_cliente;
		...
		select nombre
		  ...
		  from cliclien
		 where cod_cliente = _cod_contratante;		 
         }   -- DRN indica q es unidad 00001 Jean me lo confirmo que siempre existe y debe ser unidada 00001
		select nombre
		  into _subramo_nom
		  from prdsubra
		 where cod_ramo    = _cod_ramo
