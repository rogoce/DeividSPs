-- Cambio de Forma de  Pago de 006 ANC --> 008 de corredores_remesa  
-- Creado    : 08/03/2019 - Autor: Henry Girón 
-- SIS v.2.0 - DEIVID, S.A. 
-- execute procedure sp_cob420('GISELA'); 

drop procedure sp_cob420;
create procedure sp_cob420(a_usuario char(8),a_cod_agente char(5))
...
--set debug file to "sp_cob420.trc";
--trace on;
...
foreach with hold
	select e.no_documento,e.no_poliza		   
	 ...
	 where e.no_poliza = a.no_poliza	   
	   ...
	   and e.actualizado = 1
	   and a.cod_agente in(_cod_agente)
	 group by 1,2
	...
	call sp_pro531(_no_poliza,a_usuario,_saldo, _cod_compania, _cod_sucursal,_cambio_fp) returning _error,_error_desc;
