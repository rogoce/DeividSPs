...
drop procedure sp_rec02_JB;

create procedure "informix".sp_rec02_JB(
a_compania		char(3), 
a_agencia		char(3), 
a_periodo		char(7),
...
foreach 
---------------------------------------------
	 select a.no_reclamo, b.reserva_tot, a.numrecla 
			into _no_reclamo,
				 _monto_total,
				 _numrecla
	 from rectrmae a  
	 ...
	 (       select no_reclamo,
		 ...
		 where cod_compania	= a_compania
		   ...
		   and actualizado  	= 1
		 group by no_reclamo
		having sum(variacion) <= 0) b    on (a.no_reclamo = b.no_reclamo and a.periodo = a_periodo and a.actualizado = 1 and a.variacion <> 0 and a.numrecla[1,2] in ('02','20','23'))
    group by  a.no_reclamo, b.reserva_tot, a.numrecla
