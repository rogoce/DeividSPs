   -- Proceso que guarda en la tabla histórica del reporte Estadística Men. SuperInt. Por Polizas - Automóvil - Histórico
   DROP procedure sp_pro4970;
   CREATE procedure "informix".sp_pro4970(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
...
--------------------------------------------
	...
	DEFINE _fecha2     	      DATE;
	DEFINE _cnt_prima_nva, _cnt_prima_ren, _cnt_pol_can_cad INTEGER;
	DEFINE _cod_cobertura     CHAR(5);
	DEFINE _opcion            SMALLINT;
	DEFINE _cod_endomov       CHAR(3);
	DEFINE _nueva_renov       CHAR(1);
	DEFINE _no_unidad_uso_auto CHAR(5);
	define _tot_cant_p			INTEGER;
	define _tot_cant_c			INTEGER;
	...
	define _cnt_asegurados_p, _cnt_asegurados_c INTEGER;
...
DELETE FROM esttrp WHERE periodo = a_periodo2;
...
{IF a_periodo >= '2021-05' THEN --sp_pr26k
	CALL sp_pr26h(
	a_cia,
	a_agencia,
	a_periodo,
...
--     WHERE seleccionado = 1 
    ...
    -- La suma asegurada se lee de los endosos que son nuevos y renovados? preguntar el lunes
...
{FOREACH WITH HOLD
   SELECT no_documento,cod_ramo, prima_suscrita, sa_poliza
     INTO _no_documento,_cod_ramo, _total_pri_sus, _sa_poliza
     FROM tmp_poliza	 
	...
	WHILE _3ciclo != 4
	 ...
	 --return _no_documento,_cod_ramo, _total_pri_sus,_sa_poliza with resume; 
		FOREACH WITH HOLD
		   ...
		   {SELECT no_unidad, prima_suscrita, suma_asegurada,uso_auto,cobertura
			 INTO _no_unidad,_unidad_pri_sus, _suma_asegurada,_uso_auto,_cobertura
			 FROM tmp_unidad	 
			WHERE no_documento = _no_documento
			  AND cod_ramo = _cod_ramo}			  			  
             ...
             ELIF _3ciclo = 2 THEN
				  ...
				  let _primera_u = 1;
             ELIF _3ciclo = 3 THEN
                  LET _valor = _suma_asegurada ; --_emipouni_sa;   --_suma_asegurada ; -- 
				  LET _primera_u = 0;
             END IF 
			 let _orden = _3ciclo;
...
--SET DEBUG FILE TO "sp_pro4970.trc"; 
--trace on;
...
{	 IF _orden = 1 THEN
		IF _cnt_prima_nva + _cnt_prima_ren > _tot_cant THEN
			LET _auto_co_p_cant = _auto_co_p_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant);
			LET _sub_p_tot_cant = _sub_p_tot_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant);
			LET _tot_cant = _cnt_prima_nva + _cnt_prima_ren;
		ELIF _cnt_prima_nva + _cnt_prima_ren < _tot_cant THEN --aqui
			LET _auto_co_p_cant = _auto_co_p_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant);
			LET _sub_p_tot_cant = _sub_p_tot_cant + (_cnt_prima_nva + _cnt_prima_ren - _tot_cant);
			LET _tot_cant = _cnt_prima_nva + _cnt_prima_ren;
		END IF		
	 ...
	 IF _orden = 1 THEN
		IF _cnt_prima_nva_p + _cnt_prima_ren_p > _sub_p_tot_cant THEN
			LET _auto_co_p_cant = _auto_co_p_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant);
			LET _sub_p_tot_cant = _sub_p_tot_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant);
			--LET _tot_cant = _cnt_prima_nva_p + _cnt_prima_ren_p;
		ELIF _cnt_prima_nva_p + _cnt_prima_ren_p < _sub_p_tot_cant THEN --aqui
			LET _auto_co_p_cant = _auto_co_p_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant);
			LET _sub_p_tot_cant = _sub_p_tot_cant + (_cnt_prima_nva_p + _cnt_prima_ren_p - _sub_p_tot_cant);
			--LET _tot_cant = _cnt_prima_nva + _cnt_prima_ren;
		END IF		
		IF _cnt_prima_nva_c + _cnt_prima_ren_c > _sub_c_tot_cant THEN
			LET _auto_co_c_cant = _auto_co_c_cant + (_cnt_prima_nva_c + _cnt_prima_ren_c - _sub_c_tot_cant);
			LET _sub_c_tot_cant = _sub_c_tot_cant + (_cnt_prima_nva_c + _cnt_prima_ren_c - _sub_c_tot_cant);
			--LET _tot_cant = _cnt_prima_nva_p + _cnt_prima_ren_p;
...
----------------------------------------------------------------------------------------------		
	 ...
	 IF _orden = 2 THEN
		IF _cnt_asegurados_p > _sub_p_tot_cant THEN
			LET _auto_co_p_cant = _auto_co_p_cant + (_cnt_asegurados_p - _sub_p_tot_cant);
			LET _sub_p_tot_cant = _sub_p_tot_cant + (_cnt_asegurados_p - _sub_p_tot_cant);
		ELIF _cnt_asegurados_p < _sub_p_tot_cant THEN --aqui
			LET _auto_co_p_cant = _auto_co_p_cant + (_cnt_asegurados_p - _sub_p_tot_cant);
			LET _sub_p_tot_cant = _sub_p_tot_cant + (_cnt_asegurados_p - _sub_p_tot_cant);
		END IF		
		IF _cnt_asegurados_c > _sub_c_tot_cant THEN
			LET _auto_co_c_cant = _auto_co_c_cant + (_cnt_asegurados_c - _sub_c_tot_cant);
			LET _sub_c_tot_cant = _sub_c_tot_cant + (_cnt_asegurados_c - _sub_c_tot_cant);
		ELIF _cnt_asegurados_c < _sub_c_tot_cant THEN --aqui
			LET _auto_co_c_cant = _auto_co_c_cant + (_cnt_asegurados_c - _sub_c_tot_cant);
			LET _sub_c_tot_cant = _sub_c_tot_cant + (_cnt_asegurados_c - _sub_c_tot_cant);
		END IF		
		LET _tot_cant = _cnt_asegurados_p + _cnt_asegurados_c;
	 END  IF
...
--execute procedure sp_pro4963('001','001','2021-12', '2021-12')
