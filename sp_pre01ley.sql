...
drop procedure sp_pre01ley;		

create procedure "informix".sp_pre01ley(a_periodo1 char(7), a_periodo2 char(7))
returning char(3),
		  char(50),
		  char(10),
		  char(5),
   ...
   {	if _cod_ramo = "020" then
	...
	foreach
	 ...
	 select	cod_producto,
	  ...
	  -----
		if _cod_ramo = '002' and _cantidad > 5 then --Es colectivo
           let _cod_producto = 'C';
		elif (_cod_ramo = '004' and _cantidad > 10) or (_cod_ramo = '004'and _cod_grupo = '01016') then
           let _cod_producto = 'C';
		elif _cod_ramo = '018' and _cod_subramo = '010' then
           let _cod_producto = 'C';
		end if
