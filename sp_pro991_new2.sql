--execute procedure sp_pro991('001','001','2017-07','2017-07',"*","*","*","*","*","*",'1',"*","*")
...
foreach with hold
	select cod_ramo,
	  ...
	  from temp_det
	 where seleccionado = 1
	 group by cod_ramo,no_factura,no_documento,cod_contratante,no_poliza,no_endoso
	 having sum(prima) <> 0
	...
	if  v_prima_suscrita <> 0 then
		let _parti_reas = (_cedido_total / v_prima_suscrita) * 100;
	else
		let _parti_reas = 0;
	end if
	...
	return	v_cod_ramo,
			...
			v_descr_cia,
			v_filtros,
			_cedido_total, 	--reas cedido total
			_peri,
			_res_comprobante,
			_parti_reas,
			n_contrato,
