...
drop procedure sp_cob245;

create procedure "informix".sp_cob245(
a_compania     char(3), 
a_sucursal     char(3), 
...
FOREACH
 SELECT monto
  ...
  WHERE doc_remesa   = a_no_documento	-- Recibos de la Poliza
    ...
    AND tipo_mov     IN ('P', 'N', 'X')		-- Pago de Prima(P) y Notas de Credito(N), eliminacion de centavo
	AND periodo     <= a_periodo	    -- No Incluye Periodos Futuros
  ...
  WHERE	no_documento   = a_no_documento -- Facturas de la Poliza
	...
	if _estatus_poliza in (2,4) then --Si esta cancelada o anulada se debe, todas las facturas se hacen efectivas desde la vigencia inicial de la póliza
		...
		if _no_endoso = '00000' then
			...
			if _dia < day(_vigencia_inic_end) then
				if _mes = 12 then
					let _mes = 1;
					let _anio = _anio + 1;
				else
			...
			if _mes = 2 and _dia > 28 then
				let _dia = 28;
			elif _mes in (4,6,9,11) and _dia > 30 then
				let _dia = 30;
			end if
...
let v_monto_120 = v_monto_120;
let v_monto_150 = v_monto_150;
let v_monto_180 = v_monto_180;
