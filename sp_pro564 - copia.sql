   DROP procedure sp_pro564;
   CREATE procedure "informix".sp_pro564(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
...
--------------------------------------------
	...
	define _cod_origen, _cod_endomov        CHAR(3);
	DEFINE v_cant_polizas_ma, _cnt_prima_nva, _cnt_prima_ren, _cnt_prima_can, _cnt_pol_dif, _cantidad_aseg, v_cant_asegurados, _cnt_prima_rev, _cnt_prima_end INTEGER;
...
UPDATE ramosubr
   SET prima_suscrita  = 0, cnt_polizas = 0, cnt_reclamo = 0, incurrido_bruto = 0, pago_ded = 0, salv_rec = 0, var_reserva = 0, casos_cerrados  = 0,
	   cnt_polizas_ma  = 0, cnt_pol_nuevas  = 0, cnt_pol_ren = 0, cnt_pol_can_cad = 0, cnt_asegurados = 0;
...
FOREACH                   
 SELECT cod_ramo, total_pri_sus, cnt_prima_nva,	cnt_prima_ren, cnt_prima_can, cnt_prima_rev, cnt_prima_end, no_poliza, no_endoso
   INTO _cod_ramo, _total_pri_sus, _cnt_prima_nva, _cnt_prima_ren, _cnt_prima_can, _cnt_prima_rev, _cnt_prima_end, _no_poliza, _no_endoso
   FROM tmp_prod
  WHERE	seleccionado = 1
  ...
  IF _cnt_prima_can IS NULL THEN
  	LET _cnt_prima_can = 0;
  END IF
  IF _cnt_prima_rev IS NULL THEN
  	LET _cnt_prima_rev = 0;
  END IF
  IF _cnt_prima_end IS NULL THEN
  	LET _cnt_prima_end = 0;
  END IF
    ...
    if _cnt_prima_end = 0 and _cnt_prima_can = 0 and _cnt_prima_rev = 0 then	
		select count(*)
		  into _cantidad_aseg
		  from endeduni
		 where no_poliza = _no_poliza
...
--	if _cnt_prima_can <> 0 or _cnt_prima_rev <> 0 or _cnt_prima_end <> 0 then
--		let _cantidad_aseg = _cantidad_aseg * -1;
--	end if
   ...
   IF _cod_ramo = "019" AND _vig_fin_vida > _vig_ini_end and _nueva_renov = "N" THEN
     UPDATE ramosubr
        SET prima_suscrita = prima_suscrita + _total_pri_sus, cnt_pol_nuevas  = cnt_pol_nuevas + _cnt_prima_nva, cnt_pol_ren = cnt_pol_ren + _cnt_prima_ren, cnt_pol_can_cad  = cnt_pol_can_cad + _cnt_prima_can + _cnt_prima_rev + _cnt_prima_end, cnt_asegurados = cnt_asegurados + _cantidad_aseg
      WHERE cod_ramo     = _cod_ramo
        AND cod_subramo  = "001";
   END IF	

   IF _cod_ramo = "019" AND (_vig_fin_vida <= _vig_ini_end or _nueva_renov = "R") THEN
     UPDATE ramosubr
        SET prima_suscrita = prima_suscrita + _total_pri_sus, cnt_pol_nuevas = cnt_pol_nuevas + _cnt_prima_nva, cnt_pol_can_cad = cnt_pol_can_cad + _cnt_prima_can + _cnt_prima_rev +_cnt_prima_end, cnt_asegurados = cnt_asegurados + _cantidad_aseg
       WHERE cod_ramo     = _cod_ramo
        AND cod_subramo  = "002";
   ...
   IF _cod_ramo = '004' OR _cod_ramo = '018' THEN
	...
	if _cod_ramo = '009' and _cod_subramo in ('004', '008') then --> Se incluye el subramo 008 MARINE CARGO STP
	   ...
	   BEGIN
          ON EXCEPTION IN(-239)
             UPDATE temp_perfil2
                SET prima_suscrita = prima_suscrita + _total_pri_sus, cnt_pol_nuevas = cnt_pol_nuevas  + _cnt_prima_nva, cnt_pol_ren = cnt_pol_ren + _cnt_prima_ren, cnt_pol_can_cad  = cnt_pol_can_cad + _cnt_prima_can + _cnt_prima_rev + _cnt_prima_end, cant_asegurados = cant_asegurados + _cantidad_aseg
               WHERE cod_ramo       = _cod_ramo
          ...
          INSERT INTO temp_perfil2
              VALUES(_cod_ramo,
					 ...
					 _cnt_prima_nva,
					 _cnt_prima_ren,
					 _cnt_prima_can + _cnt_prima_rev + _cnt_prima_end,
					 _cantidad_aseg
                     );
       END
END FOREACH
...
FOREACH
   ...
   let li_anio = year(_vigencia_inic);
	...
	IF _cod_ramo = '004' OR _cod_ramo = '018' THEN
		...
		IF _cantidad > 1 then
			UPDATE ramosubr
			   SET incurrido_bruto = incurrido_bruto + v_incurrido_bruto, salv_rec = salv_rec + _salv_y_recup, pago_ded = pago_ded + _pago_y_ded, var_reserva = var_reserva + _var_reserva
			 WHERE cod_ramo        = _cod_ramo
			   AND cod_subramo     = "002";				   
		ELSE
			UPDATE ramosubr
...
call sp_pro5641(a_periodo, a_periodo2, a_origen);
let _calculo = 0;
{foreach
...
update ramosubr set cnt_polizas = cnt_pol_nuevas + cnt_pol_ren + cnt_pol_can_cad;
update ramootro set cnt_polizas = cnt_pol_nuevas + cnt_pol_ren + cnt_pol_can_cad;
...
FOREACH
    ...
    END IF
       ...
       RETURN  v_desc_subramo, v_cant_polizas, _prima_suscrita, v_desc_ramo, descr_cia, _mes, _orden, v_incurrido_bruto, _cnt_reclamo, _pago_y_ded, _salv_y_recup, _var_reserva, _cnt_cerra, v_cant_polizas_ma,
			   _cnt_prima_nva, _cnt_prima_ren, _cnt_prima_can, v_cant_asegurados WITH RESUME;
END FOREACH
