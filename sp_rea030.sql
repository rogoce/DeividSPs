...
RETURNING CHAR(25),	 -- Cuenta
   ...
   BEGIN
	 ...
	 CALL sp_rec01(a_compania,a_agencia,a_periodo1,a_periodo2,a_codsucursal,a_codgrupo,a_codramo,a_codagente,"*","*","*","*") RETURNING v_filtros; 			    							    
...
FOREACH
 SELECT distinct t.no_poliza, r.no_tranrec
   INTO i_no_poliza, i_no_tranrec
   FROM recrcmae f,tmp_sinis t, rectrmae r
  WHERE f.no_poliza    = t.no_poliza
 	and r.no_reclamo   = f.no_reclamo
    AND f.actualizado  = 1
    AND r.actualizado  = 1
	  ...
	  where no_tranrec = i_no_tranrec
		...
		FOREACH	
				...
				from sac999:reacompasie b, sac999:reacomp c
					...
					and c.no_tranrec    = i_no_tranrec

					LET i_mostrar = "";

					if trim(i_origen) =  "RECLAMOS"  then
						LET i_tipo = 'No. Transaccion';
