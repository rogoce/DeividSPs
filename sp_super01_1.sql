   ...
   DROP procedure sp_super01_1;
   CREATE procedure sp_super01_1(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
...
foreach
	select a.cod_contratante,
	 ...
	 group by a.cod_contratante, a.no_poliza
	having sum(a.prima_suscrita) >= 10000
...
foreach
	...
	insert into temp_contratantep(tipo_persona,origen,cant_polizas,prima_anualizada,es_pep)
	values(_tipo_persona,_cod_origen,_cnt_pol,_prima_suscrita,_cliente_pep);
end foreach
...
foreach
	...
	if _prima_suscrita >= 10000 then
		begin
			...
			insert into temp_asegurado2(tipo_persona,origen,cant_polizas,prima_anualizada,es_pep,cant_aseg,flag_prima)
			values(_tipo_persona,_cod_origen,_cnt_pol,_prima_suscrita,_cliente_pep,1,1);
		end
	else
		if _cliente_pep = 1 then
			begin
				...
				insert into temp_asegurado2(tipo_persona,origen,cant_polizas,prima_anualizada,es_pep,cant_aseg,flag_prima)
				values(_tipo_persona,_cod_origen,_cnt_pol,_prima_suscrita,_cliente_pep,1,0);
			end
		end if
	end if
end foreach
...
foreach
	...
	if _cliente_pep = 1 then 
		insert into temp_pagadorp(tipo_persona,origen,cant_polizas,prima_anualizada,es_pep)
		values(_tipo_persona,_cod_origen,_cnt_pol,_prima_suscrita,_cliente_pep);
	end if
