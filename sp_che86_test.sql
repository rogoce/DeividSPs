...
return 0; --se detuvo la corrida 04/10/2015 Jorge
...
-- Periodo Pasado
let _ano            = _per_ini[1,4];		  --2016
let _ano            = _ano - 1;				  --2015
let _per_ini_ap     = _ano || _per_ini[5,7];  --2015-10

let _ano            = a_periodo[1,4];		  --2017
let _ano            = _ano - 1;				  --2016
let _per_fin_ap     = _ano || a_periodo[5,7]; --2016-01

-- Diciembre

let _per_fin_dic    = _per_ini[1,4] || "-12"; --2016-12

-- Fechas de los Periodos

let _fecha_aa_ini = mdy(_per_ini[6,7], 1, _per_ini[1,4]);        --es del 01/10/2016

let _fecha_ap_ini = mdy(_per_ini_ap[6,7], 1, _per_ini_ap[1,4]);	  --es del 01/10/2015

let _fecha_aa     = sp_sis36(a_periodo);  --31/01/2017
--let _fecha_aa     = "15/10/2014";	 --esto es para extender el concurso -- Demetrio Borrar

let _fecha_ap     = sp_sis36(_per_fin_ap); --31/01/2016
...
tipo                integer,
monto_90_ap         integer     default 0,
monto_90_aa         integer     default 0
) with no log;
...
--return 1,_fecha_proceso with resume;
...
foreach
	select doc_remesa,
	  ...
	  into _no_documento,
		   ...
		   _renglon
	  from cobredet
	 where periodo     >= _per_ini			--2016-10
	   and periodo     <= a_periodo			--2017-01
	   and actualizado = 1
	   and tipo_mov    in ("P", "N")
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/10/2016' and '30/09/2017';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/10/2016' and _fecha_anulado <= '30/09/2017' then
					let _monto_dev = 0;
				end if
			end if			
		else
	...
	insert into tmp_concurso(no_documento, pri_pag,tipo)
	values (_no_documento, _monto,1);
end foreach
--**********************************************************************************************
-- Prima Pagada Anno Pasado a la fecha
--**********************************************************************************************
--let _fecha_proceso = sp_sis40();
--return 2,_fecha_proceso with resume;
...
foreach
 select doc_remesa,
   ...
   into _no_documento,
		...
		_no_poliza
   from cobredet
  where periodo     >= _per_ini_ap		--2015-10
    and periodo     <= _per_fin_ap		--2016-01
	and actualizado = 1
	and tipo_mov    in ("P", "N")
	...
	insert into tmp_concurso(no_documento, pri_pag_ap,tipo,pri_can_ap)
	values (_no_documento, _monto,2,_monto);
end foreach

--let _fecha_proceso = sp_sis40();
--return 3,_fecha_proceso with resume;
--**********************************************************************************************
-- Prima Pagada Anno Pasado periodo del concurso
--**********************************************************************************************
--let _fecha_proceso = sp_sis40();
--return 2,_fecha_proceso with resume;

let _per_siguiente = sp_sis3699(_per_fin_ap); --Trae el siguiente mes para acumular a lo ya grabado en el ciclo anterior
...
--**********************************************************************************************
-- Prima Suscrita Actual periodo del concurso  2015-10 A 2016-09
--**********************************************************************************************
foreach
	select no_poliza,
		   no_endoso,
	  ...
	  from endedmae
	 where actualizado  = 1
	   and periodo between _per_ini and a_periodo		--2015-10   a   2016-01
...
--let _fecha_proceso = sp_sis40();
--return 4,_fecha_proceso with resume;
--**********************************************************************************************
-- Prima Suscrita Anno Pasado periodo del concurso  2014-10 A 2015-09
--**********************************************************************************************
foreach
 select no_poliza,
		no_endoso,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between '2015-10' And '2016-09'  --_per_ini_ap and _per_fin_ap		--2014-10    a     2015-09
...
foreach
 select no_poliza,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between '2015-10' And _per_fin_ap 		-- 2016-01
...
--**********************************************************************************************
-- PRIMA SUSCRITA AÑO PASADO TODO EL AÑO		SP_SIS421						***************************************************
--**********************************************************************************************
...
--let _fecha_proceso = sp_sis40();
--return 7,_fecha_proceso with resume;
--**********************************************************************************************
...
foreach 
	 select no_reclamo,		
	  ...
	  where cod_compania = a_compania
		and periodo      <= _per_fin_dic 	--2016-12
		and actualizado  = 1
	  group by no_reclamo
...
--let _fecha_proceso = sp_sis40();
--return 8,_fecha_proceso with resume;
--**********************************************************************************************
-- Siniestros Pendientes Anno Actual --
--**********************************************************************************************
--cod_compania = a_compania
foreach 
 select no_reclamo,		
        SUM(variacion)
   into _no_reclamo,	
...
---****************************************************************************************************
--_fecha_fin_ap
call sp_bo077(_fecha_ap_ini,'30/09/2016' ) returning _error, _error_desc;	--01/10/2015   A   30/09/2016
...
foreach
 select no_documento,
  ...
  group by no_documento
		...
		if _cod_ramo = '020' or _cod_ramo = '004' then
...
---**********************************************************************************************
-- Polizas Nuevas y Renovadas Anno Pasado inicio del concurso a la fecha
---**********************************************************************************************
--let _fecha_proceso = sp_sis40();
--return 9,_fecha_proceso with resume;

call sp_bo077(_fecha_ap_ini, _fecha_ap) returning _error, _error_desc;	-- 01/10/2015   A 31/01/2016
...
--let _fecha_proceso = sp_sis40();
--return 10,_fecha_proceso with resume;
----**********************************************************************************************
-- Polizas Nuevas y Renovadas Anno Actual a la fecha
----**********************************************************************************************

call sp_bo077(_fecha_aa_ini, _fecha_aa) returning _error, _error_desc;	--01/10/2016    -    31/01/2017
...
--let _fecha_proceso = sp_sis40();
--return 11,_fecha_proceso with resume;
   ...
   if _cnt > 0 then
	...
	foreach
		 ...
		 select cod_agente
		  ...
		  where no_poliza = _no_poliza
{
		 if _cod_agente in('00064') then 		--Unificar Analida Martinelli con Marta Martinelli temporalmente 07/09/2016

		    let _cod_agente = "00063";
...
--let _fecha_proceso = sp_sis40();
--return 12,_fecha_proceso with resume;
--sum(pri_dev_ap),
--Aplicar Rango a los corredores tomando como base la Primas Suscrita año pasado ( 2016)
foreach
	  select cod_agente,
		 sum(pri_dev_aa),   
		 sum(pri_pag_aa),
