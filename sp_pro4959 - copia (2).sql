DROP procedure sp_pro4959;
   CREATE procedure "informix".sp_pro4959(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7), a_periodo2 CHAR(7), a_origen CHAR(3) DEFAULT '%')
...
--SET DEBUG FILE TO "sp_pro4959.trc"; 
--trace on;
...
FOREACH                   
 SELECT cod_ramo, total_pri_sus, no_poliza, no_endoso 
   INTO _cod_ramo_ori, _total_pri_sus, _no_poliza, _no_endoso 
   FROM tmp_prod 
		   ...
		   {SELECT no_unidad, prima_suscrita, suma_asegurada,uso_auto,cobertura
			...
			WHERE no_documento = _no_documento
             ...
             ELIF _3ciclo = 2 THEN
				  ...
				  let _primera_u = 1;
             ELIF _3ciclo = 3 THEN
                  LET _valor = _emipouni_sa;   --_suma_asegurada ; -- 
             END IF 
			 let _orden = _3ciclo;
...
FOREACH WITH HOLD
   SELECT no_documento,cod_ramo
     INTO _no_documento,_cod_ramo_ori
     FROM temp_perfil
	...
	WHILE _3ciclo != 4
	 ...
	 --return _no_documento,_cod_ramo, _total_pri_sus,_sa_poliza with resume; 
		...
		--	let	_tot_monto   = _sub_p_tot_monto + _sub_c_tot_monto ;
			 ...
			 UPDATE tmp_rpt
				SET auto_co_p_cant	=	auto_co_p_cant	+	_auto_co_p_cant	,
