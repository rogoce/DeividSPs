...
LET v_filtros = ap_rec704(a_compania,a_agencia, a_periodo1,a_periodo2,a_sucursal,'*', '002,023,020;','*','*','*','*',a_subramo); 
...
--	   and e.cod_origen = '001'
	   ...
	   and e.cod_subramo = '005'
	 group by r.no_reclamo, r.no_poliza, r.no_unidad
	...
	SELECT count(*)
	  INTO _cnt_unidades
	  FROM endeduni a, endmoaut b, emivehic c, emimodel d 
	 WHERE a.no_poliza = b.no_poliza
	   AND a.no_unidad = b.no_unidad
	   AND a.no_endoso = b.no_endoso
	   AND b.no_motor = c.no_motor
	   AND c.cod_modelo = d.cod_modelo
	   AND a.no_poliza = _no_poliza
	   AND a.no_unidad = _no_unidad
	   AND b.cod_tipoveh = '003'
	   AND d.cod_tipoauto not in (select cod_tipoauto from emitiaut WHERE nombre like '%BUS%');
	   
	IF _cnt_unidades IS NULL THEN
		LET _cnt_unidades = 0;
	END IF		

    IF _cnt_unidades = 0 THEN
		CONTINUE FOREACH;
	END IF	
