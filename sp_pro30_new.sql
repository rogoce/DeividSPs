...
ON EXCEPTION SET _error 
	RETURN _error, 'Error de Base de Datos al Actualizar la Facturacion ...';
END EXCEPTION           

--RETURN 1, 'Modificacion Temporal Hasta Incluir el Programa ...';
...
--TRACE _no_documento;                                                                
   ...
   END FOREACH
	...
	LET _error_desc = 'Error al Insertar Endosos';
	 ...
	 SELECT	cod_impuesto
	  ...
	  WHERE	no_poliza = _no_poliza
		...
		SELECT a.no_poliza,
		  ...
		  INTO s_no_poliza,
		       s_prima_neta
		  FROM emipouni a, emipocob b, prdcobpd c
		 WHERE a.no_poliza = _no_poliza
		   AND a.activo = 1
		   AND b.no_poliza = a.no_poliza
		   AND b.no_unidad = a.no_unidad
		   AND c.cod_producto = a.cod_producto
		   AND c.cod_cobertura = b.cod_cobertura
		   AND c.tiene_impuesto = 1
		 GROUP BY a.no_poliza;


		LET _impuesto = s_prima_neta / 100 * _factor_impuesto;
...
--		END IF
		...
		LET _error_desc = 'Error al Insertar el Impuesto';
	...
	FOREACH
	 SELECT	no_unidad,
	  ...
	  WHERE	no_poliza = _no_poliza
		...
		LET _error_desc = 'Error al Insertar Unidades';
		...
		IF _cod_cobertura IS NOT NULL THEN
			...
			LET _error_desc = 'Error al Insertar Coberturas';
		...
		FOREACH
		 SELECT	orden,
		  ...
		  ORDER BY orden
			...
			LET _error_desc = 'Error al Insertar Contratos';
