--Detalle de Totales de Prima Cobrada (Basado en polizas con contrato de reaseguro especificado)

--Creado por: Armando Moreno
--fecha:      07/08/2012

--execute procedure sp_rea037('001','001','','',"*","*","*","*","*","*","00613","*","*")

DROP PROCEDURE sp_rea037;
CREATE PROCEDURE sp_rea037(a_compania CHAR(03),a_agencia CHAR(03),a_periodo1 CHAR(07),a_periodo2 CHAR(07),a_codsucursal CHAR(255) DEFAULT "*",a_codgrupo CHAR(255) DEFAULT "*",a_codagente CHAR(255) DEFAULT "*",a_codusuario CHAR(255) DEFAULT "*",a_codramo CHAR(255) DEFAULT "*",a_reaseguro CHAR(255) DEFAULT "*",a_contrato CHAR(255) DEFAULT "*",a_serie CHAR(255) DEFAULT "*",a_subramo CHAR(255) DEFAULT "*"	)
RETURNING	char(20),
			date,
			date,
			dec(16,2),
   ...
   BEGIN
      ...
      DEFINE v_noendoso,v_cod_contrato,_cod_contrato CHAR(5);
	 ...
	 CALL sp_rea038(a_compania,a_agencia,a_periodo1,a_periodo2,a_codsucursal,a_codgrupo,
		            a_codagente,a_codusuario,a_codramo,a_contrato) RETURNING v_filtros;
	...
	CREATE TEMP TABLE tmp_ramos
     ...
     CREATE TEMP TABLE temp_produccion
               (cod_ramo         CHAR(3),
				...
				cod_coasegur 	 CHAR(3),
				no_recibo        CHAR(10),
            PRIMARY KEY(cod_ramo, cod_subramo, cod_origen, cod_contrato, cod_cobertura, desc_cob, no_poliza, no_recibo)) WITH NO LOG;
...
CREATE TEMP TABLE tmp_tabla(
		...
		cod_contrato     char(5),
        PRIMARY KEY (no_documento,vigencia_ini,vigencia_fin,cod_ramo,no_recibo)) WITH NO LOG;
...
FOREACH
 ...
 FOREACH
		...
		Select porc_impuesto,
		 ...
		 let _cuenta = sp_sis15("PPRXP", "05", _cod_origen, v_cod_ramo, v_cod_subramo);
...
FOREACH
     ...
     SELECT cod_ramo,		  --SE BUSCA POR POLIZAS
       ...
       FROM temp_produccion
	  where seleccionado = 1
	    and cod_contrato = trim(a_contrato)  
   GROUP BY cod_ramo, no_poliza,no_recibo,cod_contrato
   ORDER BY cod_ramo, no_poliza,no_recibo,cod_contrato
       ...
       FOREACH					 -- SE DESGLOSA POR TIPO, BUSCAR PRIMERO SI ES BOUQUET
		SELECT cod_contrato,
		 ...
		 WHERE cod_ramo  = v_cod_ramo
		   ...
		   and seleccionado = 1
	       and cod_contrato = trim(a_contrato)  
		 GROUP BY cod_contrato,cod_cobertura,tipo,cod_coasegur,serie,no_recibo 
		 ORDER BY cod_contrato,cod_cobertura,tipo,cod_coasegur,serie,no_recibo
...
FOREACH
	 ...
	 SELECT no_documento,
  	   ...
  	   INTO _no_documento,
			...
			v_desc_contrato
	   FROM tmp_tabla
	  WHERE cod_contrato = trim(a_contrato)  
	  ORDER BY cod_ramo

	  let _no_poliza = sp_sis21(_no_documento);

      select cod_contratante ,fecha_suscripcion, cod_subramo
	    into _cod_contratante,_fecha_suscripcion, v_cod_subramo
		from emipomae
