...
-- execute procedure sp_rea24d('001','001','2015-12','2015-12')

drop procedure sp_rea24d;
create procedure 'informix'.sp_rea24d(a_compania char(3), a_sucursal char(3), a_periodo char(7), a_periodo2 char(7))
returning	integer,                  --1
			varchar(100);              --2
...
foreach with hold
	select no_documento
	  into _doc_poliza
	  from emipoliza
	where no_documento in ('0115-00127-01')
	 --= '0816-00273-01' --cod_ramo in ('001','003')
	  --and no_documento = '0102-00365-01'--no_documento not in (select distinct no_documento from rea_saldo2 where periodo = a_periodo2)

	{
	--Pólizas Vigentes al Periodo a procesar.
	select no_documento,
		   no_poliza
	  into _doc_poliza,
		   _no_poliza
	  from emipomae
	 where _fecha between vigencia_inic and vigencia_final}

	begin work;
	
	if _doc_poliza in ('1614-00073-01','0213-01210-01','0215-00495-05') then
		commit work;
		continue foreach;
	end if
	...
	CALL sp_cob245("001","001",	_doc_poliza,a_periodo,_fecha)
			...
			foreach
				...
				if _saldo_contrato = 0 then
					if _cantidad_uni = 1 then
						let _porc_partic_suma = 1/_cnt_reaseg;
					else
						select sum(r.prima)
						  into _saldo_contrato_ac
						  from emifacon r, endedmae e
						 where r.no_poliza = e.no_poliza
						   and r.no_endoso = e.no_endoso
						   and r.no_poliza = _no_poliza
						...
						if _saldo_contrato_ac is null then
							let _saldo_contrato_ac = 0.00;
						end if

						if _saldo_contrato_ac = 0.00 then
							let _porc_partic_suma = 1/_cnt_reaseg;
						else
