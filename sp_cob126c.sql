...
DROP PROCEDURE sp_cob126c;

CREATE PROCEDURE "informix".sp_cob126c(
a_compania 		CHAR(3),
a_agencia 		CHAR(3),
a_periodo 		DATE,
a_sucursal 		CHAR(255),
...
a_monto			INT		  DEFAULT 0)
  RETURNING CHAR(50),  -- Nombre Agente
			...
			CHAR(5),   -- cod_agente
		    DEC(16,2), -- saldo de 91 o mas dias
			INTEGER,
			INTEGER;
...
define v_saldo2			   DEC(16,2);
define _cant_polizas_tot   integer;
...
LET v_filtros = TRIM(v_filtros) || " Saldo Mayor a: " || a_monto;
...
--	let	v_saldo       = 0;
	...
	foreach
		...
		if (a_periodo - _vigencia_inic) >= 90 then
			...
			let v_saldo2      = v_saldo2 + v_saldo1;
		else
			continue foreach;
		end if
	end foreach

  if _cant_polizas > 0 then
  else
	continue foreach;
  end if

  if v_saldo2 > a_monto then
  else
	continue foreach;
  end if
