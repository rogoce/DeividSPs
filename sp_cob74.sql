...
DROP PROCEDURE sp_cob74;		

CREATE PROCEDURE "informix".sp_cob74(
a_compania   CHAR(3),
a_agencia    CHAR(3),
a_fecha      DATE,   
...
define _porc_coaseguro    DEC(7,4);
 ...
 SELECT no_documento
  ...
  GROUP BY no_documento		
	...
	CALL sp_cob74b(
		 ...
		 _periodo,
		 a_fecha
		 ) RETURNING _saldo_tot, --saldo sin imp
					 _saldo_imp; --saldo con imp        
	...
	if _cod_tipoprod = "001" then  --Coas May.
	...
	FOREACH 
	 SELECT	cod_agente,
	  ...
	  WHERE	no_poliza = _no_poliza
		...
		if _cod_tipoprod = '001' then --coas may.
			LET _comis_agt  = _saldo * (_porc_coaseguro / 100) * (_porc_comis_agt/100);
		else
			LET _comis_agt  = _saldo * (_porc_comis_agt/100);
		end if
