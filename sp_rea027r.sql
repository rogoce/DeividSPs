-- Consulta de Movimientos de Cuentas Sac999 x Auxiliar -- Si es produccion 
-- Creado    : 21/04/2010 - Autor: Henry Giron 
-- SIS v.2.0 - DEIVID, S.A.	
-- execute procedure sp_rea027r('sac','01/07/2011','31/12/2011','BQ050')
DROP PROCEDURE sp_rea027r;
CREATE PROCEDURE sp_rea027r(a_db char(18), a_fecha1 date, a_fecha2 date, a_auxiliar CHAR(5) )
...
CREATE TEMP TABLE tmp_aux213r(
		...
		res1_tranrec       char(10),
		res1_desc_rea     char(50), 
		res1_no_documento char(20),
		res1_no_poliza    char(10),
		PRIMARY KEY(res1_notrx,res1_noregistro,res1_cuenta,res1_auxiliar,res1_tranrec)) WITH NO LOG;

CREATE INDEX idx1_tmp_aux213r ON tmp_aux213r(res1_notrx);
CREATE INDEX idx2_tmp_aux213r ON tmp_aux213r(res1_noregistro);
--CREATE INDEX idx3_tmp_aux213r ON tmp_aux213r(res1_linea);
CREATE INDEX idx4_tmp_aux213r ON tmp_aux213r(res1_cuenta);
CREATE INDEX idx5_tmp_aux213r ON tmp_aux213r(res1_auxiliar);
CREATE INDEX idx6_tmp_aux213r ON tmp_aux213r(res1_tranrec);
CREATE INDEX idx7_tmp_aux213r ON tmp_aux213r(res1_no_documento);
CREATE INDEX idx8_tmp_aux213r ON tmp_aux213r(res1_no_poliza);

CREATE TEMP TABLE tmp_sac213r(
		cuenta			char(12),
		comprobante		char(15),
		fechatrx		date,
		no_registro		CHAR(10),
...
call sp_sac211(a_db, a_fecha1 , a_fecha2 , a_auxiliar, "3" ) returning _error, _error_desc;
...
--SET DEBUG FILE TO "sp_sac213r.trc";
--trace on;

FOREACH 
 SELECT distinct f.no_tranrec
   INTO _no_tranrec
   FROM rectrmae f,tmp_aux t
  WHERE f.no_tranrec  = t.no_tranrec
    AND f.actualizado = 1
				...
				ON EXCEPTION IN(-239)
				  UPDATE tmp_aux213r
				   ...
				   WHERE res1_notrx     = i_notrx
					 ...
					 AND res1_tranrec    = _no_tranrec; 
				END EXCEPTION 	


					INSERT INTO tmp_aux213r(
					res1_notrx,
					res1_linea,	    
					res1_cuenta,    
					res1_auxiliar,   
		...
		FOREACH
			select a.res1_auxiliar,
			 ...
			 into r_cod_auxiliar,
				  ...
				  r_debito,
				  r_credito
			 from tmp_aux213r a ,tmp_cglterceros t	 	 
			where a.res1_cuenta = i_cuenta 
			  and a.res1_auxiliar = t.ter_codigo
			  and a.res1_tranrec   = _no_factura	
			  group by 1,2
			order by 1,2
...
DROP TABLE tmp_sac213r;
DROP TABLE tmp_aux213r;
