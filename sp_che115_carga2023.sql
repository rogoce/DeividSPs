...
DROP PROCEDURE sp_che115_carga2023;
CREATE PROCEDURE sp_che115_carga2023(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
{foreach
	select no_documento
	  into _no_documento
      from rentabilidad1
     where cod_agente in ('02264','02427','02570')
       and periodo = '2023-12'
	 
	let _valor = sp_sis101a(_no_documento,'01/01/2023',_fecha_aa, my_sessionid);
	call sp_dev06f_renta(_no_documento,_fecha_aa,'01/01/2023',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
	...
	foreach
		...
		if _no_documento[1,2] = '01' then
			let _monto2 = _monto2 * (70 / 100);
		end if
		if _cod_agente = '02570' then
			let _cod_agente = '02427';
		end if
		begin work;
		
			update rentabilidad1
			   set pri_dev_max_aa = _monto2
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2022',_fecha_ap,my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
