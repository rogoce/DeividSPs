...
--DROP PROCEDURE sp_cob742b;		

CREATE PROCEDURE "informix".sp_cob742b(
a_compania   CHAR(3),
a_agencia    CHAR(3),
a_fecha      DATE,   
...
FOREACH 
 SELECT no_documento
   INTO	_doc_poliza
   FROM emipomae p, emipoagt pa, agtagent a
  WHERE p.cod_compania       = a_compania		   -- Seleccion por Compania
    AND p.actualizado        = 1			   	   -- Poliza este actualizada
	AND (p.cod_tipoprod      = _cod_tipoprod1 OR  -- Coaseguro Mayoritario
	     p.cod_tipoprod      = _cod_tipoprod2)   -- Sin Coaseguro
	and p.no_poliza          = pa.no_poliza
	and pa.cod_agente        = a.cod_agente
	and a.tipo_agente        = "O"
