...
Drop procedure sp_par386; 
CREATE PROCEDURE "informix".sp_par386(  
a_secuencia	INTEGER 
...
define v_siniest_acumulada  dec(16,5);
define v_siniest_acu_salida dec(16,2);
...
let v_siniest_acu_salida = v_siniest_acumulada * 100;  
...
Let  _html_body1 = trim(_html_body1) || '        	Panama, 27 de junio de 2024.';
...
Let  _html_body1 = trim(_html_body1) || '			Nos dirigimos a usted muy respetuosamente en la oportunidad de hacer menci&oacute;n a su p&oacute;liza de salud Nro. '||v_no_documento||', vigente desde '||v_vigenciainicial||' hasta la fecha actual.';

Let  _html_body1 = trim(_html_body1) || '</p>';

Let  _html_body1 = trim(_html_body1) || '        	<p>';

Let  _html_body1 = trim(_html_body1) || '				Durante este per&iacute;odo, hemos tenido el agrado de proporcionarle nuestros servicios y asegurar la protecci&oacute;n que garantiza su p&oacute;liza. Sin embargo, debido a que en los &uacute;ltimos 6 (seis) a&ntilde;os ha presentado reclamos en '||v_anosreclamos||' de ellos con una siniestralidad acumulada de '||v_siniest_acu_salida||'% nos vemos en la necesidad de informarle sobre un ajuste necesario en la prima mensual de su p&oacute;liza.';				

Let  _html_body1 = trim(_html_body1) || '</p>';

Let  _html_body1 = trim(_html_body1) || '</p>';

Let  _html_body1 = trim(_html_body1) || '				<span style="text-decoration: underline;">Este ajuste, que ha sido debidamente autorizado por la Superintendencia de Seguros y Reaseguros de Panam&aacute; mediante	la Resoluci&oacute;n Nro. DRLA-043 de 27 de mayo de 2024</span>, se debe principalmente a tres factores: alta inflaci&oacute;n m&eacute;dica a nivel nacional, el hecho que no hemos ajustado las primas desde el a&ntilde;o 2019 as&iacute; como, y el frecuente y alto consumo de beneficios en su p&oacute;liza.';

Let  _html_body1 = trim(_html_body1) || '</p>';

Let  _html_body1 = trim(_html_body1) || '<p>';

Let  _html_body1 = trim(_html_body1) || '				Por lo tanto, a partir del 01 de agosto del presente a&ntilde;o, la prima mensual de su p&oacute;liza experimentar&aacute; un aumento del ';
...
Let  _html_body2 = trim(_html_body2) ||'				La Red Anc&oacute;n Premier Care es una red premium de alcance nacional en todas las especialidades m&eacute;dicas y cl&iacute;nicas de donde podr&aacute; elegir entre las ';

Let  _html_body2 = trim(_html_body2) ||'				alternativas que considere se adaptan a sus necesidades, y estar&aacute; disponible a partir del 28 de junio en nuestra p&aacute;gina web con la lista de ';
...
Let  _html_body3 = trim(_html_body3) || '				Agradecemos sinceramente su comprensi&oacute;n y deseamos seguir ';

Let  _html_body3 = trim(_html_body3) || '				siendo su aseguradora de confianza en materia de seguros de salud durante muchos a&ntilde;os m&aacute;s. Adem&aacute;s, nos comprometemos a seguir proporcionando la mejor cobertura y respaldo para usted y su familia.';

Let  _html_body3 = trim(_html_body3) || '</p>';

Let  _html_body3 = trim(_html_body3) || '<p>			Reconocemos que cualquier ajuste financiero puede generar preguntas, por lo que estamos disponibles para conversar en conjunto con su corredor cualquier inquietud que pueda tener sobre este cambio, quedamos a su disposici&oacute;n en los siguientes contactos:  ';
