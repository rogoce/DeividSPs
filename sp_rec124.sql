...
DROP PROCEDURE sp_rec124;
CREATE PROCEDURE sp_rec124(a_user char(8), a_flag smallint)
...
define _cod_asignacion2	 char(10);
...
--busca el codigo de ajustador del usuario que entro.
let _es_auditor = 0;
select cod_ajustador,
       es_auditor
  into _cod_ajustador,
...
if _es_auditor = 1 then
	...
	foreach
		...
		update atcdocde
	     ...
	     where cod_asignacion 	  = _cod_asignacion2;
		...
		foreach					  --no hay pendientes, entonces se busca el mas viejo y se devuelve.
				...
				SELECT cod_asignacion,
				 ...
				 WHERE ajustador_asignado = 0
				   ...
				   AND imcs_asignar        = 0
				   AND titulo is not null
				   AND cod_tipo           matches _cod_tipo
				 ORDER BY prioridad desc, date_added
