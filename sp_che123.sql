...
DROP PROCEDURE sp_che123;
CREATE PROCEDURE sp_che123(a_origen char(1), a_pagado  smallint, a_salud smallint default 0) 
...
FOREACH
	...
	IF a_salud = 1 THEN
       if _cod_cliente = '417605' then
		  ...
		  IF _numrecla not in ('18','04') THEN
			continue foreach;
		  END IF
       end if   
	ELSE
		IF _numrecla in ('18','04') THEN
			continue foreach;
		END IF
	END IF
