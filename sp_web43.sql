...
drop procedure sp_web43;
create procedure sp_web43(a_fecha_desde date, a_dias_vencida smallint)
returning	integer,
			varchar(100);
...
--set debug file to "sp_web43.trc";
--trace on;
...
foreach
	select distinct l.no_documento,
		   a_fecha_desde - e.fecha_primer_pago, cod_ramo
	  into _no_documento,
		   _dias_sin_pago,
		   _cod_ramo
	...
	*/
	...
	if _cod_ramo in ('008') then --'004','016','018','019') then --Ramos Personales y Fianzas
		continue foreach;
	end if
