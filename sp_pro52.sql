-- Preliminar de la Facturacion de Salud

-- Creado    : 16/10/2000 - Autor: Demetrio Hurtado Almanza 
-- Modificado: 01/11/2001 - Autor: Demetrio Hurtado Almanza

-- SIS v.2.0 - d_prod_sp_pro52_dw1 - DEIVID, S.A.

DROP PROCEDURE sp_pro52;

CREATE PROCEDURE sp_pro52(
a_compania 		 CHAR(3), 
a_sucursal 		 CHAR(3),
...
-- Periodo para la Morosidad

if month(a_vigencia_hasta) < 10 then
	let _periodo = year(a_vigencia_hasta) || "-0" || month(a_vigencia_hasta);
else
	let _periodo = year(a_vigencia_hasta) || "-" || month(a_vigencia_hasta);
end if
