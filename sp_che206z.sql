...
DROP PROCEDURE sp_che206z;

CREATE PROCEDURE sp_che206z(a_cod_agente char(5))
RETURNING DEC(16,2);
...
define _prima_n,_prima_neta2     DEC(16,2);
...
FOREACH
	...
	if _cod_ramo in("016","004") then	--NO COLECTIVO DE VIDA NI ACCIDENTES PERSONALES
		continue foreach;
	end if
	...
	if _cod_ramo in('002','020') then  --Si es soda o auto y es flota no se toman en cuenta	YA
	...
	FOREACH
	 SELECT	cod_agente,
	  ...
	  WHERE	no_remesa = _no_remesa
	   ...
	   if _cod_agente = a_cod_agente then
	   else
			continue foreach;
	   end if
	  ...
	  let _prima_neta2 = _prima_neta2 + _prima_neta;
