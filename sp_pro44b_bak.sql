...
FOREACH
	SELECT emifafac.cod_cober_reas, emifafac.cod_coasegur, emifafac.porc_partic_reas, 
	  ...
	  INTO ls_cod_cober_reas, ls_coasegur, ld_porc_reas, ld_porc_comision, ld_porc_impuesto, ls_no_cesion,
	       ld_comision, ld_impuesto, ld_suma_rea, ld_prima_rea
      FROM emifafac emifafac, reacobre reacober
     WHERE emifafac.no_poliza = a_poliza
       AND emifafac.no_endoso = a_endoso
	   AND reacobre.es_terremoto <> 1
	GROUP BY emifafac.cod_cober_reas, emifafac.cod_coasegur, emifafac.porc_partic_reas, 
	         emifafac.porc_comis_fac, emifafac.porc_impuesto, emifafac.no_cesion
