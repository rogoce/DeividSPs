-- Cambio de Forma de  Pago de 006 ANC --> 008 de corredores_remesa   MARSH    
-- Creado    : 13/03/2020 - Autor: Henry Girón 
-- SIS v.2.0 - DEIVID, S.A. 
-- execute procedure sp_cob420a('ENILDA'); 

drop procedure sp_cob420a;
create procedure sp_cob420a(a_usuario char(8))
...
foreach with hold
	select e.no_documento ,e.no_poliza --,a.cod_agente
      ...
      from emipomae e, emipoagt a, emipoliza p
	 where e.no_poliza = a.no_poliza
	   and e.no_poliza = p.no_poliza
	   and e.cod_formapag = _cod_formapag
	   and e.estatus_poliza = 1
	   and e.actualizado = 1
	   and a.cod_agente in ('01814','01853','00270')  -- MARSH SEMUSA
       and e.no_documento in (select no_documento from f9_34103 )
	 group by 1,2
  order by 1,2	
