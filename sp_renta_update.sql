...
DROP PROCEDURE sp_renta_update;
CREATE PROCEDURE sp_renta_update(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
define _pri_sus_ap,_Porc_crec      DEC(16,2);
define _cnt             integer;
     ...
     where periodo  = a_periodo
       ...
       and activo   = 1;
		...
		if _pri_susc_aa >= _prim_suscrita_min then
			...
			if  _pri_sus_ap is null or _pri_sus_ap <= 0 then
				if _pri_susc_aa = 0 then
					let _crecimiento = 0;
					let _Porc_crec   = 0;
				else
					let _crecimiento = _pri_susc_aa;
					let _Porc_crec   = 100;
				end if
			else
				let _crecimiento = _pri_susc_aa - _pri_sus_ap;
				let _Porc_crec   = (_crecimiento / _pri_sus_ap) * 100;
			end if

		    if _Porc_crec >= _crecimiento_min then
				let _siniestralidad = 0;
			    if _pri_susc_dev_ap <> 0 then 
					let _siniestralidad = (_sini_incu / _pri_susc_dev_ap) * 100;
				end if	
				let _porcentaje = 0;
