...
drop procedure sp_dev06;
create procedure sp_dev06(a_no_documento char(20),a_fecha_calculo date)
...
if a_no_documento = '0223-00508-07' then
	if _prima_diaria_acum < _monto_cobrado then	
	    let _fecha_inicio = null;
		let _dif_prima = _monto_cobrado; -- - _prima_diaria_acum;
		let _cnt_ajustar = 0;
		select count(*)
		  into _cnt_ajustar
		 ...
		 if _cnt_ajustar > 0 then
			let _ajuste = (_dif_prima/_cnt_ajustar) * -1;
		 else
			--let _ajuste = (_dif_prima/_prima_diaria_acum) * -1;
		end if
		select min(fecha)
		  into _fecha_inicio
		  from tmp_consumo_prima
		...
		while _dif_prima <> 0.00
				...
				if _fecha_inicio is null then
					...
					select prima_diaria
					 ...
					 where no_documento = a_no_documento
					   and fecha        = _fecha_inicio;				
					if _dif_prima > _prima_diaria then
							update tmp_consumo_prima
							   set prima_diaria = prima_diaria + _ajuste
							 where no_documento = a_no_documento		
							   and fecha = _fecha_inicio;
					else
							   let _ajuste = _dif_prima*-1;
							update tmp_consumo_prima
							   set prima_diaria = prima_diaria + _ajuste 
							 where no_documento = a_no_documento		
							   and fecha = _fecha_inicio;				
					...
					let _dif_prima = _dif_prima + _ajuste;
				end if
