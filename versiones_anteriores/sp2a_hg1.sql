...
--
drop procedure sp2a_hg1;
create procedure sp2a_hg1()
...
foreach
	select no_cuenta,no_tran,no_documento
	  into _no_cuenta,_no_tran,_no_documento
   ...
   select upper(nombre)
   into _nombre
   from cliclien where cod_cliente = _cod_pagador;

	return _no_tran,_cod_pagador||'-'||_nombre,null with resume;
end foreach
COMMIT WORK;
return 0,'Exito',null;
