...
--CREATE INDEX i2_no_poliza1 ON tmp_contratos2(cod_contrato);
...
--LET _fecha_emision = null;
					  ...
					  ON EXCEPTION IN(-239)
						 UPDATE tmp_contratos2
						  ...
						  WHERE no_documento  = v_documento
							...
							--cod_contrato = _cod_contrato    and 
	...
	FOREACH
	 SELECT no_unidad,
	      ...
	      INTO _no_unidad,
	        v_documento,
	        --v_desc_contrato, _porc_partic_prima,_porc_partic_suma,
			_psa_retencion,
			_psa_excedente,
			_psa_facultativos,
			_psa_cuota_parte,
	   ...
	   FROM tmp_contratos2
	  WHERE no_poliza = v_nopoliza
	  GROUP BY no_unidad,no_documento --,nombre ,porc_partic_prima,porc_partic_suma
			{psa_retencion,
			psa_excedente,
			psa_facultativos,
			psa_cuota_parte,
			...
			pp_cuota_parte,
			pp_ot_contratos	}  
	  ORDER BY no_documento,no_unidad --,nombre ,porc_partic_prima,porc_partic_suma
		  ...
		  WHERE e.no_documento = v_documento
			...
			and e.fecha_emision  <= a_periodo;	  
             ...
             if _prima_retencion > 0 then
			   let _suma_retencion = _suma ; --round(_suma * (_porc_partic_suma/100),2);	        
			 end if
             if _prima_excedente > 0 then
			   let _suma_excedente = _suma ; --round(_suma * (_porc_partic_suma/100),2);	
			 end if	
             if _prima_facultativos > 0 then
			   let _suma_facultativos = _suma ; --round(_suma * (_porc_partic_suma/100),2);	
			 end if	
             if _prima_cuota_parte > 0 then
			   let _suma_cuota_parte = _suma ; --round(_suma * (_porc_partic_suma/100),2);	        
			 end if
             if _prima_ot_contratos > 0 then
			   let _suma_ot_contratos = _suma ; --round(_suma * (_porc_partic_suma/100),2);	
       ...
       end if	
{
	...
	foreach
		...
		if _no_cambio is null then
			return 1; --,'No Existe emireaco para la Póliza: ' || trim(v_nopoliza) || ' y la Unidad: ' || trim(_no_unidad) with resume
			--continue foreach;
		end if
