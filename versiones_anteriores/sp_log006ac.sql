...
-- execute procedure sp_log006ac("I")  
-- drop procedure pr1009  

drop procedure sp_log006ac;  
create procedure "informix".sp_log006ac(a_estatus char(1) default "I")
             returning char(15),
						char(55),
						char(15),
						date,
...
define _siguiente			smallint;
 define _no_poliza_copia        char(10);
 define _copia int;
...
--set debug file to 'sp_log006_t.trc';
--trace on ;
...
--	   and (d.no_aviso = '01974') -- OR d.no_aviso = '00927')
	...
	foreach
		select user_proceso,
		  ...
		  into _user_proceso,
			   ...
			   _count_no_poliza
		  from avisocanc
		 where  (( estatus in ('I')  and cancela = "0" and imp_aviso_log = '3' )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( estatus in ('X','M')  and cancela = "0" ) and (user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
   --estatus in ('I') --,'X','Y')  -- se a̱adio X y Y ,Henry, solicitud usuario NSOLIS 2/8/2016
		   and no_aviso = (_no_aviso)
 ...
 select  d.no_documento,
	...
	into _no_documento,
		...
		_fecha_imprimir,
         ...
         _estatus
	  from avicanpar a, avisocanc d
     where  (( d.estatus in ('I')  and d.cancela = "0" and d.imp_aviso_log = '3' )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( d.estatus in ('X','M')  and d.cancela = "0" ) and (d.user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
     --d.estatus in ('I')  --,'X','Y')  -- Henry, solicitud  usuario NSOLIS 2/8/2016
     ...
     (( estatus in ('I')  and cancela = "0" and decode(imp_aviso_log,null,0,imp_aviso_log) in (1,2) )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( estatus in ('X','M')  and cancela = "0" ) and (user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
		 --  and estatus in ('I')
		   and saldo > 0
		   and no_aviso = (_no_aviso);
		...
		if _cnt_color <> 0 then
			...
			select count(*)
			  into _cnt_color
			  from avisocanc
			 where  (( estatus in ('I')  and cancela = "0" and decode(imp_aviso_log,null,0,imp_aviso_log) in (0) )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( estatus in ('X','M')  and cancela = "0" ) and (user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry

    --decode(imp_aviso_log,null,0,imp_aviso_log) in (0)
			 --  and estatus in ('I')
			   and saldo > 0
			   and no_aviso = (_no_aviso);
			...
			if _cnt_color = _count_no_poliza then
				...
				select count(*)
				  into _cnt_color
				  from avisocanc
				 where   (( estatus in ('I')  and cancela = "0" and decode(imp_aviso_log,null,0,imp_aviso_log) in (3) )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( estatus in ('X','M')  and cancela = "0" ) and (user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry


     --decode(imp_aviso_log,null,0,imp_aviso_log) in (3)
				   --and estatus in ('I')
				   and saldo > 0
				   and no_aviso = (_no_aviso);
	    ...
	    select sum(saldo)
		  into _sum_saldo
		  from avisocanc
		 where    (( estatus in ('I')  and cancela = "0"  )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( estatus in ('X','M')  and cancela = "0" ) and (user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
   ...
   --estatus in ('I')
	  ...
	  let _cnt_acreedor = 0;
	     -- 20/04/2021 JEPEREZ
	foreach
	select no_poliza --count(*)
	  into _no_poliza_copia --_cnt_acreedor
	  from avisocanc
	 where  (( estatus in ('I')  and cancela = "0"  )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( estatus in ('X','M')  and cancela = "0" ) and (user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry


  --estatus in ('I') and cancela = "0"
...
/******/
		call sp_sis389(_no_poliza_copia) returning _copia; 
		if _copia is null then
			let _copia = 1;
		end if			
		let _cnt_acreedor = _cnt_acreedor + _copia
/******/		   
	   end foreach
   ...
   --estatus in ('I')
	...
	select count(*)
	  ...
	  from avisocanc
	 where no_aviso = (_no_aviso)
     and  (( estatus in ('I')  and cancela = "0"  )  -- Correccion ya que salian incluso sin haber sido impreso 4/8/16 Henry
             or ( estatus in ('X','M')  and cancela = "0" ) and (user_marcar = 'DEIVID') ) --and d.clase in (1) )	   -- Correccion ya que salian incluso sin haber sido impreso 10/01/2020 Henry
   -- and estatus = 'I'
