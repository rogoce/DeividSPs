...
Drop procedure sp_cob778a; 

CREATE PROCEDURE "informix".sp_cob778a(
a_secuencia	INTEGER 
) RETURNING INTEGER; 
...
define _imp_aviso_log2      smallint;
...
--trace on;
 ...
 let _imp_aviso_log2 = 0;
   ...
   -- update parmailsend  set enviado = 1 ,fecha_envio  = date(current) where secuencia = a_secuencia;  
select count(*)
  into _cnt
 ...
 where t.mail_secuencia = p.secuencia             
   ...
   if _cnt > 0  then
	...
	select no_poliza,imp_aviso_log,cod_acreedor
	 ...
	 where no_aviso = _no_aviso
	   ...
	   if _cod_acreedor is null  and _clase = 1 and _imp_aviso_log <> 3  then	   	         
		   update avisocanc
			   set user_imp_aviso_log = 'DEIVID',
				   date_imp_aviso_log = _fecha_quitar,   -- Escenario solo correo JEPEREZ 10032021
				   imp_aviso_log = 3
			 where no_aviso = _no_aviso
			   and renglon = _renglon;		   
	   end if
	   
	   if _cod_acreedor is null  and _clase = 2 and _imp_aviso_log = 3  then	   	         
		   update avisocanc
			   set user_imp_aviso_log = 'DEIVID',
				   date_imp_aviso_log = _fecha_quitar,   -- Escenario solo correo JEPEREZ 10032021
				   imp_aviso_log = 3
...
select count(*)
 ...
 where t.mail_secuencia = p.secuencia             
	...
	select no_poliza, no_documento
	 ...
	 where no_aviso = _no_aviso
	   and renglon = _renglon;	   
		...
		let _msg_gestion   = "CORREO REBOTADO AVISO ENVIADO A IMPRESIÃ“N EN LOGISTICA REF: "||trim(_no_aviso);
   ...
   if _imp_aviso_log2 = 0  then
    let _imp_aviso_log2 = 1;
     set debug file to "sp_cob778t.trc";
      trace on;
        end if
