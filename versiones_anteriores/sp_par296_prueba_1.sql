...
drop procedure sp_par296_prueba_1;
create procedure "informix".sp_par296_prueba_1(a_no_registro char(10))
returning decimal(16,2),
		  char(100),dec(16,2),dec(16,2),dec(16,2),dec(16,2),dec(16,2);
...
on exception set _error, _error_isam, _error_desc
	drop table tmp_reas;
	return _error, _error_desc,0,0,0,0,0;
end exception
...
if _tipo_registro = 2 then -- Cobros
	...
	foreach
		...
		if _bouquet = 1 then
			if _porc_proporcion = 0 then		
				return 1, "% Proporcion es cero para la Rem: " || _no_remesa || " Rengl: " || _renglon,0,0,0,0,0;				 
			end if
	...
	if _cod_ramo in ("001", "003") then
		...
		foreach
			...
			update tmp_reas
			   set prima_rea    = prima_rea + (_prima_tot * _porc_inc) * (_porc_partic_prima/100), porc_inc = _porc_inc
			 where no_unidad    = _no_unidad
			   and cod_contrato = _cod_contrato
			...
			update tmp_reas
			   set prima_rea    = prima_rea + (_prima_tot * _porc_ter) * (_porc_partic_prima/100),porc_ter = _porc_ter
			 where no_unidad    = _no_unidad
			   and cod_contrato = _cod_contrato
			   and porc_partic_prima = _porc_partic_prima
			   and es_terremoto = 1;
		...
		foreach
			...
			update tmp_reas  
			   ...
			   where cod_contrato = _cod_contrato and cod_cober_reas = _cod_cober_reas;
				...
				--call sp_par297(a_no_registro, _cuenta, _debito, _credito, _tipo_comp, _periodo, _centro_costo, _fecha);
				--call sp_par298(a_no_registro, _cuenta, _debito, _credito, _tipo_comp, _cod_auxiliar, _periodo, _centro_costo, _fecha);
				return _monto,_cod_auxiliar,_prima,_monto_reas,_porc_cont_partic,_porc_comis_agt,_factor_impuesto with resume;
			end if
...
return 0, "Actualizacion Exitosa",0,0,0,0,0;
end procedure
