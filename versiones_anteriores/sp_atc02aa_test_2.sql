...
DROP PROCEDURE hp_atc02aa;

CREATE PROCEDURE "informix".hp_atc02aa(a_compania CHAR(3),a_sucursal CHAR(3),a_no_documento CHAR(20),a_ano integer, a_usuario CHAR(10), a_unidad CHAR(255) DEFAULT '*', a_membrete SMALLINT DEFAULT 0, a_monto_uni CHAR(255) DEFAULT '*')
RETURNING	CHAR(20),
			VARCHAR(100), -- PAGADOR
			VARCHAR(30),  -- CEDULA
			...
			decimal(16,2); --				as incurrido_prov;
...
--trace on;
 ...
 SELECT nombre
  ...
  WHERE cod_cliente = _cod_pagador;
   ...
   FROM cliclien
  WHERE cod_cliente = _cod_pagador2;
 ...
 FOREACH WITH HOLD
  ...
  and cod_subramo = _cod_subramo;
	 ...
	 if _ramo_sis <> 5 then		--si no es salud
		...
		select cod_tipotran
		  into _cod_tipotran3 --_cod_tipotran
		  from rectitra
				...
				foreach
					 select fecha,
					  ...
					  where no_reclamo   = _no_reclamo
					    and actualizado  = 1
--						and pagado       = 1
						and (cod_tipotran = _cod_tipotran3 --_cod_tipotran
...
FOREACH	WITH HOLD
		...
		SELECT sum(facturado),
 		 ...
 		 WHERE  year(fecha)  = a_ano
		   and seleccionado = 1
		 --seleccionado = 1                           --year(fecha)  = a_ano
		 GROUP BY no_unidad, cod_asegurado
