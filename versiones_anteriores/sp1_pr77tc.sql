...
DROP PROCEDURE sp1_pr77tc;
CREATE PROCEDURE "informix".sp1_pr77tc(a_compania CHAR(03), a_terremoto SMALLINT, a_fecha DATE, a_fecha2 DATE, a_ubica CHAR(255) DEFAULT "*") 
...
FOREACH
   SELECT d.no_poliza, e.no_endoso, d.no_documento, d.cod_contratante, d.cod_ramo, d.cod_subramo, d.vigencia_final,  d.fecha_cancelacion
    ...
    WHERE d.cod_compania = a_compania
	  ...
	  AND d.no_documento in ('0118-00183-01')
      AND e.actualizado = 1
...
{	foreach
	...
	let _sa_unidad = 0.00;	   
foreach		   
	select u.no_unidad,sum(u.suma_asegurada)
	  into _no_unidad, _sa_unidad
	  from endedmae e, endeduni u   ,endtimov t
	 where e.no_poliza = _no_poliza  
		and e.no_endoso = _no_endoso
		and e.no_poliza = u.no_poliza     
		and e.cod_endomov  = t.cod_endomov
		and e.no_endoso = u.no_endoso
		and e.actualizado = 1
	  group by u.no_unidad 
	  order by 1 desc		   
	  
		if  _sa_unidad is null then
			let _sa_unidad = 0;
		end if	
		
		if _sa_unidad <= 0 then 
			continue foreach;
		end if
