...
CREATE PROCEDURE sp_pro1106(a_fecha date)
RETURNING CHAR(20) as No_Poliza, 	
 ...
 BEGIN
   ...
   -- trace on;
	...
	--CALL sp_pro03('001','001',_fecha_hoy,"018,004;")  RETURNING _filtros;    --vigente al dia de salud. Se agrega ramo 004 a solicitud de Jean Carlos el 23/06/22
	
	--CALL sp_pro03h('001','001',_fecha_hoy,"018,004;")  RETURNING _filtros;  -- Salud -Vencidas   12/07/2022
	---drop table if exists tmp_codigos;
	--CALL sp_sis04('01822,01823,01824,01825,01826,01827,01828,01829,01830,01831,01840,01853;') returning _filtros;	
    ...
    FOREACH
       SELECT distinct y.no_poliza,
         ...
         INTO _no_poliza,
         	  ...
         	  _cod_ramo,
              _contratante			  
         FROM temp_perfil y     WHERE seleccionado = 1  and   y.no_documento = '0422-00029-03';
       -- WHERE seleccionado = 1  				
		...
		if _estatus_poliza in(1,3) then	 --Solo Vigente y Vencidas
			continue foreach;
		end if			 
				   ...
				   SELECT nombre
					...
					WHERE cod_cliente = _contratante;	   										
					 ...
					 FOREACH
						...
						if _cnt_dep > 0 and _cod_cobertura not in ('01831','01822','01825') then	
