...
------------------------------------------------
--execute procedure sp_pr860_3('001','001','2019-08','2019-08',"*","*","*","*","001,003,006,008,010,011,012,013,014,021,022;","*","2019,2018,2017,2016,2015,2014,2013,2012,2011,2010,2009,2008;",'01','*')

drop procedure sp_pr860_3;
create procedure sp_pr860_3(
a_compania		char(03),
a_agencia		char(03),
a_periodo1		char(07),
a_periodo2		char(07),
...
define v_desc_contrato		char(150);
...
--no_poliza			char(10),
primary key(cod_ramo, cod_subramo, cod_origen, cod_contrato, cod_cobertura, desc_cob, serie, desc_contrato)) with no log;
...
create index idx9_temp_produccion on temp_produccion(desc_contrato);
...
let v_prima10 = 0;
let v_prima = 0;
set debug file to "sp_pr860_3.trc";
trace on;
foreach
	select no_poliza,
		   no_endoso,
	...
	foreach
		...
		if _tipo_cont = 0 then
			if _cantidad = 0 then
				...
				let v_desc_contrato  = trim(v_nopoliza) || ": " || trim(v_desc_contrato);
				...
				foreach
					...
					let v_cod_contrato = v_cod_contrato;
			        let _porc_partic_prima = _porc_partic_prima;
			        let _porc_proporcion = _porc_proporcion;
			        let v_cobertura = v_cobertura;
					let v_prima = v_prima;
					let _porc_cont_partic = _porc_cont_partic;
					let _porc_comis_ase = _porc_comis_ase;					
					...
					let _porc_impuesto = _porc_impuesto;
					let _porc_comision = _porc_comision;
...
foreach
	select serie,
	  ...
	  into _serie,
		   ...
		   v_prima 
	  from temphg
	 where cod_coasegur in ('050','063','076','042','036','089','128','117','134','136','141','146','147','149') 
	   and anio      = _anio_reas
	   and trimestre = _trim_reas
	...
	foreach 
		select distinct cod_coasegur,
		  ...
		  from temphg
		 Where serie         = _serie
		   and cod_coasegur in  ('050','063','076','042','036','089','128','117','134','136','141','146','147','149')
