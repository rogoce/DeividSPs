...
drop procedure sp_sis602_t;
create procedure "informix".sp_sis602_t(a_cod_manzana char(255) default '*', a_poliza char(20) DEFAULT '*', a_estatus_poliza smallint default 1, a_endoso char(5), a_unidad char(5),a_suma_asegurada decimal(16,2) default 0.00, a_suma_ret decimal(16,2) default 0.00)
			...
			END FOREACH
{
	   ...
	   END FOREACH
	elif a_estatus_poliza in (2, 3) then -- Cancelada, Vencida
	   FOREACH WITH HOLD
			...
			END FOREACH
{
	   		RETURN _no_poliza, _no_unidad, _no_documento, v_asegurado, _cod_manzana,_referencia,
	          	   _suc_origen,_n_suc_origen,_suma_asegurada,_tipo_incendio,_vig_ini,_vig_fin,_suma_ret, _est_pol, _nombre_ag_acum,_suma_fac,_suma_exc,_ret_porc,_fac_porc,_exc_porc,_prima_suscrita, _coaseguro  WITH RESUME;
}
        insert into temp_sis600
		values( _no_poliza, _no_unidad, _no_documento, v_asegurado, _cod_manzana,_referencia, _suc_origen,_n_suc_origen,_suma_asegurada,_tipo_incendio,
	   		    _vig_ini,_vig_fin,_suma_ret, _est_pol, _nombre_ag_acum,_suma_fac,_suma_exc,_ret_porc,_fac_porc,_exc_porc,_prima_suscrita, _coaseguro);	
...
--	   where cod_manzana = a_cod_manzana;
	...
	if _suma_ret is null Then
	  let _suma_ret = 0.00;
	end if

	select sum(e.suma_asegurada) 
		into a_suma_ret
	from emifacon e, endedmae r , reacomae c, reacobre b
	where e.no_poliza = r.no_poliza
		and e.no_endoso = r.no_endoso
		--   and r.actualizado = 1
		and e.no_poliza = '1107450'
		and e.no_endoso = '00003'
		and e.no_unidad = '00002'
		and e.cod_contrato  = c.cod_contrato
		and c.tipo_contrato = "1"
		and e.cod_cober_reas = b.cod_cober_reas
		and b.cod_ramo in ('001','003')
		and b.es_terremoto = 0;

	if a_suma_ret is null Then
	  let a_suma_ret = 0.00;
	end if
	   
	   let _suma_ret = _suma_ret + a_suma_ret;
...
if _cnt = 0 then   
	...
	select flag,trim(mensaje)
	  ...
	  --   exit foreach;
	--RETURN _flag,_mensaje WITH RESUME;
