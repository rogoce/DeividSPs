   DROP procedure sp_pro178bk1;

   CREATE procedure "informix".sp_pro178bk1(a_cia CHAR(03),a_agencia CHAR(3),a_periodo CHAR(7))
...
{		  
drop table if exists ramozonaext;		  
    CREATE TEMP TABLE ramozonaext (   ---  ramozona
		cod_ramo CHAR(3) NOT NULL, 
		orden SMALLINT NOT NULL, 
		pais_residencia   char(50) default '',
		prima_ext DECIMAL(16,2) default 0, 
		incu_ext DECIMAL(16,2)  default 0,
		PRIMARY KEY(cod_ramo,orden,pais_residencia)) WITH NO LOG;	
}		
	   ...
	   END IF
		...
		BEGIN
          ON EXCEPTION IN(-239, -268)
             UPDATE ramozonaext
                SET prima_ext  = prima_ext + _prima_ext
              WHERE cod_ramo   = _cod_ramo
                 AND orden     = _orden
				 and pais_residencia = _pais_residencia;

          END EXCEPTION
          INSERT INTO ramozonaext(cod_ramo,orden,pais_residencia,prima_ext)
              VALUES(_cod_ramo,_orden,_pais_residencia,_prima_ext);
		END	   
...
--SET DEBUG FILE TO "hg_pro178.trc";
--trace on;
	 ...
	 -- Esta es para la parte de siniestros por sucursal
	   ...
	   END IF
		...
		BEGIN
          ON EXCEPTION IN(-239, -268)
             UPDATE ramozonaext
                SET incu_ext  = incu_ext + _incu_ext
              WHERE cod_ramo   = _cod_ramo
                 AND orden     = _orden
				 and pais_residencia = _pais_residencia;

          END EXCEPTION
          INSERT INTO ramozonaext(cod_ramo,orden,pais_residencia,incu_ext)
              VALUES(_cod_ramo,_orden,_pais_residencia,_incu_ext);
		END	   
...
FOREACH
	...
	IF _cod_cobertura = "00102" OR _cod_cobertura = "00117" THEN
		LET _orden = 1;
