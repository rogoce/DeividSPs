...
a_ano          integer, 
a_usuario      CHAR(10), 
a_unidad       CHAR(8000) DEFAULT '*', 
a_membrete     SMALLINT  DEFAULT 0, 
a_monto_uni    CHAR(8000) DEFAULT '*',
...
drop table if exists  tmp_cod_mt;
							   ...
							   or cod_tipotran  = _cod_tipotran2);	}	
{
						...
						if _cod_tipotran3 = '013' then --Declinar Reclamo
						    if _cod_contratante = _cod_cliente then
								let _no_cubierto_ase = _pago_prov + _monto_no_cubierto +  _ded + _copago + _coaseguro + _ahorro;
							else 
								let _no_cubierto_ase = 0;
							end if
						elif _cod_tipotran3 = '004' and _cod_tipopago = '001' then -- Transacciones de Pago a Proveedor
							let _inc_prov =  _pago_prov + _ahorro + _monto_no_cubierto;
							let _no_cubierto_ase = _ded + _copago + _coaseguro;
						elif _cod_tipotran3 = '004' and _cod_tipopago = '003' then -- Transacciones de Pago a Asegurado
							let _pagado_aseg = _pago_prov;
							let _no_cubierto_ase = _ded + _copago + _coaseguro + _ahorro + _monto_no_cubierto;
						end if			
...
--LET _tipo_m = sp_sis146(a_monto_uni); -- se amplia a 1255
LET _tipo_m = sp_sis146b(a_monto_uni); -- se amplia a 1255

IF trim(a_unidad) <> "*" THEN

		--LET _tipo = sp_sis04(a_unidad);  -- Separa los Valores del String en una tabla de codigos
		LET _tipo = sp_sis04b(a_unidad);  -- Separa los Valores del String en una tabla de codigos		
		...
		IF _tipo <> "E" THEN -- (I) Incluir los Registros
			...
			UPDATE tmp_rec1
			 ...
			 WHERE seleccionado = 1
			   AND trim(no_unidad) NOT IN (SELECT trim(codigo) FROM tmp_codigos);
		...
		ELSE		        -- (E) Excluir estos Registros
			...
			UPDATE tmp_rec1
			   SET seleccionado = 0
			 WHERE seleccionado = 1
			   AND trim(no_unidad) IN (SELECT trim(codigo) FROM tmp_codigos);
	...
	FOREACH	WITH HOLD
		SELECT sum(facturado),
		 ...
		 WHERE year(fecha)  = a_ano
		   and seleccionado = 1
		 GROUP BY no_unidad, cod_asegurado
		 ORDER BY no_unidad, cod_asegurado
		...
		end if
{
        ...
        SELECT monto
		  INTO _prima_depen
		  FROM tmp_cod_mt
		 WHERE trim(codigo) = trim(_no_unidad);
