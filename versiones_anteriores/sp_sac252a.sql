...
RETURNING	INTEGER   as transaccion,     -- numero de transaccion  -- 1
			CHAR(15)  as comprobante,     -- numero de comprobante  -- 2
			DATE      as fecha_registro,  -- fecha de registro      -- 3
			CHAR(3)   as tipo_comprobante,-- tipo de comprobante    -- 4
			DEC(15,2) as debito,          -- monto debito           -- 5
			DEC(15,2) as credito,         -- monto credito          -- 6
			DEC(15,2) as acumulado,       -- acumulado              -- 7
			DEC(15,2) as total,	          -- total                  -- 8
			CHAR(2)   as tipo_ingreso,	  -- tipo de ingreso        -- 9
			CHAR(3)   as centro_costo,	  -- centro de costo        -- 10
			CHAR(50)  as cia_nom,	      -- cia                    -- 11
			CHAR(50)  as cuenta,          -- cuenta                 -- 12
			CHAR(50)  as desc_centro,	  -- centro                 -- 13 
			CHAR(50)  as desc_concepto,	  -- concepto               -- 14
...
SELECT cta_cuenta,cta_nombre
  ...
  FROM cglcuentas
 WHERE cta_cuenta in ('417020105','41701010401')   
