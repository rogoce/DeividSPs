...
if _cnt >= 1 then	
		...
		select cod_ramo, no_documento
		 ...
		 where no_poliza = a_no_poliza ; --HG:27/6/2023 SD#6947 NJURADO 
			...
			if (trim(_no_documento) in ('0223-02223-09','0423-00045-01')) then 
		...
		foreach
			 SELECT (trim(agtagent.nombre)||" "||(case  when agtagent.tipo_agente = 'A' then
					(case  when agtagent.tipo_persona = 'N' then
						  ...
						  (case when _agt_ramo  in ('004','016','018','019','025')  then (case when nvl(pp_lic_vida,'') = '' then no_licencia	else pp_lic_vida end)
								when _agt_ramo = '008' then (case when nvl(pp_lic_fianza,'') = '' then no_licencia	else pp_lic_fianza end)
								when _agt_ramo not in ('004','016','018','019','025','008')  then (case when nvl(pp_lic_general,'') = '' then no_licencia	else pp_lic_general end)
								else  (case when nvl(_agt_ramo,'') = '' then ''	else no_licencia end) end) --licencia						  
