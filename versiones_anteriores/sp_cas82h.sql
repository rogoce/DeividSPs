-- Retorna el Resumen Historico de gestiones CDC por dia en detalle
-- Creado    : 20/06/2023 - Autor:Armando Moreno
--

drop procedure sp_cas82;
create procedure sp_cas82(a_cod_cobrador char(3), a_fecha_desde date, a_fecha_hasta date, a_cod_campana char(10))
returning char(50),
		  char(100),
		  char(10),
...
define _cnt_cascampana	smallint;
define _flag	        smallint;

let _nombre_campana = '';
let _no_documento = '';
let _cnt_cascliente = 0;
let _cnt_cascampana = 0;
let _flag = 0;
...
let a_cod_campana = a_cod_campana;
foreach
	select cod_pagador,
    	   cod_gestion,
		   fecha_ini,
	 ...
	 where date(fecha_ini) between a_fecha_desde and a_fecha_hasta
	   and cod_cobrador in (select cod_cobrador from cobcobra where  tipo_cobrador = 7 and activo = 1 )
       let _flag = 0;
	if _cod_gestion is not null then	--tiempos con gestion
		if _cod_pagador is not null then	--Pagador	
				select first 1 no_documento
				  into _no_documento
				  from cobgesti 
				 where cod_pagador = _cod_pagador and
					   cod_gestion = _cod_gestion and
					   user_added = _user_added and
					   date(fecha_gestion) = date(_fecha_fin);	
					   
				if _no_documento is not null then	--Poliza					   				
					select count(*) --c.cod_campana,c.cod_cliente,g.cod_gestion,c.no_documento
					  into _cnt_cascliente
					  from caspoliza c, cobgesti g
					 where c.no_documento = g.no_documento
					   and c.cod_cliente = g.cod_pagador
					   and c.cod_campana = a_cod_campana
					   and c.cod_cliente = _cod_pagador
					   and g.cod_gestion = _cod_gestion
					   and c.no_documento = _no_documento
					   and g.cod_gestion in (select cod_gestion from cobcages where date(date_added) = '22/08/2023' );
	...
	if _flag = 0 then
		let _cnt_cascampana = 0;
		select count(*)
		  into _cnt_cascampana
		  from cascampana
		 where date(_fecha_fin) between fecha_desde and fecha_hasta;
		 
			if _cnt_cascampana is null then
				let _cnt_cascampana = 0;
			end if			 
		 
			if _cnt_cascampana = 0 then		                                						
				continue foreach;
			end if		 
	end if
	...
	select trim(nombre)||" - "||trim(cod_cobrador),trim(cod_cobrador)
	  into _nombre_cobrador, _cod_cobrador
	  from cobcobra
...
foreach
	select nombre_cobrador, 
	  ...
	  into _nombre_cobrador,
		   ...
		   _no_documento
	  from temp_cobcahis
	 order by fecha_ini	,no_documento 
