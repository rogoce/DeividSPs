...
foreach
		select cod_agente
		 ...
		 select estatus_licencia,tipo_agente
		   into _est_lic,_tip_agt
		   from agtagent
...
if _nueva_renov = 'R' then
	...
	if _cod_ramo in ('002','020','023') and _cod_tipoprod <> '002' then	--se puso exclusion de coas. min por las cargas del estado, Jenniffer de F.
		foreach
			...
			let _nuevo = 0;
			select placa,nuevo,importado,mes_revisado
			  into _placa,_nuevo,_importado,_mes_revisado
			  from emivehic
			...
			if _placa = '' And _mes_revisado = 0 And _nuevo <> 1 And _importado <> 1 then
			else
