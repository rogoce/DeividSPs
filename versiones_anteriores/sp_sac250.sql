...
on exception set _error, _error_isam, _error_desc
	...
	return v_descr_cia,'',a_cuenta,'',0.00,0.00,0.00,_error,'',0,'','','','',trim(_error_desc),'';
end exception
...
if _error <> 0 then
	return v_descr_cia,'',a_cuenta,'',0.00,0.00,0.00,_error,'',0,'','','','','Generacion Prima Suscrita/Siniestro, Error: ' || trim(_error_desc),'';
end if
...
foreach
	...
	if _cnt = 0 then 
		...
		call sp_sac251a(a_periodo1,_cuenta,_cod_auxiliar) returning _error, _error_desc;	
		if _error <> 0 then
