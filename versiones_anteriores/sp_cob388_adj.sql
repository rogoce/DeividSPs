...
drop procedure sp_cob388_adj;
create procedure 'informix'.sp_cob388_adj(a_no_aviso char(10), a_usuario char(8), a_cod_agente char(5)) 
returning	smallint,
			varchar(100);
...
foreach
	select no_poliza,
	 ...
	 and estatus not in ('Y') and clase = 1
	...
	if _email_corr <> '' and _email_corr is not null then			
		...
		let _email_corr = trim(_email_corr); 
		call sp_par316_adj('00011',_email_corr,a_no_aviso,_renglon) returning _error_code,_error_desc; 
