...
DROP PROCEDURE hg_imp02;

CREATE PROCEDURE "informix".hg_imp02(a_poliza CHAR(10), a_unidad CHAR(5),a_general INT)
...
if _cod_ramo <> '002' and _cod_ramo <> '020'  and _cod_ramo <> '023' then
	...
	END IF
          ...
          let _forma_char  = "00000";
	      let _siguiente = a_unidad; 
		IF _siguiente > 9999 THEN 
			LET _forma_char = _siguiente;
		ELIF _siguiente > 999 THEN
			LET _forma_char[2,5] = _siguiente;
		ELIF _siguiente > 99  THEN
			LET _forma_char[3,5] = _siguiente;
		ELIF _siguiente > 9  THEN
			LET _forma_char[4,5] = _siguiente;
		ELSE
			LET _forma_char[5,5] = _siguiente;
		END IF
		LET a_unidad = _forma_char;
		
		  let v_prima = 0;
		  let v_descuen_sal = 0;
		  
	{  FOREACH	  
		 SELECT sum(y.prima_neta), sum(y.prima_anual-y.prima_neta)
			INTO v_prima, v_descuen_sal
			FROM emipouni x, emipocob y
			WHERE y.no_poliza = a_poliza  
			AND x.no_unidad = a_unidad
			AND x.no_poliza = y.no_poliza
			AND x.no_unidad = y.no_unidad}
	  FOREACH
	    SELECT sum(y.prima_neta), sum(y.descuento )
	      INTO v_prima, v_descuen_sal
	      FROM endeduni x, endedcob y --, prdcobpd z
	     WHERE y.no_poliza = a_poliza  
	       AND y.no_endoso = '00000'
           AND x.no_unidad = a_unidad
		   AND x.no_poliza = y.no_poliza
