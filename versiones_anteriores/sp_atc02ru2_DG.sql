...
DROP PROCEDURE sp_atc02ru;


CREATE PROCEDURE "informix".sp_atc02ru(a_compania CHAR(3),a_sucursal CHAR(3),a_no_documento CHAR(20),a_ano integer, a_usuario CHAR(10), a_unidad CHAR(255) DEFAULT '*', a_membrete SMALLINT DEFAULT 0, a_monto_uni CHAR(255) DEFAULT '*')
RETURNING	CHAR(20),
			VARCHAR(100), -- PAGADOR
			VARCHAR(30),  -- CEDULA
...
define _inc_prov            dec(16,2);
define _no_cubierto_ase     dec(16,2);
define _pagado_aseg         dec(16,2);
define _gasto_fact1         dec(16,2);
define _cod_asegurado1      char(10);
...
END FOREACH
		...
		if _pagado_aseg IS NULL THEN
			let _pagado_aseg = 0.00;
		end if
		...
		and no_unidad = _no_unidad;
		 ...
		 SELECT nombre,
		   ...
		   INTO _nombre_asegurado,
				...
				_tipo_persona
		   FROM cliclien
		  WHERE cod_cliente = _cod_asegurado1;
	    ...
	    LET v_fecha_genera = v_fecha_genera + 1 UNITS SECOND;
			...
			RETURN a_no_documento,
				   ...
				   trim(v_cargo),
				   _monto_no_cubierto, --_no_cubierto_ase,
				   _no_poliza,
				   v_fecha_genera,
				   _ded,
				   _copago,
