...
-- SIS v.2.0 - d_cobr_sp_cob748h_dw1 - DEIVID, S.A.	 -- x acreedor 
 ...
 DROP PROCEDURE sp_log015_esp; 
CREATE PROCEDURE "informix".sp_log015_esp(a_compania CHAR(3),a_cobrador CHAR(3) DEFAULT '*',a_tipo_aviso SMALLINT,a_agente CHAR(5) DEFAULT '*',a_acreedor CHAR(5) DEFAULT '*', a_asegurado CHAR(10) DEFAULT '*',a_callcenter SMALLINT DEFAULT 0, a_referencia varchar(255) default "*")
...
FOREACH
  ...
  into  _no_aviso,
    ...
    FROM avisocanc
   WHERE 
   {no_aviso   in ( select codigo FROM tmp_codigos )
