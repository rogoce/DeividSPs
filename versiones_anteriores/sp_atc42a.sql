...
DROP PROCEDURE sp_atc42a;

CREATE PROCEDURE "informix".sp_atc42a(a_cia CHAR(3), a_agencia CHAR(3), a_fecha date, a_tipo_persona CHAR(1), a_opcion SMALLINT default 0)
returning VARCHAR(250) as Contratante,
		...
		VARCHAR(10) as Celular,
           CHAR(10) as cod_contratante,
		    CHAR(5) as codigo_agt,
       VARCHAR(100) as nombre_agt,
	    VARCHAR(50) as email_agt;
...
define _nombre_agt		 VARCHAR(100);
...
CALL sp_pro03(a_cia, a_agencia, a_fecha, '*') RETURNING v_filtros;
 ...
 SELECT temp_perfil.no_poliza,temp_perfil.no_documento,temp_perfil.no_factura,temp_perfil.cod_ramo,temp_perfil.cod_subramo,temp_perfil.cod_sucursal,temp_perfil.cod_grupo,temp_perfil.cod_tipoprod,temp_perfil.cod_contratante,temp_perfil.cod_agente,temp_perfil.prima_suscrita,temp_perfil.prima_retenida,temp_perfil.vigencia_inic,temp_perfil.vigencia_final,temp_perfil.fecha_suscripcion,temp_perfil.usuario,temp_perfil.suma_asegurada,temp_perfil.prima_bruta,temp_perfil.seleccionado
  ...
  WHERE temp_perfil.seleccionado = 1  
   AND (pol.cod_no_renov in ( '039' ) or nvl(pol.cod_no_renov ,'') = '')  --  Pólizas con Estatus VIGENTES y VIGENTES con Motivo de No Renovación 039 - Cese de Coberturas (LEY 68) 
	...
	foreach
		select cod_agente,
		  ...
		  into _cod_agente,
			   _nombre_agt
		  from agtagent
		 where trim(e_mail) = trim(_email_agt)
...
foreach
 select distinct upper(Contratante),
		Identificacion,
		Telefono1,
		Telefono2,
		Telefono3,
 ...
 -- group by 1,2,3,4,5,6 
	...
	foreach
	     select distinct agt.cod_agente,
			   upper(agt.nombre)		
		  into _cod_agente,
			   _nombre_agt
		   FROM temp_perfil temp_perfil
		  ...
		  INNER JOIN emipoagt emi on emi.no_poliza = temp_perfil.no_poliza
		  INNER JOIN agtagent agt on emi.cod_agente = agt.cod_agente
		  WHERE trim(cli.e_mail) = trim(agt.e_mail)
		    AND trim(agt.e_mail) = trim(_email_agt) and  trim(temp_perfil.cod_contratante) = trim(_cod_contratante)
	    -- order by emi.porc_prtic_agt desc
