...
{drop table if exists t_bono_comerc_t;
select *
  from bono_comerc_t
  into temp t_bono_comerc_t;}

delete from bono_comerc_t
 where periodo between _per_ini and _per_fin_aa;
 
delete from fisc_bono_t;
...
foreach
	...
	insert into fisc_bono_t(no_documento, pri_pag_aa,tipo,nueva_renov,periodo)
    values (_no_documento, _monto,_fronting,_nueva_renov,_periodo);
...
foreach
	...
	insert into fisc_bono_t(no_documento, pri_pag_ap,tipo,nueva_renov,periodo)
	values (_no_documento, _monto,_fronting,_nueva_renov,_periodo);	
...
--insert into fisc_bono_t(no_documento, pri_sus_aa,nueva_renov,periodo)
insert into fisc_bono_t(no_documento, pri_sus_aa,nueva_renov,periodo,tipo)
select p.no_documento,
	   b.pbs_nueva_neto,
	   p.nueva_renov,
	   e.periodo,p.fronting
...
{foreach
	...
	insert into fisc_bono_t(no_documento, pri_sus_aa,nueva_renov,periodo,tipo)
	values (_no_documento, _prima_suscrita,_nueva_renov,_periodo,_fronting);
end foreach}

--**********************************************************************************************
-- Prima Suscrita Anno Pasado	META II
--**********************************************************************************************
insert into fisc_bono_t(no_documento, pri_sus_ap,nueva_renov,periodo,tipo)
select p.no_documento,
	   b.pbs_nueva_neto,
	   p.nueva_renov,
	   e.periodo,p.fronting
 ...
 where e.no_poliza = p.no_poliza
   ...
   --and p.cod_ramo <> '008'
   --and p.fronting = 0;
--insert into fisc_bono_t(no_documento, pri_sus_ap,nueva_renov,periodo)
...
call sp_rec01(a_compania, a_sucursal, _per_ini, _per_fin_aa,'*','*','*') returning _filtros;
...
foreach
	...
	insert into fisc_bono_t(no_documento, sin_pag_aa,nueva_renov,periodo,tipo)
	values (_no_documento, _sin_pag_aa,_nueva_renov,_per_fin_aa,_fronting);
...
foreach 
	...
	insert into fisc_bono_t(no_documento, sin_pen_ap,nueva_renov,tipo)
	values (_no_documento, _sin_pen_dic,_nueva_renov,_fronting);
end foreach
...
{	if _cod_ramo = '008' then 
	...
	insert into fisc_bono_t(no_documento, sin_pen_aa, nueva_renov, tipo)
	values (_no_documento, _sin_pen_aa, _nueva_renov, _fronting);
...
foreach
	select no_documento,
	  ...
	  into _no_documento,
		   ...
		   _no_pol_ren_ap_per
	  from fisc_bono_t	 
	 group by no_documento,nueva_renov, tipo
	 order by no_documento,nueva_renov, tipo

	let _no_poliza = sp_sis21(_no_documento);

	select cod_grupo,
		   cod_ramo, cod_subramo,
		   cod_contratante,
		   cod_tipoprod
	  into _cod_grupo,
		   _cod_ramo,_cod_subramo,
		   _cod_contratante,
		   _cod_tipoprod
	  from emipomae
	 where no_poliza = _no_poliza;
	...
	if (_cod_ramo in ('018') and _cod_subramo in ('012')) or _cod_ramo in ('001','003')  then -- condicion nueva para crecimiento 30%
		let _agrupado = '30P';
		--let _nombre_ramo = 'Ramos en Conjunto 30%';				
	else
		let _agrupado = _cod_ramo;		
	end if
	...
	let _flag = 0;
	let _error = sp_sis101a(_no_documento,_fecha_aa,_fecha_aa,my_sessionid);	--Crea tabla con el corredor con_corr  --ult.cambio:08/05/2019
