...
--			LET v_descuento = (v_prima_uni * v_porcentaje / 100) + v_descuento;
		...
		if _cod_ramo = '002' or _cod_ramo = '020'  or _cod_ramo = '023' then
			...
			{if _cod_descuen <> '001' then   --SD#10165 BOLIVAR 26042024 SOLO 002-AUTO
				RETURN
					v_nombre,
...
if _cod_ramo = '002' or _cod_ramo = '020'  or _cod_ramo = '023' then
      ...
      let v_descuen_sal = 0;
	   ...
	   FOREACH	  
		select a.cod_descuen,y.nombre,y.orden,sum(a.prima),sum(a.descuento)
		  into _cod_descuen,v_nombre,v_orden,v_prima, v_descuen_sal
		  from temp_new a, emidescu y
		 where a.cod_descuen = y.cod_descuen
		   and a.no_poliza = _no_poliza 
		   and a.no_unidad = _no_unidad
		   and y.cod_descuen in ('010','011')
		 group by a.cod_descuen,y.nombre,y.orden
		 ORDER BY y.orden
