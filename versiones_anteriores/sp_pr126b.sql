...
-----------------------------------------------------------
drop procedure sp_pr126b;
create procedure sp_pr126b(
a_compania		char(3),
a_agencia		char(3),
a_periodo1		char(7),
a_periodo2		char(7),
...
if a_contrato <> "*" then
	...
	let _tipo2 = sp_sis04(a_contrato); -- separa los valores del string
...
if a_serie <> "*" then
	...
	let _tipo2 = sp_sis04(a_serie); -- separa los valores del string
	
		select cod_contrato
		  from tmp_sinis
		 where seleccionado = 1
		  into temp tmp_cont_reas;	
	...
	if _tipo2 <> "E" then -- incluir los registros
		...
		update tmp_sinis
		   set seleccionado = 0
		 where seleccionado = 1
		   and cod_contrato not in (
		       select b.cod_contrato from tmp_codigos a,tmp_cont_reas b,reacomae c 
		        where c.serie = a.codigo and b.cod_contrato = c.cod_contrato and c.tipo_contrato in (5,7));	   
	...
	drop table tmp_codigos;
	drop table tmp_cont_reas;
end if

let _filtros = trim(_filtros)||" "|| trim(v_filtros1);
/****************/
...
FOREACH 
	select cod_ramo,
     ...
     Where cod_coasegur in("030","051","072","042","034","063",'036','117')
	   and seleccionado = 1 -- 02122020 AMORENO
...
drop table if exists tmp_codigos_serie;
