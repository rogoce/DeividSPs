...
DROP PROCEDURE sp_che95cc;
CREATE PROCEDURE sp_che95cc(a_cia CHAR(3),a_cod_agente CHAR(5) default "*",a_periodo char(7), a_tipo_pago smallint) 
RETURNING CHAR(50),CHAR(100),CHAR(5),CHAR(50),CHAR(1),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,4),DEC(16,4),CHAR(7),CHAR(1),DEC(16,2),DEC(16,2);
 -- v_nombre_cia,
 -- nombre_agente,
 -- cod_agente,
 -- nombre_tipo,
...
DEFINE _pri_susc_dev_aa,_comision  	DEC(16,2);
DEFINE _pri_susc_dev_ap,_prima_cobrada  	DEC(16,2);
...
if a_tipo_pago = 1 then
	...
	insert into chqrenta030212c(
		   ...
		   pri_susc_dev_aa,
		   pri_susc_dev_ap)
select distinct v_nombre_cia,
	       rentabilidad1.periodo,
		   trim(rentabilidad1.no_documento),
	       trim(rentabilidad1.cod_agente),
	       trim(rentabilidad1.n_agente),
	       trim(rentabilidad1.tipo),
		   trim(rentabilidad1.nombre_ramo),
	       rentabilidad1.pri_susc_ap,
	       rentabilidad1.pri_susc_aa,
		   n_cliente,
		   rentabilidad1.cod_ramo,
		   rentabilidad1.monto_90,
		   rentabilidad1.sini_inc,
		   rentabilidad1.pri_susc_dev_aa,
		   rentabilidad1.pri_susc_dev_ap --,chqrenta3.tipo_requis,rentabilidad1.cod_agente
	  from rentabilidad1
   inner join chqrenta3 on  (rentabilidad1.periodo = chqrenta3.periodo  
   and rentabilidad1.cod_agente = chqrenta3.cod_agente   
   and rentabilidad1.periodo  = a_periodo
   and chqrenta3.tipo_requis matches _tipo_pago_c);
...
FOREACH
	...
	order by 1,2,3,4,5,6,7,8 desc
			...
			if _pri_susc_dev_aa is null then
				let _pri_susc_dev_aa = 0.00;
			else
				let _prima_aplica = _pri_susc_dev_aa;
			end if
			...
			if _prima_aplica is null then
				let _prima_aplica = 0;
			end if
			if _pri_susc_dev_ap is null then
				let _pri_susc_dev_ap = 0;
			end if

		 insert into tmpche95c(prima_sus_dev,prima_cobrada,cia,asegurado,poliza,prima_suscrita,monto_90,prima_exc_monto90,reserva,prima_exc_reserva,siniestro,categoria,ramo,agente,cod_tipo,cod_agente,prima_ant)
		 values (_pri_susc_dev_ap,_prima_cobrada,v_nombre_cia,_nombre_cliente,_no_documento,_prima_aa,_monto_90,_prima_exc_m90,_reserva,_prima_aplica,_sini,_nombre_tipo,_nombre_ramo,_nombre_agente,_cod_tipo,_cod_agente,_prima_ant);			 
...
FOREACH
	select cia,
		   ...
		   sum(prima_suscrita),
		   sum(monto_90),
		   sum(prima_exc_monto90),
		   sum(reserva),
		   sum(prima_exc_reserva),
		   sum(siniestro),
		   sum(prima_ant),
		   sum(prima_cobrada),
		   sum(prima_sus_dev)
	...
	if _pri_susc_dev_ap <> 0 then
		let _siniestralidad = (_sini / _pri_susc_dev_ap) * 100;
	end if
	...
	if _porcentaje >= 0 then
		let _valor_prima = _pri_susc_dev_ap * ( _porcentaje / 100);	--_pri_susc_dev_ap tiene la prima suscrita devengada.
		...
		foreach
			select sum(comision)
			  ...
			  from chqrenta3
			 where periodo = a_periodo
			   and tipo_g = _cod_tipo
			   and cod_agente = _cod_agente
			exit foreach;
		end foreach
		if _comision is not null and _comision <> 0 then
			let _valor_prima = _comision;
		end if

		RETURN  v_nombre_cia,	  	--1
				_nombre_agente,		--2
				_cod_agente,		--3
				_nombre_tipo,		--4
				_cod_tipo,			--5
				_prima_aa,			--6
				_monto_90,			--7
				_prima_exc_m90,		--8 PRIMA SUSCRITA AA
				_reserva,			--9
				_prima_aplica,		--10
				_sini,				--11
				_prima_ant,			--12 PRIMA SUSCRITA AP
				_crecimiento,		--13
				_Porc_crec,      	--14
				_siniestralidad, 	--15
				_porcentaje,     	--16
