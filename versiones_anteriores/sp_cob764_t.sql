...
-- Modificado: Amado Perez M. 16/01/2014 -- se retorna el codigo de estafeta para que se pueda actualizar en el modulo de aviso de cancelación .
...
foreach
	...
	foreach
		select cod_cobrador
		  into _cobrador
		  from cobcobra
		 where usuario = _usuario2   -- a_usuario
		   and activo = '1'
	     order by 1 asc
		  exit foreach;
	end foreach
	...
	foreach
		...
		if _saldo_c = 0 and a_tab <> 7 then
			continue foreach;
		end if	
...
--				   and saldo < _exigible;
			...
			if _hay_pago >= 1 or _exigible_c <= 0 then
				...
				if (_saldo <= _saldo_sin_mora and abs(_saldo - _saldo_sin_mora) <= 5.00) and a_tab <> 7 then
					continue foreach;
				else
		...
		if a_tab = 1 then      -- x corredores
			...
			{if _no_aviso = '00891' and _no_poliza not in ('136755','201942','342030','517853','516308','835925','833398') then
				continue foreach;
			end if}
		  ...
		  select fecha_adicion 
		    into _fecha_vence
		    from cobccert0
           where numero = _reporte_certifica;
		   
		     --if isnull(_fecha_vence) then
		      --     let _fecha_vence = date('00/00/0000');
			  --end if
		
				 if _nombre_acreedor <> '... SIN ACREEDOR ...' and _nombre_acreedor is not null then 
						if _imp_aviso_log = 3 then
							let _impreso = 1;
						else
							let _impreso = 0;
			...
			if a_proceso = 3 then	        --  Última Gestión
				if _estatus not in ("X") Then
					continue foreach;
				end if
		...
		elif a_tab = 8 then     -- X seleccionar cancelado	  sp_cob753 X - A cancelar
			--let _marcar_entrega = 1;
		...
		let _direccion = trim(_direccion_1) || " " || trim(_direccion_2);
		 ...
		 call sp_cob782(_no_poliza,_renglon,_no_aviso,'C') returning _ult_gestion; --_estatus;
		 
		 if _ult_gestion is null then 
		     let _ult_gestion = 'I';  --estatus
		 end if
