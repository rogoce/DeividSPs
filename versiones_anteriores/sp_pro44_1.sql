...
DROP PROCEDURE sp_pro44_2;
--DROP TABLE tmp_arregloxls;

CREATE PROCEDURE "informix".sp_pro44_2(a_poliza CHAR(10), a_endoso CHAR(5), a_flota INT DEFAULT 0)
			RETURNING   CHAR(100)    as contratante,
...
--	     _tipo_mov = 5 then
  ...
  SELECT ano_tarifa  
   ...
   WHERE emiautor.no_poliza = a_poliza 
	...
	select cod_producto,impuesto_r,suma_aseg
	  into var_Cod_Producto, var_impuesto,var_suma_aseg
       FROM emireaut
       WHERE no_poliza = _no_poliza  
	   and no_unidad = v_unidad
	   and estatus_ren = '1';	   
...
{	FOREACH		
  ...
  GROUP BY orden, 
	...
	SELECT deducible_o ,
       ...
       WHERE  no_poliza = a_poliza 
	   and no_unidad = v_unidad
	   and cod_cobertura = va_cod_cobertura
  	 GROUP BY deducible_o,
			limite_1_o,
			limite_2_o ;	 
		} 
