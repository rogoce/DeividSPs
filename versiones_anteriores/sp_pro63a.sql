...
RETURNING CHAR(50), 	 --cia
		  ...
		  CHAR(5),
		  CHAR(50),
		  CHAR(50),char(5),char(30),smallint;
...
let _estatus_poliza = 0;
    ...
    select fecha_cancelacion, cod_grupo, estatus_poliza
	  into _fecha_cancelacion, _cod_grupo, _estatus_poliza
	  from emipomae
   ...
   select nombre
	    ...
	    LET v_prima_suscrita = v_prima_suscrita + _prima;
		 ...
		 BEGIN
			...
			END EXCEPTION
				...
				VALUES (v_nopoliza, _cod_contrato, v_desc_contrato, _tipo_contrato, _orden, _suma, _prima,_no_unidad,_prima,_suma,_estatus_poliza);
	       END
	...
	FOREACH
	 SELECT no_unidad,
	        nombre,estatus_poliza,
			sum(prima_suscrita),
			sum(suma_asegurada),
	        SUM(suma),
	        SUM(prima)
	   INTO _no_unidad,
	        v_desc_contrato,_estatus_poliza,
			v_prima_suscrita,
			v_suma_asegurada,
	        _suma,
	        _prima
	   FROM tmp_contratos
	  WHERE no_poliza = v_nopoliza
	  GROUP BY no_unidad,nombre,estatus_poliza 
	  ORDER BY no_unidad,nombre,estatus_poliza
