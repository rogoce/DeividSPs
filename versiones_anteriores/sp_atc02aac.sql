...
DROP PROCEDURE spc_atc02aa;
CREATE PROCEDURE "informix".spc_atc02aa(
a_compania     CHAR(3),
a_sucursal     CHAR(3),
a_no_documento CHAR(20),
a_ano          integer, 
a_usuario      CHAR(10), 
a_unidad       CHAR(12000) DEFAULT '*', 
a_membrete     SMALLINT  DEFAULT 0, 
a_monto_uni    CHAR(8000) DEFAULT '*',  --1255
...
LET _tipo_m = sp_sis146c(a_monto_uni); -- se amplia a 1255 luego 8000
set debug file to "spc_atc02aa.trc";
trace on;
      SELECT count(*)
		  INTO _cnt
		  FROM tmp_cod_mt;
    --     WHERE trim(codigo) = trim(_no_unidad); 
      SELECT count(*)
		  INTO _cnt
		  FROM tmp_rec1 WHERE seleccionado = 1;	
let _cnt = _cnt;	
      SELECT count(*)
		  INTO _cnt
		  FROM tmp_rec1 WHERE seleccionado = 0;	
let _cnt = _cnt;	
trace off;		 
IF a_unidad <> "*" THEN
