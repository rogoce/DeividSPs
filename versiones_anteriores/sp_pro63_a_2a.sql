 ...
 DROP procedure sp0_pro63_a_2;
CREATE procedure "informix".sp0_pro63_a_2(a_cia CHAR(3),a_agencia CHAR(3),a_codsucursal CHAR(255) DEFAULT "*",a_periodo DATE, a_cod_ramo CHAR(255) DEFAULT "*", a_codcliente CHAR(255) DEFAULT "*" ,a_subramo CHAR(255) DEFAULT "*", a_no_documento CHAR(255) DEFAULT "*")
...
--Filtro de Poliza
      ...
      END IF
--IF a_no_documento  in ('0119-00276-01','0121-01472-01','0123-00135-01','2222-00013-03')then
   set debug file to "sp_pro63a.trc";
    trace on;
--end if
FOREACH
...
{
--Leer las pÃ³lizas vigentes al fecha xx, considerando la suma asegurada y prima actualizada a esa fecha xx,
--     es decir si la pÃ³liza o unidad ha tenido endosos de aumento de suma con prima adicional o
--     por el contrario disminuciÃ³n de suma con devoluciÃ³n de prima,
--     el reporte debe mostrar el resultado neto o final de esos movimientos,
--     siempre y cuando estÃ©n antes de la fecha corte indicada para generarlo.
--Si la pÃ³liza fue cancelada y rehabilitada antes de esa fecha xx, si la debe incluir el reporte y traer la informaciÃ³n de suma y prima.
--     Pero, si por ejemplo la rehabilitaciÃ³n fue despuÃ©s de la fecha corte a la que se pidiÃ³ el reporte,
--     entonces no deberÃ­a incluirla porque estarÃ­a aun cancelada.
--Si una unidad fue eliminada de la pÃ³liza antes de la fecha de corte no deberÃ­a incluirla el reporte,
---    pero si la unidad fue eliminada posterior a esa fecha xx, entonces si la debe incluir.
--Si una unidad es incluida en la pÃ³liza antes de la fecha de corte si deberÃ­a incluirla el reporte,
--     pero si la unidad fue incluida posterior a esa fecha xx, entonces no la debe incluir.
}
{
â€¢	Si se cancelÃ³ antes del 30/06 no se muestra.
â€¢	Si la PÃ³liza se cancelo y se rehabilitÃ³ antes del 30/06 se debe mostrar.
â€¢	Si la PÃ³liza se cancelÃ³ antes del 30/06 y se rehabilitÃ³ despuÃ©s del 30/06 NO se Muestra.
...
--LET _fecha_emision = null;
        ...
        SELECT es_terremoto
		 ...
		 WHERE cod_cober_reas = _cod_cober_reas;
						...
						{	IF v_documento  in ('0119-00276-01','0121-01472-01','0123-00135-01','2222-00013-03') then
							  let _es_terremoto = 1;
							end if	}	 
		...
		IF _flag = 1 and v_documento  in ('0119-00276-01','0121-01472-01','0123-00135-01','2222-00013-03') then
