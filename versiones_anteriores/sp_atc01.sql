...
-- SIS v.2.0 - DEIVID, S.A.         execute procedure hg_atc01('001','001','1815-00196-01',2020,'HGIRON',0)

drop procedure hg1_atc01;
create procedure "informix".hg1_atc01(
a_compania		char(3),
a_sucursal		char(3),
...
define _gasto_fact1, _pago_prov1, _monto_no_cubierto1, _ded1, _copago1, _coaseguro1, _ahorro1,_inc_prov1 decimal(16,2);
define _ded,_copago,_coaseguro,_ahorro,_inc_prov decimal(16,2);
define _pago_prov_real  decimal(16,2);
define _ahorro_real  decimal(16,2);
define _monto_no_cubierto_real  decimal(16,2);
define _inc_prov_real decimal(16,2);
 ...
 if _ramo_sis <> 5 then		--si no es salud
	...
	select cod_tipotran
	 ...
	 where tipo_transaccion = 4;
	 	drop table if exists hgrecrcmae;
	  select	numrecla,
	        fecha_siniestro,
			no_reclamo,
			no_unidad,
			no_poliza,
			periodo,no_documento,actualizado
	 
	   from recrcmae
	  where	no_documento   = a_no_documento
	    and actualizado    = 1 into temp hgrecrcmae;
	...
	foreach
	 select	numrecla,
	  ...
	  where	no_documento   = a_no_documento
	    and actualizado    = 1
		drop table if exists hgrectrmae;
		 select fecha,
					no_tranrec,
					fecha_factura,
					cod_tipopago,
					cod_tipotran,
					cod_cliente,no_reclamo ,actualizado

			   from rectrmae
			  where no_reclamo   = _no_reclamo
			    and actualizado  = 1  into temp hgrectrmae;
		...
		foreach
			...
			if _cod_tipotran = '013' or _cod_tipotran = '004' then --Pago a Asegurado
			...
			{	select sum(facturado),
				   ...
				   let _pago_prov1 = 0.00;
				   let _monto_no_cubierto1 = 0.00;
				   let _ded1 = 0.00;
				   let _copago1 = 0.00;
				   let _coaseguro1 = 0.00;
				   let _ahorro1 = 0.00;
				...
				--•	Gastos Incurridos por Proveedor: sumatoria de las columnas Pagado, Ahorro y 
				-- No Cubierto de todas las Transacciones de Pago (004) de Tipo de Pago a Proveedor (001).	
					if _cod_tipotran = '004'  and _cod_tipopago = '001' then --Pago incurrido 36556
					    let _pago_prov_real =  _pago_prov1;	
						let _ahorro_real = _ahorro1;
						let _monto_no_cubierto_real = _monto_no_cubierto1;						
					else
					    let _pago_prov_real = 0;	
						let _ahorro_real = 0;
						let _monto_no_cubierto_real = 0;
					end if
					let _inc_prov1  = _pago_prov_real + _ahorro_real  + _monto_no_cubierto_real;
				 
				--  Gastos No Cubiertos:  sumatoria de las columnas A Deducible, CoPago, Coaseguro de todas 
				-- las Transacciones de Pago (004) y Declinar Reclamo (013) + la sumatoria de las columnas No Cubierto 
				-- y Ahorro de todas las Transacciones de Pago (004) y de Tipo de Pago a Asegurado (003) y Declinar Reclamo (013).	
				     
					if _cod_tipopago = '003' and (_cod_tipotran = '004' or _cod_tipotran = '013' ) then --Pago a Ahorro y monto no cubierto 36556
						let _ahorro_real = _ahorro1;
						let _monto_no_cubierto_real =  _monto_no_cubierto1;
					else
						let _monto_no_cubierto_real  = 0;
					...
					let _monto_no_cubierto1 =  _ded1 + _copago1 + _coaseguro1 + _monto_no_cubierto_real + _ahorro_real;						

					
				 --• Gastos Pagados:  sumatoria de la columna Pagado de todas las Transacciones de Pago (004) 
				 --  de Tipo de Pago a Asegurado (003).
				 	if  _cod_tipotran = '004' and _cod_tipopago = '003' then --Pago a Asegurado  36556
					else
						let	_pago_prov1	= 0;									
					end if
					...
					if _pago_prov1 is null then
						let _pago_prov1 = 0;
					end if
					if _monto_no_cubierto1 is null then
						let _monto_no_cubierto1 = 0;
					end if
					if _ded1 is null then
						let _ded1 = 0;
					end if
					if _copago1 is null then
						let _copago1 = 0;
					end if
					if _coaseguro1 is null then
						let _coaseguro1 = 0;
					end if
					if _ahorro1 is null then
						let _ahorro1 = 0;
					end if
					if _inc_prov1 is null then
						let _inc_prov1 = 0;
					end if
				   let _gasto_fact = _gasto_fact + _gasto_fact1;
				   let _pago_prov = _pago_prov + _pago_prov1;
				   let _monto_no_cubierto = _monto_no_cubierto + _monto_no_cubierto1;
				   let _ded = _ded + _ded1;
				   let _copago = _copago + _copago1;
				   let _coaseguro = _coaseguro + _coaseguro1;
				   let _ahorro = _ahorro + _ahorro1;
                   let _inc_prov = _inc_prov + _inc_prov1;				   
