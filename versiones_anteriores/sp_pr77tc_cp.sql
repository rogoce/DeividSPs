...
DROP PROCEDURE sp_pr77tc_cp;
CREATE PROCEDURE "informix".sp_pr77tc_cp(a_compania CHAR(03), a_terremoto SMALLINT, a_fecha DATE, a_fecha2 DATE, a_ubica CHAR(255) DEFAULT "*") 
...
FOREACH
   SELECT d.no_poliza, e.no_endoso, d.no_documento, d.cod_contratante, d.cod_ramo, d.cod_subramo, d.vigencia_final,  d.fecha_cancelacion
    ...
    WHERE d.cod_compania = a_compania
	  AND d.cod_ramo IN ('017')   AND d.cod_subramo IN ('001','004')
	  ...
	  --AND d.no_documento in ('0108-00553-01')
      AND e.actualizado = 1
		...
		END FOREACH				   
			...
			into _ld_ps_casco
				 ...
				 WHERE prdcober.cod_cobertura = emipocob.cod_cobertura
				   ...
				   AND (nombre like '%CASCO Y MAQUINARIA%' )			
		   ...
		   if _ld_ps_casco is null then
			...
			into _ld_ps_rctercero
				 ...
				 WHERE prdcober.cod_cobertura = emipocob.cod_cobertura
				   AND ( emipocob.no_poliza = _temp_poliza ) 
				   AND ( emipocob.no_unidad = _no_unidad)	
				   AND (nombre like '%RESPONSABILIDAD CIVIL A TERCEROS%' )			
	...
	--					   AND no_endoso = _no_endoso
		...
		END FOREACH					
			...
			into _ld_ps_casco
				  FROM prdcober, emipocob
				 WHERE prdcober.cod_cobertura = emipocob.cod_cobertura
				   AND ( emipocob.no_poliza = _no_poliza ) 
				   AND ( emipocob.no_unidad = _no_unidad)	
				   AND (nombre like '%CASCO Y MAQUINARIA%' )			
