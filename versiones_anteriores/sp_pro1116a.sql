...
--------------------------------------------
    ...
    DEFINE v_prima_suscrita,v_prima_retenida,v_suma_asegurada, _prima_bruta DECIMAL(16,2);
    DEFINE v_vigencia_inic,v_vigencia_final,v_fecha_suscrip, _fecha_ini, _fecha_fin   DATE;
    ...
    IF a_codramo = "*" THEN
		FOREACH WITH HOLD
			SELECT d.no_poliza,d.no_documento,d.no_factura,d.sucursal_origen,
			       ...
			       d.user_added,d.suma_asegurada,d.fecha_cancelacion,d.estatus_poliza,d.prima_bruta
			  INTO _no_poliza,_no_documento,_no_factura,v_cod_sucursal,
			       ...
			       v_usuario,v_suma_asegurada,_fecha_cancelacion,_estatus_poliza,_prima_bruta
			  FROM emipomae d
...
--                  OR  d.fecha_cancelacion > a_periodo)
	...
	--let _fecha_fin = mdy(12, 31, _ano);   Observacion: JEPEREZ 25032024 hay que tomar en cuenta las vencidas actuales
	IF a_codramo = "*" THEN
	   FOREACH WITH HOLD
          SELECT  d.no_poliza,d.no_documento,d.no_factura,d.sucursal_origen,
                  d.cod_grupo,d.cod_ramo,d.cod_subramo,d.cod_tipoprod,
