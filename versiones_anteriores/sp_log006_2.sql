...
drop procedure sp_log006_2;  
create procedure "informix".sp_log006_2(a_estatus char(1) default "I") 
              returning char(15), 
						char(55),  
						char(15), 
						date, 
...
foreach  
    select d.no_aviso, a.nombre, count(d.no_poliza)
     ...
     where a.cod_avican = d.no_aviso
	   and (d.no_aviso  >= '02584' ) -- >= '00951') -- OR d.no_aviso = '00927')	  
	...
	foreach
		select user_proceso, 
		  ...
		  into _user_proceso,
			   ...
			   _count_no_poliza			   
		  from avisocanc 
		 where estatus in ('I') --,'X','Y')  -- se añadio X y Y ,Henry, solicitud usuario NSOLIS 2/8/2016
		   and no_aviso = (_no_aviso)
