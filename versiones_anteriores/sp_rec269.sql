...
returning		
	...
	char(10)       	 as cod_icd,
	char(100)      	 as nom_icd,
	date	 as Fecha_Ultimo_Mov_Reserva,
	dec(16,2)	as Monto_Ultimo_Mov_Reserva,
	date	 as Fecha_Trans_Mov_Mes_actual,
...
DEFINE v_no_tranrec_all  CHAR(10);
DEFINE v_no_tranrec_act  CHAR(10);
DEFINE v_variacion_act 	 DEC(16,2);
DEFINE v_fecha_act_all 	 DATE;
...
--return '','01/01/1900','01/01/1900','01/01/1900',_error_desc,_error,0.00,0.00,_nom_compania,_desc_paso,0.00,'','','','',0.00,0.00,0.00,'','','','01/01/1900',0.00,'01/01/1900';
  return '','','','','01/01/1900','01/01/1900','01/01/1900',_error_desc	,_error	,0,0,_nom_compania	,_desc_paso	,0,'','','','','01/01/1900',0,'01/01/1900',0,0,0	;	
end exception
  ...
  SELECT nombre 
   ...
   WHERE cod_ramo = _cod_ramo
	...
	if _estatus_reclamo = 'A' then
		...
		----------------------------
		--set debug file to "sp_rec269.trc";
		--trace on;
		
	let v_no_tranrec_act = '';
	let v_fecha_act_all = '01/01/1900';
	let v_fecha_act = '01/01/1900';
	let v_variacion_act = 0;
	
	foreach
    select transaccion,fecha 
      into v_no_tranrec_act,v_fecha_act
      from rectrmae
     where numrecla = _numrecla  
	   and actualizado = 1	
	   order by 2 desc, transaccion desc
	   exit foreach;
	   end foreach
	   
    -- Monto del Ultimo Movimiento Reserva
	foreach
    select variacion
      into v_variacion_act
      from rectrmae
     where numrecla = _numrecla
	     and fecha = v_fecha_act
       --and periodo  = a_periodo
       --and cod_tipotran in ('001','002','003','010','011','012','013','014')
	   and transaccion = v_no_tranrec_act
	   and actualizado = 1	
	   exit foreach;
	   end foreach	   
	   
	-- Fecha Transacción del Movimiento Mes actual	
	foreach
    select fecha
      into v_fecha_act_all
      from rectrmae
     where numrecla = _numrecla
       and periodo  = a_periodo           
	   and actualizado = 1	
	...
	if v_variacion_act is null then
	   let v_variacion_act =	0 ;
	end if	   
	if _variacion is null then
	   let _variacion =	0 ;
