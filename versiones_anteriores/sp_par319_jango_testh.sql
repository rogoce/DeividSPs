...
drop procedure sp_par319_jango_test;

create procedure "informix".sp_par319_jango_test() 
...
define _date_added, _fecha_envio      date;
define _dias            integer;
define _porcbandaedad   dec(5,2);
define _cnt_salud       smallint;
...
foreach
	select cod_tipo,
	  ...
	  into _cod_tipo,
		   ...
		   _grupo_trans
	  from parmailtipo
     where cod_tipo in ('00055') and envio_jango = 0
	 order by prioridad ,cod_tipo
	 
--	if _sender_tipo = 'cobrosancon@asegurancon.com' then
--		let _sender_tipo = 'cobros@asegurancon.com';
--	end if

	let _ruta_image2 = trim(_ruta_image) || trim(_nom_tipo) || '\' ;

	foreach
		select first 10 email,
			   enviado,
			   adjunto,
			   trim(html_body),
			   secuencia,
	 	 ...
	 	 where cod_tipo = _cod_tipo
		   and enviado in (0)
		   and secuencia in (6540407) --6483623,6482828,6482435)--(3490156)
       ...
       let _email = 'hgiron@asegurancon.com;jeperez@asegurancon.com;'; 

		if _cod_tipo = '00050' then
			select poliza			
			  into _no_documento
			  from deivid_tmp:carta84
			 where secuencia = _secuencia;	

            let _email = "atencionalcliente@asegurancon.com";
		end if

		if _cod_tipo = '00051' then
			select poliza			
			  into _no_documento
		...
		if _cod_tipo = '00053' then
			let _porcbandaedad = 0.00;
			
			select porcbandaedad,
                   poliza			
			  into _porcbandaedad,
			       _no_documento
			...
			if _porcbandaedad is null then
				let _porcbandaedad = 0.00;
			end if
 			 
			if _porcbandaedad <> 0.00 then
				continue foreach;
			end if
		...
		if _cod_tipo in ('00050','00051','00052','00053') then
			let _email = "sbarragan@asegurancon.com;jeperez@asegurancon.com;";
			
	{		let _cnt_salud = 0;
			
			select count(*) 
			  into _cnt_salud
			  from deivid_tmp:prueba_carta
			 where poliza = _no_documento;
			 
			if _cnt_salud is null then
				let _cnt_salud = 0;
			end if
			
			if _cnt_salud = 0 then
				continue foreach;
			end if
			 }
		end if
		
	--   let _sender_send = 'jeperez@asegurancon.com';
		...
		if _enviado = 3 and _cod_tipo in ('00030','00031','00004','00017','00015','00019','00021','00023','00047') then
		--	continue foreach;
		end if

		let _asunto = _asunto_orig;

		if _cod_tipo in ('00004','00015','00047') then
			foreach
				Select asegurado
				  into _cod_cliente
				  from parmailcomp
        ...
        elif _cod_tipo in ('00050','00051','00052','00053') then
			let _asunto = 'PRUEBA ENVIO CARTA SALUD POLIZA ' || trim(_no_documento);		
        elif _cod_tipo in ('00055') then --SD#11059 JEPEREZ HG:24/07/24

			select poliza,email_corredor	
			  into _no_documento,_sender_tipo
			  from deivid_tmp:carta_021
			 where secuencia = _secuencia
			 and (enviado = 0 or enviado is null);	
			 
            let _email = 'hgiron@asegurancon.com;jeperez@asegurancon.com;'; 
			let _sender_tipo = "atencionalcliente@asegurancon.com";
		
			let _asunto = 'PRUEBA IT - Comunicacion Red de Proveedores - Poliza:  ' || trim(_no_documento);					
		...
		if _cod_tipo not in ('00050','00051','00052','00053','00055') then
			let _asunto = 'PRUEBAS DE IT';
		end if
