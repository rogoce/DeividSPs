...
drop procedure sp_bo077;

create procedure "informix".sp_bo077(
a_fecha_ini	date,
a_fecha_fin	date
) returning integer,
            char(50);
...
---------------------------------------------
	...
	-- Demetrio Hurtado (02/10/2012)
	  ...
	  { 	if year(a_fecha_ini) = 2011 then

	 		if _vigencia_inic < _vigencia_soda then
				let _no_poliza_per = 0;
			end if

		elif year(a_fecha_ini) = 2012 then
		
	 		if _vigencia_inic < _vigencia_soda2 then
				...
				if _reemplaza_pol is not null then
					let _no_documento = _reemplaza_pol;
				end if
	...
	if _nueva_renov = "N" then
		...
		if _estatus_poliza = 2 then  --esta cancelada
			...
			if _fecha_canc <= "15/03/2013" then
...
--------------------------------------------------------------
--	let _no_documento = sp_sis21(_no_documento);

--	if _no_documento is null then
--		return 1, _no_poliza with resume;
--	end if
--------------------------------------------------------------
end foreach
 ...
 {  let li_dia = day(_vigen_ini);
   ...
   let _ano_ant = _periodo_fin[1,4] - 1;
	...
	If li_mes = 2 Then
		If li_dia > 28 Then
			let li_dia = 28;
	        let	_vigencia_ant = mdy(month(_vigen_ini), li_dia, _ano_ant);
