...
drop procedure sp_sis421;
create procedure sp_sis421()
returning integer;
...
define _concurso,_unificar,_fronting     smallint;
...
foreach
	select no_poliza,
	  ...
	  from endedmae
	 where actualizado  = 1
	   and periodo between '2025-01' and '2025-12'
	
	select cod_ramo, cod_subramo, cod_tipoprod,cod_grupo,fronting
	  into _cod_ramo, _cod_subramo, _cod_tipoprod,_cod_grupo,_fronting
	  from emipomae
   ...
   select count(*)
	...
	if _cnt = 0 then
		foreach
			...
			insert into prisusap(no_documento, prima_suscrita, cod_agente)
