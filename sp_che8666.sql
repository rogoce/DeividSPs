...
DROP PROCEDURE sp_che8666;
CREATE PROCEDURE sp_che8666(a_compania CHAR(3), a_sucursal CHAR(3))
...
if a_periodo > "2017-09" then    --"2015-10"
...
let _per_fin_ap     = _ano || a_periodo[5,7]; --2016-04
...
foreach
 select doc_remesa,
  ...
  where periodo     >= _per_ini_ap		--2015-10
    and periodo     <= _per_fin_ap		--2016-07
	and actualizado = 1
	and tipo_mov    in ("P", "N")
...
foreach
 select no_poliza,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between '2015-10' And _per_fin_ap --   2014-10    A    2015-01
...
foreach 
	 select no_reclamo,		
	  ...
	  where cod_compania = a_compania
		and periodo      <= _per_fin_dic 	--2015-12
		and actualizado  = 1
	  group by no_reclamo
...
call sp_bo077(_fecha_ap_ini, _fecha_fin_ap) returning _error, _error_desc;
...
---**********************************************************************************************
-- Polizas Nuevas y Renovadas Anno Pasado_fecha_fin_ap
---**********************************************************************************************
--let _fecha_proceso = sp_sis40();
...
call sp_bo077(_fecha_aa_ini, _fecha_aa) returning _error, _error_desc;	--01/10/2014    -    30/09/2015
...
--sum(pri_dev_ap),
--Aplicar Rango a los corredores tomando como base la Primas Suscrita año pasado ( 2015)
foreach
	  select cod_agente,
		 sum(pri_dev_aa),   
		 sum(pri_pag_aa),
