...
drop procedure sp_sis245b2;

create procedure sp_sis245b2(a_periodo char(7))
...
foreach
	select tmp.no_poliza,
	  ...
	  into _no_poliza_n,
		   ...
		   _prima_neta,
		   _prima_bruta
	  from tmp_renov_serafin tmp
	 inner join emipomae emi on emi.no_poliza = tmp.no_poliza
	 inner join emipouni uni on uni.no_poliza = emi.no_poliza
	 inner join emipocob cob on cob.no_poliza = uni.no_poliza and cob.no_unidad = uni.no_unidad
	 where tmp.no_documento not in ('0922-00698-01')
	
	update emipouni
	   set impuesto = _prima_bruta,
		   prima_bruta = _prima_neta + _prima_bruta	
	...
	update emipocob
	   set prima_neta = _prima_bruta  ,prima_anual = _prima_bruta ,prima = _prima_bruta,limite_1 = _limite_1
	 where no_poliza = _no_poliza_n;
	 
	call sp_proe02(_no_poliza_n, '00001', '001') returning _error;
	call sp_proe24(_no_poliza_n, '00001') returning _error;
	call sp_proe03(_no_poliza_n, '00001') returning _error;
	...
	if _return <> 0 Then
		...
		return 0,0,_no_documento,_no_poliza_n with resume;
	--end if
end foreach
end
end procedure;