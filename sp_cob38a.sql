...
DROP PROCEDURE sp_cob38a;

CREATE PROCEDURE sp_cob38a(a_compania CHAR(3), a_agencia CHAR(3), a_periodo1 CHAR(7), a_periodo2 CHAR(7),a_ramo CHAR(255) DEFAULT "*")
	RETURNING	CHAR(10)		as no_recibo,
				CHAR(30)		as poliza,
				varchar(100)	as cliente, 
				DATE			as fecha_pago,
				DECIMAL(16,2)	as prima_neta_cobrada,
				DECIMAL(16,2)	as impuesto_1porc,
				DECIMAL(16,2)	as impuesto_5porc,
				DECIMAL(16,2)	as monto_total,
				CHAR(50)		as ramo,
				CHAR(50)		as compania,
				CHAR(255)		as filtros,
				dec(16,2)		as impuesto_2porc,
				varchar(50)	    as Motivo,
				char(1)		    as tipo;

DEFINE _motivo				varCHAR(50);
...
--    AND renglon     <> 0
	...
	if v_imp_5 = 0.00 then
		let _motivo = 'PÓLIZA FACTURADA SIN IMPUESTO.';
	end if
	...
	If _aplica_impuesto = 1 Then -- Verifica si a la Poliza se le Aplican los Impuestos (Exterior No Llevan) 
		...
		If _imp_gob = 1 Then -- Verifica si al Ramo se le Aplican los Impuestos
			...
			let _motivo = 'RAMO NO APLICA IMPUESTO 2%.';
		end if
	else
		let _motivo = 'PÓLIZA DE ORIGEN EXTERIOR.';
	end if
