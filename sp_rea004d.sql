...
-- execute procedure sp_rea004d("2010-04","2010-04") 
-- SIS v.2.0 - DEIVID, S.A. 
  
drop procedure sp_rea004d;

create procedure sp_rea004d(a_periodo1 char(7), a_periodo2 char(7)) 
returning char(3),
		  char(50),
		  dec(16,2),
		  dec(16,2),
		...
		foreach
		 select cod_coasegur,
	      ...
	      where cod_contrato   = _cod_contrato
			...
			If _cod_contrato in ("00595","00596","00597") then
			   LET _grupo = 2;
			End if

			If _cod_contrato in ("00576","00585","00593") then
			   LET _grupo = 3;
			End if


			If _cod_contrato in ("00574","00584","00594") then
			   LET _grupo = 4;
			End if

			insert into tmp_ramo
			values (_cod_coasegur, _cod_ramo, 0.00, 0.00, 000, 0.00, 0.00, _por_pagar,_cod_contrato,_serie,_grupo);
		...
		foreach
		 select cod_coasegur,
	      ...
	      where cod_contrato   = _cod_contrato
			...
			insert into tmp_ramo
			values (_cod_coasegur, _cod_ramo, 0.00, 0.00, 0.00, 0.00, _monto_reas, 0.00,_cod_contrato,_serie,_grupo);
			...
			foreach
			 select cod_coasegur,
		      ...
		      where cod_contrato   = _cod_contrato
				...
				insert into tmp_ramo
				values (_cod_coasegur, _cod_ramo, _monto_reas, _comision, _impuesto, _por_pagar, 0.00, 0.00,_cod_contrato,_serie,_grupo);
			...
			foreach
			 select cod_coasegur,
		      ...
		      where cod_contrato   = _cod_contrato
				...
				If _cod_contrato in ("00595","00596","00597") then	 --50%MAPFRE
				   LET _grupo = 2;
				End if

				If _cod_contrato in ("00576","00585","00593") then	 --FACILIDAD INCENDIO
				   LET _grupo = 3;
				End if

				If _cod_contrato in ("00574","00584","00594") then	 --FACILIDAD CAR
				   LET _grupo = 4;
				End if
...
foreach
 select serie,
  ...
  order by 1, 2, 3, 4
		...
		if _grupo = 1 then
			let _nombre_grupo = "CONTRATOS NO BOUQUET";
		end if
		if _grupo = 2 then
			let _nombre_grupo = "CONTRATOS 50 % Retencion MAPFRE";
		end if
		if _grupo = 3 then
			let _nombre_grupo = "CONTRATOS Facilidad INCENDIO";
		end if
		if _grupo = 4 then
			let _nombre_grupo = "CONTRATOS Facilidad CAR";
		end if
