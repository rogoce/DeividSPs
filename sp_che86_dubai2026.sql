...
--*************************************************************************************
-- Prima neta cobrada devengada periodo convencion 2025-01 Al periodo se esta evaluando
--*************************************************************************************
...
foreach
	...
	call sp_dev06f(_no_documento,_fecha_aa,'01/01/2025',_fecha_aa) returning _valor,_mensaje,_pri_dev_aa;
...
--**********************************************************************
-- Prima Pagada periodo convencion 2025-01 Al periodo se esta evaluando
--**********************************************************************
...
foreach
	...
	foreach
		...
		select pagado,
		 ...
		 where no_requis = _no_requis
		   and fecha_impresion between '01/01/2025' and '31/12/2025';

		if _pagado = 1 then
			if _fecha_anulado is not null then
				if _fecha_anulado >= '01/01/2025' and _fecha_anulado <= '31/12/2025' then
					let _monto_dev = 0;
				end if
			end if			
		else
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2025','31/12/2025',my_sessionid); --se cambia a tabla fisica 05/06/2018
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
call sp_bo077_2('01/01/2024', _fecha_ap, _fecha_aa) returning _error, _error_desc;
...
----**********************************************************************************************
call sp_bo077_2('01/01/2025', _fecha_aa, _fecha_aa) returning _error, _error_desc;
