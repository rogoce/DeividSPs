-- Procedimiento que Genera la Morosidad Total por Ramo	con el 5 y 1 %
-- 
...
--
-- SIS v.2.0 - DEIVID, S.A.	igual al sp_cob05

DROP PROCEDURE sp_cob138;

CREATE PROCEDURE "informix".sp_cob138(
a_compania CHAR(3),
a_agencia  CHAR(3),
a_fecha    DATE
)
...
DEFINE _imp_5			  DECIMAL(16,2);
DEFINE _imp_1			  DECIMAL(16,2);
...
CREATE TEMP TABLE tmp_moros(
		...
		imp5_saldo		DEC(16,2)	DEFAULT 0 NOT NULL,
		imp1_saldo		DEC(16,2)	DEFAULT 0 NOT NULL,
    ...
    FOREACH
	    ...
	    SELECT factor_impuesto
		 ...
		 IF _factor_imp = 1.00 THEN
		 	LET _imp_1 = _saldo - _saldo / (1 +( _factor_imp / 100));
		 ELIF _factor_imp = 5.00 THEN
		 	LET _imp_5 = _saldo - _saldo / (1 +( _factor_imp / 100));
		 END IF
	END FOREACH
