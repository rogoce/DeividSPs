...
DROP PROCEDURE sp_cob114;

CREATE PROCEDURE "informix".sp_cob114(
a_compania CHAR(3),
a_agencia  CHAR(3),
a_periodo  CHAR(7),
a_periodo2 CHAR(7)
...
FOREACH
 SELECT doc_remesa, 
  ...
  WHERE actualizado  = 1			              -- Recibo este actualizado
    AND tipo_mov     IN ('P', 'N', 'X')           	  -- Pago de Prima(P)
...
{	select tipo_cobro
	...
	if _tipo_cobro = "G" then
		let _doc_poliza = "0503-00026-01";	
	end if
}
