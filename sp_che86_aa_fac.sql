...
--*****************************************************************************************
DROP PROCEDURE sp_che86_aa_fac;
CREATE PROCEDURE sp_che86_aa_fac(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
foreach
	select cob.doc_remesa,
	   ...
	   into _no_documento,
			 ...
			 _porc_proporcion
	   from cobredet cob 
	  inner join cobreaco c on c.no_remesa = cob.no_remesa  and c.renglon   = cob.renglon
	  inner join reacomae r on r.cod_contrato = c.cod_contrato
	  where cob.periodo >= '2024-01'
	    and cob.periodo  <= '2024-12'
...
--	let _monto = _monto - _monto_fac_ac;
	...
	*/
	...
	foreach
		...
		insert into tmp_concurso1(no_documento, monto_90_ap, cod_agente,tipo)
...
--**********************************************************************************************
INSERT INTO tmp_concurso1(no_documento, pri_dev, cod_agente) 
