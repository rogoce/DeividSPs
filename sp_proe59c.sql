...
drop procedure sp_proe59;
create procedure "informix".sp_proe59(a_compania CHAR(03), a_fecha DATE,a_cod_manzana char(255) DEFAULT '*', a_poliza char(20) DEFAULT '*', a_estatus_poliza smallint,a_agente char(255) DEFAULT '*',a_grupo char(255) DEFAULT '*',a_periodo1 CHAR(07),a_periodo2 CHAR(07),a_ramo char(255) DEFAULT '*',a_subramo char(255) DEFAULT '*')
...
CALL sp_pro34("001","001",a_periodo1,a_periodo2,"*",a_grupo,a_agente,"*","001,003;","*",1) RETURNING v_filtros;

FOREACH WITH HOLD
   SELECT y.no_documento,y.no_poliza,y.cod_grupo,y.no_endoso
     INTO _no_documento,_no_poliza,_grupo, _no_endoso
     FROM temp_det y
    WHERE y.seleccionado = 1
    ORDER BY y.cod_ramo,y.no_documento

	SELECT estatus_poliza,cod_ramo,cod_subramo
	  INTO _est_pol,_cod_ramo,_cod_subramo
	  FROM emipomae
	   			...
	   			ON EXCEPTION IN (-239)
					UPDATE temp_ubica			   
					   SET SUMA_ASEGURADA		= SUMA_ASEGURADA	   + _suma_asegurada,	 
						   RETENCION			= RETENCION			   + _suma_ret,
						   CONTRATOS			= CONTRATOS			   + _suma_otros,
						   FACULTATIVO			= FACULTATIVO		   + _suma_fac,	 
						   TOTAL_PRIMA_SUSCRITA = TOTAL_PRIMA_SUSCRITA + _prima_suscrita,
						   PRIMA_INC			= PRIMA_INC			   + _prima_incendio,
						   ...
						   PRIMA_TERREMOTO	   	= PRIMA_TERREMOTO	   + _prima_terremoto,
...
FOREACH WITH HOLD
   SELECT a.COMPANIA, 			 
	...
	order by  a.RAMO,a.SUBRAMO,a.UBICACION,a.MANZANA,a.POLIZA,a.UNIDAD
		  ...
		  if _suma_asegurada = 0 and _suma_ret = 0  and _suma_otros = 0  and _suma_fac = 0  and _prima_suscrita = 0  then	
				continue foreach;
		end if			
	 ...
	 {	  let _si_hay = 0;
		...
		select count(*)
		 into _si_hay
		 from temp_filtro b
		where TRIM(b.NO_DOCUMENTO) = _no_documento 		
		  and b.NO_UNIDAD  = _no_unidad	
		  and b.NO_MANZANA  = _cod_manzana
		  and b.NO_CORREDOR	= _cod_agente
		  and b.NO_GRUPO    = _grupo
		  and b.ESTATUS	    = _est_pol
