...
DROP PROCEDURE sp_cob128new;
CREATE PROCEDURE "informix".sp_cob128new(
a_compania CHAR(3),
a_agencia  CHAR(3),
a_periodo1 CHAR(7),
a_periodo2 CHAR(7),
...
DEFINE _renglon,_valor	  SMALLINT;
...
FOREACH WITH HOLD
 SELECT t.no_recibo,
		...
		t.prima_agt,		
		t.coas_neto_pagar,
		c.asegurado,no_poliza,c.vigencia_inic,c.vigencia_final,c.monto_cedido,c.cod_coasegur,c.cod_ramo
   INTO v_no_recibo,
   		v_no_documento,
   		v_cod_contratante,
   		v_fecha,
		...
		_prima_agt,		
		_coas_neto_pagar,
		_asegurado,_no_documento,_vig_ini,_vig_fin,_monto_cedido,_cod_coasegur2,_cod_ramo2
   FROM tmp_prod t full JOIN tmp_coas c ON t.no_documento = c.no_poliza
  order by t.cod_coasegur,t.cod_ramo,t.no_documento
...
if v_no_recibo is null or v_no_recibo = "" then
	let v_codramo = _cod_ramo2;
	let _vigencia_inic = _vig_ini;
	let _vigencia_final = _vig_fin;
end if
--Selecciona los nombres de Ramos
	...
	if v_no_recibo is null or v_no_recibo = "" then
		...
		let _cod_coasegur = _cod_coasegur2;
		let v_no_documento = _no_documento;
	end if
