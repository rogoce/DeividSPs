...
-- COMPROMISO DE NEGOCIO - 2010 : copia REALIZADO 23/01/2012 POR HENRY

DROP PROCEDURE sp_che115;

CREATE PROCEDURE sp_che115(
a_compania          CHAR(3),
a_sucursal          CHAR(3)
) RETURNING SMALLINT;
...
foreach
 select doc_remesa,
  ...
  where periodo     between _per_ini and a_periodo 
	...
	insert into tmp_che115(no_documento, pri_pag, tipo)
...
foreach
 select doc_remesa,
  ...
  where periodo     between _per_ini_ap and _per_fin_ap	  --2009-01  a  2009-03
	...
	insert into tmp_che115(no_documento, pri_pag_ap, tipo)
	values (_no_documento, _monto, 2);

end foreach

--*********************
-- Prima Devengada  AA --	   falta restar el % por reserva de prima no devengada
--*********************
...
--*********************
-- Prima Devengada  AP --	   falta restar el % por reserva de prima no devengada	  Anio Pasado
--*********************
...
foreach
 select no_documento,
   ...
   from endedmae
  where actualizado  = 1
	and periodo between _per_ini_ap and _per_fin_ap	    --2009-01  a  2009-03
  group by no_documento
...
foreach
 select no_documento,
		...
		sum(pri_sus_pag),			--   prima Suscrita devengada	   Actual
		sum(pri_sus_pag_ap)			--   prima Suscrita devengada Anio Pasado
   into _no_documento,
	    _pri_pag,
		_sin_pag_aa,
	    _sin_pen_aa,
...
{	 if _concurso = 0 then -- Excluir del Concurso
	...
	foreach
	 ...
	 {if trim(_cod_agente) in ("00874","00628","01709","00702") then	 -- x corredor
	    -- sigue
	 else
	 	continue foreach;
	 end if}
	...
	let _pri_sus_orig  	  = _prima_suscrita;
	let _porc_res_xramo	  = _porc_res_mat;


	let _porc_res_mat   = 100 - _porc_res_mat;
	let _pri_sus        = _prima_suscrita;
	let _prima_suscrita = _prima_suscrita * _porc_res_mat / 100;

	-- Prima Cobrada Devengada		 -- Modificado 14/11/2010 Henry
	let _pri_cob_dev = _pri_pag * _porc_res_mat / 100;
	 ...
	 foreach
	  SELECT cod_agente
	   ...
	   WHERE no_poliza = _no_poliza
		...
		nombre_tipo,
		prima_neta,
		comision,		   -- Comision a pagar
		porcentaje,
		por_crecimiento,
		por_siniestro,
		prima_max,		   -- Maximo Suscrito Devengado
		prima_devengada,   -- Prima Sucrita Devengada
		pri_cob_dev,	   -- Prima Cobrada Devengada
		pri_cob_dev_max,   -- Maximo Cobrado Devengado
		pri_cob,		   -- Prima Cobrada
		pri_cob_max,       -- Maximo Cobrado
		pri_sus_dev,	   -- Prima Suscrita Devengada 
		monto_90,		   -- Prima Suscrita Devengada 
		pri_sus_orig,  	   -- Prima Suscrita Original 
		porc_res_xramo,	   -- Primax Ramo		
		pri_pag_aa,		   -- Prima Pagada Anio Actual
		pri_pag_ap		   -- Prima Pagada Anio Pasado
		)          
		values(
		_cod_agente, 
		_no_documento, 
