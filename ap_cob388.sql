...
drop procedure ap_cob388;
create procedure 'informix'.ap_cob388(a_no_aviso char(10), a_usuario char(8)) 
returning	smallint,
			varchar(100);
...
foreach
	select no_poliza,
	 ...
	 where no_aviso = a_no_aviso
	   and no_poliza not in (
	   select b.no_poliza
	     from cobgesti b
	    where date(b.fecha_gestion) = '06/06/2022'
	      and desc_gestion like '%02254%'
	 group by no_poliza)	 
    ...
    let _fecha_gestion = current - 18 units day;
