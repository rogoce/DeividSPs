...
drop procedure sp_rea29;
create procedure "informix".sp_rea29(a_fecha date)
...
define _porc_partic_reas		smallint;
...
--set debug file to "sp_rea29.trc"; 
--trace on;
...
foreach with hold
	...
	foreach
		...
		call sp_sis122c(_no_poliza,_no_unidad) returning _error,_error_desc;
		...
		foreach
			...
			let _suma_aseg_reas = _suma_aseg_uni* (_porc_partic_reas/100) * (_porc_partic_prima/100)*(_porc_cont_partic/100);
			...
			if _contrato_xl = 1 then
				...
				let _suma_aseg_xl_prorrata = _suma_aseg_xl * _prorrata;
