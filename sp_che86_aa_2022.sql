...
--*****************************************************************************************
DROP PROCEDURE sp_che86_aa_2022;
CREATE PROCEDURE sp_che86_aa_2022(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
--**********************************************************************************************
-- Prima Pagada Anno Pasado periodo del concurso 2021-01 A 2021-12
--**********************************************************************************************
foreach
 select doc_remesa,
        prima_neta,
  ...
  where periodo     >= '2021-01'
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2021','31/12/2021',my_sessionid);	--Crea tabla tmp_corr con el corredor
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
		...
		insert into fis_concurso1_2021(no_documento, pri_can_ap, cod_agente,tipo)
...
--**********************************************************************************************
INSERT INTO fis_concurso1_2021(no_documento, pri_dev, cod_agente) 
...
foreach 
	 select no_reclamo,		
	  ...
	  where cod_compania = a_compania
		and periodo      <= _per_fin_dic 	--2021-12
		and actualizado  = 1
	  group by no_reclamo
	 having sum(variacion) > 0 
		...
		foreach
			...
			insert into fis_concurso1_2021(no_documento, sin_pen_ap, cod_agente)
