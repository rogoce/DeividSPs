...
foreach
	 select no_documento,
			...
			sum(pri_can_ap),
			sum(pri_dev_ap),
		    sum(pri_sus_pag)			--   prima Suscrita devengada
	   into _no_documento,
			_pri_pag,
			_pri_can,
			_pri_dev,
	...
	select beneficios
	 ...
	 IF _tipo_prod = 4 or _tipo_prod = 3 THEN 		-- Excluir Reaseguro Asumido y Coas. Mayoritario (2)
		CONTINUE FOREACH;
	 END IF
	...
	-- Morosidades Mayores a 90 Dias (No Se Incluyen) -- Año Actual
	...
	-- Morosidades Mayores a 90 Dias (No Se Incluyen) -- Año Pasado
	call sp_cob33(a_compania, a_sucursal, _no_documento, _per_fin_ap, _fecha_ap)
	     returning v_por_vencer,    
	               v_exigible,      
	               v_corriente,    
	               v_monto_30,      
    ...
    --/*CATEGORIZAR*/
    --A-AUTOMOVIL                   ="002","020"
    --C-PATRIMONIALES               ="003","005","006","015","011","013","010","009","017","014","021","012","007"
	...
	if  _cod_ramo in ("003","005","006","015","011","013","010","009","017","014","021","012","007")  then
	   let _tipo = 'C';
	end if
...
--		if _pri_sus_pag_ap = 0 then	 -- se quito para tomar en cuenta los corredores que no tuvieron movimiento el año pasado, incremento 100 %
--			continue foreach;		 -- Solicitado: MARGARITA Fecha: 19/02/2010
--		end if
		...
		--************************************************
		--   Calculos para incremeto de PSP 2009 vs 2008
		--************************************************
		...
		--************************************************
		--    Calculos % de siniestralidad 2009	
		--************************************************
		let _siniestralidad = 0;
