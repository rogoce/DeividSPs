...
drop procedure sp_bo003;

create procedure "informix".sp_bo003(a_periodo char(7))
returning char(1),
          char(100);
...
on exception set _error, _error_isam, _error_desc
	if _doc_poliza is null then
		let _doc_poliza = '';
	end if
	
	let _error_desc = 'sp_bo003 ' || trim(_doc_poliza) || ' ' || trim(_error_desc);
...
--		exit foreach;
	...
	update cobmoros4
	   set cobros_por_vencer      = _por_vencer,
		   ...
		   fecha_ult_pago 		  = _fecha_ult_pago,
	       monto_ult_pago 		  = _monto_ult_pago,
		   facultativo 			  = _facultativo
	 where no_documento           = _doc_poliza
	   and periodo                = a_periodo;
