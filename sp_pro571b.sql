...
DROP PROCEDURE sp_pro571b;

CREATE PROCEDURE sp_pro571b(a_no_documento CHAR(20))
...
--set debug file to "sp_pro571b.trc";
--trace on;
...
--  end foreach
  ...
  let _porc_aumento = _porc_aumento - 4.4;
  
  LET _prima_nueva = _prima_nueva / 1.05;
  let _prima_nueva = _prima_nueva - _prima_nueva * 4.4 / 100;
...
{		 if _edad <> _edad2 then
	  ...
	  if _edad_desde <> _edad_desde2 then
		LET _prima_plan = _prima_plan - (_prima_plan * 4.4 / 100);
		LET _cambio_edad = _prima_plan / _prima_nueva * 100;
		LET _cambio_edad = _cambio_edad * _porc_aumento / 100;
		--LET _cambio_edad = ((_prima_plan / _prima_plan2) - 1);-- * 100;
	  end if

	  let _cambio_edad_t = _cambio_edad_t +_cambio_edad;
