			...
			select nombre_ramo,
			 ...
			 order by 1,2
				...
				if  _ProdAntRam is null or _ProdAntRam = 0 then
					let _Ramo_crec 		= _ProdActRam;
					let _Ramo_Porc_crec = 100;
				else
					let _Ramo_crec 		= _ProdActRam - _ProdAntRam ;
					let _Ramo_Porc_crec = ( _Ramo_crec / _ProdAntRam ) * 100;
				end if

				let _crecimiento = _crecimiento + _Ramo_crec ;
				let _Porc_crec   = _Porc_crec + _Ramo_Porc_crec;						
				let _Ramo_crec 		= 0;
				let _Ramo_Porc_crec = 0;
...
--************************************************
	...
	if _cod_tipo = 'E' then	  --  Fianzas
		if _prima_aa >= 50000 then 
			if _Porc_crec >= 30 then						
	...
	{		foreach
			select porcentaje,comision
			  into _porcentaje,_valor_prima
			  from chqrenta3
