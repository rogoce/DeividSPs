...
DEFINE _aprobado      VARCHAR(50);
DEFINE v_cod_tipopago  CHAR(3);
...
LET _wf_apr_j	   = NULL;
LET _wf_apr_jt	   = NULL;
LET _wf_apr_jt_2   = NULL;
LET _wf_apr_g	   = NULL;
LET _aprobado      = NULL;
LET v_cod_tipopago = NULL;
LET v_tipopago     = NULL;
...
foreach
   ...
   IF v_cod_tipopago IS NOT NULL AND TRIM(v_cod_tipopago) <> "" THEN
	select nombre 
	  into v_tipopago
	  from rectipag
	 where cod_tipopago = v_cod_tipopago;
   END IF
	
   if v_numrecla[1,2] = "18" then
    select descripcion
	  into _aprobado
	  from insuser
	 where usuario = v_user_added;
   elif _wf_apr_g	is not null and trim(_wf_apr_g) <> "" then
    select descripcion
	  into _aprobado
	  from insuser
	 where usuario = _wf_apr_g;
   elif _wf_apr_jt_2 is not null and trim(_wf_apr_jt_2) <> ""	then
    select descripcion
	  into _aprobado
	  from insuser
	 where usuario = _wf_apr_jt_2;
