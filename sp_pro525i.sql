...
drop procedure sp_pro525i;
create procedure sp_pro525i(a_vigencia_inic date, a_vigencia_final date)
returning	char(3)		as Cod_Ramo,			--_cod_ramo,
			varchar(50)	as Ramo,				--_nom_ramo,
			integer		as Letra,				--_no_letra,
			integer		as Cantidad_Registros,	--_cantidad,
			integer		as Dias_Pago,			--_dias_pago,
			dec(16,2)	as Promedio_Dias,		--_promedio_pagos;
...
on exception set _error, _error_isam, _error_desc
	rollback work;
	return '',_error_desc,_error,_error_isam,0,0.00,_no_poliza;
end exception
...
dias_pago	integer,
cantidad	integer,
primary key (cod_ramo, no_letra)) with no log;
...
foreach with hold
	...
	if (_fecha_pago is null or _fecha_pago = '') and _pagada = 1 then
		commit work;
		continue foreach;
	end if

	if _fecha_vencimiento >= today and _pagada = 0 then
		commit work;
		continue foreach;
	end if
