...
-- Modficado : para adicionar en reporte el campo reporto_ssrp 
-- execute procedure sp_leg01('001','001','01/08/2015',today,"*","*","*","*","*","*","*",0,0,0)
...
a_cuantia1          dec(16,2) default 0,
a_cuantia2          dec(16,2) default 0,
a_reporto_ssrp      smallint  default 0)
returning	char(50)		as Compania, 		--1	--v_compania_nombre,
			...
			char(30)			as Estatus_Actual,	--16--_n_estatus_actual
			...
			char(2)         as reporto_ssrp;  -- 26	new tipo_estado		
...
define _n_estatus_actual    char(30);
...
define _reporto_ssrp		smallint;
define _reporto_ssrp_txt	char(2);

let _reporto_ssrp = 0;
let _reporto_ssrp_txt = 'NO';
...
foreach
	select no_demanda,
	 ...
	 where date(date_added) >= a_fecha
	   ...
	   if _reporto_ssrp is null then
	      let _reporto_ssrp = 0;
	   end if
	   
	   if _reporto_ssrp = 0 then
	       let _reporto_ssrp_txt = 'NO';
	   else
	       let _reporto_ssrp_txt = 'SI';	   
	   end if
...
if a_cuantia1 >= 0 and a_cuantia2 >= 0 then   -- rango del monto de la cuantia
	if a_cuantia1 <> 0 or a_cuantia2 <> 0 then 
		let v_filtros = trim(v_filtros) || " Cuantia Desde: " ||  cast(a_cuantia1 as varchar(10))|| " Cuantia Hasta: " ||  cast(a_cuantia2 as varchar(10));	
		update tmp_prod
		   set seleccionado = 0
		 where monto_cuantia < a_cuantia1 
		   or monto_cuantia > a_cuantia2;
	   else
	   end if
end if

if a_reporto_ssrp <> "2"  then 
	if a_reporto_ssrp = "1"  then   -- Reporto a SSRP
		let v_filtros = trim(v_filtros) || " Reportado a SSRP?: SI. " ;
		update tmp_prod
		   set seleccionado = 0
		 where reporto_ssrp_txt = 'NO';
	end if

	if a_reporto_ssrp = "0"  then   -- Reporto a SSRP
		let v_filtros = trim(v_filtros) || " Reportado a SSRP?: NO. " ;
		update tmp_prod
		   set seleccionado = 0
		 where reporto_ssrp_txt = 'SI';
	end if
end if
...
foreach
	...
	if _estatus_actual = 1 then
		...
		let _n_estatus_actual = 'CERRADA';
	elif _estatus_actual = 2 then
		let _n_estatus_actual = 'CERRADO CON RECLAMO ABIERTO';		
	end if
