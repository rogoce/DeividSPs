...
--execute procedure sp_che81aa("001","001","2010-02","informix")

DROP PROCEDURE sp_che81aa;

CREATE PROCEDURE sp_che81aa
(
a_compania          CHAR(3),
a_sucursal          CHAR(3),
a_periodo		    CHAR(7),
...
--let _valor = sp_che108aa(a_compania,a_sucursal,a_periodo); --06/04/2010,acumula prima cobrada hasta mes que se va a pagar, para luego evaluar si cumple para el bono.
...
FOREACH
	select cod_agente,
	  ...
	  into _cod_agente,
		   ...
		   _retro
	  from chqboagt
     where cod_agente in('00620','00547','00013')
	 ...
	 order by cod_agente,retroactivo
	  ...
	  if _prima_cobrada >= 35000 then	--si es >= a 35000, se toma en cuenta al corredor para el pago.
	  else
...
foreach
	...
	SELECT cod_agente
	  ...
	  FROM chqboni
     WHERE periodo = a_periodo
	   and cod_agente in('00620','00547','00013')
	 GROUP BY cod_agente
	 ORDER BY cod_agente
