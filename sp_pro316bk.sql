...
drop procedure sp_pro316bk;

create procedure "informix".sp_pro316bk(a_poliza char(10))
returning integer;
...
select centro_costo
 ...
 --***********************
  ...
  if _cnt > 0 then
		if _cod_ramo in('002') then --AUTO
	   	...
	   	elif _cod_ramo in('020') then	  --SODA
		else											 --PATRIMONIALES
 ...
 --Excepcion de Cobros	 *
 --***********************
    select cod_formapag,cod_contratante
	  into _cod_formapag,_cod_contr
	  from emipomae
	 where no_poliza = a_poliza;
