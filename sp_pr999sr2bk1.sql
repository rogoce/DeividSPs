...
--------------------------------------------
--execute PROCEDURE sp_pr999sr2bk1('001','001','2012-12','2012-12',"*","*","*","*","001,003,006;","*","*","2013,2012,2011,2010,2009,2008","*")
DROP PROCEDURE sp_pr999sr2;
CREATE PROCEDURE sp_pr999sr2(a_compania CHAR(03),a_agencia CHAR(03),a_periodo1 CHAR(07),a_periodo2 CHAR(07),a_codsucursal CHAR(255) DEFAULT "*",a_codgrupo CHAR(255) DEFAULT "*",a_codagente CHAR(255) DEFAULT "*",a_codusuario CHAR(255) DEFAULT "*",a_codramo CHAR(255) DEFAULT "*",a_reaseguro CHAR(255) DEFAULT "*",a_contrato CHAR(255) DEFAULT "*",a_serie CHAR(255) DEFAULT "*",a_subramo CHAR(255) DEFAULT "*"	)
RETURNING CHAR(3),CHAR(50),SMALLINT,DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),DEC(16,2),CHAR(255),CHAR(50),DEC(16,2),char(1),smallint;
   BEGIN
...
CREATE TEMP TABLE tmp_tabla(
		...
		p_fac_car	     DEC(16,2),
		serie            SMALLINT,
        PRIMARY KEY (cod_ramo,serie)) WITH NO LOG;
...
FOREACH
     SELECT cod_ramo,		  --SE BUSCA POR POLIZAS
       ...
       FROM temp_produccion
	  where seleccionado = 1
   GROUP BY cod_ramo, no_poliza, serie
   ORDER BY cod_ramo, no_poliza, serie
...
FOREACH
	 SELECT cod_ramo,		
  	   ...
  	   INTO v_cod_ramo, 
			...
			_serie
	   FROM tmp_tabla
	  where cod_ramo in('006','IN1','IN3','TE1','TE3') --RESP CIVIL, INCENDIO MULT Y TERREMOTO	anterior al 2010 deben ir al 2010
        and serie < 2010
	  ORDER BY cod_ramo
	...
	if _cnt = 0 then
		...
		UPDATE tmp_tabla
		   SET p_cobrada        = p_cobrada     + v_prima,
			   ...
			   cant_polizas     = cant_polizas  + _cantidad
		 WHERE serie            = 2010
		   and cod_ramo         = v_cod_ramo;
...
FOREACH
	...
	if v_cod_ramo = 'IN1' or v_cod_ramo = 'IN3' then
	   LET v_desc_ramo = 'Incendio';
	   let _n_cober = "2";
	end if

	if v_cod_ramo = 'TE1' or v_cod_ramo = 'TE3' then
	   LET v_desc_ramo = 'Terremoto';
	   let _n_cober = "2";
	end if

	if v_cod_ramo in("010","011","012","013","014","021","022") then
