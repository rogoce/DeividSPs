...
DROP PROCEDURE sp_che117apm;
CREATE PROCEDURE sp_che117apm(a_origen char(1), a_pagado  smallint) 
    ...
    if _cnt = 0 then
	  ...
	  SELECT sum(chqchmae.monto)
	   ...
	   WHERE ( agtagent.cod_agente = chqchmae.cod_agente ) and
			 ...
			 ( agtagent.agente_agrupado = _agente_agrupado)  and
		   (chqchmae.no_cheque = 3903);
