...
drop procedure sp_par71;
create procedure "informix".sp_par71(a_no_tranrec CHAR(10))
returning integer,
		  char(100);
...
define _no_tranrec_anulo	char(10);
...
define _variacion_ced		dec(16,2);
...
define _cantidad,_flag   	smallint;
...
--		end if
	...
	let _variacion_ced = _variacion_bru - _variacion_net;

	-- Reserva de Siniestros en Tramite (221)
	...
	cod_intercta	cuenta	nombre	
	RPRDSMR			222		RESERVA DE SINIESTROS MONTO RECUPERABLE	
	RGADRST			553		AUMENTO DISMINUCION RESERVA SINIESTROS TRAMITE	
	NIIFPRSMR		149		PARTICIPACION/MONTO RECUPERABLE EN SINIESTRO - REA	
	RGADRSTRE		554		AUM.DISM PART REA RES SIN EN TRAMITE	

	*/

	-- Reserva de Siniestros Monto Recuperable(149)
	let _monto2 = _variacion_ced;
	...
	if _monto2 <> 0.00 then
		...
		--let _cuenta    = sp_sis15('RPRDSMR', '01', _no_poliza);-- antes iba a la 222
	...
	*/
	...
	-- Aumento/Disminucion de Reserva Bruta(553)
	...
	if _variacion_bru <> 0.00 then
		...
		if _variacion_bru >= 0.00 then
			let _debito  = _variacion_bru;
		else
			let _credito = _variacion_bru;
		end if
	...
	-- Aumento/Disminucion de Reserva Cedida (554)
	if _variacion_ced <> 0.00 then
		let _variacion_ced = _variacion_ced* -1;

		let _debito  = 0.00;
		let _credito = 0.00;

		if _variacion_ced >= 0.00 then
			let _debito  = _variacion_ced;
		else
			let _credito = _variacion_ced;
		end if
...
if _tipo_transaccion  = 4 and
   _monto            <> 0 then
	...
	if _cod_tipoprod   = "002" then -- Coaseguro Minoritario
		...
		if _anular_nt <> '' then -- Verificar la cuenta que afectó la transaccion original 
			let _cuenta = sp_sis15('RPRCXA');
			let _no_tranrec_anulo = '';

			foreach
				select a.no_tranrec
				  into _no_tranrec_anulo
	...
	if _flag14103 = 1 then -- Reclamos de Coaseguro por Aplicar
...
If (_monto <> 0.00 OR _monto is null) And _flag = 1 Then
