...
drop procedure sp_cob370a;
create procedure sp_cob370a()
returning	integer,
			varchar(100);
...
foreach
	...
	select count(*)
	  into _cnt_caspoliza
	  from caspoliza
	 where cod_campana in (select cod_campana from cascampana where tipo_campana = 3 and cod_campana <> _cod_campana )
	...
	if _cnt_caspoliza = 0 then
		delete from cascliente
		 where cod_cliente = _cod_pagador
		   and cod_campana in (select cod_campana from cascampana where tipo_campana = 3 and cod_campana <> _cod_campana );
	end if
