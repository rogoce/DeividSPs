...
drop procedure sp_sis448_2;
create procedure "informix".sp_sis448_2(a_secuencia integer, a_fecha date)
returning char(3000);
...
if _cod_tipo in ('00021','00023') then --Notificaciones de Rechazos TCR/ACH
 ...
 SELECT nombre_cliente	
  ...
  WHERE no_aviso  = _no_remesa
   ...
   let _fecha_letra = sp_fecha_letra(a_fecha);
   ...
   let _html = replace(trim(_html),'%_nombre%',trim(_nom_cliente));	
...
elif _cod_tipo = '00004' then -- Comprobantes de pagos
	...
	let _html = replace(trim(_html),'%_pagador%',_nom_cliente);
	let _html = replace(trim(_html),'%_mes%',_fecha_char);
