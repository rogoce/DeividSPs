...
-- *********************************
DROP PROCEDURE sp_aud17bk2;
CREATE PROCEDURE sp_aud17bk2(a_db CHAR(18), a_fecha1 date, a_fecha2 date ) 
	...
	order by a.res_noregistro    				
			...
			if v_auxiliar = "S" then
	           ...
	           SELECT SUM(res1_debito), 
				...
				-- Segun descuadre 26410 --
				{LET _cuenta = s_cuenta;
				let _cant = 0;
				
				if trim(_cuenta) = '26410' then				
					select count(*)
					  into _cant
					  from mart_agos
					 where res1_notrx = s_notrx
					   and res1_auxiliar = s_auxiliar;
