...
drop procedure sp_rea24d_prueba;
create procedure sp_rea24d_prueba(a_compania char(3), a_sucursal char(3), a_periodo char(7), a_flag smallint)
returning	integer,
			varchar(100);
...
foreach with hold
	select no_documento,
	 ...
	 where periodo = a_periodo
	   and saldo_pxc <> 0
	   and no_documento = "1614-00078-01"
	...
	if _doc_poliza in ('1604-00024-01','1617-00619-09','1618-00023-01','0417-00008-01') then
		commit work;
		continue foreach;
	end if
	...
	if _cod_ramo = '016' then
		call sp_sis188(_no_poliza) returning _valor,_error_desc;
	end if
	...
	if _cod_ramo in ('001','003') then
		...
		foreach
			...
			update deivid_tmp:fic_emireaco
			   set porc_partic_prima  = porc_partic_prima * (_porc_cober_reas / 100)
			 where no_poliza      = _no_poliza
			   and no_unidad      = _no_unidad
			   and cod_contrato   = _cod_contrato
			   and no_cambio      = _no_cambio
	...
	return 0, 'Exito. Diferencia entre cobmoros y provisión: ' || 0;
	foreach
		select cod_contrato,
			   cod_cober_reas,
			   porc_partic_prima,
