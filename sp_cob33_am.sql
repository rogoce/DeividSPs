...
drop procedure sp_cob33_am;
CREATE PROCEDURE sp_cob33_am(
a_compania     CHAR(3), 
a_sucursal     CHAR(3), 
...
{RETURNING	DEC(16,2)	as monto_resto,	
			DEC(16,2)	as mto_primero,	
...
{if a_no_documento = '1824-00007-10' then
...
foreach
	select prima_bruta,
	 ...
	 where no_documento   = a_no_documento -- facturas de la poliza
	   and actualizado    = 1			 	-- factura este actualizada
	   and activa         = 1
	   and no_factura = '01-2962843'  --fecha_emision >= '20/05/2025'
	...
	if _estatus_poliza in (2,4) then -- or _cod_endomov = '019' then --Si esta cancelada o anulada se debe, todas las facturas se hacen efectivas desde la vigencia inicial de la pÃÂ³liza
		let _no_pagos = 1;
	...
	--RETURN  _monto_resto,_monto_primero,_no_pagos,_no_factura,_fecha_letra,a_fecha - _fecha_letra,0; 
	...
	for _ciclo = 1 to _no_pagos
		...
		--RETURN  v_por_vencer,0,v_corriente,v_monto_30,v_monto_60,v_monto_90,v_saldo with resume; 
	end for
	--RETURN  v_por_vencer,0,v_corriente,v_monto_30,v_monto_60,v_monto_90,v_saldo; 
end foreach
