...
CREATE PROCEDURE "informix".sp_pro219b(a_compania CHAR(03), a_terremoto SMALLINT, a_fecha DATE, a_fecha2 DATE, a_ubica CHAR(255) DEFAULT "*", a_serie char(255) default "*")
RETURNING   CHAR(50),  -- Ubicacion
			...
			char(50),
			dec(16,2),
			CHAR(5), char(255),SMALLINT, CHAR(5), CHAR(50), CHAR(3);					
...
if _error <> 0 then
	RETURN _error_desc,_error,0.00,'','',0,'','','','',0.00,'','',0.00,'',v_filtros,0,'','','';		   
end if
-- HG:CASO: 31242
-- Filtro por Serie
IF a_serie <> "*" THEN
...
--TRACE ON;
  FOREACH WITH HOLD
	  ...
	  SELECT cod_ubica,
			 ...
			 orden,
			 no_unidad,
             suma_aseg_total,serie,cod_contrato,desc_contrato,cod_cobertura		 
		INTO _cod_ubica,
			 _tipo_incendio,
			 v_cnt_poliza,     
			 v_suma_asegurada, 
			 ...
			 _orden,
			 _no_unidad,
			 _suma_aseg_total,_serie,_cod_contrato,_desc_contrato,_cod_cobertura	
	   FROM temp_ubica
	   WHERE seleccionado = 1
	  order by orden,tipo_incendio,no_unidad
  ...
  if _cod_manzana is null then
   ...
   SELECT cod_provincia,
	...
	RETURN v_ubicacion,
		   ...
		   _n_dist,
		   _prima_unidad,
		   _no_unidad,v_filtros ,_serie,_cod_contrato,_desc_contrato,_cod_cobertura	      
		   WITH RESUME;
