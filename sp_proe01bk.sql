...
DROP PROCEDURE sp_proe01bk;
CREATE PROCEDURE "informix".sp_proe01bk(a_poliza CHAR(10), a_unidad CHAR(5), a_cia CHAR(3))
			   ...
			   if _tipo_descuento = 1 and _nueva_renov = 'N' then	--> Descuento RC, solo polizas nuevas
					...
					if _descuento_max > 0 then
						...
						if _descuento_mod > 0 then
							...
							--#########################################################################################
							let _desc_porc   = _descuento_mod / 100;
							let _desc_cob    = ld_prima_acu * _desc_porc;
							let ld_prima_acu = ld_prima_acu - _desc_cob;
							let _desc_cob_total = _desc_cob + _desc_cob_total;
							--#########################################################################################	
