...
Let  _html_body1 = trim(_html_body1) || '        	Panama, 08 de junio de 2024.';
...
Let  _html_body1 = trim(_html_body1) || '        	Ref.: Su p&oacute;liza de Salud Nro. '||v_no_documento||' - Asegurado(a) '||v_asegurado;
...
Let  _html_body1 = trim(_html_body1) || '				Durante este tiempo, nos ha complacido brindarle nuestros servicios y asegurarle la protecci&oacute;n que garantiza su p&oacute;liza. Tomando en cuenta que en los &uacute;ltimos 6 (seis) a&ntilde;os usted ha presentado reclamos en 					'||v_anosreclamos||' de esos a&ntilde;os con una siniestralidad acumulada de '||v_siniest_acumulada||'% nos vemos en la obligaci&oacute;n de informarle sobre un ajuste necesario en la prima mensual de su p&oacute;liza.';				

Let  _html_body1 = trim(_html_body1) || '</p>';

Let  _html_body1 = trim(_html_body1) || '</p>';

Let  _html_body1 = trim(_html_body1) || '				<span style="text-decoration: underline;">Este ajuste, que ha sido debidamente autorizado por la Superintendencia de Seguros y Reaseguros de Panam&aacute; mediante 				la Resoluci&oacute;n Nro. DRLA-043 de 27 de mayo de 2024</span>, se debe principalmente a tres factores: la alta inflaci&oacute;n m&eacute;dica a nivel nacional, el hecho que no 				hemos ajustado los costos desde el a&ntilde;o 2019 y el frecuente y alto consumo de beneficios en su p&oacute;liza.';

Let  _html_body1 = trim(_html_body1) || '</p>';

Let  _html_body1 = trim(_html_body1) || '<p>';

Let  _html_body1 = trim(_html_body1) || '				Por lo tanto, a partir del 01 de julio del presente a&ntilde;o, la prima mensual de su p&oacute;liza experimentar&aacute; un aumento del ';

Let  _html_body2 = trim(_html_body2) || '				'||v_aumento||'%,  equivalente a B/. '||v_montoaumento||',  pasando de B/. '||v_prima_actual||' a B/. '||v_prima_nueva||'; el nuevo monto de prima se compone ';

Let  _html_body2 = trim(_html_body2) || '				de un incremento del '||v_porcbandaedad||'%, equivalente a B/. '||v_montobandaedad||', por incremento de Banda de Edad y Recargos m&aacute;s un incremento ';

Let  _html_body2 = trim(_html_body2) || '				de '||v_porcsiniest||'%, equivalente a B/. '||v_montosiniest||' debido a inflaci&oacute;n m&eacute;dica en la siniestralidad.';

Let  _html_body2 = trim(_html_body2) || '</p>';

Let  _html_body3 = trim(_html_body3) || '<p>';

Let  _html_body3 = trim(_html_body3) || '				Agradecemos sinceramente su comprensi&oacute;n y su continuo compromiso con nuestra empresa. Esperamos seguir ';
