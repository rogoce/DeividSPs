...
Define _dia_cob,_mes_num,_anio	smallint;
Define _error				smallint;
Define flag,_filt_periodo	smallint;
define _tipo_campana,_nueva_renov        smallint;
...
delete from campoliza
 where cod_campana = a_cod_campana
   and cod_grupo in ('1090', '124', '125', '148','1122','77960','77982','78020');   --CASO: 30140 USER: ASTANZIO grupo: 148 desde: 18/12/2018 5pm -- F9:30295 1122 ASTANZIO  15/01/2019    -- SD#3010 07/04/2022 4:00pm -- SD#5708 23/02/2023 HG
...
select count(*) 
 ...
 --Puesto por AMM, caso 13923,Milagros G.
 if _nueva_renov = 0 then --Todas, No hace nada.
 elif _nueva_renov = 1 then --Solo Nuevas
	delete from campoliza
	 where no_documento not in(
	select c.no_documento
	  from campoliza c, emipoliza e, emipomae t
	 where c.no_documento = e.no_documento
 ...
 if _filt_periodo = 1 then
	...
	let _mes_num = month(_fecha_hasta);
	let _anio = year(_fecha_hasta);
	
	select * from campoliza
	 where cod_campana = a_cod_campana
	   and month(vigencia_inic) = _mes_num
	   and year(vigencia_inic) = _anio
	   into temp prueba;
