...
drop procedure sp_sac164apm;		

create procedure sp_sac164apm()
...
foreach
    ...
    if _no_factura <> "01-2505140" then
		if abs(_pbs_endoso - _pbs_emifacon) > 0.01 then
		
		    let _error_cod = sp_reainv_amm7(_no_poliza, _no_endoso);
	...
	foreach
		...
		--	let _error_cod = sp_reainv_amm8(_no_poliza, _no_endoso, _no_unidad);
