...
drop procedure sp_sis119;

create procedure sp_sis119(a_usuario char(8))
 returning integer,
           char(200),
           char(5);
...
--set debug file to "sp_sis119.trc";
--trace on;
...
FOREACH	WITH HOLD
	...
	begin work;
            ...
            select cod_ramo into _cod_ramo from emipomae where no_poliza = _no_poliza;

            if _cod_ramo = '002' then
				let _ruta = '00557';  --Ruta 100/50/50 para auto
			else
				let _ruta = '00558';  --Ruta 100/50/50 para autoflota
			end if
			...
			let _error = sp_proe05bk1(_no_poliza, _no_unidad, _ruta, _suma_asegurada, _no_endoso);
