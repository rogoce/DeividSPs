...
--DROP PROCEDURE ap_pro30;

CREATE PROCEDURE ap_pro30(a_compania CHAR(3), a_sucursal CHAR(3), a_vigencia_desde DATE, a_vigencia_hasta DATE, a_usuario CHAR(8)) RETURNING INTEGER,
			CHAR(100), integer;
...
IF _cod_ruta IS NULL THEN
	RETURN 1, 'No Existe Distribucion de Reaseguro ...', _contador;
END IF
...
IF _porc_partic_prima <> 100 THEN
	RETURN 1, 'La Distribucion No Es 100%, Verifique la Ruta ' || _cod_ruta,_contador;
END IF
...
IF _cod_cober_reas IS NULL THEN
	RETURN 1, 'No Existe Cobertura de Reaseguro ...',_contador;
END IF
...
-- Procedure que realiza el calculo de las tarifas nuevas de salud 
	...
	call ap_pro30c(_no_poliza) returning _error, _error_desc;
	 ...
	 SELECT	no_unidad,
	  ...
	  order by no_unidad
		...
		if _prima_neta = 0.00 then
			...
			RETURN 1, _error_desc,_contador;
		end if
