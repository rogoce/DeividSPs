--Procedimiento para realizar la carga en Rentabilidad1 para bono de RENTABILIDAD 2026
--Usando prima neta cobrada devengada
--Creado 26/03/2026	Armando Moreno M.

drop procedure sp_che115_carga2026;
create procedure sp_che115_carga2026(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
define _cnt,_ano_salud  integer;
...
define _renglon,_ano_rent integer;
...
define _vigente,_pagado,_periodo_espera         smallint;
...
define _bono_rent2,_fronting,_tiene_fac        smallint;
...
let a_periodo   = '2026-02';

update parparam
   set agt_per_fidel = a_periodo;

let _fec_aa_ini     = "01/01/2026";	--para sacar cancelada o anulada en el periodo
let _per_ini        = "2026-01";
let _per_ini_ap     = "2025-01";
let _ano            = a_periodo[1,4];
let _ano            = _ano - 1;
let _per_fin_ap     = _ano || a_periodo[5,7];

let _per_fin_dic    = "2025-12";         
...
--*****************************************
-- Prima Neta Cobrada Devengada Año Actual
--*****************************************
foreach
	select doc_remesa
	  into _no_documento
	 ...
	 where periodo     >= _per_ini
	   ...
	   and tipo_mov    in ("P", "N")
	 group by doc_remesa
	 order by doc_remesa desc

	let _tiene_fac = 0;
	let _no_poliza = sp_sis21(_no_documento);
	let _tiene_fac = sp_sis439(_no_poliza);
	
	let _valor = sp_sis101a(_no_documento,'01/01/2026',_fecha_aa, my_sessionid);
	if _tiene_fac = 1 then	--Tiene Facultativo
		call sp_dev06f_renta(_no_documento,_fecha_aa,'01/01/2026',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
	else	
		call sp_dev06f(_no_documento,_fecha_aa,'01/01/2026',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
	end if
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2025',_fecha_ap,my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
foreach
	...
	if _cod_grupo in('00000','1000','00990') then --Grupo del estado se excluye, (se excluye grupo BHN puesto 17/03/2025)
		continue foreach;
	end if
    ...
    if _cod_ramo in("018","004","016","002","020","023","012","014","017","010","022","013","001","003","006","015","005","011","021","009","007") then
	else
		continue foreach;
    end if
	...
	if _tipo_prod = 4 then	--Excluye Reaseguro Asumido
	   continue foreach;
	end if
	...
	else	--PARA SALUD, SOLO POLIZAS NUEVAS
		let _ano_salud = year(_vigencia_inic);
		let _ano_rent  = year(_fec_aa_ini);
		if _ano_salud  = _ano_rent then		--Para Salud, solo pólizas nuevas
		else
			continue foreach;
		end if
		if _cod_subramo <> '012' then --Subramo colectivo
			let _cod_tipo = '004';    --Para salud individual
		else
			if _periodo_espera = 2 then	--Para colectivos solo con CONDICION DE PERIODO DE ESPERA
			else
				continue foreach;
			end if
			let _cod_tipo = '005';   --Para salud colectivo
		end if
	end if
	...
	--se excluye 02442 caso 11579
	if _cod_agente in('02442','02569','02656') then	--se excluyen estos corredores por instr. Analisa, correo del 09/07/2019
		continue foreach;
	end if
	...
	--************************************************
	--   Calculos para incremeto de PSP 2021 vs 2020
	--************************************************
		let _pri_sus_aa_tmp = 0;
		let _pri_sus_ap_tmp = 0;		
