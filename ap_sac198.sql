...
-- execute procedure ap_sac198('01','231020203','%','RE034','2011','01','2011','01','sac') 

DROP PROCEDURE ap_sac198;

CREATE PROCEDURE ap_sac198(a_tipo char(2),a_cuenta char(12),a_ccosto char(3),a_aux char(5),a_anio char(4),a_mes char(2),a_anio2 char(4),a_mes2 char(2),a_db CHAR(18))
...
FOREACH	
	...
	if v_origen = 'COB' THEN 
		select semana
		  into _semana
		  from deivid:chqpagco
		 where v_fecha between fecha_desde and fecha_hasta;
	elif v_origen = 'CHE' THEN
		select semana
		  into _semana
		  from deivid:chqpagco
		 where v_fecha - 2 units day between fecha_desde and fecha_hasta;
		-- where date(fecha_fin) = v_fecha;
	end if	
	
	if _semana is null then
		LET _semana = 0;
	end if
