...
drop procedure sp_rec02_reserva;

create procedure "informix".sp_rec02_reserva(
a_compania		char(3), 
a_agencia		char(3), 
a_periodo		char(7),
...
foreach 
 select no_reclamo,
         porc_partic_coas, cod_ramo
   into _no_reclamo, 
        _porc_coas,_cod_ramo
   from tmp_sinis 
	...
	-- Variacion de Reserva
	 ...
	 where no_reclamo  = _no_reclamo
	   and actualizado = 1;
                      ...
                      --   if _numrecla = "02-0412-01213-10" then
                  --trace on;
	          --      end if;
...
---------------------------------------------------------------------------------			 
			...
			if _existe = 0 then				   					 
				...
				let _cod_cober_reas_no = '';
					--  Combinaciones Casco en RC
					if (_cod_ramo = '002' and _cod_cober_reas = '031') or 
					   (_cod_ramo = '002' and _cod_cober_reas = '034') or 
					   (_cod_ramo = '023' and _cod_cober_reas = '034') or 
					   (_cod_ramo = '023' and _cod_cober_reas = '031' ) then
					...
					--  Combinaciones RC en Casco
					if (_cod_ramo = '002' and _cod_cober_reas = '002') or 
					   (_cod_ramo = '002' and _cod_cober_reas = '033') or 
					   (_cod_ramo = '023' and _cod_cober_reas = '002') or 
					   (_cod_ramo = '023' and _cod_cober_reas = '033' ) then
						select first 1 cod_cober_reas
						  into _cod_cober_reas_no
						  from rectrrea
						 where no_tranrec     = _no_tranrec
			...
			if _porc_reas is null then
				let _porc_reas = 0;
                        ...
                        if _numrecla = "02-0412-01213-10" then
                            ...
                            let _no_tranrec = _no_tranrec;
                            let _cod_cober_reas = _cod_cober_reas;
                            let _monto_total = _monto_total;
                            let _monto_cob = _monto_cob;
                            let _monto_bruto = _monto_bruto;
                            let _monto_neto = _monto_neto;
                            let _porc_coas = _porc_coas;
                            let _porc_reas = _porc_reas;
