		...
		foreach
	     ...
	     if _codcliente is not null then
		   ...
		   { FOREACH
		    ...
		    --if v_saldo1 <= 0 and _procedencia <> 2 then se quita 29/05/2012 por instr. de Jessica Miller(Jefe de cobros) Armando.
		   	--	continue foreach;
		    --end if
		 end if
		 ...
		 -- Lectura de Cobruter2
			...
			foreach
				SELECT cod_cobrador	
				  ...
				  FROM cascliente
				 WHERE cod_cliente = _cod_pagador
				 if _cod_cobrador_o is null or _cod_cobrador_o = '' then
					continue foreach;
				 end if
				 exit foreach;
			end foreach
