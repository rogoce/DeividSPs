   ...
   RETURNING CHAR(50),INT,DECIMAL(16,2),CHAR(50),CHAR(45),INT,SMALLINT,DECIMAL(16,2),INTEGER,DECIMAL(16,2),DECIMAL(16,2),DECIMAL(16,2),INTEGER;
--------------------------------------------
---  APADEA
 ...
 SELECT cod_ramo,
  ...
  END IF
   ...
   IF _cod_ramo = '004' OR _cod_ramo = '018' THEN
	...
	if _cod_ramo = '009' and _cod_subramo = '004' then
			UPDATE ramosubr
			   SET prima_suscrita  = prima_suscrita + _total_pri_sus
			 WHERE cod_ramo        = _cod_ramo
			   AND cod_subramo     = "004";
...
FOREACH WITH HOLD
   ...
   IF v_cod_ramo = "014" OR v_cod_ramo = "013" THEN	--car y montaje
	...
	if v_cod_ramo = '009' and v_cod_subramo = '004' then	
			UPDATE ramosubr
			   SET cnt_polizas = cnt_polizas + 1
			 WHERE cod_ramo    = v_cod_ramo
			   AND cod_subramo = "004";
...
END FOREACH
--Actualizar los reclamos cerrados en el mes
foreach
	select no_reclamo
	  into _no_reclamo
      from rectrmae
   ...
   {ELIF	v_cod_ramo = "007" THEN
     UPDATE ramosubr
        SET cnt_polizas     = cnt_polizas + v_cant_polizas,
			...
			casos_cerrados  = casos_cerrados  + _cnt_cerra			
      WHERE cod_ramo        = "015"
        AND cod_subramo     = "002";}
   END IF
END FOREACH
