...
-- d_sac_sp_sac216_dw1
-- *********************************
DROP PROCEDURE sp_sac216; 
CREATE PROCEDURE sp_sac216(a_db char(18), a_fecha1 date, a_fecha2 date, a_auxiliar CHAR(5) ) 
RETURNING char(3),
	        char(30),
            date,
            integer,
...
call sp_sac215(a_db, a_fecha1 , a_fecha2 , a_auxiliar  ) returning _error, _error_desc;

--set debug file to "sp_sac216.trc";	
--trace on;
	...
	-- Validaciones
	  ...
	  SELECT sum(res_debito),
       ...
       INTO total_db,
	    ...
	    and  res_no_documento = b_no_documento;
	...
	FOREACH
	  SELECT res_notrx,
       ...
       INTO c_notrx,
	    ...
	    where res_notrx = b_notrx
		  and res_no_documento = b_no_documento
      ORDER BY b_no_documento,res_noregistro
		...
		IF l_auxiliar = "S" THEN 
	           ...
	           SELECT SUM(res1_debito), 
			    ...
			    WHERE ( res1_noregistro = c_linea ) AND
					  ...
					  (	res1_notrx = b_notrx ) and
					  (	res1_no_documento = b_no_documento) ;
			...
			FOREACH 
			  SELECT res1_linea,
			   ...
			   WHERE ( res1_noregistro = c_linea ) AND
					 ...
					 ( res1_notrx = b_notrx ) and
					 ( res1_no_documento = b_no_documento) 
			order by res1_no_documento,res1_cuenta,res1_linea
