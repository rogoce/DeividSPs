--Procedimiento para realizar la carga en Rentabilidad1 para bono de RENTABILIDAD 2022
--Usando prima neta cobrada devengada
--Creado 17/03/2022	Armando Moreno M.

DROP PROCEDURE sp_che115_carga2022;
CREATE PROCEDURE sp_che115_carga2022(a_compania CHAR(3), a_sucursal CHAR(3))
RETURNING SMALLINT;
...
let _fec_aa_ini     = "01/01/2022";	--para sacar cancelada o anulada en el periodo
let _per_ini        = "2022-01";
let _per_ini_ap     = "2021-01";
let _ano            = a_periodo[1,4];
let _ano            = _ano - 1;
let _per_fin_ap     = _ano || a_periodo[5,7];

let _per_fin_dic    = "2021-12";         
...
foreach
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2022',_fecha_aa, my_sessionid);
	call sp_dev06f_renta(_no_documento,_fecha_aa,'01/01/2022',_fecha_aa) returning _valor,_mensaje,_pri_devengada;
...
foreach	with hold
	...
	let _valor = sp_sis101a(_no_documento,'01/01/2021',_fecha_ap,my_sessionid);
	foreach
		select cod_agente,
		       porcentaje
		  into _cod_agente,
...
foreach
	...
	{if _tipo_prod = 4 or _tipo_prod = 3 then  se pone en comentario 16/03/2023 para incluir ahora polizas con coaseguro minoritario
	   continue foreach;
	end if}
	...
	if _cod_agente in('02569','02656','02825') then	--se excluyen estos corredores por instr. Analisa, correo del 09/07/2019
		continue foreach;
	end if
