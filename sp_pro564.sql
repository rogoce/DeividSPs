...
--------------------------------------------
	...
	DEFINE v_cant_polizas_ma, _cnt_prima_nva, _cnt_prima_ren, _cnt_prima_can, _cnt_pol_dif, _cantidad_aseg, v_cant_asegurados, _cnt_prima_rev, _cnt_prima_end, _cnt_polizas, _cnt_asegurados INTEGER;
...
FOREACH
	SELECT cod_ramo,
	  ...
	  INTO _cod_ramo,
		   ...
		   _cnt_asegurados
	  FROM ramosubrbase
	 WHERE origen LIKE a_origen
	
	UPDATE ramosubr
	   SET cnt_polizas_ma = _cnt_polizas,
	       cnt_asegurados = _cnt_asegurados
...
FOREACH                   
 SELECT cod_ramo, total_pri_sus, cnt_prima_nva,	cnt_prima_ren, cnt_prima_can, cnt_prima_rev, cnt_prima_end, no_poliza, no_endoso
  ...
  END IF
	...
	if _cnt_prima_can <> 0 or _cnt_prima_rev <> 0 then
   ...
   IF _cod_ramo = "019" AND _vig_fin_vida > _vig_ini_end and _nueva_renov = "N" THEN
     UPDATE ramosubr
        SET prima_suscrita = prima_suscrita + _total_pri_sus, cnt_pol_nuevas  = cnt_pol_nuevas + _cnt_prima_nva, cnt_pol_ren = cnt_pol_ren + _cnt_prima_ren, cnt_pol_can_cad  = cnt_pol_can_cad + _cnt_prima_can + _cnt_prima_rev, cnt_asegurados = cnt_asegurados + _cantidad_aseg
      WHERE cod_ramo     = _cod_ramo
        AND cod_subramo  = "001";
   END IF	
   ...
   IF _cod_ramo = '004' OR _cod_ramo = '018' THEN
	...
	if _cod_ramo = '009' and _cod_subramo in ('004', '008') then --> Se incluye el subramo 008 MARINE CARGO STP
	   ...
	   BEGIN
          ON EXCEPTION IN(-239)
             UPDATE temp_perfil2
                SET prima_suscrita = prima_suscrita + _total_pri_sus, cnt_pol_nuevas = cnt_pol_nuevas  + _cnt_prima_nva, cnt_pol_ren = cnt_pol_ren + _cnt_prima_ren, cnt_pol_can_cad  = cnt_pol_can_cad + _cnt_prima_can + _cnt_prima_rev, cant_asegurados = cant_asegurados + _cantidad_aseg
               WHERE cod_ramo       = _cod_ramo
                AND cod_subramo    = _cod_subramo;
...
update ramosubr set cnt_polizas = cnt_polizas_ma + cnt_pol_nuevas - cnt_pol_can_cad;
update ramootro set cnt_polizas = cnt_polizas_ma + cnt_pol_nuevas - cnt_pol_can_cad;
