...
DROP PROCEDURE sp_pro586h;

CREATE PROCEDURE "informix".sp_pro586h(
		a_compania  CHAR(3),
		a_agencia   CHAR(3),
		a_periodo1  CHAR(7),
		a_periodo2  CHAR(7),
...
FOREACH
 SELECT a.no_reclamo,
   ...
   INTO _no_reclamo,
		...
		_no_tranrec,
		_cod_tipotran
   FROM rectrmae a, recrcmae b, emipoagt c
  WHERE a.no_reclamo = b.no_reclamo
    ...
    AND a.monto        <> 0
	AND c.cod_agente in (select cod_agente from agtagent where no_licencia[1,3] = "OAL")
...
FOREACH 
 SELECT a.no_reclamo,	
  ...
  WHERE a.no_reclamo = b.no_reclamo
	...
	AND a.cod_compania = a_compania
    AND a.actualizado  = 1
    AND a.cod_tipotran MATCHES '*' -- Para que incluya el indice creado
	AND a.periodo      >= a_periodo1 
	AND a.periodo      <= a_periodo2
    AND a.variacion    <> 0
    AND c.cod_agente in (select cod_agente from agtagent where no_licencia[1,3] = "OAL")
