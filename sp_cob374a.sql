...
drop procedure sp_cob374a;
create procedure sp_cob374a()
returning	integer,
			varchar(100);
...
define _fecha_venc_pen		date;
define _periodo_gracia		date;
define _vigencia_final		date;
define _fecha_venc_pag		date;
...
on exception set _error, _error_isam, _error_desc
	--rollback work;
	let _error_desc = trim(_error_desc) || 'no_documento: ' || trim(_no_documento);
...
foreach with hold
	...
	let _monto_pen = 0.00;
	let _fecha_venc_pen = null;

	select min(fecha_vencimiento)
	  into _fecha_venc_pen
	  from emiletra
	 where no_documento = _no_documento
	   and pagada = 0;

	if _fecha_venc_pen is null then
		continue foreach;
	end if

	select max(fecha_vencimiento)
	  into _fecha_venc_pag
	  from emiletra
	 where no_documento = _no_documento
	   and pagada = 1
	   and monto_letra > 1;

	if _fecha_venc_pag > _fecha_venc_pen then
		call sp_pro545(_no_documento) returning _error,_error_desc;

		if _error <> 0 then
			--rollback work;
			return _error, 'sp_pro545  ' || _error_desc;
		else
			call sp_cob346a(_no_documento) returning _error,_error_desc;
			
			if _error <> 0 then
				--rollback work;
				return _error,'sp_cob346a ' || _error_desc;
			end if
			...
			if _error <> 0 then
				--rollback work;
				return _error,'sp_pro544 ' || _error_desc;
			end if
		end if
