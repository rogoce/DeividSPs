...
FOREACH
  SELECT a.no_documento,
	...
	INTO v_poliza,
		 ...
		 _cod_contratante
    FROM emipomae a
   WHERE a.cod_ramo in ('002','020','023') 
	 AND a.cod_tipoprod in ('001','005')
     AND a.vigencia_inic <= date(current)
     AND a.vigencia_final >= date(current)
     AND a.actualizado = 1
     AND a.estatus_poliza = 1
     --AND a.fecha_suscripcion >= _fecha_hoy - 5 units month
	-- and a.no_documento in ('0219-13239-47','0218-03961-09')
	 --AND a.fecha_suscripcion = today
