...
DROP PROCEDURE sp_rwf39;
CREATE PROCEDURE "informix".sp_rwf39(a_no_tranrec CHAR(10), a_fecha datetime hour to fraction(5)) 
...
-- Buscando si se unifica o no -- Ramo Salud
 IF _ramo_sis = 5 THEN
...
--    LET _no_requis_n = "ultimus";
    ...
    IF _ramo_sis = 5 or _ramo_sis = 9 THEN	--> Solo salud se paga con ACH Salud o Accidentes Personales
		if _agrega_acreedor = 0 then 		
           if _ramo_sis = 5 then		
				...
				if _cod_tipopago = '001' then
					CALL sp_rec183 (_cod_compania,_cod_cliente,_cod_banco,_cod_chequera) returning _cod_banco, _cod_chequera, _tipo_requis;
				elif _cod_tipopago in ('003','004','001') then -- Asegurado o Tercero Amado SD 13377 -- 10-04-2025 -- Se agrega proveedor 001 SD 13821 -- Amado 23-05-2025
		...
		if _ramo_sis = 1 then
			if _agrega_acreedor = 0 then
				if _cod_tipopago = '001' then --Proveedor
					CALL sp_rec183 (_cod_compania,_cod_cliente,_cod_banco,_cod_chequera) returning _cod_banco, _cod_chequera, _tipo_requis;
				elif _cod_tipopago = '003' then --Asegurado
					if _perd_total_rec = 0 and _perd_total_tr = 0 then
						CALL sp_rec183 (_cod_compania,_cod_cliente,_cod_banco,_cod_chequera) returning _cod_banco, _cod_chequera, _tipo_requis;
					end if		
                elif _cod_tipopago = '004' then	--> Tercero para las subrogaciones
					select count(*)
					  into _cont
					  from rectrcon
					 where no_tranrec = a_no_tranrec
...

{
	SELECT valor_parametro 
	 ...
	 WHERE codigo_compania  = _cod_compania
	   ...
	   AND aplicacion       = "REC"
	   AND version          = "02"
	   AND codigo_parametro = "banco_default";
	...
	SELECT valor_parametro 
	 ...
	 WHERE codigo_compania  = _cod_compania
	   ...
	   AND aplicacion       = "REC"
	   AND version          = "02"
	   AND codigo_parametro = "chequera_default";
 }
	...
	END IF
     ...
     BEGIN
		ON EXCEPTION SET _error 
		 --	rollback work;
		 	RETURN _error, "Error al actualizar CHQCHREC", 0;         
		END EXCEPTION 
