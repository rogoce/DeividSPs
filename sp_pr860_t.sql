...
--execute procedure sp_pr860_t('001','001','2023-01','2023-01',"*","*","*","*","008;","*","2024,2023,2022,2021,2020,2019,2018,2017,2016,2015,2014,2013,2012,2011,2010,2009,2008;",'09')
drop procedure sp_pr860_t;
create procedure sp_pr860_t(a_compania char(03),a_agencia char(03),a_periodo1 char(07),a_periodo2 char(07),a_codsucursal char(255)	default "*",a_codgrupo char(255) default "*",
...
foreach
	select serie,
	  ...
	  into _serie,
		   ...
		   v_prima
	  from temphg
	 where cod_coasegur in ('122','153','050','063','076','042','036','089','128','117','134','136','141','146','147','149','077','153','034','164') --SD#6343 add 164 27/04/2023-HGIRON --agregu̩ el 153 porque es nuevo APM 12-01-2022, 
	   and anio      = _anio_reas                                                                                                                          --Se agrega el 173 qe es nuevo - Amado 29-04-2025 SD 13543 ,'173' lo quite 14-05-2025
	   and trimestre = _trim_reas
	...
	foreach
		select distinct cod_coasegur,
		  ...
		  from temphg
		 Where serie         = _serie
		   and cod_coasegur in  ('122','050','063','076','042','036','089','128','117','134','136','141','146','147','149','077','153','034','164') --SD#6343 add 164 27/04/2023-HGIRON --agregu̩ el 153 porque es nuevo APM 12-01-2022
		   and cod_ramo      = v_cod_ramo                                                                                                                 --Se agrega el 173 qe es nuevo - Amado 29-04-2025 SD 13543 ,'173' lo quite 14-05-2025
		   and cod_contrato  = v_cod_contrato
		   and cod_cobertura = v_cobertura
		   and anio          = _anio_reas
		   and trimestre     = _trim_reas
	...
	foreach
		...
		if _borderaux = '01' and _cod_coasegur = '042' and _serie >= 2012 then
			if v_cod_ramo = '001' or v_cod_ramo = '003' then
				...
				let v_clase = '011';
				select siniestros_pagados   -- abs(siniestros_pagados) Amado 23-04-2025 SD 13497
				  into _siniestro
		...
		if _borderaux = '01' and _cod_coasegur = '042' and _serie = 2011 then
			if v_cod_ramo = '001' and _siniestro2 <> 0 then
				 let _sini_dif = 0;
				 let _sini_dif = abs(_siniestro) - abs(_siniestro2); -- le puse abs a ambas variables -- Amado 23-04-2025 SD 13497
		...
		if v_cod_ramo = '019' and _serie >= 2024 then --Amado 13-05-2025 -- Caso 13500 Josue a partir del 2024 no debe aparecer los siniestros
			let _siniestro = 0.00;
		end if
...
if a_serie = '*' then
	...
	foreach
		select cod_coasegur,
			   ...
			   cod_cobertura,
			   p_partic,
				(case  when cod_contrato = '2018' then
							  (case  when desc_contrato = '00689' then '00734'
							   when desc_contrato =  '00688' then '00733'
					   else desc_contrato  end)
				when cod_contrato = '2019' then
							  (case  when desc_contrato = '00696'  then '00735'
							   when desc_contrato =  '00695' then '00736'
					   else desc_contrato  end)
				when cod_contrato = '2020' then
								(case  when desc_contrato = '00705' then '00738'
							   when desc_contrato =  '00704' then '00737'
					   else desc_contrato  end)
				when cod_contrato = '2021' then
								(case  when desc_contrato = '00714' then '00739'
							   when desc_contrato = '00713'then '00740'
					   else desc_contrato end)
				else desc_contrato end) desc_contrato,	
			   sum(prima),
			   sum(comision),
			   sum(impuesto),
			   sum(prima_neta),
		 ...
		 where anio = _anio_reas
		   and trimestre = _trim_reas
		   and borderaux in(_borderaux)
		 group by cod_coasegur,cod_clase,cod_contrato,cod_cobertura,p_partic,6 --desc_contrato
