-- Procedimiento para el reporte de Registros pendiendes de Gestion
-- Creado    : 19/08/2011 - Autor: Roman Gordon

drop procedure sp_cob284c;
create procedure "informix".sp_cob284c(a_cod_campana char(10)) 
...
foreach
	...
	foreach
		...
		if _fecha_ult_pago > _fecha_desde_camp then
			...
			insert into temp_campana(no_documento,no_poliza,cobrada,no_remesa)
			values (_no_documento,_no_poliza,1,_no_remesa);
		else
			insert into temp_campana(no_documento,no_poliza,cobrada,no_remesa)
			values (_no_documento,_no_poliza,0,'');
		end if		
	end foreach		 
end foreach
