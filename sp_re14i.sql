-- Siniestralidad por Fronting - Detalle Ramo
--
-- Creado    : 12/09/2000 - Autor: Demetrio Hurtado Almanza
-- Modificado: 16/09/2000 - Autor: Demetrio Hurtado Almanza
--
-- SIS v.2.0 - d_recl_sp_rec14i_dw9 - DEIVID, S.A.

DROP PROCEDURE sp_rec14i;

CREATE PROCEDURE "informix".sp_rec14i(
a_compania CHAR(3),
a_agencia  CHAR(3),
a_periodo1 CHAR(7),
a_periodo2 CHAR(7),
...
FOREACH
 SELECT cod_contrato,
  ...
  WHERE seleccionado = 1
    AND fronting     = 1
  GROUP BY cod_contrato, cod_ramo
  ORDER BY cod_contrato, cod_ramo
