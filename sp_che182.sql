...
DROP PROCEDURE sp_che182;
CREATE PROCEDURE sp_che182(a_origen_cheque char(1) default "3") 
...
DEFINE _cod_chequera_dist   char(3);  
...
FOREACH
  SELECT chqchmae.cod_cliente,
   ...
   WHERE ( cliclien.cod_cliente = chqchmae.cod_cliente ) and
         ...
         ( ( chqchmae.origen_cheque = a_origen_cheque ) AND
		 ( chqchmae.cod_banco = '001' ) AND
         ( chqchmae.cod_chequera <> _cod_chequera_dist ) AND
   ...
   END FOREACH
    ...
    if a_origen_cheque = "3" then
	   if _numrecla[1,2] not in ('02','20','23') then
			continue foreach;
	   end if
	elif a_origen_cheque = "M" then
